...
# <a name="dns-requirements-for-a-standard-edition-server-in-lync-server-2013"></a>Lync Server 2013의 Standard Edition 서버에 대한 DNS 요구 사항
...
<td><p>이름 다중 작업 업데이트-r2를 사용 하는 내부 A 레코드 &lt;장치 업데이트&gt; 웹 서비스의 STANDARD Edition server 호스팅 IP 주소를 확인 하는 SIP 도메인입니다. UC 장치가 켜져 있지만 사용자가 장치에 로그인 한 적이 없는 경우에는 A 레코드를 통해 장치가 장치 업데이트 웹 서비스를 호스팅하는 서버를 검색 하 고 업데이트를 얻을 수 있습니다. 그렇지 않으면 장치는 사용자가 처음 로그인 할 때 대역내 프로비저닝 기능을 통해 서버 정보를 가져옵니다. 자세한 내용은 운영 설명서의 <a href="lync-server-2013-device-update-web-service.md">Lync Server 2013에서 장치 업데이트 웹 서비스</a> 를 참조 하세요.</p></td>
</tr>
<tr class="even">
