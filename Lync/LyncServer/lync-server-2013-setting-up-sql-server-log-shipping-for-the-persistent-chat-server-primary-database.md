...
SQL Server Management Studio를 사용 하 여 영구 채팅 서버 보조 로그 전달 데이터베이스 인스턴스에 연결 하 고 SQL Server 에이전트가 실행 중인지 확인해 보십시오.

영구 채팅 기본 데이터베이스 인스턴스에 연결 된 SQL Server Management Studio를 사용 하 여 다음 단계를 수행 합니다.

1.  SQL Server 에이전트가 실행 되 고 있는지 확인해 보십시오.

2.  mgc 데이터베이스를 마우스 오른쪽 단추로 클릭한 후 **속성**을 클릭합니다.

3.  **페이지 선택** 아래에서 **트랜잭션 로그 전달**을 클릭합니다.

4.  **이 데이터베이스를 로그 전달 구성의 기본 데이터베이스로 사용** 확인란을 선택합니다.
...
7.  백업 폴더가 주 서버에 있는 경우 백업 폴더의 기본 서버에 백업 폴더의 로컬 경로를 입력 하 고 **폴더의 로컬 경로 (예: c: \\ 백업)** 상자를 입력 합니다. (백업 폴더가 주 서버에 없는 경우에는이 상자를 비워 둘 수 있습니다.)
    ...
    > [!IMPORTANT]  
    > 기본 서버의 SQL Server 서비스 계정이 로컬 시스템 계정으로 실행 되는 경우 주 서버에 백업 폴더를 만들고 해당 폴더에 대 한 로컬 경로를 지정 해야 합니다.
...
9.  **백업 작업** 아래의 **예약** 상자에 나열된 백업 일정을 확인합니다. 설치 일정을 사용자 지정 하려면 **예약**을 클릭 하 고 필요에 따라 SQL Server 에이전트 일정을 조정 합니다.
...
14. **보조 데이터베이스 초기화** 탭에서 **예, 기본 데이터베이스의 전체 백업을 생성 하 고 보조 데이터베이스에 복원 합니다 (존재 하지 않는 경우 보조 데이터베이스를 만듭니다)**.

15. **파일 복사** 탭의 **복사한 파일의 대상 폴더: (이 폴더는 일반적으로 보조 서버에 있습니다.)** 상자에 트랜잭션 로그 백업을 복사할 폴더의 경로를 입력합니다. 이 폴더는 대개 보조 서버에 있습니다.

16. **복사 작업** 아래 **일정** 상자에 나열된 복사 일정을 확인합니다. 설치 일정을 사용자 지정 하려면 **예약**을 클릭 하 고 필요에 따라 SQL Server 에이전트 일정을 조정 합니다. 이 일정은 백업 일정과 대략적으로 같아야 합니다.
...
20. **복원 작업** 아래 **일정** 상자에 나열된 복원 일정을 확인합니다. 설치 일정을 사용자 지정 하려면 **예약**을 클릭 하 고 필요에 따라 SQL Server 에이전트 일정을 조정한 후에 **확인**을 클릭 합니다. 이 일정은 백업 일정과 대략적으로 같아야 합니다.
