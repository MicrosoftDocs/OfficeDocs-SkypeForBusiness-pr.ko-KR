---
title: 'Lync Server 2013: 마이그레이션 및 동시 사용 cmdlet'
description: 'Lync Server 2013: 마이그레이션 및 동시 사용 cmdlet'
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
f1.keywords:
- NOCSH
TOCTitle: Migration and coexistence cmdlets
ms:assetid: ff1a56e0-e883-473d-92fe-ca77ea4eb63b
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Gg415682(v=OCS.15)
ms:contentKeyID: 48185968
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: eecd1adad53d8f917d7db7ca201b25442a7d5e85
ms.sourcegitcommit: d42a21b194f4a45e828188e04b25c1ce28a5d1ae
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 10/17/2020
ms.locfileid: "48561014"
---
# <a name="migration-and-coexistence-cmdlets-in-lync-server-2013"></a>Lync Server 2013의 마이그레이션 및 동시 사용 cmdlet

<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="https://msdn.microsoft.com/">

<div data-asp="https://msdn2.microsoft.com/asp">



</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**마지막으로 수정 된 항목:** 2012-06-26_

[Move-cslegacyuser](https://technet.microsoft.com/library/Gg413025(v=OCS.15)) cmdlet은 사용자 계정을 Office Communications server 2007 또는 Microsoft lync server 2010에서 Microsoft lync server 2013로 이동할 수 있는 방법을 제공 합니다. 예를 들어 Microsoft Lync Server 2013에서 Microsoft Lync Server 2010로 사용자 계정을 "뒤로" 이동 해야 하는 경우에는 [csuser](https://technet.microsoft.com/library/Gg398528(v=OCS.15)) cmdlet을 사용 합니다.

  - <span></span>  
    [Import-cslegacyconferencedirectory](https://technet.microsoft.com/library/Gg398418(v=OCS.15))

<!-- end list -->

  - <span></span>  
    [Import-cslegacyconfiguration](https://technet.microsoft.com/library/Gg412923(v=OCS.15))

<!-- end list -->

  - <span></span>  
    [병합-Merge-cslegacytopology](https://technet.microsoft.com/library/Gg425870(v=OCS.15))

<!-- end list -->

  - <span></span>  
    [CsApplicationEndpoint 이동](https://technet.microsoft.com/library/Gg398188(v=OCS.15))

<!-- end list -->

  - <span></span>  
    [이동-Move-cslegacyuser](https://technet.microsoft.com/library/Gg413025(v=OCS.15))

<!-- end list -->

  - [변환-CsUserData](https://technet.microsoft.com/library/JJ205337(v=OCS.15))

  - [수출-CsUserData](https://technet.microsoft.com/library/JJ204897(v=OCS.15))

  - [가져오기-CsUserData](https://technet.microsoft.com/library/JJ205373(v=OCS.15))

  - [업데이트-CsUserData](https://technet.microsoft.com/library/JJ205358(v=OCS.15))

<div>

## <a name="see-also"></a>참고 항목


[Lync Server PowerShell 블로그](https://go.microsoft.com/fwlink/p/?linkid=203150)  
  

</div>

</div>

<span> </span>

</div>

</div>

</div>

