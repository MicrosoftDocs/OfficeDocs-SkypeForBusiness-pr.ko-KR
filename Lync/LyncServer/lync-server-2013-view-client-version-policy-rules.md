...
클라이언트 버전 정책은 클라이언트 버전 정책 규칙 집합으로 구성 됩니다. 이러한 규칙은 사용자가 특정 클라이언트 및 클라이언트 버전을 사용하여 로그온을 시도할 때 수행할 동작을 정의합니다. Lync Server 2013 제어판 또는 Lync Server 2013 관리 셸에서 클라이언트 버전 정책 규칙을 볼 수 있습니다.
...
5.  **클라이언트 버전 정책 편집** 페이지에 규칙이 표시 됩니다. 규칙에 대 한 세부 정보를 보려면 규칙을 선택한 다음 **세부 정보 표시**를 클릭 합니다.
...
자세한 내용은 [Get CsClientVersionPolicyRule](https://docs.microsoft.com/powershell/module/skype/Get-CsClientVersionPolicyRule) cmdlet에 대 한 도움말 항목을 참조 하세요.
