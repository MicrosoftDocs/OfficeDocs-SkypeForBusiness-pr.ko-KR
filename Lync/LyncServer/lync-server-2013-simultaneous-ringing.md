...
호출한 상대방이 동시 신호음을 사용 하도록 설정 되어 있는 경우, Location-Based 라우팅은 발신자의 위치와 호출한 파티의 끝점을 분석 하 여 통화를 라우팅해야 하는지 여부를 결정 합니다.
...
<th>알 수 없는 네트워크 사이트에 있거나 Location-Based 라우팅을 사용 하도록 설정 되지 않음</th>
</tr>
</thead>
<tbody>
<tr class="odd">
...
다음 표에서는 동일한 네트워크 사이트, 다른 네트워크 사이트 또는 알 수 없는 네트워크 사이트에서 Lync 사용자 (예: Lync 발신자) 로부터의 통화를 보여 줍니다. 수신자는 PSTN 끝점 (즉, 휴대폰)을 동시 링 대상으로 구성 합니다. 이 시나리오에서 라우팅을 Location-Based 호출을 수신자의 동시 링 대상 (예: 휴대폰)으로 라우팅해야 하는지 결정 합니다.
...
<td><p>발신자의 사이트 음성 라우팅 정책을 통해 허용 되는 동시 벨 울림</p></td>
<td><p>발신자의 음성 정책을 통해 허용 되는 동시 링이 Location-Based 라우팅에 대해 사용 하도록 설정 되지 않음 트렁크</p></td>
</tr>
</tbody>
</table>
