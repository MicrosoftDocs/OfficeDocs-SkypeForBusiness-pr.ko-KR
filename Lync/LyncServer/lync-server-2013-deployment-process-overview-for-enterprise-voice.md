---
title: 'Lync Server 2013: Enterprise Voice에 대한 배포 프로세스 개요'
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
TOCTitle: Deployment process overview for Enterprise Voice
ms:assetid: cf92adbe-aa90-4b05-8e1a-f3794ca68132
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Gg398878(v=OCS.15)
ms:contentKeyID: 48185526
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: 0eaccdf8aad78b35fb773a30082e0c8d4ced52af
ms.sourcegitcommit: bb53f131fabb03a66f0d000f8ba668fbad190778
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 05/11/2019
ms.locfileid: "40980180"
---
<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="http://msdn.microsoft.com/en-us/">

<div data-asp="http://msdn2.microsoft.com/asp">

# <a name="deployment-process-overview-for-enterprise-voice-in-lync-server-2013"></a>Lync Server 2013의 Enterprise Voice에 대한 배포 프로세스 개요

</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**마지막으로 수정한 주제:** 2012-09-22_

따라야 하는 배포 및 구성 단계는 Enterprise Voice 기능 또는 Lync Server 2013 환경에 추가 하는 기능에 따라 달라 집니다.

<div>

## <a name="feature-deployment-overviews"></a>기능 배포 개요

PSTN 연결 배포에 대 한 개요는 다음을 참조 하세요.

  - [Lync Server 2013에 대한 SIP 트렁크 배포 검사 목록](lync-server-2013-sip-trunk-deployment-checklist.md)

  - [Lync Server 2013의 직접 SIP 배포 옵션](lync-server-2013-direct-sip-deployment-options.md)

  - [Lync Server 2013에서 아웃바운드 음성 라우팅 계획](lync-server-2013-planning-outbound-voice-routing.md)

Exchange UM (통합 메시징) 배포에 대 한 개요는 다음을 참조 하세요.

  - [온-프레미스 통합 메시징과 Lync Server 2013의 통합을 위한 배포 프로세스](lync-server-2013-deployment-process-for-integrating-on-premises-unified-messaging.md)

통화 허용 제어 배포에 대 한 개요는 다음 항목을 참조 하세요.

  - [Lync Server 2013의 통화 허용 제어 서비스에 대한 배포 검사 목록](lync-server-2013-deployment-checklist-for-call-admission-control.md)

응급 서비스 배포 프로세스에 대 한 개요는 다음을 참조 하세요.

  - [Lync Server 2013에서 응급 전화에 대한 요구 사항 정의](lync-server-2013-defining-your-requirements-for-emergency-calls.md)

  - [Lync Server 2013의 E9-1-1 서비스 공급자 선택](lync-server-2013-choosing-an-e9-1-1-service-provider.md)

  - [Lync Server 2013의 E9-1-1에 대 한 배포 검사 목록](lync-server-2013-deployment-checklist-for-e9-1-1.md)

개인 전화선을 배포 하는 방법에 대 한 개요는 [Lync Server 2013를 사용 하 여 사설 전화선을 계획](lync-server-2013-planning-for-private-telephone-lines.md)하는 "혼합 배포의 비공개 전화선" 섹션을 참조 하세요.

통화 처리 기능 (통화 주차, 알림 신청 및 응답 그룹) 배포에 대 한 개요는 다음을 참조 하세요.

  - [Lync Server 2013의 통화 공원 배포 프로세스](lync-server-2013-deployment-process-for-call-park.md)

  - [Lync Server 2013의 알림 신청에 대 한 배포 프로세스](lync-server-2013-deployment-process-for-the-announcement-application.md)

</div>

</div>

<span> </span>

</div>

</div>

</div>

