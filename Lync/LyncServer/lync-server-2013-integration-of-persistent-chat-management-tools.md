---
title: 'Lync Server 2013: 영구 채팅 관리 도구 통합'
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
f1.keywords:
- NOCSH
TOCTitle: Integration of Persistent Chat management tools
ms:assetid: 5231aec7-5046-4052-800b-3b63806b9558
ms:mtpsurl: https://technet.microsoft.com/en-us/library/JJ688057(v=OCS.15)
ms:contentKeyID: 49733651
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: f1a2d5bf8d3cc15051799f24895e7eb886cdfc08
ms.sourcegitcommit: 831d141dfc5a49dd764cb296b73b63e5a9f8e599
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 02/21/2020
ms.locfileid: "42214564"
---
<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="https://msdn.microsoft.com/">

<div data-asp="https://msdn2.microsoft.com/asp">

# <a name="integration-of-persistent-chat-management-tools-in-lync-server-2013"></a>Lync Server 2013의 영구 채팅 관리 도구 통합

</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**마지막으로 수정 된 항목:** 2012-08-16_

Lync Server 2013에서는 관리 도구를 나머지 Lync Server에 사용 된 도구와 통합 하 여 영구 채팅 서버를 간편 하 게 관리할 수가 있습니다.

영구 채팅 서버에는 Lync Server 제어판에 통합 된 관리 사용자 인터페이스 환경이 포함 되어 있습니다. 또한 영구 채팅 서버에는 영구 채팅 서버 범주를 관리 하 고 관리 하는 Windows PowerShell cmdlet 모음 (채팅방 삭제 및 사용 되지 않는 콘텐츠 제거) 및 추가 기능이 포함 되어 있습니다.

<div>

## <a name="see-also"></a>참고 항목


[Lync Server 2013의 영구 채팅 서버 개요](lync-server-2013-overview-of-persistent-chat-server.md)  


[Lync Server 2013, 영구 채팅 서버 관리](managing-lync-server-2013-persistent-chat-server.md)  
  

</div>

</div>

<span> </span>

</div>

</div>

</div>

