...
<p>Windows PowerShell의 원격 인스턴스를 사용 하 여 실행 하는 경우 Get-cskerberosaccountassignment cmdlet을 실행 하는 권한이 있는 RBAC 역할을 사용자에 게 할당 해야 합니다. 이 cmdlet을 사용할 수 있는 모든 RBAC 역할의 목록을 보려면 Windows PowerShell 프롬프트에서 다음 명령을 실행 합니다.</p>
...
Get-cskerberosaccountassignment cmdlet을 사용 하면 Kerberos 계정이 지정 된 사이트와 연결 되었는지,이 계정이 제대로 구성 되었는지, 계정이 예상 대로 작동 하는지 확인할 수 있습니다. Kerberos 계정은 IIS (인터넷 정보 서비스)를 실행 하는 사이트의 모든 컴퓨터에 대 한 인증 주체로 작동할 수 있는 컴퓨터 계정입니다. 이러한 계정은 Kerberos 인증 프로토콜을 사용 하기 때문에, 계정을 Kerberos 계정 이라고 하며, 새 인증 프로세스를 Kerberos 웹 인증 이라고 합니다. 이렇게 하면 단일 계정을 사용 하 여 모든 IIS 서버를 관리할 수 있습니다.
...
기본적으로 Get-cskerberosaccountassignment는 화면에 거의 출력이 표시 되지 않습니다. 대신, cmdlet에서 반환 된 정보가 HTML 파일에 기록 됩니다. 따라서 Test-Get-cskerberosaccountassignment를 실행할 때마다 Verbose 매개 변수와 Report 매개 변수를 포함 하는 것이 좋습니다. Verbose 매개 변수는 cmdlet이 실행 되는 동안 약간 더 자세한 출력을 화면에 제공 합니다. Report 매개 변수를 사용 하면 Get-cskerberosaccountassignment에서 생성 되는 HTML 파일의 파일 경로와 파일 이름을 지정할 수 있습니다. 보고서 매개 변수를 포함 하지 않는 경우 HTML 파일이 사용자 폴더에 자동으로 저장 되 고 다음과 같은 이름이 지정 됩니다. ce84964a-c4da-c4da-4622-ad3-c54ff3ed36-1.html.

Get-cskerberosaccountassignment를 실행할 때 사이트 Id도 지정 해야 합니다. Kerberos 계정은 사이트 범위에서 할당 됩니다.

다음 명령은 Get-cskerberosaccountassignment를 실행 하 고 출력을 C:\\Logs\\KerberosTest 이라는 파일에 저장 합니다.
...
Get-cskerberosaccountassignment cmdlet은 성공 또는 실패를 나타내는 간단한 표시를 반환 하지 않습니다. 대신 Internet Explorer를 사용 하 여 생성 된 HTML 파일을 확인 해야 합니다.
...
다음은 Get-cskerberosaccountassignment에서 오류가 발생할 수 있는 몇 가지 일반적인 이유입니다.
