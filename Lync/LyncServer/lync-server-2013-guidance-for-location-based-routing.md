---
title: 'Lync Server 2013: Location-Based 라우팅에 대 한 지침'
description: 'Lync Server 2013: Location-Based 라우팅에 대 한 지침입니다.'
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
f1.keywords:
- NOCSH
TOCTitle: Guidance for Location-Based Routing
ms:assetid: ce88cb39-f486-46d4-af48-cdf047ad3444
ms:mtpsurl: https://technet.microsoft.com/en-us/library/JJ994074(v=OCS.15)
ms:contentKeyID: 51803985
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: 4759fe077c754bb75b6c17e72c746a7668bba82d
ms.sourcegitcommit: d42a21b194f4a45e828188e04b25c1ce28a5d1ae
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 10/17/2020
ms.locfileid: "48564144"
---
# <a name="guidance-for-location-based-routing-in-lync-server-2013"></a>Lync Server 2013의 Location-Based 라우팅에 대 한 지침

<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="https://msdn.microsoft.com/">

<div data-asp="https://msdn2.microsoft.com/asp">



</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**마지막으로 수정 된 항목:** 2013-02-21_

Location-Based 라우팅을 사용자의 끝점 네트워크 사이트 위치나 PSTN 게이트웨이의 네트워크 사이트 위치에 적용할 수 있습니다. 이 항목에서는 Location-Based 라우팅이 적용 되는 방식에 대 한 지침을 제공 합니다.

<div>

## <a name="in-this-section"></a>이 섹션의 내용

  - [Lync Server 2013의 사용자 위치](lync-server-2013-user-s-location.md)

  - [Lync Server 2013의 PSTN 게이트웨이 위치](lync-server-2013-pstn-gateway-s-location.md)

</div>

<div>

## <a name="see-also"></a>참고 항목


[Lync Server 2013의 Location-Based 라우팅 계획](lync-server-2013-planning-for-location-based-routing.md)  
  

</div>

</div>

<span> </span>

</div>

</div>

</div>

