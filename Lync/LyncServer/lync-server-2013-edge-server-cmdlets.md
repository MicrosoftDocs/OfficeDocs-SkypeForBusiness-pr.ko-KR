...
# <a name="edge-server-cmdlets-in-lync-server-2013"></a>Lync Server 2013의 Edge 서버 cmdlet
...
Edge 서버는 내부 네트워크에 로그인 하지 않은 사용자에 게 Microsoft Lync Server 2013의 기능을 확장할 수 있는 방법을 제공 합니다. 예를 들어 내부 네트워크를 통하지 않고 인터넷을 통해 Lync Server 2013에 로그온 하는 원격 사용자가 인증 된 사용자가 있는 경우, Lync Server 액세스 Edge 서비스를 실행 하는 Edge 서버를 설정 해야 합니다. 또한 다른 조직과의 페더레이션을 설정 하려는 경우 또는 사용자에 게 Yahoo\!, AOL, MSN 등의 공공 인스턴트 메시징 서비스와 계정이 있는 사용자에 게 통신 하는 권한을 부여 하려는 경우에는 Edge 서버가 필요 합니다.
...
## <a name="edge-server-cmdlets"></a>Edge 서버 Cmdlet

다음은 Edge 서버 관리와 직접 관련 된 cmdlet의 목록입니다.
