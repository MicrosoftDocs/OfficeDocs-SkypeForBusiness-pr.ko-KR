...
> [!IMPORTANT]  
> Lync Windows 스토어 앱에 대 한 다단계 인증을 사용 하도록 AD FS를 구성 하려는 경우에는 다음과 같은 중요 한 사항을 고려해 야 합니다. 
> <UL>
> <LI>
> <P>Lync server 2013에 대 한 누적 업데이트는 최소 2013 6 월 2013 일 필수입니다. Lync 2013 데스크톱 클라이언트는 lync Server 2013 2013에 대 한 누적 업데이트가 필요 하지 않으므로 Lync 2013 클라이언트가 인증을 수행할 수 있기 때문에 수동 인증이 작동 하는 것 처럼 보일 수 있습니다. 그러나 Lync Windows 스토어 앱 클라이언트에 대 한 인증 프로세스가 완료 되지 않으며 알림 또는 오류 메시지가 표시 되지 않습니다.</P>
> <LI>
> <P>수동 인증이 제공 되는 유일한 인증 유형이 되도록 서버를 구성 해야 합니다.</P>
> <LI>
> <P>하드웨어 부하 분산 장치를 사용 하는 경우에는 Lync Windows 스토어 앱 클라이언트의 모든 요청이 동일한 프런트 엔드 서버에서 처리 되도록 부하 분산 장치에서 쿠키 지 속성을 사용 하도록 설정 합니다.</P>
> <LI>
> <P>Lync Server와 AD FS 서버 간에 신뢰 당사자 트러스트를 설정 하는 경우 Lync 모임의 최대 길이를 확장 하는 데 충분 한 길이의 토큰 수명을 할당 합니다. 일반적으로 240 분의 토큰 수명은 충분 합니다.</P></LI></UL>
...
## <a name="the-time-and-date-are-not-set-accurately-on-the-device-running-lync-windows-store-app"></a>Lync Windows 스토어 앱을 실행 하는 장치에 시간 및 날짜가 정확히 설정 되지 않음

장치의 시간 설정은 서버의 시간 설정과 동기화 되어야 합니다. 이는 Microsoft Surface와 같은 장치 및 도메인에 가입 되지 않은 Windows RT를 실행 하는 기타 장치에 특히 중요 합니다. 시간 서버에서 이러한 장치에 대 한 시간을 자동으로 설정 하려면 장치의 관리자 권한 명령 프롬프트에서 다음 명령을 실행 합니다.
```console
w32tm /resync
```
</div>
...
## <a name="lync-windows-store-app-cannot-access-the-lync-server-or-services"></a>Lync Windows 스토어 앱이 Lync server 또는 서비스에 액세스할 수 없음

Lync Windows 스토어 앱은 Windows 8을 실제 장치로 등록 하지 않은 네트워크 어댑터 (예: 4G LTE USB 모뎀)를 통해 Lync server 또는 서비스에 액세스 하지 못할 수 있습니다. 데스크톱 앱과 브라우저에서 다른 서버 및 웹 사이트에 액세스할 수 있는 경우에도 Lync Windows 스토어 앱이이 문제가 발생할 수 있습니다.

</div>

<div>

## <a name="lync-windows-store-app-cannot-sign-in-with-lync-server-2010-and-office-communications-server-2007-r2-edge-server"></a>Lync Windows 스토어 앱은 Lync Server 2010 및 Office Communications Server 2007 R2에 지 서버를 사용 하 여 로그인 할 수 없음

토폴로지가 Office Communications Server 2007 R2에 지 서버를 사용 하 여 Lync Server 2010로 구성 된 경우 Lync Server 용 누적 업데이트 2010:7 월 2013에 제공 되는 업데이트 된 버전의 토폴로지 작성기를 실행 해야 합니다. 이전 버전의 토폴로지 작성기에서는 Office Communications Server 2007에 지 서버에 필요한 매핑을 만들지 않으므로 Lync Windows 스토어 앱 클라이언트에서 로그인 할 수 없습니다. 다음 단계를 수행 해야 합니다.

1.  Lync server 2010에 대 한 누적 업데이트를 설치 합니다 2010 2010 2013. |

2.  다음을 수행 하 여 Lync 자동 검색 구성을 업데이트 하 여 외부 SIP 진입점이에 지 서버 주소 임을 나타냅니다.
...
2.  화면 오른쪽에서 살짝 밀기 마우스를 사용 하는 경우 화면의 오른쪽 위 모서리를 가리킨 다음 마우스 포인터를 화면 아래로 이동 합니다.
...
4.  이전에 **진단 로그가** 해제 된 경우 Lync를 다시 시작 해야 합니다. Lync를 다시 시작 하려면 다음 중 하나를 수행 합니다.
      ...
      - Lync 작업을 종료 하 고 앱을 다시 시작 합니다. 작업을 종료 하려면 Windows 작업 관리자를 열고 **Lync**를 선택한 다음 **작업 끝내기**를 탭 합니다. Lync가 나열 되지 않으면 **세부 정보** 를 탭 하 고 **백그라운드 프로세스**에서 Lync를 찾습니다.
...
3.  화면 오른쪽에서 살짝 밀기 마우스를 사용 하는 경우 화면의 오른쪽 위 모서리를 가리킨 다음 마우스 포인터를 화면 아래로 이동 합니다.
