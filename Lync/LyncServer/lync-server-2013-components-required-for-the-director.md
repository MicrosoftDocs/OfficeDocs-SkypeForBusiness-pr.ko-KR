...
단일 디렉터 서버 또는 디렉터 풀을 구현할 수 있습니다. 디렉터는 Lync Server 2013의 다른 서버 역할에 collocated 되지 않고 항상 별도의 서버 또는 풀입니다.

<div>


> [!NOTE]  
> 디렉터를 배포 하지 않으면 프런트 엔드 서버 또는 프런트 엔드 풀은 디렉터 역할을 가정 합니다.



</div>

디렉터 풀은 부하 분산 되어야 합니다. 다음 중 하나를 실행할 수 있습니다.

  - 웹 서비스에 대 한 하드웨어 부하 분산 장치 및 다른 트래픽 유형에 대 한 DNS (Domain Name System) 부하 분산을 사용 하는 토폴로지를 만듭니다.
