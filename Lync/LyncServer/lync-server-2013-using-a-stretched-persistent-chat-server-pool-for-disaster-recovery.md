...
고가용성을 위해 SQL Server 미러링을 구성 하는 방법에 대 한 자세한 내용은 [Lync server 2013에서 Sql server 미러링을](lync-server-2013-sql-server-mirroring.md)참조 하세요. 재해 복구용으로 데이터베이스를 장애 조치 하는 방법에 대 한 자세한 내용은 lync server [2013에서 영구 채팅 서버 기본 데이터베이스에 대 한 Sql Server 로그 전달을 설정](lync-server-2013-setting-up-sql-server-log-shipping-for-the-persistent-chat-server-primary-database.md) 하 고 [lync server 2013에서 기본 미러 서버와 로그 전달 보조 데이터베이스 간에 sql Server 로그 전달을 설정](lync-server-2013-setting-up-sql-server-log-shipping-between-the-primary-mirror-and-the-log-shipping-secondary-database.md)하는 방법에 대해 알아보세요.
