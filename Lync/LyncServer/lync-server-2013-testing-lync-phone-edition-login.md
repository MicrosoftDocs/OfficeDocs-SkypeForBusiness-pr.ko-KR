...
<p>Windows PowerShell의 원격 인스턴스를 사용 하 여 실행 하는 경우 사용자에 게 Test-CsPhoneBootstrap cmdlet을 실행 하는 권한이 있는 RBAC 역할을 할당 해야 합니다. 이 cmdlet을 사용할 수 있는 모든 RBAC 역할의 목록을 보려면 Windows PowerShell 프롬프트에서 다음 명령을 실행 합니다.</p>
...
오류: 오류-Web-Ticket 서비스에 대 한 응답을 받지 못했습니다.
...
Test-CsPhoneBootstrap 실패 하면 다음 시간에 Verbose 매개 변수를 포함 하 여 테스트를 다시 실행할 수 있습니다.

    Test-CsPhoneBootstrap -PhoneOrExt "+12065551219" -Pin "0712" -Verbose

Verbose 매개 변수를 포함 하면 Test-CsPhoneBootstrap는 지정 된 사용자가 Lync Server에 로그온 할 수 있는지 확인할 때 시도한 각 작업의 단계별 계정을 반환 합니다. 예를 들어 다음과 같이 잘못 된 PIN이 포함 된 세션 인 경우 실패 한 로그온에 대 한 출력 부분을 확인할 수 있습니다.
...
Test-CsPhoneBootstrap 실패할 수 있는 몇 가지 일반적인 이유는 다음과 같습니다.
  ...
  - 등록자 풀이 DHCP를 사용할 수 없습니다. 등록자 풀이 DHCP에 대해 사용 하도록 설정 되어 있는지 확인 하려면 Get-CsRegistrarConfiguration cmdlet을 실행 하 여 EnableDHCPServer 속성의 값을 확인 합니다. 예를 들면 다음과 같습니다.
