...
# <a name="scaled-consolidated-edge-dns-load-balancing-with-public-ip-addresses-in-lync-server-2013"></a>Lync Server 2013의 조정된 통합 에지, 공용 IP 주소의 DNS 부하 분산
...
Edge 서버 풀 토폴로지에서는 두 개 이상의 Edge 서버가 데이터 센터의 경계 네트워크에 부하 분산 풀로 배포 됩니다. DNS (Domain Name System) 부하 분산이 외부 및 내부 Edge 인터페이스에 대 한 트래픽 소통에 사용 됩니다.

조직에서 15000 이상의 액세스에 지 서비스 클라이언트 연결을 지원 해야 하는 경우, 1000 활성 Lync Server 웹 회의 서비스 클라이언트 연결 또는 500 동시 A/V Edge 세션을 사용 하거나, Edge 서버의 가용성을 높게 하는 것이 중요 한 경우이 토폴로지에서는 확장성 및 장애 조치 지원의 이점을 제공 합니다.
