...
Lync Server 2010에 자동 검색이 도입 된 경우 기존 서버 배포에 대 한 잠재적인 인증서 변경이 필요한 서비스를 구현 하기 위해 수행 해야 하는 손상이 있었습니다. 자동 검색은 HTTPS에 대 한 포트 TCP 443 또는 HTTP에 대 한 포트 TCP 80를 통해 사용할 수 있습니다. HTTPS를 사용 하기로 결정 한 경우 필요한 및 DNS 레코드를 수용할 수 있도록 역방향 프록시, 디렉터 및 프런트 엔드 서버의 인증서를 다시 발급 해야 `lyncdiscover.<domain>` `lyncdiscoverinternal.<domain>` 합니다. 결정이 HTTP를 사용 하는 경우에는 DNS CNAME (또는 별칭) 레코드를 사용 하 여 인증서의 기존 이름을 사용 하 여 인증서를 다시 발급 하지 못할 수 있습니다. HTTP를 사용 하는 경우 초기 통신이 암호화 되지 않았음을 의미 합니다.

Lync Server 2013에서는 모든 클라이언트에 자동 검색을 사용 하기 때문에 주요 시나리오는 HTTPS를 독점적으로 사용 하 고 lyncdiscover를 사용 하 여 인증서를 만드는 것입니다.\<domain\> 역방향 프록시, 디렉터 및 프런트 엔드 서버 구성의 일부분입니다. Lync Server 2010에서 업그레이드 된 배포에 자동 검색을 구현 하는 경우 HTTP를 사용 하 여 인증서를 재발급 하지 않을 수 있습니다. 두 시나리오에 대 한 지침은 다음 섹션에서 제공 됩니다.
