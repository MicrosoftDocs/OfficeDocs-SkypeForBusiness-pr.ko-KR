...
용량 계획 시 다음 두 대역폭을 고려합니다.
  ...
  - FEC 없는 최대 **대역폭**     FEC 없이 시나리오에 사용 되는 일반적인 코덱 및 스트림의 일반적인 작업을 포함 하 여 스트림이 소비 하는 최대 대역폭입니다.이는 스트림이 100% activity이 고 FEC 사용을 트리거하는 패킷 손실이 없는 경우의 대역폭입니다.이는 지정 된 시나리오에서 코덱을 사용 하도록 허용 하기 위해 할당 해야 하는 대역폭의 양을 계산 하는 데 유용 합니다. 

  - FEC의 최대 **대역폭**     스트림의 일반적인 작업 및 FEC을 사용 하는 시나리오에서 사용 되는 일반적인 코덱의 등 스트림이 소비 되는 최대 대역폭입니다. 이는 stream이 100% activity이 고 FEC 사용을 트리거하여 품질을 개선 하기 위해 패킷 손실이 발생 하는 경우의 대역폭입니다. 이는 지정 된 시나리오에서 코덱을 사용 하도록 허용 하기 위해 할당 해야 하는 대역폭의 양을 계산 하 고 FEC을 사용 하 여 패킷 손실 조건에 품질을 유지할 수 있도록 하는 데 관심을 두어야 합니다. 
