...
# <a name="dns-summary---single-consolidated-edge-with-public-ip-addresses-in-lync-server-2013"></a>Lync Server 2013의 DNS 요약 - 공용 IP 주소의 단일 통합 에지
...
다음과 같이 Edge 서버에서 두 개의 네트워크 어댑터를 구성할 수 있습니다.
  ...
  - **네트워크 어댑터 2 (외부 인터페이스)**
    ...
    이 네트워크 어댑터에는 세 가지 공용 IP 주소 (예: 액세스 131.107.155.10 용), 131.107.155.20 for 웹 회의 Edge, AV Edge에 대 한 131.107.155.30이 지정 됩니다.
    ...
    액세스에 지 공용 IP 주소는 기본 게이트웨이가 공용 라우터 (131.107.155.1)로 설정 된 기본입니다.
    
    웹 회의 및 A/V Edge 공용 IP 주소는 Windows Server의 **로컬 영역 연결 속성** 에 대 한 **인터넷 프로토콜 버전 4 (Tcp/ip)** 및 **ipv6(인터넷 프로토콜 버전 6 (tcp/ip)** 의 **고급** 섹션에 있는 추가 IP 주소입니다.
...
### <a name="dns-records-required-for-single-consolidated-edge-with-public-ip-addresses-example"></a>공용 IP 주소를 사용 하 여 통합 된 단일 Edge에 필요한 DNS 레코드 (예)
...
> [!IMPORTANT]
> 이전 표에 나열 된 레코드는 <EM>.net</EM> 확장 또는 <EM>.com</EM> 확장명으로 표시 되어 분할 하는 DNS를 사용 하지 않는 경우에는 해당 영역이 있어야 하는 영역을 강조 표시 합니다. 분할 하는 DNS를 사용 하는 경우에는 모든 레코드가 같은 영역에 있고 내부 또는 외부 버전에 있는지만 구별 됩니다. 자세한 내용은 <A href="lync-server-2013-determine-dns-requirements.md">Lync Server 2013의 dns 요구 사항 결정</A>에서 "분할-두뇌 dns"를 참조 하세요.
