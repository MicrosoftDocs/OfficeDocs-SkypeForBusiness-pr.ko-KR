---
title: 'Lync Server 2013: 통화 대기 응용 프로그램 cmdlet'
description: 'Lync Server 2013: 통화 대기 응용 프로그램 cmdlet'
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
f1.keywords:
- NOCSH
TOCTitle: Call Park application cmdlets
ms:assetid: 30cc001f-b29e-4d44-bad7-65e1133e67b1
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Gg415639(v=OCS.15)
ms:contentKeyID: 48183764
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: 25d37e8cfad022e9e8478e68f158e1838bdcca5b
ms.sourcegitcommit: d42a21b194f4a45e828188e04b25c1ce28a5d1ae
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 10/17/2020
ms.locfileid: "48570094"
---
# <a name="call-park-application-cmdlets-in-lync-server-2013"></a>Lync Server 2013의 통화 대기 응용 프로그램 cmdlet

<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="https://msdn.microsoft.com/">

<div data-asp="https://msdn2.microsoft.com/asp">



</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**마지막으로 수정 된 항목:** 2012-03-21_

통화 대기 응용 프로그램은 사용자가 통화를 보류 한 다음 다른 휴대폰에서 해당 통화를 검색할 수 있도록 허용 합니다. 이러한 cmdlet을 사용 하 여 통화 대기 궤도 및 통화 대기 응용 프로그램에 대 한 설정을 구성 합니다.

<div>

## <a name="call-park-application-cmdlets"></a>통화 대기 응용 프로그램 Cmdlet

다음 cmdlet을 사용 하 여 통화 대기 응용 프로그램을 관리할 수 있습니다.

**통화 대기 응용 프로그램**

  - <span></span>  
    [Get-cscallparkorbit](https://technet.microsoft.com/library/Gg398554(v=OCS.15))

  - <span></span>  
    [Get-cscallparkorbit](https://technet.microsoft.com/library/Gg398936(v=OCS.15))

  - <span></span>  
    [Get-cscallparkorbit을 제거 합니다.](https://technet.microsoft.com/library/Gg412901(v=OCS.15))

  - <span></span>  
    [Get-cscallparkorbit](https://technet.microsoft.com/library/Gg398796(v=OCS.15))

<!-- end list -->

  - <span></span>  
    [Set-cscallparkservicemusiconholdfile](https://technet.microsoft.com/library/Gg412836(v=OCS.15))

<!-- end list -->

  - <span></span>  
    [New-cscpsconfiguration](https://technet.microsoft.com/library/Gg398948(v=OCS.15))

  - <span></span>  
    [New-cscpsconfiguration](https://technet.microsoft.com/library/Gg412919(v=OCS.15))

  - <span></span>  
    [New-cscpsconfiguration을 제거 합니다.](https://technet.microsoft.com/library/Gg398358(v=OCS.15))

  - <span></span>  
    [New-cscpsconfiguration](https://technet.microsoft.com/library/Gg412721(v=OCS.15))

</div>

<div>

## <a name="see-also"></a>참고 항목


[Lync Server PowerShell 블로그](https://go.microsoft.com/fwlink/p/?linkid=203150)  
  

</div>

</div>

<span> </span>

</div>

</div>

</div>

