...
- MSTeams 모듈 설치됨 ````  (Install-Module -Name MicrosoftTeams -Force -AllowClobber) ````
- MSOnline 모듈 설치됨 ```` Install-Module -Name MSOnline -Force -AllowClobber ````
2)  테넌트 관리 권한이 있습니다.
...
![cmdlet을 사용하여 빌드되는 자동 감쇠 호출 흐름의 다이어그램.](media/create-a-phone-system-auto-attendant-via-cmdlets.png)
