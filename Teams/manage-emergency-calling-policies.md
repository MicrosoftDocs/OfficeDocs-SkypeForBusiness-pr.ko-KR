...
2. **추가** 를 클릭합니다.
3. 정책의 이름과 설명을 입력합니다.
4. 응급 전화가 걸려 오면 조직 내 사용자에게 일반적으로 보안 센터에 알리는 방법을 설정할 수 있습니다. 이렇게하려면 알림 모드에서 **다음** 중 하나를 선택합니다.
    - **알림 보내기만:** 지정한 사용자 및 그룹에 Teams 채팅 메시지가 전송됩니다.
    - **음소거 및** 음소거를 음소거할 수 없는 회의: Teams 채팅 메시지는 사용자가 지정한 사용자 및 그룹으로 보내지며 발신자 및 PSAP 운영자 간의 대화에서 듣기(참여하지 않을 수 없습니다)입니다.
    - **음소거** 중이지만 음소거를 퇴치할 수 있습니다. Teams 채팅 메시지는 사용자가 지정한 사용자 및 그룹으로 보내지며 발신자와 PSAP 운영자 간의 대화를 듣고 참여하기 위해 음소거를 퇴장할 수 있습니다.
5.  음소거된 알림  모드에서 회의를 선택한 경우 전화  번호에서 긴급 통화 알림 상자에 전화를 걸 사용자 또는 그룹의 PSTN 전화 번호를 입력하여 전화를 걸고 긴급 통화에 참가할 수 있습니다. 예를 들어, 조직의 보안 데스크 번호를 입력합니다. 긴급 전화가 걸려오면 전화를 받고 통화를 들을 수 있는 사람. 모드가 음소거된 회의로 설정되어 있지만 음소거를 할 수 있는 경우에도 PSTN 휴대폰의 음소거를언할 **수 없습니다.**
