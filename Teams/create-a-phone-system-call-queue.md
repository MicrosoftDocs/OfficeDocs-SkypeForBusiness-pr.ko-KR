...
1. **리소스 계정** 에서 **추가** 단추를 선택하여 이 호출 큐에 대한 리소스 계정을 추가합니다.
...
1. **리소스 계정** 에서 **추가** 단추를 선택하여 리소스 계정을 추가합니다.
1. **계정 추가** 창에서 결과 드롭다운을 끌어올 문자 집합을 검색합니다.
1. 결과 아래쪽 **에서 + 리소스 계정 추가** 단추를 선택합니다.
1. **리소스 계정 추가** 창에서 다음을 수행합니다.
...
- [Microsoft Teams 전화 시스템 리소스 계정 라이선스를 할당합니다](manage-resource-accounts.md#assign-a-license).
...
> [!NOTE]
> Microsoft Teams 서비스에서 음악 또는 오디오 파일을 사용하는 데 필요한 모든 권리와 권한을 독립적으로 지우고 보호해야 하며, 여기에는 아티스트를 포함할 수 있는 모든 관련 권리 보유자의 오디오 파일에 지적 재산권 및 기타 음악, 음향 효과, 오디오, 브랜드, 이름 및 기타 콘텐츠가 포함될 수 있습니다.  배우, 공연자, 음악가, 작곡가, 작곡가, 음반 레이블, 음악 출판사, 노조, 길드, 권리 사회, 집단 관리 조직 및 음악 저작권, 음향 효과, 오디오 및 기타 지적 재산권을 소유, 제어 또는 허가하는 기타 당사자.
...
[Teams 채널을 사용하여 큐를 관리하려면 다음을 수행합니다](https://support.microsoft.com/office/9f07dabe-91c6-4a9b-a545-8ffdddd2504e).

1. **팀 선택** 라디오 단추를 선택하고 **채널 추가를** 선택합니다.
1. 사용하려는 팀을 검색하고 선택하고 **추가** 를 선택합니다.
1. 사용하려는 채널을 선택하고(표준 또는 비공개 채널만 지원됨) **적용** 을 선택합니다.
...
> 
> 비공개 채널에 팀 구성원의 하위 집합만 있는 경우에도 통화는 팀의 모든 구성원에게 배포됩니다.
...
1. **사용자 추가** 를 선택하고, 사용자를 검색하고, **추가** 를 선택한 다음, **추가** 를 선택합니다.

큐 **에 그룹을 추가** 하려면 다음을 수행합니다.

1. **그룹 추가** 를 선택하고, 그룹을 검색하고, **추가** 를 선택한 다음, **추가** 를 선택합니다. 
...
- Microsoft Teams 전화 시스템 버전 1449/1.0.94.2020051601 이상
...
> [!NOTE]
> 위치 기반 라우팅에 사용하도록 설정된 직접 라우팅 게이트웨이에서 큐로 라우팅되는 호출에는 회의 모드가 지원되지 않습니다.
>
> 비즈니스용 Skype 서버 큐로 라우팅되는 호출에는 회의 모드가 지원되지 않습니다.
> 
...
**통화 시간 제한: 최대 대기 시간은** 통화가 리디렉션되거나 연결이 끊어지기 전에 큐에서 통화가 보류될 수 있는 최대 시간을 지정합니다.
...
- Microsoft Teams Mac 클라이언트
- [가상화된 데스크톱 인프라](teams-for-vdi.md)의 Microsoft Teams(Windows Virtual Desktop, Citrix 및 VMware)
...
[Microsoft Teams 전화 시스템에서 얻을 수 있는 것은 다음과 같습니다.](here-s-what-you-get-with-phone-system.md)
