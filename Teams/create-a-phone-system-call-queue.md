...
description: Microsoft Teams 대규모 조직에 대한 통화 큐를 설정하는 방법을 알아봅니다. 통화 큐는 인사말 메시지를 제공하고, 음악을 누르고, 통화 리디렉션 및 기타 기능을 제공합니다.
...
- 호출 라우팅 - *FIFO(First In, First Out* ) 순서에서 에이전트로
...
Teams 데스크톱 사용자 및 표준 통화 큐의 경우 호출 큐의 멤버에 대한 호출자 ID를 호출 큐의 서비스 번호 또는 적절한 자동 전화 교환으로 설정하는 것이 좋습니다. 자세한 내용은 [Microsoft Teams 호출자 ID 정책 관리를 참조](caller-id-policies.md)하세요.
...
큐에 개별 사용자 또는 그룹을 추가하려면 **사용자 및 그룹 선택을 선택합니다**. 
...
|기능                          |Teams Desktop<sup>1</sup> |Teams Web | Teams Mobile<sup>2</sup> |Lync |IP 휴대폰 | 표준 호출 큐 |채널 기반 통화 큐 | 코멘트 |
|:--------------------------------|:------------------------:|:--------:|:--------------:|:---:|:--------:|:--------------------:|:------------------------:|:--------|
|**에이전트 라우팅 메서드**        |                          |          |                |     |          |                      |                          |   |
...
|**에이전트 라우팅 옵션**        |                          |          |                |     |          |                      |                          |   |
|`Presence Based Routing`<sup>3</sup>|Y                      |Y         |Y               |N    |Y         |Y                     |Y                         |*권장* |
|`Agents can Opt-out`               |Y                       |Y         |Y               |Y<sup>7</sup>|Y<sup>7</sup>|Y          |Y                         |*기본*     |
|**전송 모드**               |                          |          |                |     |          |                      |                          |   |
|`Conference Mode`<sup>5</sup>    |Y                         |Y         |Y               |N    |Y<sup>6</sup>|Y                  |Y                         |*권장* |
|`Transfer Mode`                  |Y                         |Y         |Y               |Y    |Y         |Y                     |Y                         |*기본*              |
|**공동 작업 통화**        |                          |          |                |     |          |                      |                          |   |
...
참고:
1. Microsoft Teams Windows 클라이언트, Microsoft Teams Mac 클라이언트는 가상화된 데스크톱 인프라에서 Microsoft Teams.
...
7. 의 사용자 설정 포털 페이지를 https://aka.ms/vmsettings통해
8. 공용 채널만 지원됩니다.
...
다음 클라이언트는 통화 큐의 통화 에이전트에 대해 지원됩니다.
  ...
  - Microsoft Teams Android 앱
    ...
    > [!NOTE]
    > 직접 라우팅 번호가 할당된 통화 큐는 비즈니스용 Skype 클라이언트, Lync 클라이언트 또는 비즈니스용 Skype IP Phone을 에이전트로 지원하지 않습니다. Teams 클라이언트는 [TeamsOnly의 공존 모드](/microsoftteams/setting-your-coexistence-and-upgrade-settings)에서만 지원됩니다.
