...
1. 결과 맨 아래에 **있는 + 리소스 계정 추가** 단추를 선택합니다.
...
> [!NOTE]
> ID를 호출하는 데 사용되는 리소스 계정에는 **Microsoft Teams 전화 Resource Account** 라이선스와 다음 중 하나가 할당되어 있어야 합니다.
>
...
1. 결과 맨 아래에 **있는 + 리소스 계정 추가** 단추를 선택합니다.
1. **리소스 계정 추가** 창에서 다음을 수행합니다.
    1. 호출 받는 사람에게 표시되는 설명 **이 포함된 표시 이름을** 입력합니다.
    1. 리소스 계정에 대한 설명이 포함된 **사용자 이름을** 입력합니다.
    1. **리소스 계정 유형** 드롭다운을 선택하고 **통화 큐를** 선택합니다.
1. 창 아래쪽에서 **저장** 단추를 선택합니다.
1. **리소스 계정** 창에서 **추가** 단추를 선택합니다.
...
이 언어는 시스템 생성 음성 프롬프트 및 음성 메일 기록을 사용하도록 설정하는 경우 사용됩니다.
...
> [!NOTE]
> Microsoft Teams 서비스에서 음악 또는 오디오 파일을 사용하는 데 필요한 모든 권한과 권한을 독립적으로 지우고 보호해야 하며, 여기에는 아티스트를 포함할 수 있는 모든 관련 권리 보유자의 오디오 파일에서 지적 재산권 및 기타 음악, 음향 효과, 오디오, 브랜드, 이름 및 기타 콘텐츠가 포함될 수 있습니다.  배우, 공연자, 음악가, 작곡가, 음반 레이블, 음악 출판사, 노조, 길드, 권리 사회, 집단 관리 조직 및 음악 저작권, 음향 효과, 오디오 및 기타 지적 재산권을 소유, 제어 또는 허가하는 기타 당사자.

인사말 및 대기 음악을 선택한 후 **통화 큐 추가** 페이지 아래쪽에 있는 **다음** 단추를 선택합니다.

# <a name="step-3-call-answering"></a>[3단계: 전화 응답](#tab/call-answering)

## <a name="step-3-set-up-who-will-answer-incoming-calls"></a>3단계: 수신 전화에 응답할 사용자 설정
...
1. 팀 라디오 **선택** 단추를 선택하고 **채널 추가를** 선택합니다.
1. 사용하려는 팀을 검색하고, 선택하고, **추가** 를 선택합니다.
1. 사용할 채널을 선택하고(표준 채널만 지원됨) **적용** 을 선택합니다.

통화 큐에 Teams 채널을 사용할 때 지원되는 클라이언트는 다음과 같습니다.
...
>
> 팀에 200명 이상의 구성원이 있는 경우 처음 200명의 멤버만 사전순으로 호출 큐에 에이전트로 추가됩니다.
...
1. **사용자 추가** 를 선택하고 사용자를 검색한 다음 **추가** 를 클릭한 다음 **추가** 를 클릭합니다.
...
**전화 회의 모드** 는 에이전트가 통화를 수락한 후 호출자가 에이전트에 연결하는 데 걸리는 시간을 줄입니다. 회의 모드가 작동하려면 통화 큐의 에이전트가 다음 클라이언트 중 하나를 사용해야 합니다.

- 최신 버전의 Microsoft Teams 데스크톱 클라이언트, Android 앱 또는 iOS 앱
- Microsoft Teams 전화 버전 1449/1.0.94.2020051601 이상
  
에이전트의 Teams 계정은 TeamsOnly 모드로 설정해야 합니다. 요구 사항을 충족하지 않는 에이전트는 통화 라우팅 목록에 포함되지 않습니다. 에이전트가 호환되는 클라이언트를 사용하는 경우 통화 큐에 전화 회의 모드를 사용하도록 설정하는 것이 좋습니다.
...
>
> 전화 통화가 비즈니스용 Skype 서버 큐로 라우팅되는 경우 회의 모드가 지원되지 않습니다.
> 
> Teams 사용자가 통화 큐를 사용하여 통화를 상담/전송해야 하는 경우 회의 모드가 필요합니다.
>
> 에이전트는 처음 통화에 참가할 때 최대 2초 동안 대기 중인 구성된 음악을 들을 수 있습니다.


통화 응답 옵션을 선택한 후 통화 **큐 추가** 페이지 아래쪽에 있는 **다음** 단추를 선택합니다.
...
- **직렬 라우팅** 은 모든 **콜 에이전트** 를 콜 에이전트 목록에 지정된 순서대로 하나씩 울립니다. 에이전트가 통화를 해제하거나 수신하지 않으면 호출이 다음 에이전트에 울림됩니다. 이 작업은 호출이 선택되거나 시간이 초과될 때까지 반복됩니다.
...
>
> **가장 긴 유휴 상태** 인 경우 에이전트가 사용할 수 없게 된 직후에 큐에서 전화를 받거나 사용할 수 있게 된 후 큐에서 전화를 받는 데 짧은 지연이 있을 수 있습니다.
>
...
### <a name="presence-based-call-routing"></a>현재 상태 기반 호출 라우팅

**현재 상태 기반 호출 라우팅** 은 호출 에이전트의 가용성 상태를 사용하여 선택한 라우팅 방법에 대한 호출 라우팅 목록에 에이전트를 포함해야 하는지 여부를 결정합니다.
...
모든 라우팅 방법으로 **프레즌스 기반 호출 라우팅** 을 사용하도록 설정할 수 있습니다.

에이전트가 통화를 옵트아웃하면 해당 에이전트가 수신 가능 여부가 설정된 상태와 관계없이 통화 라우팅 목록에 포함되지 않습니다.

> [!TIP]
> **현재 상태 기반 라우팅을** **켜** 는 것이 좋습니다.

> [!NOTE]
> **가장 긴 유휴** 상태를 라우팅 방법으로 선택하면 현재 상태 기반 라우팅 토글이 **꺼** 지고 회색으로 표시되더라도 현재 상태 기반 라우팅이 필요하고 자동으로 사용하도록 설정됩니다.
>
> 현재 상태 기반 라우팅을 사용하도록 설정하지 않고 큐에 여러 호출이 있는 경우 시스템은 현재 상태와 관계없이 에이전트에 이러한 호출을 동시에 표시합니다. 이 작업을 수행하면 특히 일부 에이전트가 표시되는 초기 호출에 응답하지 않는 경우 에이전트에 대한 여러 통화 알림이 발생합니다.
>
> **현재 상태 기반 라우팅을** 사용하는 경우 에이전트가 사용할 수 없게 된 직후 큐에서 전화를 받거나 사용할 수 있게 된 후 큐에서 전화를 받는 데 잠시 지연이 있을 수 있습니다.
>
...
### <a name="call-agents-can-opt-out-of-taking-calls"></a>통화 에이전트는 통화 수신을 옵트아웃할 수 있습니다.

호출 에이전트가 통화 수신을 옵트아웃할 수 있는지 여부를 지정할 수 있습니다.

통화 에이전트를 켜 **면 통화 수신을 옵트아웃할 수 있습니다**.
...
에이전트 호출 라우팅 옵션을 선택한 후 통화 **큐 추가** 페이지의 맨 아래에 있는 **다음** 단추를 선택합니다.
...
4. 표준 통화 큐의 일부로 개별 사용자를 추가할 때만 주문을 설정할 수 있습니다. 메일 그룹 또는 Teams 채널이 사용되는 경우 순서는 사전순입니다.
...
- Microsoft Teams Android 앱
  ...
  > [!NOTE]
  > 직접 라우팅 번호가 할당된 통화 큐는 비즈니스용 Skype 클라이언트, Lync 클라이언트 또는 비즈니스용 Skype IP 통화가 에이전트로 지원되지 않습니다. Teams 클라이언트는 [Teams만의 공존 모드](setting-your-coexistence-and-upgrade-settings.md)에서만 지원됩니다.
