...
- 채팅해야 하는 외부 도메인의 사용자가 있습니다. 예를 들어 Rob@contoso.com 및 Ann@northwindtraders.com은 contoso.com 및 northwindtraders.com 도메인의 다른 사람들과 함께 프로젝트를 진행하고 있습니다.
...
|:---------|:-----------------------|
|비즈니스용 Skype 온-프레미스가 아닌 사용자. 여기에는 TeamsOnly 사용자 및/또는 비즈니스용 Skype Online 사용자가 있는 조직이 포함됩니다.| Teams 관리 센터를 사용하는 경우: <br>- 통신하려는 도메인이 외부 액세스가 허용되는지 확인하세요.<br><br>PowerShell을 사용하는 경우:<br>- 테넌트가 페더레이션에 대해 `Get-CsTenantFederationConfiguration`이 `AllowFederatedUsers=true`를 표시하는지 확인합니다. <br>- `CsExternalAccessPolicy`의 사용자 유효 값이 `EnableFederationAccess=true`인지 확인하세요.<br>- 공개 페더레이션을 사용하지 않는 경우, 대상 도메인이 `CsTenantFederationConfiguration`의 `AllowedDomains`에 나열되어 있는지 확인합니다. |
...
> [!IMPORTANT]
> Teams 사용자가 조직 외부의 관리되지 않는 Teams 사용자와 통신할 수 있도록 하기 위해 **Teams 도메인** 을 허용된 도메인으로 추가할 필요가 없습니다. 모든 **관리되지 않는 Teams 도메인** 이 허용됩니다.
