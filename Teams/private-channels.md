...
## <a name="private-channel-creation"></a>개인 채널 만들기

기본적으로 팀 소유자 또는 팀 구성원은 비공개 채널을 만들 수 있습니다. 게스트는 만들 수 없습니다. 개인 채널을 만드는 기능을 팀 수준 및 조직 수준에서 관리할 수 있습니다. 정책을 [사용하여](teams-policies.md) 조직에서 개인 채널을 만들 수 있는 사용자를 제어합니다. 정책을 설정하면 팀 소유자는 팀의 설정 탭에서 구성원이 개인 채널을 만들  수 있는 기능을 해제하거나 설정할 수 있습니다.
...
팀 구성원은 추가된 개인 채널만 볼 수 있습니다.

## <a name="adding-and-removing-owners-and-members"></a>소유자 및 멤버 추가 및 제거

비공개 채널 소유자가 하나 이상의 비공개 채널을 운영하는 마지막 소유자일 경우 Teams 클라이언트를 통해 제거될 수 없습니다.

개인 채널 소유자가 조직을 떠났거나 팀과 연결된 Microsoft 365 그룹에서 제거된 경우 개인 채널의 구성원이 자동으로 개인 채널 소유자로 승격됩니다.
...
각 사설 채널에는 구성원을 추가 및 제거하고, 탭을 추가하고, 전체 채널에 대한 @mentioning 기능을 포함하여 채널 소유자가 관리할 수 있는 자체 설정이 있습니다. 이러한 설정은 상위 팀 설정과는 독립됩니다. 비공개 채널을 만들면 상위 팀에서 설정이 상속되며, 이후에 해당 설정을 변경하면 상위 팀 설정과 독립적으로 지정됩니다.
...
|---------|---------|---------|---------|---------|---------|---------|
|비공개 채널 만들기|관리자 제어|관리 및 팀 소유자 제어|아니요|해당 없음|해당 없음|해당 없음|
...
|탭 및 앱 관리|아니요|해당 없음|해당 없음|예, 팀에 앱을 설치해야 합니다.|채널 소유자 제어|아니요|

## <a name="manage-the-lifecycle-of-private-channels"></a>개인 채널의 수명 주기 관리

조직의 개인 채널 수명 주기를 관리하는 방법에 대한 지침은 [Teams의](private-channels-life-cycle-management.md) 개인 채널 수명 주기 관리를 참조하세요. 여기에는 조직 내 어떤 사용자가 비공개 채널을 만들 수 있도록 할지 여부를 제어하는 방법, 팀 소유자 대신 비공개 채널을 만드는 방법, 보관 및 감사 목적으로 모든 비공개 채널 메시지 목록을 얻는 방법, 및 기타 관리 작업에 대한 설명이 포함되어 있습니다.  

## <a name="private-channel-sharepoint-sites"></a>비공개 채널 SharePoint 사이트

각 개인 채널에는 자체 SharePoint 사이트가 있습니다. 개별 사이트는 개인 채널 파일에 대한 액세스가 개인 채널의 구성원만 제한되도록 보장하기 위한 것입니다. 이러한 사이트는 기본적으로 문서 라이브러리를 사용하여 만들어지며 사이트 관리 인터페이스를 통해 전체 기능을 사용하는 사이트로 쉽게 [개선할 수 있습니다.](https://support.office.com/article/A2F2A5C2-093D-4897-8B7F-37F86D83DF04) 각 사이트는 부모 팀의 사이트와 동일한 지리적 지역에 만들어집니다. 이러한 간단한 사이트에는 사용자 지정 템플릿 ID인 "TEAMCHANNEL#0"이 있어 PowerShell 및 Graph API를 통해 보다 손쉽게 관리할 수 있습니다.

개인 채널 사이트는 데이터 분류를 동기화하고 부모 팀의 사이트에서 게스트 액세스 권한을 상속합니다. 사이트 소유자 및 구성원 그룹에 대한 멤버 자격은 Teams 내의 개인 채널 멤버 자격과 동기화됩니다. SharePoint의 소유자 또는 구성원 그룹의 멤버 자격에 대한 변경 내용은 4시간 이내에 자동으로 개인 채널 멤버 자격으로 되찾습니다. 특정 사용자가 비공개 채널 메시지에 액세스하지 않고 문서에 액세스해야 할 경우 해당 사용자를 사이트의 방문자 그룹에 추가하거나 소유자 및 구성원과 구분된 새 그룹에 추가하세요.

Teams는 개인 채널 사이트의 수명 주기를 관리합니다. 사이트가 Teams 외부에서 삭제된 경우 백그라운드 작업은 비공개 채널이 계속 활성화된 한 4시간 이내에 사이트를 복원합니다. 사이트가 영구적으로 삭제되면 개인 채널에 대해 새 사이트가 프로비전됩니다.

개인 채널 또는 개인 채널이 포함된 팀이 복원되면 사이트가 복원됩니다. 개인 채널 사이트가 복원되고 개인 채널에 대한 30일 소프트 삭제 기간을 초과하는 경우 사이트는 독립 실행형 사이트로 운영됩니다.
...
기존 전자 필기장이 개인 채널에 탭으로 추가된 경우 개인 채널에 대한 액세스는 변경되지 않습니다. 전자 필기장에는 기존 사용 권한이 유지됩니다.
