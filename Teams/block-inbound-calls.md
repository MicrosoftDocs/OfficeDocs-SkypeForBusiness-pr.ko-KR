...
전화 시스템 직접 라우팅 및 통화 계획은 PSTN(공용 전환 전화 네트워크)에서 인바운드 호출 차단을 지원합니다. 이 기능을 사용하면 테넌트에 들어오는 모든 PSTN 호출의 발신자 ID를 일치하는 목록에 대해 확인할 수 있도록 테넌트 전역 번호 패턴 목록을 정의할 수 있습니다. 일치하는 경우 들어오는 호출이 거부됩니다.
...
새, **Get,** **Set,**    - **CsInboundBlockedNumberPattern** cmdlet을 사용하여 숫자 패턴을 관리합니다. 특정 패턴의 활성화를 전환하는 기능을 포함하여 이러한 cmdlet을 사용하여 주어진 패턴을 관리할 수 있습니다.
...
전체 호출 차단 기능을 보고 활성화하는 것은 Get **,** **Set**  - **CsTenantBlockingCallingNumbers** cmdlet을 통해 관리됩니다.
...
이 예제에서는 **사용 및** 설명 매개 **변수가** 선택 사항입니다.
...
이 예제에서는 **ID 매개 변수가** 필요합니다.

```powershell
Remove-CsInboundBlockedNumberPattern -Identity “<identity>”
```
 
ID를 알 수 없는 경우 **Get-CsInboundBlockedNumberPattern** cmdlet을 사용하여 먼저 적절한 패턴을 찾고 ID를 기록합니다. 그런 다음 **Remove-CsTenantBlockedNumberPattern** cmdlet을 실행하고 적절한 ID 값을 전달합니다.
...
새 , Get, **Set,**    - **CsTenantBlockNumberExceptionPattern** cmdlet을 사용하여 차단된 숫자 패턴에 예외를 추가할 수 있습니다.
...
이 예제에서는 새 숫자 예외 패턴이 만들어지며 기본적으로 사용 가능한 패턴을 추가합니다. 사용 **및** **설명 매개 변수는** 선택 사항입니다.
...
이 예제에서는 **ID 매개** 변수가 필수입니다. **Set-CsTenantBlockedNumberExceptionPattern** cmdlet을 사용하면 주어진 숫자 패턴 ID에 대해 하나 이상의 매개 변수를 수정할 수 있습니다.
...
이 예제에서는 **ID 매개 변수가** 필요합니다. 이 cmdlet은 테넌트 목록에서 주어진 숫자 패턴을 제거합니다.  ID를 알 수 없는 경우 **Get-CsInboundBlockedNumberPattern** cmdlet을 사용하여 먼저 적절한 패턴을 찾고 ID를 기록합니다. 그런 다음 **Remove-CsTenantBlockedNumberExceptionPattern** cmdlet을 실행하고 적절한 ID 값을 전달합니다.테스트하고 유효성을 검사하기 전에 복제 시간을 허용합니다.  
...
이 예제에서는 **PhoneNumber** 및 **테넌트 매개 변수가** 필요합니다. **PhoneNumber** 매개 변수는 + 또는 -와 같은 추가 문자가 없는 숫자 문자열이 해야 합니다. TRPS에서 **테넌트 매개 변수는** 선택 사항입니다. 결과적으로 **isNumberBlocked** 매개 변수는 테넌트에서 숫자가 차단된 경우 True 값을 반환하고, 차단되지 않은 경우 False를 반환합니다.
