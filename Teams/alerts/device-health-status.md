...
ms.locfileid: "64737606"
---
# <a name="microsoft-teams-device-health-monitoring"></a>Microsoft Teams 디바이스 상태 모니터링

Microsoft Teams 관리 센터에서 디바이스 상태를 모니터링하면 다양한 Teams 디바이스의 상태를 사전에 모니터링할 수 있습니다. 조직의 모니터링된 디바이스가 오프라인 상태가 되면 디바이스의 오프라인 상태를 모니터링하고 실시간으로 경고를 수신합니다.  
...
1. Microsoft Teams 관리 센터의 왼쪽 탐색에서 **알림 & alertsRules를** >  선택합니다.
...
|필드 |설명  |
|--------|-------------|
|**규칙 유형**   |디바이스 상태 규칙은 효과적으로 관리하는 데 도움이 됩니다. Teams 디바이스 관리 유형으로 분류됩니다. 나중에 디바이스 관리 유형의 더 많은 규칙을 사용하여 다른 관련 기능을 모니터링할 수 있습니다(예: 비정상 디바이스 및 디바이스의 로그인 상태 포함).|
|**조건**   |디바이스가 오프라인으로 전환되면 디바이스의 상태를 모니터링할 수 있습니다. Teams 관리 센터에서 디바이스 관리에 대해 [자세히 알아봅니](../devices/device-management.md)다. |
...
|**작업** >  **채널 경고**   |작업 섹션에서 경고를 받을 팀 채널을 지정할 수 있습니다. 현재 **Admin Alerts 및 Notifications라는** 기본 팀과 **MonitoringAlerts** 라는 채널이 만들어집니다. 여기서 알림이 전달됩니다. <BR/> <BR/> 테넌트에 있는 전역 관리자 및 Teams 관리자가 이 기본 팀에 자동으로 추가됩니다.|
...
알림은 **관리자 경고 및 알림** 팀의 자동 생성된 **MonitoringAlerts** 채널에 전달됩니다. 디바이스가 오프라인으로 전환된 후 15분 이내에 경고가 수신됩니다. 
