...
이 비디오에서는 자동 참석을 만드는 방법에 대한 기본 예제를 Microsoft Teams.
...
각 자동 참석자를 가상 사용자 전화 시스템 할당해야 합니다. 통화 요금제로 Teams 전화 경우 가상 사용자 전화 시스템 수가 제공되어 더 이상 요청할 필요가 없습니다. 그러나 향후 더 많은 것이 필요한 경우 가상 사용자 라이선스 의 지침에 따라 전화 시스템 [수 있습니다.](../teams-add-on-licensing/virtual-user.md)
...
> [!NOTE]
> 처음 통화 요금제로 Teams 전화 설정하는 단계에 따라 회사의 주 전화 번호에 대한 자동 수행자 **설정:** 이 탭의 단계를 이미 완료했습니다. 다음 탭으로 이동: 자동 참석자 일반 [정보](?tabs=general-info#steps)입니다.
