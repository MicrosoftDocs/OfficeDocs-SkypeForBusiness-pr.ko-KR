...
Microsoft Endpoint Configuration Manager 또는 그룹 정책 또는 광범위한 배포를 위해 타사 배포 메커니즘을 사용하기 위해 Microsoft는 관리자가 Teams의 대량 배포에 사용할 수 있는 MSI 파일(32비트 및 64비트 모두)을 제공하여 사용자 또는 컴퓨터를 선택했습니다. 관리자는 이러한 파일을 사용하여 Teams를 원격으로 배포할 수 있으므로 사용자가 Teams 앱을 수동으로 다운로드할 수 없습니다. 배포되면 해당 머신에 로그인하는 모든 사용자에 대해 Teams가 자동으로 실행됩니다. (앱을 설치한 후 자동 실행을 사용하지 않도록 설정할 수 있습니다. [아래를 참조하세요.)](#disable-auto-launch-for-the-msi-installer) 컴퓨터에 패키지를 배포하는 것이 좋습니다. 따라서 컴퓨터의 모든 새 사용자도 이 배포의 이점을 하게 됩니다.
...
클라이언트가 서비스에서 새 버전을 사용할 수 있는 것으로 감지되면 자동으로 업데이트하기 때문에 MSI를 사용하여 업데이트를 배포하지 않습니다. 최신 설치 관리자를 다시 배포하려면 아래에 설명된 MSI를 다시 배포하는 프로세스를 사용하세요. 이전 버전의 MSI 패키지를 배포하는 경우 가능한 경우 클라이언트가 자동으로 업데이트됩니다(VDI 환경 제외). 매우 오래된 버전이 배포되면 사용자가 Teams를 사용하기 전에 MSI에서 앱 업데이트를 트리거합니다.
...
VDI에 Teams 데스크톱 앱을 배포하는 방법에 대한 전체 지침은 가상화된 데스크톱 인프라용 [Teams를 참조하세요.](teams-for-vdi.md)
...
> [!IMPORTANT]
> 다음 단계에서는 레지스트리를 수정하는 방법에 대한 정보가 포함되어 있습니다. 레지스트리를 수정하기 전에 백업하고 문제가 발생하는 경우 레지스트리를 복원하는 방법을 알고 있는지를 확인해야 합니다. 레지스트리를 백업, 복원 및 수정하는 방법에 대한 자세한 내용은 고급 사용자의 Windows 레지스트리 [정보를 참조하세요.](https://support.microsoft.com/help/256986)
...
> [!CAUTION]
> 이미 Teams를 배포한 경우 Teams 자동 시작을 사용하지 않도록 이 정책을 설정하려는 경우 먼저 그룹 정책 설정을 원하는 값으로 설정한 다음 사용자에 따라 [Teams](scripts/powershell-script-teams-reset-autostart.md) 자동 시작 재설정 스크립트를 실행합니다.
...
> [!Note]
> MSI를 수동으로 실행하는 경우 상승된 권한으로 실행해야 합니다. 관리자 권한으로 실행한 경우에도 관리자 권한으로 실행하지 않으면 설치 관리자가 자동 시작을 사용하지 않도록 옵션을 구성할 수 없습니다.
