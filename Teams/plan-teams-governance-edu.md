...
-   **접두사-접미사 이름명 정책** 접두사 또는 접미사를 사용하여 팀(그룹)의 이름 규칙을 정의할 수 **GRP_US_My Group_Engineering.** 접두사와 접미사는 팀을 만드는 사용자를 기반으로 이름에 추가되는 고정 문자열 또는 사용자 특성(예: **[부서])일** 수 있습니다.
...
경험에 따라 사용자 교육은 책임감 있는 Teams 사용을 보장하는 데 도움이 됩니다. 사용자가 팀을 만드는 것이 익명이 아니라는 것을 이해하는 즉시, 팀을 부수적으로 만드는 의미를 이해하고 도구를 잘못 사용하는 것을 주저하는 경향이 있습니다.
...
학교 학기 또는 분기가 끝날 때 Teams 데이터를 처리하는 방법에 대해 먼저 생각하는 것이 좋습니다. 학생이 과정을 완료한 후에도 해당 데이터를 삭제할지 아니면 학생이 사용할 수 있도록 유지할지 여부를 생각해 보는 것이 좋습니다. 설정한 정책이 공휴일과 충돌하지 않는지 학교 일정에 유의해야 합니다. 다음 도구를 사용하여 전략을 구현할 수 있습니다.
...
-   **만료 정책:** 특정 일 수 후에 만료하도록 팀을 구성합니다. 만료되기 30일 전에 팀의 모든 소유자는 팀을 갱신해야 하다는 알림을 습니다. 그렇지 않으면 삭제됩니다(관리자가 삭제된 팀을 30일 동안 복구할 수 있습니다). 이 설정은 사용되지 않는 팀이 해소된 경우 매우 유용합니다. [Microsoft 365 그룹 만료 정책에 대해 자세히 알아보고](https://support.office.com/article/office-365-group-expiration-policy-8d253fe5-0e09-4b3c-8b5e-f48def064733)
