---
title: IT 직원을 준비하여 Microsoft Teams
author: dstrome
ms.author: dstrome
manager: serdars
ms.topic: article
ms.service: msteams
audience: admin
ms.reviewer: landerl
description: 조직에서 IT 직원을 배포 및 지원하기 위해 준비하는 방법에 대해 Microsoft Teams.
localization_priority: Normal
search.appverid: MET150
f1.keywords:
- CSH
ms.custom: Teams-upgrade-guidance
ms.collection:
- Teams_ITAdmin_JourneyFromSfB
- M365-collaboration
- seo-marvel-apr2020
appliesto:
- Microsoft Teams
ms.openlocfilehash: 2dc29f8656b4fdd69d0e6e59338e91341d32cc7e283b4bdd2cfb5680192e248b
ms.sourcegitcommit: a17ad3332ca5d2997f85db7835500d8190c34b2f
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/05/2021
ms.locfileid: "54343862"
---
# <a name="prepare-your-it-staff-for-microsoft-teams"></a>IT 직원을 준비하여 Microsoft Teams

![기술 준비 단계 강조하는 업그레이드 여정 다이어그램](media/upgrade-banner-tech-readiness.png "기술 준비 단계가 강조된 업그레이드 여정의 단계")

이 문서는 사용자 준비 단계와 병렬로 완료한 작업인 업그레이드 여정의 기술 준비 단계의 일부입니다. 계속하기 전에 이전 단계에서 이러한 작업을 완료한지 확인합니다.

- [프로젝트 관계자 인리스트](upgrade-enlist-stakeholders.md)
- [프로젝트 범위 정의](./upgrade-define-project-scope.md)
- [비즈니스용 Skype 및 상호운용성을 Teams](./teams-and-skypeforbusiness-coexistence-and-interoperability.md)
- [업그레이드 여정을 선택했습니다.](upgrade-and-coexistence-of-skypeforbusiness-and-teams.md)

조직 Microsoft 365 Office 365, 기술 리드 및 지원 데스크는 고품질의 사용자 환경을 구동할 책임이 있습니다. 여기에는 네트워크가 지원할 준비가 되어 Teams 사용자에 대한 Teams 구성하고, 발생할 수 있는 문제를 효과적으로 해결하고 해결할 수 있습니다.

다음 리소스를 IT 직원과 공유하고 업그레이드를 시작하기 전에 사용자를 지원할 준비가 됐는지 Teams.

- [사용자에 대한 관리자 Microsoft Teams](itadmin-readiness.md)
- [비즈니스용 제품에 대한 고객 지원 센터 문의 - 관리자 도움말](/microsoft-365/admin/contact-support-for-business-products)
- [Microsoft Teams 클라이언트의 연결 문제 해결](connectivity-issues.md)
- [로그 파일을 사용하여 문제 해결 Microsoft Teams](log-files.md)



|&nbsp; |&nbsp; |
|---|---|
| ![의사 결정 지점을 묘사하는 아이콘](media/audio_conferencing_image7.png) <br/>의사 결정 지점|<ul><li>배포 및 지원에 참여할 가능성이 높은 모든 지원 직원을 Teams.</li><li>업그레이드가 진행될 때 추가 직원을 온보드하기 위한 교육 계획을 개발해 보신가요?</li></ul> |
| ![다음 단계를 묘사하는 아이콘](media/audio_conferencing_image9.png)<br/>다음 단계|<ul><li>IT 직원에게 필요한 정보가 있는지 확인하세요.</li><li>새 기능이 릴리스될 때 교육 및 준비 계획을 다시 확인합니다.</li></ul>|

IT 직원을 준비한 후 Teams 모든 요구 사항을 충족하는지 [확인해야 합니다.](upgrade-plan-journey-prerequisites.md)