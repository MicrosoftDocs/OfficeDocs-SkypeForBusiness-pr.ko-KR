...
- 디바이스 절반 듀플렉스 AEC 이벤트 비율을 확인합니다. 허브 또는 도킹 스테이션에 연결할 때 USB 오디오 드롭아웃으로 인하여 장치 글리치 또는 마이크 글리치가 잡음 또는 정적을 발생시겠습니까?  
...
호출 수 및 사용자 수 측정값을 계산하기 위해 데이터 집합의 호출 또는 사용자 식별자에 대해 고유한 카운트프 작업이 수행됩니다. 큰 데이터 집합에서 고유 카운트프 연산에 내재된 최대 0.2%의 오류가 있습니다. 가장 정확한 볼륨의 경우 이 고유한 countif 작업에는 사용되지 않습니다. 데이터 볼륨을 줄이기 위해 필터링하면 오류를 줄일 수 있지만 고유한 호출 및 사용자 수에서 이 오류 원본을 제거하지는 않을 수 있습니다. 측정값이 영향을 받는 [통화](dimensions-and-measures-available-in-call-quality-dashboard.md) 품질 대시보드에서 사용할 수 있는 차원 및 측정값을 참조하세요.
...
이를 염두에 두어 사용 현황 보고를 위해 통화 품질 관리 도구를 사용하지 않는 것이 좋습니다. Teams 관리 센터는 일련의 사용 현황 보고서를 [](teams-analytics-and-reports/meeting-attendance-report.md) 제공하며, [](teams-analytics-and-reports/teams-reporting-reference.md)모임 참석 보고서를 클라이언트에서 직접 Teams 있습니다.
...
CQD Teams(isTeams = 1)에서만 필터링하면 첫 번째 엔드포인트가 Teams.  두 *번째 엔드포인트가* 비즈니스용 Skype CQD 보고서에 해당 정보가 표시됩니다. CQD는 고객의 시나리오에 따라 데이터 커넥터 호출을 비즈니스용 Skype 서버 2019 호출을 포함할 [수](/skypeforbusiness/hybrid/plan-call-data-connector.md) 있습니다. 또한 봇 Skype(AA, CVI, VDI), 라이브 이벤트 및 PSTN 호출도 포함할 수 있습니다.
