...
> [!IMPORTANT]
> 사용자가 대규모 교육 기관에 소속되어 있는 경우 Microsoft Teams 관리 포털 환경을 사용하여 각 사용자에 대해 정책을 설정하는 작업이 어려울 수 있습니다. PowerShell을 통해 일괄적으로 정책을 할당하는 것이 좋습니다. 사용자에게 필요한 경우 [교육 기관에서 대규모 사용자 집합에 정책을 할당](batch-group-policy-assignment-edu.md)하는 방법에 대해 몇 가지 EDU 관련 정보를 제공하며 대규모 사용자 그룹에 대한 정책 및 설정을 관리하는 또 다른 유용한 방법인 정책 패키지에서 아래 섹션을 확인할 수도 있습니다.
...
다음은 각 모임 생성 정책에 대한 논리를 설명하는 표입니다.

| 모임 생성 정책 | 모임 생성 | 무인 모임 시작 | 가입 시 로비 바이패스 | 모임 종료 |
| --- | --- | --- | --- | --- |
| **켬(예: 강사)** | 예 | 예 | [모임 옵션](https://go.microsoft.com/fwlink/?linkid=2093366)에 의해 결정됨 | 예, 이끌이로
| **끄기 (예: student)** | 아니오 | 아니오\*\* | [모임 옵션](https://go.microsoft.com/fwlink/?linkid=2093366)에 의해 결정됨 | 아니오

> [!NOTE]
> \*\* 이는 .EDU 라이선스가 있는 사용자에게만 적용되며 모임, 채널 모임, 즉석 모임 및 인스턴트 채널 모임에 적용됩니다.
...
다음은 모임에서 대화를 허용하기 위한 논리를 설명하는 표입니다.

| '모임에서 대화 허용' 정책 | 언제든지 채팅 기록 보기 | 모임 중에 메시지 게시 | 회의 전후에 메시지 게시 |
| --- | --- | --- | --- | 
| **모두에게 켬** | 예 | 예 | 예 |
| **모두에게 꺼짐** | 해당 없음 | 해당 없음 | 해당 없음 |
| **교육자용 및 학생용 꺼짐** | 강사: 네<br>학생 예 | 강사: 네<br>학생 예 | 강사: 네<br>학생 번호\*\* | 

> [!NOTE]
> \*\*이것은 EDU 라이선스 사용자에게만 적용되며 회의 및 즉석 모임에 적용됩니다. 채널 모임이나 즉석 채널 모임에는 적용되지 않습니다.
