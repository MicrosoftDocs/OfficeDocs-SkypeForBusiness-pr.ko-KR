...
Microsoft 팀에서 게스트 액세스를 설정 하 고 구성 하는 데 도움이 되는 검사 목록을 사용 합니다.
...
4. **외부 공동 작업 설정** 페이지에서 사용 하도록 설정할 정책을 선택 합니다.
  ...
  - **게스트 사용자 권한이 제한 됨**:이 정책은 디렉터리의 게스트에 대 한 사용 권한을 결정 합니다. **예** 를 선택 하 여 사용자, 그룹 또는 기타 디렉터리 리소스 열거와 같은 특정 디렉터리 작업에서 게스트를 차단 합니다. 게스트를 사용 하 여 디렉터리 데이터에 대 한 액세스 권한을 다른 사용자와 동일 하 게 지정 하려면 ( **아니요** )를 선택 합니다.
   ...
   - **구성원 초대 가능**: 해당 디렉터리의 관리자가 아닌 구성원이 게스트를 초대 하도록 허용 하려면이 정책을 **예로**설정 합니다.
       ...
       > [!IMPORTANT]
       > 게스트의 모든 팀에서 작업에 액세스 하려면 구성원을 **예**에 **초대할 수** 있도록 설정 해야 합니다.   
   - **게스트 초대**: 게스트가 다른 게스트를 초대할 수 있도록 하려면이 정책을 **Yes**로 설정 합니다.
       > [!IMPORTANT]
       > 현재 팀은 게스트 inviter 역할을 지원 하지 않으므로, 게스트가 **예**에 **초대할 수** 있도록 설정한 경우에도 게스트는 다른 게스트를 팀에 초대할 수 없습니다.
...
게스트를 초대할 수 있는 사용자를 제어 하는 방법에 대 한 자세한 내용은 [Azure Active DIRECTORY B2B 공동 작업에 대 한 초대 위임을](https://docs.microsoft.com/azure/active-directory/b2b/delegate-invitations)참조 하세요.
