---
title: 워크샵 & 도구 - 계획, 제공, & 채택 Teams
author: SerdarSoysal
ms.author: serdars
manager: serdars
ms.topic: article
ms.service: msteams
audience: admin
ms.reviewer: karuanag
description: 조직이 창업을 시작할 수 있도록 도구, 워크샵 및 시작 키트를 Microsoft Teams.
ms.localizationpriority: medium
search.appverid: MET150
f1.keywords:
- NOCSH
appliesto:
- Microsoft Teams
ms.custom: seo-marvel-mar2020
ms.openlocfilehash: 159ff348fcc519b2ddc837021263f5c6851214d1
ms.sourcegitcommit: 556fffc96729150efcc04cd5d6069c402012421e
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/26/2021
ms.locfileid: "58636672"
---
# <a name="tools-and-workshops-plan-deliver-and-adopt-microsoft-teams"></a>도구 및 워크샵: 계획, 제공 및 채택 Microsoft Teams

이러한 도구를 다운로드하여 조직에서 계획, 배달 및 채택을 Teams 진행합니다.


- [Microsoft Teams 고객 성공 키트](https://aka.ms/TeamsCustomerSuccess)

    이 키트에는 조직의 다양한 프레젠테이션, 전자 Teams 메일 템플릿, 팁 & 및 포스터가 포함되어 있습니다. 관리자 및 팀 리더를 위한 2개의 빠른 시작 가이드와 모든 부서에 대한 사용 사례 예제가 포함되어 있습니다. 이러한 리소스를 사용하여 마케팅, 영업Teams 재무, 인사, IT, 엔지니어링 및 관리 등 조직의 다양한 기능에 대한 Project 활용합니다.

- [Teams를 배포하는 방법](./deploy-overview.md)

    권장되는 경로를 따라 롤아웃을 Teams.


- [기술 Teams 교육을 받을 수 있습니다.](itadmin-readiness.md)

    이러한 기술 교육 리소스를 사용하여 직접 교육하거나, 다른 사용자에 대해 가르치거나, 워크샵을 실행합니다.

- [Office 365 Government 고객](plan-for-government-gcc.md)

   비즈니스를 Teams 경험은 엔터프라이즈 고객의 경험과 매우 입니다. 테넌트에서 Teams 일반적으로 사용할 수 있도록 설정되어 있는 경우, 정부 Microsoft 365 [계획 -](plan-for-government-gcc.md) GCC 관리자 빠른 시작 가이드 를 시작하는 것이 [좋습니다.](https://download.microsoft.com/download/F/3/9/F39B4F10-5720-4516-87E1-91E5A5678EFB/MicrosoftTeams-AdminQuickStart-EnableTeams.docx) 보안 [및 규정 준수](security-compliance-overview.md) 개요를 통해 사용자 환경에서의 보안 Teams 확신할 수 있습니다.

- [교육용 Teams - 관리자에 대한 빠른 시작](./teams-quick-start-edu.yml)

    이 자습서를 사용하여 교육 조직에 대한 Teams 롤아웃을 진행합니다.

### <a name="see-also"></a>참고 항목

[Teams 교육](training-microsoft-teams-landing-page.md)

[IT 관리자에 대한 Teams](ITAdmin-readiness.md)

[채택을 위한 도구](adopt-tools-and-downloads.md)

[비즈니스용 Skype 계획 Teams 워크숍을 진행할 수 있습니다.](https://www.microsoft.com/microsoft-365/partners/intelligentcommunications/skype-for-business-teams)

[사용자를 온-프레미스에서 Teams로 이동](/skypeforbusiness/hybrid/move-users-from-on-premises-to-teams)

[조직의 Teams 설정 관리](enable-features-office-365.md)
