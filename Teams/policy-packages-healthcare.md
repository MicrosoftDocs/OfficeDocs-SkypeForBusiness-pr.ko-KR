...
|---------|---------|---------|
|의료 의료 의료진  |의료 조직의 의료 종사자  |등록된 간호사, 간호사, 의사, 사회복지사와 같은 의료 근로자가 채팅, 통화, 교대 근무 관리 및 모임에 대한 모든 권한을 부여하는 정책 및 정책 설정 집합을 만듭니다. |
...
각 개별 정책에는 정책 패키지의 이름이 지정되어 정책 패키지에 연결된 정책을 쉽게 식별할 수 있습니다. 예를 들어 조직의 의료진에게 의료 의료진 정책 패키지를 할당하면 패키지의 각 Healthcare_ClinicalWorker 정책이 만들어집니다.
...
의료 정책 패키지를 시작하려면 Microsoft 관리 센터 온보팅 허브에서 **의료를** 선택한 다음 역할에 따라 정책 **설정 할당을 선택합니다.** 시작할 준비가 됐을 때 조직의 개인을 할당할 정책 패키지를 결정해야 합니다.
