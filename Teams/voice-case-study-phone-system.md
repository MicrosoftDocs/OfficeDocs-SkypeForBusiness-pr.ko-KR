...
전체 Microsoft 전화 시스템 솔루션을 구현하기 위해 Contoso는 PSTN(공용 전환 전화 네트워크)에 연결하기 위해 다음 옵션 중 전화 시스템 각 사이트 유형에 대해 결정해야 &mdash; &mdash; 합니다.
...
조직에 적합한 솔루션을 결정하기 위해 Contoso는 [Microsoft](/SkypeForBusiness/hybrid/msft-telephony-solutions) 전화 통신 솔루션과 Ignite 2019 세션 [호출을](https://myignite.techcommunity.microsoft.com/sessions/83170?source=sessions)Microsoft Teams.  
...
전화 번호를 할당해야 하는 사용자 수로 인해 Contoso는 PowerShell을 사용하여 전화 번호를 할당하기로 결정했습니다. Contoso에서 PowerShell 개요 를 사용한 다른 설정 외에도 PowerShell을 사용하여 숫자를 Teams &mdash; &mdash; 방법을 [알아보았다.](teams-powershell-overview.md)  

### <a name="direct-routing-sites"></a>직접 라우팅 사이트

Contoso의온-프레미스 전화 통신 인프라를 연결하기 위해 Microsoft Teams Contoso의 관리자는 직접 [](direct-routing-configure.md) 라우팅 구성의 단계를 수행하고 지침에 따라 비디오 직접 라우팅을 Microsoft Teams [검토했습니다.](https://www.youtube.com/watch?v=1ASftX_Msb8&index=10&list=PLaSOUojkSiGnKuE30ckcjnDVkMNqDv0Vl)  Contoso는 인증된 SBC 공급업체에서 직접 라우팅 배포 설명서를 참조했습니다. 

SBC 및 Microsoft 전화 시스템 간에 직접 라우팅을 구성한 후 Contoso가 구성을 테스트해야 합니다. 이를 위해 Contoso 관리자는 [Ignite 2019의](https://myignite.techcommunity.microsoft.com/sessions/83178?source=sessions)직접 라우팅 업데이트 세션에서 설명한 SIP 테스터 클라이언트를 사용했습니다. SIP Tester 클라이언트 스크립트 및 설명서는 PowerShell 스크립트에서 다운로드하여 직접 라우팅 세션 테두리 컨트롤러 연결을 테스트했습니다.   
...
품질 문제를 해결하기 위해 Contoso는 내부 앱에 대한 연결이 VPN에 남아 있는 동안 Office 365 트래픽이 인터넷을 트래버스할 수 있도록 VPN 분할 터널링을 구현했습니다. VPN 분할 터널링을 구현하기 위해 Contoso는 에 대한 VPN 분할 [터널링 구현의 지침을 Office 365.](/office365/enterprise/office-365-vpn-implement-split-tunnel)  
