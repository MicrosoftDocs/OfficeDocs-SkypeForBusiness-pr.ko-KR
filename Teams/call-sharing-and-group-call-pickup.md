...
그룹 통화 선택은 다른 형태의 통화 공유(예: 통화 전달 또는 동시 연결)보다 받는 사람에게 덜 방해가 됩니다. 사용자가 수신 공유 통화에 대한 알림을 받는 방법을 구성할 수 있으며(Teams 앱에서 오디오 및 시각적 알림, 시각적 알림만 또는 배너를 통해) 응답할지 여부를 결정할 수 있기 때문에 수신자에게 덜 방해가 됩니다.
...
중요: 관리자가 사용자에 대한 통화 그룹을 끄면(통화 그룹 관계가 설정되고 통화 그룹 관계가 구성된 후) 잘못된 통화 라우팅을 방지하기 위해 Teams 관리 센터의 사용자에 대한 통화 그룹 관계를 정리해야 합니다. 
