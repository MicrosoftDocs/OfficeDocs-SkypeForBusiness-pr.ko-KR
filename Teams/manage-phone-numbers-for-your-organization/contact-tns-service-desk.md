...
새로운 전화 번호 서비스 센터에서 TNS(전화 번호 서비스) 서비스 데스크와 상호 작용하는 새로운 프로세스가 **[있습니다](https://pstnsd.powerappsportals.com)**. 이제 Teams 관리 센터와 통합된 단일 위치에서 티켓을 열고 티켓을 보고 통신을 추적할 수 있습니다. 해당 작업은 다음 섹션에서 자세히 설명합니다.
...
> [!NOTE]
> 동일한 테넌트에서만 사례를 만들 수 있습니다. 예를 들어 @fabrikam.com 사용자가 @contoso.com 대신 사례를 만들 수 없습니다.

새 사례를 만들려면 다음 단계를 수행합니다.

1. 다음 위치 중 하나에서 **새 사례 만들기** 를 선택합니다.

   - **전화 번호 서비스 센터** 홈페이지에서 페이지 위쪽 또는 아래쪽 타일에 있습니다.
...
- **인벤토리 형식 변경** – 전화 번호 유형을 변경합니다. 예를 들어 Microsoft 구독자 번호를 서비스 번호로 변경하려고 합니다. Microsoft에서 지원하는 전화 번호 유형에 대한 자세한 내용은 [전화 번호 유형](../different-kinds-of-phone-numbers-used-for-calling-plans.md)을 참조하세요.
  ...
  - **확인란 – 인벤토리 유형을 업데이트하려면 전화 번호를 할당 취소해야 합니다** . 테넌트 내의 전화 번호가 할당되지 않는 한 Microsoft는 전화 번호 유형 변경 요청을 처리할 수 없습니다. 이후 날짜에 대해 이 변경을 요청하는 경우 요청된 날짜 및 시간 전에 번호 할당을 취소해야 합니다.
...
- **새 TN 취득** – Microsoft에서 새 전화 번호를 구입하세요.
  ...
  - **Teams 관리 센터 포털에서 전화 번호를 받으려고** 했습니다. Microsoft Teams 관리 센터에서 이러한 전화 번호를 구입하려고 했나요?
...
- **포트 인** – 현재 서비스 공급자의 기존 전화 번호를 Microsoft로 포팅합니다.
  ...
  - **요청된 포팅 날짜/시간*** - 번호를 Microsoft로 포팅하려는 날짜/시간입니다. 현재 번호 소유자가 먼저 포트 요청을 승인해야 하므로 이는 보장된 포팅 날짜가 아닙니다. 날짜 및 시간을 참조하세요.
...
- 기존 사례를 열고 아래로 스크롤하여 **메모 추가** 를 선택하여 사례와 관련하여 **TNS 서비스 데스크와 통신** 합니다. 새 창이 나타납니다. 메모 상자에 메시지를 입력합니다. 지원 문서(사용 가능한 경우)를 첨부한 다음 **제출** 을 선택합니다.
