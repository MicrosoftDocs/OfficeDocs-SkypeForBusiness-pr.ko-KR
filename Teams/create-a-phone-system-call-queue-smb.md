...
통화 큐를 만들기 위해 이 문서에 설명된 단계는 Teams 관리 센터를 사용합니다. PowerShell을 사용하여 통화 큐를 만드는 지침은 [PowerShell cmdlet을 사용하여 통화 큐 만들기를 참조하세요](create-a-phone-system-call-queue-via-cmdlets.md).
...
에이전트의 Teams 계정은 모드로 `TeamsOnly` 설정해야 합니다. 요구 사항을 충족하지 않는 에이전트는 통화 라우팅 목록에 포함되지 않습니다. 에이전트가 호환되는 클라이언트를 사용하는 경우 통화 큐에 대해 회의 모드를 사용하도록 설정하는 것이 좋습니다.

> [!TIP]
> **회의 모드** 를 **켜** 기로 설정하는 것이 좋습니다.

> [!NOTE]
> Location-Based 라우팅에 사용하도록 설정된 직접 라우팅 게이트웨이에서 큐로 라우팅되는 호출에는 회의 모드가 지원되지 않습니다.
>
...
3. 에이전트 라우팅 방법에 대해 가장 긴 유휴를 선택하면 Presence-Based 라우팅이 자동으로 활성화됩니다.
4. 에이전트가 호출될 순서를 설정할 수 없습니다.
5. 전화 통화가 Location-Based 라우팅에 사용하도록 설정된 직접 라우팅 게이트웨이에서 큐로 라우팅되는 경우 회의 모드가 지원되지 않습니다.
...
1. 아래에서 **테스트 실행을** 선택하여 Microsoft 365 관리 센터 진단을 채웁니다.
