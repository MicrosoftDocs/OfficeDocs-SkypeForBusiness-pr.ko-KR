...
description: 통화 & 모임에 대한 Teams 정책 기반 녹음/녹화에 대해 알아봅니다.
f1.keywords:
- CSH
ms.custom:
- Adopt
...
ms.locfileid: "65681989"
---
# <a name="introduction-to-teams-policy-based-recording-for-callings--meetings"></a>통화 & 모임에 대한 Teams 정책 기반 녹음/녹화 소개

정책 기반 기록을 사용하면 통화 및 모임에 대한 Microsoft Teams 채택하여 관련 회사 또는 규제 정책에 따라 후속 처리 및 보존을 위해 통화 및 온라인 모임을 자동으로 기록하고 캡처해야 하는 경우 관리 정책을 사용하여 규정할 수 있습니다.

Teams Teams 통신을 구성, 관리, 기록, 저장 및 분석하기 위한 엔드 투 엔드 솔루션을 제공하는 데 필요한 플랫폼 기능, 사용자 환경 및 관리 인터페이스를 비롯한 타사 기록 솔루션의 통합을 지원하도록 향상되었습니다. 향상된 기능에는 다음을 제공하는 통신 플랫폼 API 및 기록을 위한 이벤트가 포함됩니다.

- 오디오, 비디오, 화면 공유 및 채팅을 위해 디바이스 및 지원되는 모든 엔드포인트에서 원활하고 고품질의 미디어 캡처

- Teams 사용자와 지원되는 호출 엔드포인트(Teams, Teams Mobile, 비즈니스용 Skype, PSTN) 간의 상호 작용 캡처 지원
...
규정 준수 기록 솔루션 통합 기능은 Ignite 2019에서 [준수 기록 및 Microsoft Teams 세션에서](https://myignite.microsoft.com/archives/IG19-VCE40)도 검토되었습니다.
...
Teams 모임 및 라이브 이벤트의 [편리](./cloud-recording.md)하고 기능적인 녹음을 위한 다양한 기능을 제공합니다. 조직 녹음/녹화는 통화 및 모임에 대한 Teams 채택하는 조직이 관련 회사 또는 규제 정책에 따라 후속 처리 및 보존을 위해 통화 및 온라인 모임을 자동으로 기록하고 캡처해야 하는 경우 관리 정책을 통해 규정할 수 있도록 하는 것을 의미합니다. 이 정책의 사용자는 Teams 디지털 상호 작용이 기록되고 있지만 녹음을 사용하지 않도록 설정할 수 없으며 상호 작용이 완료되면 녹음/녹화에 액세스할 수 없다는 것을 알게 됩니다. 녹음/녹화는 eDiscovery, 법적 보존 및 기타 회사 보존 사용에 대한 규정 준수 및 법률 담당자가 사용할 수 있는 조직 보관 파일의 일부가 됩니다.
...
규정 준수 기록 솔루션은 다음 다이어그램과 같이 Teams 통합됩니다.

> [!div class="mx-imgBorder"]
> ![팀 사용자 지정 앱 설정을 보여 주는 스크린샷.](media/hp-compliance-recording-for-teams-calling-and-meetings.jpg "이미지는 Teams 모임 또는 통화가 전송되고 수신될 때의 흐름을 보여줍니다.")

> [!NOTE]
> 이 솔루션은 Teams 정책 기반 규정 준수 기록을 사용하도록 특별히 설계되었습니다. 이 솔루션의 다른 사용은 지원되지 않습니다.

## <a name="recorder"></a>레코더

규정 준수 기록 솔루션의 핵심 구성 요소는 레코더입니다.
레코더는 [Microsoft의 통신 플랫폼을 사용하고 Microsoft](/graph/cloud-communications-concept-overview) Graph 애플리케이션으로 등록하는 확장 가능한 Azure 기반 서비스(봇)로 빌드됩니다. 레코더는 Teams 통화 및 모임 [통신 플랫폼 API](/graph/api/resources/communications-api-overview)와의 직접 상호 작용을 제공하고 미디어 수집을 위한 엔드포인트를 제공합니다.
...
[updateRecordingStatus](/graph/api/call-updaterecordingstatus?tabs=http) 및 [incomingContext](/graph/api/resources/incomingcontext)에 대한 특정 API에 대한 Graph 설명서는 여기에서 찾을 수 있습니다.

레코더 서비스의 정확한 구현은 파트너에 따라 다르지만 대기 시간을 Teams 레코더로 줄이기 위해 배포의 고가용성 및 지리적 배포를 달성하기 위해 여러 레코더를 지원하도록 설계되어야 합니다. 또한 레코더 자체는 복원력과 중복성을 염두에 두고 설계될 것으로 예상됩니다.
...
Azure 및 Windows VM 요구 사항은 Teams Bot 구성 요소에만 적용됩니다. 즉, 파트너가 규정 준수 기록의 관련 성능 및 기능 요구 사항을 충족할 수 있는 경우 선택한 나머지 플랫폼을 구현할 수 있습니다.
...
파트너가 CCaaS 솔루션을 개발하고 Teams 통합할 수 있도록 공개적으로 사용할 수 있는 API를 게시하는 것 외에도 Microsoft Teams 인증 프로그램에 대한 규정 준수 기록을 개발하여 각 참여 파트너의 솔루션이 Microsoft 솔루션에서 기대하는 품질, 호환성 및 안정성을 제공하도록 테스트 및 검증되었음을 고객에게 제공합니다.  

다음 파트너는 Microsoft Teams 솔루션을 인증했습니다.<br/><br/>
...
<br/>
다음 파트너는 Microsoft Teams 대한 솔루션을 인증하는 중입니다.<br/><br/>
...
인증 프로그램에 참여하려는 공급업체인 경우 [Teamscategorypartner@microsoft.com](mailto:Teamscategorypartner@microsoft.com) 메일을 보냅니다.
