...
| 유형                   | 편의성(일반 Teams 녹화) | Org - 규제(준수 기록) |
| ---------------------- | ------------------ | --------------- |
| 초기자              | 사용자               | 관리자(시스템)  |
| 대상                 | 통화당/모임 | 사용자별        |
...
<td>규정 준수 책임자</td>
<td><ul>
<li><p>적절한 Teams 규정 준수 의무를 충족하는 데 필요한 방식으로 모든 통신을 수집합니다.</p></li>
<li><p>통신 관련 메타데이터 또는 상호 작용 콘텐츠를 기반으로 상호 작용을 검색합니다. 일반적인 예는 다음과 같습니다.</p>
<ul>
<li><p><strong></strong> - 메타데이터 참가자, 시간, 방향, 전화 걸기 번호, 원본 번호, 사용자 지정 비즈니스 데이터</p></li>
...
레코더는 Microsoft의 통신 플랫폼을 사용하여 [Microsoft](/graph/cloud-communications-concept-overview) 애플리케이션으로 등록하는 확장성 있는 Azure 기반 서비스(봇)로 Graph. 레코더는 호출 및 모임 Teams 플랫폼 [API](/graph/api/resources/communications-api-overview)와 직접 상호 작용을 제공하며 미디어 정보의 엔드포인트를 제공합니다.

[봇](https://github.com/microsoftgraph/microsoft-graph-comms-samples/tree/a3943bafd73ce0df780c0e1ac3428e3de13a101f/Samples/BetaSamples/LocalMediaSamples/ComplianceRecordingBot)을 구성하고, 앱 인스턴스를 만들고, 규정 준수 정책을 할당하는 방법을 보여주는 샘플 준수 레코더 애플리케이션을 사용할 수 있습니다. 또한 샘플에는 들어오는 호출 라우팅 [처리, 기록](https://github.com/microsoftgraph/microsoft-graph-comms-samples/blob/a3943bafd73ce0df780c0e1ac3428e3de13a101f/Samples/BetaSamples/LocalMediaSamples/ComplianceRecordingBot/FrontEnd/Bot/CallHandler.cs#L135-L138) 상태 변경 및 기록되는 사용자 제거 [](https://github.com/microsoftgraph/microsoft-graph-comms-samples/blob/a3943bafd73ce0df780c0e1ac3428e3de13a101f/Samples/BetaSamples/LocalMediaSamples/ComplianceRecordingBot/FrontEnd/Http/Controllers/PlatformCallController.cs#L199-L244) 와 같은 특정 상호 작용을 기록하기 위한 API 사용에 대한 예제도 [있습니다](https://github.com/microsoftgraph/microsoft-graph-comms-samples/blob/a3943bafd73ce0df780c0e1ac3428e3de13a101f/Samples/BetaSamples/LocalMediaSamples/ComplianceRecordingBot/FrontEnd/Bot/CallHandler.cs#L121-L126).
Graph API에 대한 자세한 설명서는 [UpdateRecordingStatus](/graph/api/call-updaterecordingstatus?tabs=http) 및 [들어오는Context에 대해 여기에서 찾을 수 있습니다](/graph/api/resources/incomingcontext).
...
알림에 대한 지원은 클라이언트 환경을 사용하여 Teams 있습니다. 환경은 시각적 또는 오디오일 수 있습니다.
...
> 사용자가 인터넷 정전을 경험하고 SBA를 사용하여 PSTN 호출을 받고 있는 경우 규정 준수 기록이 작동하지 않습니다.
...
인증 프로그램에 참가하기를 원하는 공급업체인 경우 해당 공급업체에 메일을 [Teamscategorypartner@microsoft.com](mailto:Teamscategorypartner@microsoft.com).
