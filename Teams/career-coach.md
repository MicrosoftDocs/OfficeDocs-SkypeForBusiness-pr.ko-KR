...
커리어 코치 EES(등록용 EES), CSP(클라우드 서비스 공급자) 및 웹 직접(웹 직접)을 통해 자격을 갖춘 고등 교육 기관에 대한 추가 Microsoft 365 관리 센터 사용할 수 있습니다. 추가 Microsoft Teams 라이선스를 구입하려면 Microsoft 365 A3/A5 또는 Office 365 A1/A3/A5가 커리어 코치 있어야 합니다. 학생 및 교직원/교직원 사용자를 위해 별도의 라이선스가 제공됩니다.

25명 학생 및 25개 교직원 라이선스에 대해 표준 90일 무료 평가판을 사용할 수 있습니다. 평가판 라이선스는 Microsoft 365 관리 센터 구매 자격을 갖춘 테넌트에 의해 커리어 코치.
...
## <a name="career-coach-settings-status"></a>커리어 코치 상태

커리어 코치 관리 센터의 Teams 설정 페이지에서 앱을 구성하기 위한 불완전, 보류 중, 완료 및 실패한 단계의 상태 보고서를 제공합니다. 이러한 상태는 테넌트에 릴리스할 준비가 커리어 코치 있는지 여부를 확인하는 데 도움이 될 수 있습니다.

### <a name="configuration-status"></a>구성 상태

앱 설정 페이지의 구성 상태 섹션에 현재 상태가 표시됩니다.

![경력 코치 앱의 구성 상태 섹션입니다.](media/career-coach-config-status.png)

| 범주              | 상태                    | 설명                                                 |
| --------------------- | ------------------------- | ----------------------------------------------------------- |
| 서비스 프로비전  | 보류 중                   | 앱이 테넌트에 추가됩니다. 추가 작업이 필요하지 않습니다. |
| 서비스 프로비전  | 완료                  | IT 관리자가 설정을 제출할 준비가 완료되었습니다.                      |
| 브랜드 및 기본 설정 | 시작되지 않은 경우               | 설정 제출해야 합니다.                              |
| 브랜드 및 기본 설정 | 필수 _필드가 누락된 경우_ | IT 관리자는 누락된 필드를 추가하거나 업로드해야 합니다.         |
| 브랜드 및 기본 설정 | 완료                  | 추가 작업이 필요하지 않습니다.                                   |
| 과정 카탈로그        | 시작되지 않은 경우               | 카탈로그를 제출해야 합니다.                              |
| 과정 카탈로그        | 불완전                | 해결 방법에 대한 자세한 내용은 섭취 상태를 검사합니다.   |
| 과정 카탈로그        | 완료                  | 추가 작업이 필요하지 않습니다.                                   |
| LinkedIn 연결   | 시작되지 않은 경우               | LinkedIn 학교 페이지 URL을 제출해야 합니다.             |
| LinkedIn 연결   | 보류 중                   | LinkedIn 학교 페이지 관리자 승인을 기다리고 있습니다.               |
| LinkedIn 연결   | 완료                  | 추가 작업이 필요하지 않습니다.                                   |
| 연구 분야       | 시작되지 않은 경우               | 문서를 제출해야 합니다.                             |
...
문서가 업로드된 후 코스 카탈로그 상태가 코스 카탈로그 설정 페이지에 표시되어 문서 업로드 및 처리에 대한 세부 정보를 제공합니다.


![커리어 코치 앱의 과정 카탈로그 업로드 상태입니다.](media/career-coach-course-catalog-status.png)

| 열           | 값     | 설명                                                                                        |
| ---------------- | --------- | -------------------------------------------------------------------------------------------------- |
| 업로드된 시간    | 타임스탬프 | IT 관리자가 문서를 업로드한 날짜 및 시간입니다.                                                     |
| 완료된 시간   | 타임스탬프 | 문서가 완전히 처리된 날짜 및 시간입니다.                                               |
| 업로드된 과정 | 정수   | 문서에서 찾을 수 있는 과정의 수입니다.                                                           |
| 섭취 상태 | 보류 중   | 처리를 위해 큐에 있는 문서입니다.                                                                  |
| 섭취 상태 | 실행 중   | 문서는 현재 처리됩니다. 이 프로세스는 최대 60분 정도 걸릴 수 있습니다.                        |
| 섭취 상태 | 성공   | 섭취 프로세스는 완료 및 과정으로 완전히 구성되면 앱에서 사용할 수 있습니다. |
| 섭취 상태 | 실패했습니다.    | 문서 형식을 확인하고 다시 로드합니다.                                                            |
| 중복       | 정수   | 문서에 있는 중복된 과정의 수입니다.                                                 |

> [!NOTE]
> 열이 비어 있는 경우 문서가 현재 처리되고 있으며 해당 값을 사용할 수 없습니다. 문서가 처리되고 나면 값이 채워지게 됩니다. 페이지를 새로 고쳐 업데이트를 확인할 수 있습니다.
