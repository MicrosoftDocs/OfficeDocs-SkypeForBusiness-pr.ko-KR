...
자동 전화 연결은 대부분의 경우 시스템의 노드로, 발신자에게 사용자 운영자가 아닌 음성 프롬프트 또는 음성 파일을 일련의 음성 프롬프트 또는 오디오 파일을 제공합니다. 사용자가 자동 전화 접속과 연결된 번호로 전화를 걸면 해당 사용자가 선택한 전화가 사용자에게 리디렉션되거나 조직에서 다른 사람을 찾은 다음 해당 사용자에게 연결할 수 있습니다. DTMF(휴대폰 키패드) 또는 음성 인식을 사용하여 선택 항목을 표현하고 메뉴 시스템과 상호 작용할 수 있습니다. 다른 자동 전화 걸기 또는 통화 큐로 통화를 리디렉션할 수도 있습니다.
...
- 이렇게 하면 전화가 걸려와 조직 내 사용자에 대한 메시지에 도달하거나 메시지를 남길 수 있습니다.
...
- 자동 전화 번호에 전화 번호를 할당할 때 엄격하게 말하면 해당 자동 전화 연결과 연결된 리소스 계정에 전화 번호를 할당하는 것입니다. 이렇게 하면 두 개 이상의 전화 번호가 자동 전화 접속에 액세스할 수 있습니다. 대부분의 경우 리소스 계정은 비용 없는 전화 시스템 가상 사용자 라이선스를 사용하게 됩니다. 이 라이선스는 조직 수준에서 전화 번호에 전화 시스템 기능을 제공하며, 자동 전화 회의 및 통화 큐를 만들 수 있습니다.

> [!NOTE]
> 자동 전화 걸기 및 통화 큐에 대한 직접 라우팅 서비스 번호는 Microsoft Teams 사용자 및 통화 에이전트에만 지원됩니다.

   > [!TIP]
   > 전화 시스템 라이선스가 있는 온라인 사용자인 운영자  또는 메뉴 옵션으로 통화를 리디렉션하려면 통화에 대한 계정을 사용하도록 설정하거나 Enterprise Voice 요금제에 할당해야 합니다. [Microsoft Teams 추가 기능 라이선스 할당을 참조합니다.](teams-add-on-licensing/assign-teams-add-on-licenses.md) 또한 다음을 사용할 Windows PowerShell. 예를 들어 다음을 실행합니다.  `Set-CsUser -identity "Amos Marble" -EnterpriseVoiceEnabled $true`
...
- 전체 시스템의 비대화 수준 자동 전화 걸기 또는 통화 큐는 아웃바운드 PSTN 통화를 하는 경우 전화 번호만 필요합니다.
...
이름별 전화 걸기 및 내선 번호로 전화 걸기에서 특정 사용자를 검색할 때 지원할 수 있는 Active Directory 사용자 수에는 제한이 없습니다. 호출자는 부분 또는 전체 이름(FirstName + LastName 및 LastName + FirstName)을 입력할 수 있지만 전체 확장 번호가 필요합니다. 단일 자동 참석자가 음성 인식 사용을 지원할 수 있는 최대 이름 목록 크기는 80,000명입니다.
...
> [!NOTE]
> 음성 인식으로 Dial by Name을 사용하지만 조직의 Active Directory가 80,000명을 넘고 다이얼 범위 기능을 사용하여 전화로 전화 걸기의 범위를 제한하지 않은 경우 전화기 키패드를 사용하여 발신자에 대해 전화 걸기 기능을 사용할 수 있으며 음성 입력은 다른 모든 시나리오에서 사용할 수 있습니다. 다이얼 범위 기능을 사용하면 특정 자동 전화 접속자에 대해 Dial by Name의 범위를 변경하여 도달 가능한 이름을 좁힐 수 있습니다.
...
- 조직에서 설정한 모든 기존 호출 큐입니다. 호출 큐에 대한 자세한 내용은 클라우드 호출 큐 [만들기를 참조합니다.](/SkypeForBusiness/what-is-phone-system-in-office-365/create-a-phone-system-call-queue)

### <a name="business-hours-and-call-handling"></a>업무 시간 및 통화 처리

각 자동 참석자에 업무 시간을 설정할 수 있습니다. 업무 시간을 설정하지 않은 경우 24/7 일정이 기본적으로 설정되어 있기 때문에 해당 날의 모든 일 및 모든 시간은 업무 시간으로 간주됩니다. 업무 시간은 하루 중 시간의 시간으로 설정할 수 있으며, 근무 시간으로 설정되지 않은 모든 시간은 시간 후로 간주됩니다. 업무 시간 및 근무 시간 이후의 다른 수신 통화 처리 옵션 및 다른 인사말(선택 사항)을 설정할 수 있습니다.
...
이러한 옵션은 메뉴 프롬프트가 재생될 때 자동 전화 요청에 의해 발신자에 표시됩니다. 예: "판매를 위해 1을 누르고 서비스에 대해 2를 누르기. 운영자에게 말하기 위해, 아무 때나 0을 누르면 됩니다."

### <a name="set-menu-options"></a>설정 메뉴 옵션
...
Microsoft 서비스 번호, 직접 라우팅 번호 또는 하이브리드 번호를 자동 전화 담당자의 연결된 리소스 계정(또는 여러 개의 전화 번호가 필요한 경우 여러 리소스 계정에 할당)할 수 있습니다. 자세한 내용은 직접 라우팅 계획(Plan [Direct Routing)을](direct-routing-plan.md) 참조합니다.

서비스 번호를 할당하려면 기존 무료 또는 무료 서비스 번호를 얻거나 이식해야 합니다. 무료 또는 무료 서비스 전화 번호를 받을 경우 비즈니스용 **Skype** 관리 센터 음성 전화 번호에  >    >  **표시됩니다.** **번호 유형은** 서비스 **- 무료로 나열됩니다.** 서비스 번호를 확인하려는 경우 비즈니스용 Skype 및 [Microsoft Teams의](/microsoftteams/getting-service-phone-numbers) 서비스 전화 번호 확인 또는 기존 서비스 번호를 이전하려는 경우 Teams로 전화 번호 이전을 [참조하세요.](phone-number-calling-plans/transfer-phone-numbers-to-teams.md)
