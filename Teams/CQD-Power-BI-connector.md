...
> [!NOTE]
> CQD Power BI 커넥터는 Power BI에서 DirectQuery만 지원됩니다. 가져오기 모드는 지원되지 않습니다. 
...
이 Microsoft Teams용 Power BI 커넥터의 새 버전이 릴리스된 경우 *사용자* 지정 커넥터 디렉터리의 이전 커넥터 파일을 새 파일로 바꾸기만 합니다.
...
5. 마지막으로, CQD에 대한 전체 데이터 모델을 보여주는 최종 프롬프트가 표시될 것입니다. 이 시점에는 데이터가 표시되지 않습니다. CQD에 대한 데이터 모델만 표시됩니다. *[로드]를* 선택하여 설치 프로세스를 완료합니다.

6. 이 시점에서 Power BI는 창의 오른쪽에 데이터 모델을 로드합니다. 그렇지 않으면 페이지가 비어 있으며 기본적으로 쿼리가 로드되지 않습니다. 아래에서 **쿼리** 작성을 진행하여 쿼리를 작성하고 데이터를 반환합니다.
...
3. 다음으로 필터링할 차원을 선택하고 필터 창에서  이 시각적 필드의 필터로 *끌어다* 놓습니다. CQD Power BI 커넥터는  현재 기본 필터링(가능한 차원 값 목록에서 값 선택), *고급* 필터링(고급 CQD와 유사하게 필터링할 값 및 피연산자  지정)  및 상대 날짜 필터링(종료 시간 및 시작 시간 차원에만 사용 가능)을 제공합니다.  상위 N에 따라 *필터링은* CQD에서 지원되지 않습니다.
...
4. 마지막으로 시각화 *창* 내에서 서식  탭을 선택하여 쿼리의 스타일을 지정하고 서식을 지정합니다.
    ...
    > [!NOTE]
    > CQD 쿼리를 실행하려면 하나 이상의 측정값이 요구됩니다. 쿼리가 로드되지 않는 경우 쿼리에 측정값이 포함되어 있지 않은지 다시 한 번 검사합니다.
...
호출 품질 커넥터 쿼리는 드릴스루 기능을 염두에 두어 설계해야 합니다. 모든 데이터를 한 번 로드한 다음 필터를 사용하여 아래로 스크롤하는 대신 더 광범위한 카디널리티 쿼리로 시작하고 높은 카디널리티 쿼리로 드릴다운합니다. 예를 들어 품질 문제에 가장 많이 기여하는 서브넷을 진단하려고 할 때 먼저 문제에 기여하는 해당 지역 및 국가를 식별한 다음 해당 지역 또는 국가의 서브넷으로 드릴다운하는 것이 유용합니다. 호출 품질 커넥터 템플릿은 예제 역할을 하도록 이러한 방식으로 디자인되었습니다.
...
> **이 시각적 개체에** 대한 데이터를 로드할 수 없습니다. OLE DB 또는 ODBC 오류: [Expression.Error] 데이터 원본에 식을 접을 수 없습니다. 더 간단한 식을 사용해 주세요.

날짜 슬라이서는 Power BI 커넥터에서 지원되지 않습니다. 날짜 범위를 지정하기 위해 보고서에 두 개의 필터를 적용하여 날짜보다 작고 큰 값을 지정합니다.
...
**<sup>1</sup>** 특정 프로세스 및 앱(예: OneDrive)으로 인해 문서 루트 폴더가 변경될 수 있습니다. Power BI Desktop 사용자 지정 커넥터 디렉터리가 현재 루트 폴더 *\\ Documents* 폴더 안에 있는지 확인합니다.
...
### <a name="why-does-the-connector-seem-slower-compared-to-advanced-cqd-in-the-browser-what-can-i-do-to-improve-performance"></a>브라우저에서 고급 CQD에 비해 커넥터가 느리게 보이는 이유는 무엇입니까? 성능을 향상하기 위해 무엇을 할 수 있는가

다양한 템플릿에 대한 쿼리 성능은 실제로 브라우저와 커넥터에서 동일합니다.  다른 독립 실행형 앱과 마찬가지로 Power BI는 성능에 인증 및 렌더링 시간을 추가합니다. 또한 차이점은 실행되는 동시 쿼리 수에 있습니다. CQD의 브라우저 내 버전에는 덜 잘 개발되고 정보 밀도가 높은 시각화 옵션이 있기 때문에 대부분의 보고서는 한에 2-3개 쿼리를 로드하는 것으로 제한되었습니다. 반면에 커넥터 템플릿은 종종 20개 이상의 동시 쿼리를 표시합니다. 이전처럼 응답성이 뛰어난 보고서를 빌드하려는 경우 탭당 쿼리가 2~3개가 넘지 않습니다.
