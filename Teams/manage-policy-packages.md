...
- **[보기:](#view-the-settings-of-a-policy-in-a-policy-package)** 정책 패키지에서 정책을 본다. 그런 다음 패키지를 할당하기 전에 패키지에서 각 정책의 설정을 확인합니다. 각 설정을 이해해야 합니다. 미리 정의한 값이 조직에 적합한지 또는 조직의 요구에 따라 더 제한적이거나 유의적으로 변경해야 하는지 여부를 결정하세요.
