...
- **image** - 이미지.
- **inAppNotification**- 사용자가 앱에서 활성 상태인 동안 알림을 탭할 때 트리거됩니다.
...
- **notification/notification_clicked** – 알림을 탭할 때 트리거됩니다.
...
- **SignInWithOTP** - 사용자가 OTP(일회성 암호)를 사용하여 게스트로 로그인하는 옵션을 선택합니다. 
- **signUp** - **무료 계정 만들기** 또는 **무료 가입** 이 선택됩니다.
- **SignUpFromSignIn**- 사용자가 로그인에서 **새 계정 만들기** 옵션을 탭합니다.
...
- **SSOAccountListItem**: 사용자가 SSO 계정을 탭하여 로그인할 때 트리거됩니다.
...
- **전환** - 테넌트 또는 계정이 앱에서 전환됩니다. 이는 계정/테넌트 전환 문제를 사전에 측정하고 원활한 계정/테넌트 전환 환경을 제공하는 데 필요합니다.
...
- **appInstall**: 사용자가 설치 후 처음으로 앱을 열 때 트리거됩니다.
...
- **signInSSOPage**: 사용자가 로그인되어 있는 동안 Single Sign-On 페이지를 볼 때 트리거됩니다.
-- **signInError**: 사용자가 로그인하는 동안 오류가 발생하면 트리거됩니다. 로그인되어 있는 동안 사용자가 직면하는 문제를 사전에 식별하고 해결하는 데 필요합니다. 
...
- **auth_adal_tokens**- 자동 인증을 수행하려면 서비스 호출이 필요합니다. 사용자가 앱을 시작할 때 트리거되거나 만료 시 토큰이 새로 고쳐집니다.
- **chat_add_giphy** - Giphy GIF 렌더링 작업이 성공했는지 실패했는지 확인합니다.
- **chat_send_message_sfc**- SfC interop 채팅에서 채팅 메시지를 보낼 때 트리거됩니다.
...
- **create_new_chat_thread_sfc**- SfC interop 채팅에 대한 새 채팅 스레드가 만들어질 때 트리거됩니다.
...
- **interactiveAuthNopa2** – 암호 사용자가 대화형 인증을 수행하는 데 방해가 되지 않을 때 트리거됩니다.
...
- **saveMeProfile**- 사용자가 프로필을 저장할 때 트리거되는 필수 서비스 호출
