...
모임 **> -Teams** 회의 브리지에서 또는 비즈니스용 Skype Online PowerShell cmdlet Get-CsOnlineDialInConferencingBridge를 사용하여 관리 센터에서 서비스 번호를 찾을 수 있습니다. 자세한 내용은 의 오디오 회의 번호 목록을 [Microsoft Teams.](see-a-list-of-audio-conferencing-numbers-in-teams.md)
...
전화 접속 Teams 전화 회의 브리지의 기본 서비스 번호에서 오는 것입니다. 오디오 회의 브리지의 기본 서비스 번호에 대한 자세한 내용은 오디오 회의 브리지의 전화 번호 변경을 [참조하세요.](change-the-phone-numbers-on-your-audio-conferencing-bridge.md)
...
음성 경로는 모임에서 전화를 걸 수 있는 전화 번호를 기반으로 통화를 라우팅하는 데 사용할 PSTN Teams 결정됩니다. 음성 경로는 모임에서 전화한 전화 번호를 regex 패턴으로 Teams 특정 통화를 라우팅하는 데 사용할 PSTN 게이트웨이를 결정합니다. 음성 경로를 만들 때 경로가 하나 이상의 PSTN 사용량에 연결되어야 합니다.
