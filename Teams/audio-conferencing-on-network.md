...
ms.custom:
- Audio Conferencing
description: 다음은 오디오 회의 대한 온-네트워크 설명입니다.
ms.openlocfilehash: 9b986bb8cd4d432c563e60f03e2dcdf496749f36
ms.sourcegitcommit: 296862e02b548f0212c9c70504e65b467d459cc3
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 05/25/2022
ms.locfileid: "65675960"
---
# <a name="on-network-conferencing-for-audio-conferencing"></a>오디오 회의 대한 네트워크 회의

온-네트워크 회의를 통해 조직은 직접 라우팅을 통해 인바운드 및 아웃바운드 오디오 회의 전화를 Microsoft 전화 접속 번호로 보낼 수 있습니다. 이 기능은 오디오 회의 지원을 타사 전화 접속 번호로 확장하기 위한 것이 아닙니다. Microsoft 오디오 회의 Bridge에서 타사 전화 접속 전화 번호 또는 PSTN에 대한 아웃바운드 통화를 통해 오디오 회의 서비스에 대한 인바운드 통화를 라우팅하는 데 사용되는 경우 온-네트워크 회의는 지원되지 않습니다.
...
- 오디오 회의 대해 사용하도록 설정되었거나 사용하도록 설정된 조직의 모든 사용자가 모든 모임에 Teams 사용하고 있는지 확인합니다. 온-네트워크 회의를 통한 인바운드 및 아웃바운드 오디오 회의 호출의 라우팅은 Teams 모임에서만 지원됩니다.

- 온-네트워크 회의를 사용할 모든 사용자에게 오디오 회의 라이선스를 할당합니다.

- 오디오 회의 서비스를 설정합니다. 자세한 내용은 [Microsoft Teams 대한 오디오 회의 설정을](set-up-audio-conferencing-in-teams.md) 참조하세요.
...
- 직접 라우팅을 위해 SBC(세션 테두리 컨트롤러)를 설정합니다. 자세한 내용은 [직접 라우팅 계획](direct-routing-plan.md) 및 [직접 라우팅 구성을 참조하세요](direct-routing-configure.md).
  ...
  오디오 회의 목적으로만 직접 라우팅을 설정하는 경우 온-네트워크 회의를 위해 "1단계: SBC 커넥트"만 완료하면 됩니다.

## <a name="enable-the-routing-of-dial-in-calls-to-microsoft-audio-conferencing-through-direct-routing"></a>직접 라우팅을 통해 Microsoft 오디오 회의 대한 전화 접속 통화 라우팅 사용

직접 라우팅을 통해 온-프레미스 사용자가 수행한 전화 접속 통화를 오디오 회의 서비스로 라우팅하려면 SBC 및 PBX(Private Branch Exchange)에 대한 적절한 라우팅 규칙을 구성해야 합니다.

직접 라우팅 트렁크를 통해 조직의 전화 회의 브리지의 서비스 번호로 통화를 라우팅하도록 사이트의 전화 통신 장비를 구성해야 합니다.

**모임 -> 회의 브리지** 의 Teams 관리 센터에서 또는 비즈니스용 Skype Online PowerShell cmdlet Get-CsOnlineDialInConferencingBridge를 사용하여 서비스 번호를 찾을 수 있습니다. 자세한 내용은 [Microsoft Teams 오디오 회의 숫자](see-a-list-of-audio-conferencing-numbers-in-teams.md) 목록을 참조하세요.

> [!NOTE]
> 분당 지불 오디오 회의 라이선스를 가진 사용자는 이 기능을 사용할 수 없습니다.

## <a name="enable-the-routing-of-teams-meeting-dial-out-calls-through-direct-routing"></a>직접 라우팅을 통해 Teams 모임 전화 걸기 통화 라우팅 사용

Teams 모임 전화 통화는 조직의 모임 내에서 전화 통화 및 모임에 새 참가자를 가져오기 위한 전화 통화를 포함하여 PSTN 번호로 시작됩니다.
...
Teams 모임의 전화 접속 전화는 회의 브리지의 기본 서비스 번호에서 제공됩니다. 오디오 회의 브리지의 기본 서비스 번호에 대한 자세한 내용은 [오디오 회의 브리지의 전화 번호 변경을 참조하세요](change-the-phone-numbers-on-your-audio-conferencing-bridge.md).
...
음성 경로는 Teams 모임에서 전화를 건 전화 번호를 기반으로 통화를 라우팅하는 데 사용해야 하는 PSTN 게이트웨이를 결정합니다. 음성 경로는 Teams 모임에서 전화를 건 전화 번호를 regex 패턴과 일치시켜 지정된 통화를 라우팅하는 데 사용해야 하는 PSTN 게이트웨이를 결정합니다. 음성 경로를 만들 때 경로는 하나 이상의 PSTN 사용량에 연결되어야 합니다.
...
오디오 회의 음성 라우팅 정책은 모임 전화 접속 통화의 대상 전화 번호에 따라 이끌이의 모임에서 시작된 통화를 라우팅하는 데 사용할 수 있는 경로를 결정합니다. 오디오 회의 음성 라우팅 정책은 하나 이상의 PSTN 사용과 연결되어 있으므로 정책과 연결된 이끌이의 모임 전화 걸기 호출에 사용할 수 있는 경로를 결정합니다.
...
기본적으로 Teams 사용자는 E.164 형식( +\<country code\>\<number\>)으로 PSTN 번호로 전화를 걸 수 있습니다. 그러나 다이얼 플랜을 사용하여 사용자가 다른 형식(예: 4자리 확장)으로 전화 번호를 걸 수 있도록 할 수 있습니다.
