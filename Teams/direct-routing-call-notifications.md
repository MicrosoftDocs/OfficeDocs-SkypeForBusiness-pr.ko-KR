...
ms.locfileid: "62457188"
---
# <a name="manage-call-notifications"></a>통화 알림 관리

이 문서에서는 직접 라우팅 사용자의 통화 알림을 관리하는 방법을 설명합니다. PBX(개인 분기)Teams(PBX) 또는 세션 경계 컨트롤러(Exchange)에 대해 호출 엔드포인트를 구성할 수 있습니다. 이 설정은 사용자의 모바일 및 책상 전화에 동시에 전화를 보내고자 하는 경우와 같은 유용합니다.   
...
호출이 도착하면 SBC는 직접 라우팅과 타사 SBC 간에 호출을 포크합니다.
...
호출은 호출의 통화 세부 정보 레코드에 Teams 전화 시스템 호출로 등록되지 않습니다. 호출은 최종 SIP 코드 "487", 최종 Microsoft 하위 코드 "540200", 최종 SIP 코드 구 "다른 곳에서 완료된 호출"으로 "시도"로 등록됩니다.  (통화 세부 정보 레코드를 확인하려면 Teams 관리 센터 -> 및 **ReportsUsage** ->  보고서로 이동하고 **PSTN** 사용량을 선택합니다.)
