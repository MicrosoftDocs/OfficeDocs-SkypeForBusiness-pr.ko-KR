...
이 문서에서는 를 사용하여 디바이스의 통합된 종 :::no-loc text="Microsoft Teams Rooms"::: 단-종단-종단 모니터링을 설정하고 배포하는 방법을 설명합니다 :::no-loc text="Azure Monitor":::.

를 관리하는 :::no-loc text="Log Analytics"::: :::no-loc text="Azure Monitor"::: 데 도움이 되는 기본 원격 분석 및 경고를 제공하도록 내에서 구성할 수 있습니다 :::no-loc text="Microsoft Teams Rooms":::. 관리 솔루션이 완성되면 추가 데이터 및 관리 기능을 배포하여 디바이스 가용성 및 성능에 대한 자세한 보기를 만들 수 있습니다.

이 가이드를 따라 다음 예제와 같은 대시보드를 사용하여 디바이스 가용성, 애플리케이션 및 하드웨어 상태 및 :::no-loc text="Microsoft Teams Rooms"::: 애플리케이션 및 운영 체제 버전 배포에 대한 자세한 상태 보고를 얻을 수 있습니다.
...
2. [관리 설정에 대한 테스트 :::no-loc text="Log Analytics"::: 디바이스 구성](azure-monitor-deploy.md#configure_test_devices)
3. [사용자 지정 필드 매핑](azure-monitor-deploy.md#Custom_fields)
4. [에서 보기 :::no-loc text="Microsoft Teams Rooms"::: 정의 :::no-loc text="Log Analytics":::](azure-monitor-deploy.md#Define_Views)
...
> [!IMPORTANT]
> 최소한의 구성으로 운영 :::no-loc text="Azure Monitor"::: :::no-loc text="Log Analytics"::: :::no-loc text="Windows"::: 체제를 실행하는 컴퓨터를 모니터링할 수 있습니다. :::no-loc text="Microsoft Teams Rooms"::::::no-loc text="Microsoft Teams Rooms"::: 에이전트를 모든 장치에 배포하기 전에 수행해야 하는 몇 가지 단계가 있습니다.
...
에서 로그 :::no-loc text="Log Analytics"::: 수집을 시작하려면 작업 영역이 필요합니다 :::no-loc text="Microsoft Teams Rooms":::. 작업 영역은 고유한 데이터 리포지토리 :::no-loc text="Log Analytics"::: , 데이터 원본 및 솔루션이 있는 고유한 환경입니다. 기존 작업 :::no-loc text="Log Analytics"::: :::no-loc text="Microsoft Teams Rooms"::: :::no-loc text="Log Analytics"::: :::no-loc text="Microsoft Teams Rooms"::: 영역이 이미 있는 경우 배포를 모니터링하는 데 사용할 수도 있습니다. 또는 또는 모니터링 요구에 따라 전용 작업 영역도 만들 수 있습니다.

새 작업 :::no-loc text="Log Analytics"::: 영역 만들기가 필요한 경우 포털 [:::no-loc text="Log Analytics"::: 에서 작업 영역 만들기 문서의 지침을 따릅니다:::no-loc text="Azure":::.](/azure/azure-monitor/learn/quick-create-workspace)

> [!NOTE]
> 를 :::no-loc text="Log Analytics"::: 사용하려면 :::no-loc text="Azure Monitor":::활성 구독이 필요합니다 :::no-loc text="Azure"::: . 구독이 :::no-loc text="Azure"::: 없는 경우 무료 평가판 구독을 시작점 [으로 만들 수](https://azure.microsoft.com/free) 있습니다.

### <a name="configure-no-loc-textlog-analytics-to-collect-no-loc-textmicrosoft-teams-rooms-event-logs"></a>이벤트 :::no-loc text="Log Analytics"::: 로그를 수집하도록 :::no-loc text="Microsoft Teams Rooms"::: 구성

:::no-loc text="Log Analytics"::: 설정에 지정된 :::no-loc text="Windows"::: 이벤트 로그에서만 이벤트를 수집합니다. 각 로그에 대해 선택한 심각도의 이벤트만 수집됩니다.

디바이스 및 애플리케이션 :::no-loc text="Log Analytics"::: :::no-loc text="Microsoft Teams Rooms"::: 상태를 모니터링하는 데 필요한 로그를 수집하도록 구성해야 합니다. :::no-loc text="Microsoft Teams Rooms"::: 를 **:::no-loc text="Skype Room System":::** 사용하여 이벤트 로그를 사용할 수 있습니다.

:::no-loc text="Log Analytics"::: 이벤트를 수집하도록 :::no-loc text="Microsoft Teams Rooms"::: 구성하려면 [:::no-loc text="Windows"::: 에서 이벤트 로그 데이터 원본을 참조하세요:::no-loc text="Azure Monitor":::.](/azure/azure-monitor/platform/data-sources-windows-events)

![이벤트 로그 설정 스크린샷.](../media/Deploy-Azure-Monitor-2.png "이벤트 로그 설정")

> [!IMPORTANT]
> 이벤트 :::no-loc text="Windows"::: 로그 설정을 구성하고 이벤트 **:::no-loc text="Skype Room System":::** 로그 이름으로 입력한 다음 **오류, 경고** 및 정보 확인란을 선택합니다.

## <a name="configure-test-devices-for-azure-monitoring"></a>Azure Monitoring에 대한 테스트 디바이스 구성
<a name="configure_test_devices"> </a>

관련 이벤트를 :::no-loc text="Log Analytics"::: 모니터링할 :::no-loc text="Microsoft Teams Rooms":::수 있게 준비해야 합니다. 먼저 에이전트 :::no-loc text="Microsoft Monitoring"::: :::no-loc text="Microsoft Teams Rooms"::: 를 물리적으로 액세스할 수 있는 하나 또는 두 개의 디바이스에 배포하고 해당 :::no-loc text="Log Analytics"::: 테스트 디바이스를 사용하여 일부 데이터를 생성하고 작업 영역으로 푸시해야 합니다.

### <a name="install-no-loc-textmicrosoft-monitoring-agents-to-test-devices"></a>디바이스 :::no-loc text="Microsoft Monitoring"::: 를 테스트하기 위해 에이전트 설치

의 :::no-loc text="Microsoft Monitoring"::: 서비스에 커넥트 지침을 사용하여 에이전트를 테스트 [디바이스 :::no-loc text="Windows"::: 에 :::no-loc text="Log Analytics"::: 배포합니다:::no-loc text="Azure":::](/azure/azure-monitor/platform/agent-windows). 이 문서에서는 :::no-loc text="Microsoft Monitoring"::: *:::no-loc text="Windows"::::::no-loc text="Log Analytics":::**배포** 에 연결된 디바이스를 얻기 위한 작업 영역 ID_ 및 _ *_primary key_**:::no-loc text="Microsoft Teams Rooms"::: :::no-loc text="Azure Monitor"::: :::no-loc text="Log Analytics"::: 에 대한 에이전트를 배포하는 단계 및 인스턴스에 대한 에이전트 연결을 확인하는 단계에 대한 자세한 정보를 제공합니다.

### <a name="generate-sample-no-loc-textmicrosoft-teams-rooms-events"></a>샘플 이벤트 :::no-loc text="Microsoft Teams Rooms"::: 생성

에이전트를 :::no-loc text="Microsoft Monitoring"::: 테스트 디바이스에 배포한 후 필요한 이벤트 로그 데이터가 에 의해 수집되는지 확인 :::no-loc text="Azure Monitor":::

> [!NOTE]
> 에이전트를 :::no-loc text="Microsoft Monitoring"::: 설치한 후 디바이스를 다시 부팅하고 모임 :::no-loc text="Microsoft Teams Rooms"::: 앱이 시작된지 확인하여 이벤트 로그에 새 이벤트를 생성할 수 있도록 합니다.

1.  포털에 [:::no-loc text="Microsoft Azure"::: 로그인](https://portal.azure.com) 하고 작업 영역 :::no-loc text="Log Analytics"::: 으로 이동하여 선택합니다.

2.  디바이스에서 생성된 하트비트 이벤트를 나열 :::no-loc text="Microsoft Teams Rooms"::: 합니다.
    1.  작업 영역 및 로그로 이동하고 쿼리  를 사용하여 에 대한 사용자 지정 필드가 있는 하트비트 레코드를 검색합니다:::no-loc text="Microsoft Teams Rooms":::.
    2.  샘플 쿼리: `Event | where Source == "SRS-App" and EventID == 2000`

3.  쿼리가 모임 앱에서 생성된 이벤트를 포함 하는 :::no-loc text="Microsoft Teams Rooms"::: 로그 레코드를 반환 하는지 확인 합니다.

4.  하드웨어 문제를 생성하고 필요한 이벤트가 에 로그인되어 있는지 유효성을 검사합니다 :::no-loc text="Azure Log Analytics":::.
    1.  테스트 시스템의 주변 장치 중 하나를 확정합니다 :::no-loc text="Microsoft Teams Rooms"::: . 카메라, 스피커, 마이크 또는 Front Room 디스플레이일 수 있습니다.
    2.  이벤트 로그가 에 채워지기까지 10분 정도 기다립니다 :::no-loc text="Azure Log Analytics":::.
...
사용자 지정 필드를 사용하여 이벤트 로그에서 특정 데이터를 추출합니다. 나중에 타일, 대시보드 보기 및 경고와 함께 사용할 사용자 지정 필드를 정의해야 합니다. 사용자 [지정 필드를 만들기 :::no-loc text="Log Analytics":::](/azure/azure-monitor/platform/custom-fields) 전에 에서 사용자 지정 필드를 참조하고 개념에 익숙해지면 됩니다.

캡처된 이벤트 로그에서 사용자 지정 필드를 추출하려면 다음 단계를 수행합니다.

1.  포털에 [:::no-loc text="Microsoft Azure"::: 로그인](https://portal.azure.com) 하고 작업 영역 :::no-loc text="Log Analytics"::: 으로 이동하여 선택합니다.

2. 디바이스에서 생성된 이벤트를 나열 :::no-loc text="Microsoft Teams Rooms"::: 합니다.
   1.  로그 **로 이동** 하고 쿼리를 사용하여 사용자 지정 필드가 있는 레코드를 검색합니다.
   2.  샘플 쿼리: `Event | where Source == "SRS-App" and EventID == 2000`

3. 레코드 중 하나를 선택하고 왼쪽 단추를 선택하고 필드 추출 마법사를 시작합니다.
4. RenderedDescription에서 추출할 데이터를 강조 표시하고 필드 제목을 제공합니다. 사용할 필드 이름은 표 1에 제공됩니다.
5. 표 1에 표시된 매핑 *을 사용합니다*. :::no-loc text="Log Analytics"::: 새 필드를 **\_정의하면 CF** 문자열이 자동으로 추가됩니다.

> [!IMPORTANT]
> 모든 JSON 및 필드는 :::no-loc text="Log Analytics"::: 대소문자 구분입니다.
> 
> 아래 표의 각 사용자 지정 필드에 필요한 쿼리에 주의하세요. 사용자 지정 필드 값을 :::no-loc text="Log Analytics"::: 성공적으로 추출하려면 올바른 쿼리를 사용해야 합니다.
> 
**표 1**
...
## <a name="define-the-no-loc-textmicrosoft-teams-rooms-views-in-no-loc-textlog-analytics"></a>에서 보기 :::no-loc text="Microsoft Teams Rooms"::: 정의 :::no-loc text="Log Analytics":::
...
>
> [!IMPORTANT]
> [Azure Monitor의 뷰 디자이너가 2023년 8월 31일부터 사용 중지되고 2020년 11](https://azure.microsoft.com/updates/view-designer-in-azure-monitor-is-retiring-on-31-august-2023/) 월 30일에는 기능 만들기 및 복제가 비활성화됩니다. 통합 문서는 대신 사용할 수 있습니다. 통합 문서로 보기 디자이너 전환 가이드에 대한 자세한 내용은 미리 설정된 보기 디자이너 템플릿을 사용하여 [빠른 시작을 참조하세요](/azure/azure-monitor/visualize/view-designer-conversion-tasks#quickstart-with-preset-view-designer-templates).
...
1.  갤러리 **에서 &** 번호 목록을 선택한 다음 새 타일을 추가합니다.
2.  일반 **속성을** 정의합니다.<br>
    **그룹 제목:** 비워 두기<br>
    **새 그룹:** 선택되지 않았습니다.
3.  **타일 속성을** 정의합니다.<br>
...
1.  갤러리 **에서 도넛 &** 목록을 선택한 다음 새 타일을 추가합니다.
2.  일반 **속성을** 정의합니다.<br>
...
### <a name="create-a-tile-that-displays-no-loc-textmicrosoft-teams-rooms-application-versions"></a>애플리케이션 버전을 표시하는 타일 :::no-loc text="Microsoft Teams Rooms"::: 만들기
...
:::no-loc text="Azure Monitor"::: 콘솔에서 :::no-loc text="Microsoft Teams Rooms"::: 문제가 발생할 때 관리자에게 알리기 위해 경고를 제기할 수 있습니다.
...
[의 :::no-loc text="Azure Monitor":::](/azure/azure-monitor/platform/alerts-unified-log) 경고에 대한 자세한 내용은 에서 경고 로그를 참조합니다:::no-loc text="Azure Monitor":::.

> [!NOTE]
> 다음 예제에서는 디바이스가 :::no-loc text="Microsoft Teams Rooms"::: 하드웨어 또는 애플리케이션 오류를 생성할 때 전자 메일 경고를 전송합니다.

### <a name="configure-an-email-alert-for-no-loc-textmicrosoft-teams-rooms-hardware-issues"></a>하드웨어 문제에 대한 전자 메일 경고 :::no-loc text="Microsoft Teams Rooms"::: 구성

지난 1시간 :::no-loc text="Microsoft Teams Rooms"::: 이내에 하드웨어 문제가 발생한 디바이스를 검사하는 경고 규칙을 구성합니다.
1.  포털에 [:::no-loc text="Microsoft Azure"::: 로그인](https://portal.azure.com) 하고 작업 영역 :::no-loc text="Log Analytics"::: 으로 이동하여 선택합니다.

2. 작업 영역 :::no-loc text="Log Analytics"::: 으로 이동하고 **경고** 를 선택한 다음 새 경고 **규칙을 선택합니다.**
...
7. 작업 그룹을 구성합니다.
    ...
    3.  고유한 작업 이름을 *지정* 하고 **전자 메일/SMS/푸시/음성** 을 선택한 다음 세부 정보 편집 **을 선택합니다**.
    4.  전자 메일 **확인** 란을 선택하고 경고를 받을 사람 또는 그룹의 전자 메일 주소를 제공합니다.
...
### <a name="configure-an-email-alert-for-no-loc-textmicrosoft-teams-rooms-application-issues"></a>애플리케이션 문제에 대한 전자 메일 경고 :::no-loc text="Microsoft Teams Rooms"::: 구성
...
<a name="configure_all_devices"> </a> 대시보드 및 경고 :::no-loc text="Microsoft Monitoring"::: :::no-loc text="Microsoft Teams Rooms"::: 를 구성한 후 모든 디바이스에서 에이전트를 설정하고 구성하여 모니터링 배포를 완료할 수 있습니다.

각 디바이스에 :::no-loc text="Microsoft Monitoring"::: 에이전트를 수동으로 설치하고 구성할 수 있지만 기존 소프트웨어 배포 도구 및 메서드를 활용하는 것이 좋습니다.

처음으로 디바이스 :::no-loc text="Microsoft Teams Rooms"::: :::no-loc text="Microsoft Monitoring"::: 를 빌드하는 경우 빌드 프로세스의 일부로 에이전트 설정 및 구성 단계를 포함해야 할 수 있습니다. 자세한 내용은 명령줄 [을 사용하여 에이전트 설치를 참조하세요](/azure/azure-monitor/platform/agent-windows#install-the-agent-using-the-command-line).
...
구현하기 :::no-loc text="Microsoft Teams Rooms"::: :::no-loc text="Azure Monitoring":::전에 디바이스를 이미 배포한 경우 제공 :::no-loc text="Active Directory"::: 된 스크립트를 사용하여 그룹 정책 개체를 사용하여 에이전트를 설정하고 구성할 수 있습니다.

1.  공유 네트워크 경로를 만들고 도메인 컴퓨터 그룹에 대한 읽기 **액세스 권한을 부여** 합니다.

2.  에서 에이전트의 64비트 버전 :::no-loc text="Microsoft Monitoring"::: 다운로드 :::no-loc text="Windows":::<https://go.microsoft.com/fwlink/?LinkID=517476>

3.  설정 패키지의 내용을 네트워크 공유로 추출합니다.
    1.  명령 프롬프트 창을 열고 / **c에서MMASetup-AMD64.exe 실행합니다.**
    2.  방금 만든 공유를 지정하고 콘텐츠를 추출합니다.

4.  새 그룹 정책 개체를 만들고 컴퓨터 :::no-loc text="Microsoft Teams Rooms"::: 계정이 있는 조직 단위에 할당합니다.

5.  PowerShell 실행 정책 구성:
    1.  새로 만든 그룹 정책 개체를 편집하고 컴퓨터 구성 \\ 정책 관리 \\ 템플릿 구성 요소로 이동합니다 \\ :::no-loc text="Windows"::: . \\ :::no-loc text="Windows PowerShell":::
    2.  스크립트 실행 **턴을 사용하도록 설정하고** **실행** 정책을 설정하여 **로컬 스크립트를 허용합니다**.

6.  시작 스크립트를 구성합니다.
    1.  다음 스크립트를 복사하여 다음 스크립트로 Install-MMAgent.ps1.
    2.  WorkspaceId, WorkspaceKey 및 SetupPath 매개 변수를 구성과 일치하도록 수정합니다.
    3.  동일한 그룹 정책 개체를 편집하고 스크립트에서 컴퓨터 \\ \\ :::no-loc text="Windows"::: 구성 \\ 정책으로 설정(시작/종료)
    4.  두 번 클릭하여 **시작** 을 선택한 다음 **PowerShell 스크립트를 선택합니다**.
    5.  파일 **표시를** 선택한 다음 해당 **Install-MMAgent.ps1파일을 복사** 합니다.
    6.  추가 **를** 선택한 다음 찾아 **보기를 선택합니다**.
    7.  방금 복사한 ps1 스크립트를 선택합니다.

7.  :::no-loc text="Microsoft Teams Rooms"::: 두 번째 재 :::no-loc text="Microsoft Monitoring"::: 부팅을 통해 에이전트를 설치하고 구성해야 합니다.
...
> [!NOTE]
> 에이전트를 [:::no-loc text="Log Analytics":::](/azure/azure-monitor/platform/agent-manage) 다시 구성하거나, 다른 작업 영역으로 이동하거나, 초기 설치 후 프록시 설정을 수정해야 할 때 에이전트 관리 및 유지 관리 문서를 참조할 수 있습니다.

## <a name="additional-solutions"></a>추가 솔루션
<a name="Solutions"> </a>

:::no-loc text="Azure Monitor":::환경을 모니터링할 수 있도록 솔루션 갤러리를 [](/azure/azure-monitor/insights/solutions) 통해 기본 제공 관리 솔루션을 제공합니다. 작업 영역도 [경고](/azure/azure-monitor/platform/alert-management-solution) [:::no-loc text="Azure Log Analytics"::: 관리 및 에이전트 상태](/azure/azure-monitor/insights/solution-agenthealth) 솔루션을 추가하는 것이 좋습니다.

> [!NOTE]
> 에이전트 상태 솔루션은 :::no-loc text="Microsoft Monitoring"::: 사용자 환경 내에서 손상된 에이전트를 식별하는 데 도움이 될 수 있으며 경고 관리 솔루션은 특정 기간 내에 발생된 경고에 대한 세부 정보를 제공합니다.
