...
이 문서에서는를 사용 :::no-loc text="Microsoft Teams Rooms"::: :::no-loc text="Azure Monitor":::하 여 통합 된 디바이스의 종단 간 관리를 설정 하 고 배포 하는 방법에 대해 설명 합니다.

내에서 :::no-loc text="Log Analytics"::: :::no-loc text="Azure Monitor"::: 구성 하 여 회의실 장치를 관리 :::no-loc text="Microsoft Teams Rooms"::: 하는 데 도움이 되는 기본 원격 분석 및 알림을 제공할 수 있습니다. 관리 솔루션이 완성 됨에 따라 추가 데이터 및 관리 기능을 배포 하 여 디바이스 가용성 및 성능에 대 한 자세한 뷰를 만들 수 있습니다.
...
2. [관리 설정에 맞게 :::no-loc text="Log Analytics"::: 테스트 장치 구성](azure-monitor-deploy.md#configure_test_devices)
...
> [!IMPORTANT]
> 최소 :::no-loc text="Azure Monitor"::: :::no-loc text="Log Analytics"::: 구성에서 :::no-loc text="Windows"::: 운영 체제를 실행 하는 컴퓨터를 모니터링할 수 있지만, 모든 :::no-loc text="Microsoft Teams Rooms"::: :::no-loc text="Microsoft Teams Rooms"::: 장치에 에이전트 배포를 시작 하기 전에 수행 해야 하는 몇 가지 특정 단계가 있습니다.
...
장치에서 :::no-loc text="Microsoft Teams Rooms"::: 로그 수집을 :::no-loc text="Log Analytics"::: 시작 하려면 작업 영역이 있어야 합니다. 작업 영역은 고유한 데이터 리포지토리 :::no-loc text="Log Analytics"::: , 데이터 원본 및 솔루션이 있는 고유한 환경입니다. 기존 :::no-loc text="Log Analytics"::: 작업 영역이 이미 있는 경우이를 사용 하 여 :::no-loc text="Microsoft Teams Rooms"::: 배포를 모니터링 하거나 :::no-loc text="Log Analytics"::: :::no-loc text="Microsoft Teams Rooms"::: 모니터링 요구 사항에 맞는 전용 작업 영역을 만들 수 있습니다.

새 :::no-loc text="Log Analytics"::: 작업 영역을 만들어야 하는 경우 [ :::no-loc text="Log Analytics"::: :::no-loc text="Azure"::: 포털에서 작업 영역 만들기](https://docs.microsoft.com/azure/azure-monitor/learn/quick-create-workspace) 문서의 지침을 따르세요.

> [!NOTE]
> 에서을 :::no-loc text="Log Analytics"::: 사용 하려면 활성 :::no-loc text="Azure"::: 구독이 있어야 합니다. :::no-loc text="Azure Monitor"::: :::no-loc text="Azure"::: 구독을 보유 하 고 있지 않은 경우 [무료 평가판 구독을](https://azure.microsoft.com/free) 시작 점으로 만들 수 있습니다.

### <a name="configure-no-loc-textlog-analytics-to-collect-no-loc-textmicrosoft-teams-rooms-event-logs"></a>이벤트 :::no-loc text="Log Analytics"::: 로그를 :::no-loc text="Microsoft Teams Rooms"::: 수집 하도록 구성

:::no-loc text="Log Analytics":::설정에 지정 된 :::no-loc text="Windows"::: 이벤트 로그 에서만 이벤트를 수집 합니다. 각 로그에는 선택한 심각도의 이벤트만 수집 됩니다.

장치 및 응용 프로그램 :::no-loc text="Log Analytics"::: 상태를 모니터링 :::no-loc text="Microsoft Teams Rooms"::: 하는 데 필요한 로그를 수집 하도록 구성 해야 합니다. :::no-loc text="Microsoft Teams Rooms":::장치는 이벤트 **:::no-loc text="Skype Room System":::** 로그를 사용 합니다.

이벤트를 :::no-loc text="Log Analytics"::: 수집 하도록 구성 하려면 다음 [ :::no-loc text="Windows"::: 의 :::no-loc text="Azure Monitor"::: 이벤트 로그 데이터 원본을](https://docs.microsoft.com/azure/azure-monitor/platform/data-sources-windows-events) 참조 하세요. :::no-loc text="Microsoft Teams Rooms":::

![이벤트 로그 설정 스크린샷](../media/Deploy-Azure-Monitor-2.png "이벤트 로그 설정")

> [!IMPORTANT]
> 이벤트 :::no-loc text="Windows"::: 로그 설정을 구성 하 고 **:::no-loc text="Skype Room System":::** 이벤트 로그 이름으로 입력 한 다음 **오류**, **경고**및 **정보** 확인란을 선택 합니다.

## <a name="configure-test-devices-for-azure-monitoring"></a>Azure 모니터링을 위한 테스트 장치 구성
<a name="configure_test_devices"> </a>

관련 이벤트를 모니터링할 :::no-loc text="Log Analytics"::: :::no-loc text="Microsoft Teams Rooms":::수 있도록 준비 해야 합니다. 먼저 물리적으로 액세스할 수 있는 장치 하나 :::no-loc text="Microsoft Monitoring"::: 또는 두 개 :::no-loc text="Microsoft Teams Rooms"::: 에 에이전트를 배포 하 고 해당 테스트 장치에서 일부 데이터를 생성 하 여 :::no-loc text="Log Analytics"::: 작업 영역에 푸시하는 것이 필요 합니다.
...
> [!NOTE]
> :::no-loc text="Microsoft Monitoring"::: 에이전트를 설치한 후 장치를 다시 부팅 하 고 이벤트 로그에 새 :::no-loc text="Microsoft Teams Rooms"::: 이벤트를 생성할 수 있도록 모임 앱이 시작 되었는지 확인 합니다.

1.  포털에 로그인 하 고 작업 영역으로 :::no-loc text="Log Analytics"::: 이동 하 여 선택 합니다. [ :::no-loc text="Microsoft Azure"::: ](https://portal.azure.com)

2.  :::no-loc text="Microsoft Teams Rooms"::: 장치에서 생성 된 하트 비트 이벤트를 나열 합니다.
    1.  작업 영역을 선택 하 고 **로그** 로 이동한 후 쿼리를 사용 하 여 사용자 지정 필드가 포함 될 하트 비트 레코드 :::no-loc text="Microsoft Teams Rooms":::를 검색 합니다.
    2.  쿼리 예제:`Event | where Source == "SRS-App" and EventID == 2000`

3.  쿼리가 :::no-loc text="Microsoft Teams Rooms"::: 모임 앱에서 생성 된 이벤트를 포함 하는 로그 레코드를 반환 하는지 확인 합니다.

4.  하드웨어 문제를 생성 하 고 필요한 이벤트가 로그인 되어 있는지 확인 :::no-loc text="Azure Log Analytics":::합니다.
    1.  테스트 :::no-loc text="Microsoft Teams Rooms"::: 시스템에서 주변 장치 중 하나를 분리 합니다. 카메라, 스피커폰, 마이크 또는 프론트 실 디스플레이로 표시 될 수 있습니다.
...
5.  응용 프로그램 문제를 생성 하 고 필요한 이벤트가 기록 되는지 확인 합니다.
    1.  응용 :::no-loc text="Microsoft Teams Rooms"::: 프로그램 구성을 수정 하 고 잘못 된 SIP (세션 초기화 프로토콜) 주소/암호 쌍을 입력 합니다.
    2.  이벤트 로그가 채워질 때까지 10 분간 기다립니다 :::no-loc text="Azure Log Analytics":::.
...
2. :::no-loc text="Microsoft Teams Rooms"::: 장치에서 생성 된 이벤트 나열:
...
5. *표 1*에 표시 된 매핑을 사용 합니다. :::no-loc text="Log Analytics":::새 필드를 정의할 때 ** \_CF** 문자열이 자동으로 추가 됩니다.

> [!IMPORTANT]
> 모든 JSON 및 :::no-loc text="Log Analytics"::: 필드는 대/소문자를 구분 한다는 점에 유의 하세요.
> 
> 아래 표의 각 사용자 정의 필드에 필요한 쿼리를 주의 해 서 살펴 보세요. 사용자 지정 필드 값을 성공적으로 추출 :::no-loc text="Log Analytics"::: 하려면 올바른 쿼리를 사용 해야 합니다.
> 
...
데이터를 수집 하 고 사용자 지정 필드를 매핑한 후에는 뷰 디자이너를 사용 하 여 이벤트를 모니터링 :::no-loc text="Microsoft Teams Rooms"::: 하기 위해 다양 한 타일이 포함 된 대시보드를 개발할 수 있습니다. 보기 디자이너를 사용 하 여 다음 타일을 만듭니다. 자세한 내용은 다음 [에서 :::no-loc text="Log Analytics"::: 뷰 디자이너를 사용 하 여 사용자 지정 보기 만들기](https://docs.microsoft.com/azure/azure-monitor/platform/view-designer) 를 참조 하세요.
...
:::no-loc text="Microsoft Teams Rooms"::: 대시보드를 가져와 장치 모니터링을 빠르게 시작할 수 있습니다. 대시보드를 가져오려면 다음 단계를 수행 합니다.

1.  Omsview 대시보드 파일 SkypeRoomSystems_v2을 가져옵니다 [.](https://go.microsoft.com/fwlink/?linkid=835675)
2.  포털에 로그인 하 고 작업 영역으로 :::no-loc text="Log Analytics"::: 이동 하 여 선택 합니다. [ :::no-loc text="Microsoft Azure"::: ](https://portal.azure.com)
3.  **뷰 디자이너**를 엽니다.
...
3.  타일 **:::no-loc text="Microsoft Teams Rooms":::** 의 이름을 이름으로 합니다.
...
### <a name="create-a-tile-that-displays-no-loc-textmicrosoft-teams-rooms-operating-system-versions"></a>운영 체제 버전을 표시 :::no-loc text="Microsoft Teams Rooms"::: 하는 타일 만들기
...
### <a name="create-a-tile-that-displays-no-loc-textmicrosoft-teams-rooms-application-versions"></a>응용 프로그램 버전을 표시 :::no-loc text="Microsoft Teams Rooms"::: 하는 타일 만들기
...
:::no-loc text="Azure Monitor":::콘솔에 :::no-loc text="Microsoft Teams Rooms"::: 문제가 발생할 경우 관리자에 게 알림을 발생 시킬 수 있습니다.
...
-   서비스에서 :::no-loc text="Azure Automation"::: runbook 시작

의 알림에 대 한 자세한 내용은 [ :::no-loc text="Azure Monitor"::: 의 경고 로그](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-unified-log) 를 참조 :::no-loc text="Azure Monitor":::하세요.

> [!NOTE]
> 다음 예제에서는 장치에서 :::no-loc text="Microsoft Teams Rooms"::: 하드웨어 또는 응용 프로그램 오류를 생성할 때 전자 메일 알림을 보냅니다.

### <a name="configure-an-email-alert-for-no-loc-textmicrosoft-teams-rooms-hardware-issues"></a>하드웨어 문제에 대 한 :::no-loc text="Microsoft Teams Rooms"::: 전자 메일 알림 구성

지난 시간 내에 하드웨어 문제가 발생 :::no-loc text="Microsoft Teams Rooms"::: 한 장치를 확인 하는 알림 규칙을 구성 합니다.
1.  포털에 로그인 하 고 작업 영역으로 :::no-loc text="Log Analytics"::: 이동 하 여 선택 합니다. [ :::no-loc text="Microsoft Azure"::: ](https://portal.azure.com)

2. :::no-loc text="Log Analytics"::: 작업 영역으로 이동 하 고 **알림을** 선택한 다음 **새 알림 규칙** 을 선택 합니다.
...
### <a name="configure-an-email-alert-for-no-loc-textmicrosoft-teams-rooms-application-issues"></a>응용 프로그램 문제에 대 :::no-loc text="Microsoft Teams Rooms"::: 한 전자 메일 알림 구성
...
처음으로 :::no-loc text="Microsoft Teams Rooms"::: 장치를 작성 하는 경우 빌드 프로세스의 일부로 :::no-loc text="Microsoft Monitoring"::: 에이전트 설정 및 구성 단계를 포함할 수 있습니다. 자세한 내용은 [명령줄을 사용 하 여 에이전트 설치](https://docs.microsoft.com/azure/azure-monitor/platform/agent-windows#install-the-agent-using-the-command-line)를 참조 하세요.

### <a name="deploying-no-loc-textmicrosoft-monitoring-agent-by-using-a-group-policy-object-gpo"></a>GPO :::no-loc text="Microsoft Monitoring"::: (그룹 정책 개체)를 사용 하 여 에이전트 배포

구현 :::no-loc text="Microsoft Teams Rooms"::: :::no-loc text="Azure Monitoring":::하기 전에 장치를 이미 배포한 경우 제공 된 스크립트를 사용 하 여 그룹 정책 개체를 사용 :::no-loc text="Active Directory"::: 하 여 에이전트를 설정 하 고 구성할 수 있습니다.

1.  공유 네트워크 경로를 만들고 **도메인 컴퓨터** 그룹에 대 한 읽기 권한을 부여 합니다.

2.  의 :::no-loc text="Microsoft Monitoring"::: 에이전트 :::no-loc text="Windows"::: 64 비트 버전을 다운로드 합니다.<https://go.microsoft.com/fwlink/?LinkID=517476>

3.  네트워크 공유에 설치 패키지의 콘텐츠를 추출 합니다.
    1.  명령 프롬프트 창을 연 다음 **MMASetup-AMD64 실행/c**
    2.  방금 만든 공유를 지정 하 고 콘텐츠를 추출 합니다.

4.  새 그룹 정책 개체를 만들고 :::no-loc text="Microsoft Teams Rooms"::: 컴퓨터 계정이 있는 조직 구성 단위에 할당 합니다.
...
> [!NOTE]
> 에이전트 상태 솔루션은 환경 내에서 오래 되거나 손상 :::no-loc text="Microsoft Monitoring"::: 된 에이전트를 식별 하는 데 도움이 될 수 있으며, 알림 관리 솔루션은 지정 된 기간 내에 발생 한 경고에 대 한 세부 정보를 제공 합니다.

## <a name="see-also"></a>참고 항목

[관리 :::no-loc text="Microsoft Teams Rooms"::: 계획:::no-loc text="Azure Monitor":::](azure-monitor-plan.md)

[장치 :::no-loc text="Microsoft Teams Rooms"::: 관리:::no-loc text="Azure Monitor":::](azure-monitor-manage.md)
