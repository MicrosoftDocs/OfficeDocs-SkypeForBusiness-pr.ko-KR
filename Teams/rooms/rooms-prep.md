...
- Active Directory 또는 Azure AD(Azure Active Directory) 인스턴스뿐만 아니라 Microsoft Exchange 및 Microsoft Teams 액세스합니다.
...
2. **설정** 탭한 다음 **Windows 이동** 단추를 탭한 다음 **관리자 로그인으로 이동** 단추를 탭한 다음 **관리자** 단추를 클릭합니다(컴퓨터가 도메인에 가입된 경우 **다른 사용자를** 선택한 다음 .\admin을 사용자 이름으로 사용).
...
> [!NOTE]
> Microsoft Teams 룸 디바이스에서 도메인에 대한 신뢰가 끊어지는 경우(예: 도메인에 가입된 후 도메인에서 Microsoft Teams 룸 제거하는 경우) 디바이스에 인증하고 설정 열 수 없습니다. 해결 방법은 로컬 관리자 계정으로 로그인하는 것입니다. 
