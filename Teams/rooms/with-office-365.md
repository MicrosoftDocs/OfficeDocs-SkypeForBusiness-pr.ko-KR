...
모든 리소스 계정은 단일 Microsoft Teams 룸 설치에 고유하거나 핫 데스크 구현을 표시할 Teams 있습니다.
...
|Active Directory  <br/> |Active Directory Enterprise 관리자, 도메인 관리자 또는 사용자를 만들 수 있는 위임된 권한이 있습니다. 동기화 권한을 Azure Active Directory 커넥트.  <br/> |
...
- **모임 Teams** 리소스 계정을 공유 장치(예: Microsoft Teams 회의실 또는 핫 데스크와 Teams 디스플레이)와 연결하고 참석자가 이를 사용하여 비디오 및 오디오를 프레젠테이션할 수 있도록 Teams 모임에 참가하려면 미팅룸 라이선스가 필요합니다. 회의실 라이선스에 대한 자세한 내용은 [Teams 미팅룸 라이선스](rooms-licensing.md)를 참조하세요.
...
**1단계 -** [새 리소스 계정을 만듭니](#create-a-resource-account)다. 또는 회의실 사서함이 이미 있고 리소스 계정으로 변환하려는 경우 [기존 Exchange 회의실 사서함을 수정](?tabs=existing-account#create-a-resource-account)할 수 있습니다.

**2단계 -**  그런 다음 Teams 모임에 대한 [계정을 구성](#configure-mailbox-properties)합니다.

**3단계 -**  리소스 계정이 핫 데스크로 표시되는 Teams 같은 공유 디바이스와 연결될 경우 [암호 만료를 해제합니다](#turn-off-password-expiration).

**4단계 -**  마지막으로 계정이 Microsoft Teams 액세스할 수 있도록 [회의실 라이선스를 할당](#assign-a-meeting-room-license)합니다.
...
> [!TIP]
> 리소스 계정의 이름을 지정할 때는 전자 메일 주소의 시작 부분에 표준 명명 규칙을 사용하는 것이 좋습니다. 이렇게 하면 동적 그룹을 만들어 Azure Active Directory 관리가 용이합니다. 예를 들어 Microsoft Teams 룸 연결할 모든 리소스 계정에 "mtr-"을 사용할 수 있습니다.

> [!TIP]
> Exchange Online 및 Azure Active Directory 사용하여 모든 리소스 계정을 만드는 것이 좋습니다.
...
9. **라이선스 및 앱** 섹션에서 디바이스가 설치될 국가 또는 지역으로 **위치 선택을** 설정합니다. 그런 다음, 할당할 라이선스(예: 미팅룸)를 선택하고 **변경 내용 저장** 을 선택합니다. 라이선스는 조직에 따라 달라질 수 있습니다.
...
1. [PowerShell](/powershell/exchange/connect-to-exchange-online-powershell)을 Exchange Online 커넥트.

    ``` PowerShell
    Connect-ExchangeOnline
    ```

2. 기본적으로 회의실 사서함에는 연결된 계정이 없습니다. Microsoft Teams 사용하여 인증할 수 있도록 회의실 사서함을 만들 때 계정을 추가합니다.
...
Exchange 하이브리드 구성에 있지 않은 경우 다음 단계인 [사서함 속성 구성](#configure-mailbox-properties)을 계속할 수 있습니다.

Exchange 하이브리드 구성에 있는 경우 온-프레미스 도메인 계정의 이메일 주소를 추가해야 합니다. 자세한 내용은 [온-프레미스 동기화 및 Office 365 사용자 계정 디렉터리를 참조하세요](https://support.microsoft.com/topic/how-to-use-smtp-matching-to-match-on-premises-user-accounts-to-office-365-user-accounts-for-directory-synchronization-75673b94-e1b8-8a9e-c413-ee5a2a1a6a78).

#### <a name="with-exchange-server"></a>[**Exchange Server**](#tab/exchange-server)

  1. Exchange 관리 셸에 커넥트. [Exchange 관리 셸을 열](/powershell/exchange/exchange-server/open-the-exchange-management-shell)거나 [원격 PowerShell을 사용하여 Exchange 서버에 연결합니다](/powershell/exchange/exchange-server/connect-to-exchange-servers-using-remote-powershell).
...
#### <a name="modify-an-existing-exchange-room-mailbox"></a>[**기존 Exchange 회의실 사서함 수정**](#tab/existing-account)
...
> [!NOTE]
> Surface Hub Teams Room에 대해 이 계정을 만드는 경우 이 계정에서도 ActiveSync를 사용하도록 설정해야 합니다. 이렇게 하면 Surface Hub 직접 전자 메일을 보낼 수 있으며, 화이트보드와 같은 기능에 사용할 수 있습니다. 자세한 내용은 [디바이스 계정(Surface Hub)에 ActiveSync 정책 적용](/surface-hub/apply-activesync-policies-for-surface-hub-device-accounts)을 참조하세요.
...
온라인 또는 온-프레미스의 Exchange PowerShell에서 회의실 사서함에서 다음 설정을 구성하여 모임 환경을 개선합니다.
...
- **DeleteComments: `$false`** 들어오는 모임 요청의 메시지 본문에 텍스트를 보관합니다. 원터치 조인 환경을 제공하기 위해 외부 Teams 및 타사 모임을 처리하는 데 필요합니다.
...
먼저 Active Directory PowerShell에 커넥트.
...
#### <a name="azure-active-directory-10"></a>[**Azure Active Directory 1.0**](#tab/azure-active-directory1-password/)
 ...
 1. MSOnline PowerShell에 커넥트:

       ```PowerShell
       Connect-MsolService
       ```

       Active Directory에 대한 자세한 내용은 [Azure Active Directory(MSOnline)](/powershell/azure/active-directory/overview?view=azureadps-1.0)를 참조하세요.
...
1. Active Directory PowerShell에 커넥트:
...
Microsoft Teams 로그인하려면 리소스 계정에 Microsoft 365 또는 Office 365 라이선스가 필요합니다.
...
1. Azure AD 커넥트
    ...
    ```
     ...
     Active Directory에 대한 자세한 내용은 [Graph Azure Active Directory PowerShell을](/powershell/azure/active-directory/overview?view=azureadps-2.0) 참조하세요.
...
1. MSOnline PowerShell에 커넥트.
   ...
   ```
    ...
    Active Directory에 대한 자세한 내용은 [Azure Active Directory(MSOnline)를](/powershell/azure/active-directory/overview?view=azureadps-1.0) 참조하세요.
...
계정 만들기 및 라이선스 할당의 유효성을 검사하려면 만든 계정을 사용하여 Teams Client에 로그인합니다.
...
공동 작업을 위해 PowerPoint Live, Whiteboard 및 공유 노트를 켭니다. 모임 정책 설정 "비공개 모임에서 지금 모임"을 사용하도록 설정하는 것이 좋습니다. Teams 룸 대한 참가자 및 게스트 설정을 조정하는 모임 정책을 만들 수 있습니다. 예를 들어 모임에 자동으로 허용할 참석자와 같은 로비 설정을 검토합니다. Teams 모임 정책에 대한 자세한 내용은 [Microsoft Teams 모임 정책 관리를](/microsoftteams/meeting-policies-overview) 참조하세요.
...
> [!NOTE]
> 디바이스 리소스 계정에 통화 정책을 할당하여 공유 디바이스에 대한 음성 메일을 해제하는 것이 좋습니다. 자세한 내용은 [Teams 통화 및 통화 전달](../teams-calling-policy.md)을 참조하세요.

### <a name="configure-distribution-groups-for-teams-calendar"></a>Teams 일정에 대한 메일 그룹 구성

회의실 위치를 구성하려면 디바이스 리소스 계정을 Exchange 메일 그룹에 추가할 수 있습니다. 예를 들어 세 개의 서로 다른 지리적 위치에 사무실이 있는 경우 세 개의 배포 그룹을 만들고 각 위치에 적절한 리소스 계정을 추가할 수 있습니다. 자세한 내용은 [회의실 목록 만들기를 참조하세요](/exchange/recipients/room-mailboxes?view=exchserver-2019#create-a-room-list).

### <a name="configure-places-for-outlook-calendar"></a>Outlook 일정 위치 구성
회의실 위치를 Outlook 회의실 찾기에 표시하려면 Set-Place Exchange PowerShell cmdlet을 사용해야 합니다. Set-Place Outlook 회의실 찾기를 채울 뿐만 아니라 회의실의 용량이나 회의실의 바닥과 같은 추가 메타데이터를 추가할 수 있습니다. 자세한 내용은 [Set-Place](/powershell/module/exchange/set-place)를 참조하세요.
