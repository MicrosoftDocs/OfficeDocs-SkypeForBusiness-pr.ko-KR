...
모든 텍스트 편집기를 사용 하 여 설정 파일을 만들 수 있습니다. **XML 요소** 표에서는이 샘플 SkypeSettings.xml (필수 파일 이름) 구성 파일에 표시 되는 요소에 대해 설명 합니다.
...
|\<Password\> |문자열 3  || Password 매개 변수는 비즈니스용 Skype 디바이스 계정 로그인에 사용 되는 암호와 동일 합니다.  |
...
|\<DisableTacCommunication\> |부울 &#x2777;  |첫 번째 &#x2776; |True 인 경우 팀 관리 센터 장치 관리와의 모든 통신을 사용할 수 없습니다. |
|\<SendLogs\> |컨트롤러 |첫 번째 &#x2776;  |  |
...
| \<Devices\>  |컨트롤러 |첫 번째 &#x2776;  | 자식 요소의 연결 된 오디오 장치 이름은 장치 관리자 앱에 나열 된 값과 동일 합니다. 구성에는 현재 콘솔에 연결 되지 않은 A/V 장치와 같이 현재 시스템에 존재 하지 않는 장치가 포함 될 수 있습니다. 해당 디바이스에 대 한 구성이 유지 됩니다.  |
...
| \<Theming\>  |컨트롤러 |첫 번째 &#x2776;  |XML 파일과 함께 적용할 수 있는 기능 중 하나는 조직의 사용자 지정 테마입니다. 테마 이름, 배경 이미지 및 색을 지정할 수 있습니다. |
...
시작 시 Microsoft 팀 대화방 콘솔이 SkypeSettings.xml 이라는 이름의 XML 파일 `C:\Users\Skype\AppData\Local\Packages\Microsoft.SkypeRoomSystem_8wekyb3d8bbwe\LocalState` 을 찾으면 xml 파일에 지정 된 구성 설정을 적용 하 고 xml 파일을 삭제 합니다.
...
- [Microsoft 팀 대화방에 대 한 그룹 정책 구성](rooms-operations.md#GroupPolicy)
- [PowerShell을 사용 하](rooms-operations.md#RemotePS) 고 [파일 항목을 구성 하는](https://technet.microsoft.com/library/cc772536%28v=ws.11%29.aspx) 원격 관리
...
사용자 지정 테마 이미지 파일은 폴더에 배치 해야 합니다 `C:\Users\Skype\AppData\Local\Packages\Microsoft.SkypeRoomSystem_8wekyb3d8bbwe\LocalState` . 변수에 파일 이름과 확장명을 입력 합니다 \<CustomThemeImageUrl\> .
