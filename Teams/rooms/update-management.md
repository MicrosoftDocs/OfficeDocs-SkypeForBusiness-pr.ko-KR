...
- **자동으로 관리**: 업데이트 관리 서비스 평가에 따라 회의실 디바이스에 설치됩니다. 포트폴리오에서 관리되는 업데이트에는 개입이 필요하지 않습니다.
...
> [!NOTE]
> 일반적인 업데이트 관리 전략으로 **"업데이트 강제 적용"은 권장하지 않습니다**. 유효성 검사 단계에 있는 업데이트를 푸시하는 경우 이미 알고 있는 문제가 발생할 수 있습니다. 이러한 경우 이러한 회의실에 대한 인시던트 해결은 최선을 다할 것입니다.  
