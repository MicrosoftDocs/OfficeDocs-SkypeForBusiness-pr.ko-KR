---
title: 업데이트 관리 Microsoft Teams 룸
author: donnah007
...
Microsoft Teams 룸 관리형 서비스는 조직의 각 회의실이 항상 준비되고 제대로 작동하는 공간을 제공하기 위해 권장 수준에서 유지 관리된다는 보증을 제공합니다. Microsoft의 목표는 인텔리전스 및 자동화를 통해 운영 직원의 복잡성과 레그워크를 줄이는 것입니다. 문제 해결 또는 진단은 가능한 한 빨리 수행됩니다. 

## <a name="transitioning-a-device-to-managed-services"></a>디바이스를 관리되는 서비스로 전환 
관리형 서비스에 온보딩하는 회의실 디바이스에는 일반적으로 지침과 다른 변경 관리 기록 및 사례가 있습니다.  

- Managed Services의 이점을 활용하려면 Managed Services 포트폴리오의 모든 업데이트에 대한 변경 관리를 전환해야 합니다.
...
관리 서비스 업데이트를 받기 위한 몇 가지 필수 구성 요소가 있습니다. 

|소프트웨어 |지침 |
|:- |:- |
|Logitech Sync Services  |Logitech 회의실 디바이스에 설치하고 실행해야 합니다. 필수 동기화 서비스는 차단되지 않는 한 Windows 업데이트에서 자동으로 설치됩니다. 전체 동기화 패키지를 설치할 수도 있습니다. |
|OS 업데이트 Windows |WSUS로 리디렉션되거나 네트워킹 관점에서 차단되지 않도록 설정해야 합니다. GPO 또는 MDM 정책은 OS 업데이트를 관리하는 데 사용해서는 안 됩니다. |
|업데이트 Microsoft Store   |해제해야 합니다. Managed Services는 Microsoft Store 업데이트를 해제합니다(있는 경우). |
|바이러스 백신 소프트웨어 |이러한 디바이스에서 AV 소프트웨어를 실행하는 경우 AV에 Teams 및 Skype dll에 대한 제외가 있는지 확인해야 합니다. 자세한 내용은 여기를 참조하세요. |
...
- **자동으로 관리**: 업데이트는 관리 서비스 평가에 따라 회의실 디바이스에 설치됩니다. 포트폴리오에서 관리되는 업데이트에는 개입이 필요하지 않습니다.
...
자동으로 관리되도록 선택하는 경우 해당 부분의 업데이트에 대한 작업이 필요하지 않습니다. 그러나 Managed Services에서 지원하는 업데이트의 현재 포트폴리오를 검토해야 합니다. 포트폴리오는 지속적으로 새로운 추가를 얻고 있으며, 객실 안정성을 보장하기 위해 가장 빈약하고 영향력 있는 업데이트를 다루는 것이 우리의 우선 순위입니다. 현재 목록(이 문서의 "업데이트 관리" 섹션 아래)을 확인하여 조직에 필요한 추가 변경 관리를 계획합니다.  

**추천:** 관리되는 모든 디바이스에 Managed Services에서 적용되는 업데이트를 직접 설치하지 마세요. 문제가 발생하면 포털에서 인시던트를 보고하세요.

### <a name="ring-validation"></a>링 유효성 검사

링 유효성 검사를 선택할 때 Managed Services에서 링이 작동하는 방식과 조직에 맞게 사용자 지정하는 데 사용할 수 있는 옵션에 대한 다음 섹션을 검토합니다. 링 유효성 검사를 수행하더라도 Managed Services는 권장 업데이트로 인해 회의실이 지나지 않도록 합니다. 상황에 따라 회의실에서 관리되는 서비스 권장 사항을 준수하는지 확인하기 위해 "catch up" 업데이트를 받을 수 있습니다.  

 포트폴리오에서 새로운 유형의 소프트웨어 및 펌웨어를 사용할 수 있게 됨에 따라 포털 홈페이지 및 Managed Services 설명서 내에서 공지 사항을 확인합니다. Managed Services 전문가는 디바이스 포트폴리오에서 매일 업데이트 버전을 검토하므로 특정 문제를 해결하고 필요에 따라 업데이트를 대상으로 합니다.  

### <a name="scheduling"></a>일정 
관리형 업데이트는 회의실의 장비에 따라 예약되며 해당 소프트웨어 및 펌웨어에 대한 관리 서비스 표준을 충족하지 않는 경우 예약됩니다. 

- 고객이 변경 관리 요구 사항을 충족할 수 있도록 준비 링에서 수요일에 업데이트 배포를 시작 **합니다** . 중요한 업데이트가 필요한 경우 이 일정을 무시하고 사용 가능한 즉시 업데이트를 릴리스합니다. 

- 업데이트는 특정 회의실의 필요에 따라 순서가 지정됩니다. 
- 업데이트의 유효성을 검사하는 설치 링이 있는 경우 업데이트가 링 순서를 통해 진행됩니다. 
- 상황에 따라 객실 안정성이 향상되었다고 판단되면 새 업데이트가 대기 중인 업데이트를 대체할 수 있습니다.  
- 업데이트는 일반적으로 야간 유지 관리 기간 동안 적용되며, 이는 모든 종류의 중단을 방지하기 위해 현지 시간 **으로 오전 12:00 ~ 오전 5:00** 입니다. 
...
모든 관리되는 회의실에서 균일한 표준을 유지하고 추세 문제를 효율적으로 식별할 수 있도록 지원 및 구독 서비스 사용 약관에 따라 MTR 앱 소프트웨어의 두 가지 최신 주 또는 부 릴리스(N, N-1)를 지원하고 배포합니다. 필요에 따라 업데이트 링을 우회하여 비준수 회의실을 자동으로 최신 상태로 유지합니다. 

N-1 정책은 타사 소프트웨어에도 적용됩니다.  

## <a name="update-management-experience-walk-through"></a>업데이트 관리 환경 연습  
업데이트를 보려면 관리되는 서비스 포털에 로그인하고 업데이트 페이지로 이동합니다.

![관리되는 서비스 업데이트 스크린샷](../media/update-management-001.jpg)
...
자세한 회의실 보기는 업데이트 노드 아래에 관련 업데이트 및 해당 상태를 표시 **합니다** .  

![관련 업데이트 및 변경 내용 스크린샷](../media/update-management-003.jpg)

### <a name="rings"></a>반지  

링은 기능 업데이트 배포에서 파생된 문제의 위험을 줄이는 데 사용됩니다. 이 작업은 전체 사이트에 업데이트를 점진적으로 배포하여 수행됩니다. 각 링에는 Microsoft Teams 룸 목록과 해당 출시 일정이 있어야 합니다. 링 정의는 일반적으로 일회성 이벤트(또는 최소한 드물게 발생)이지만 IT는 순서가 여전히 올바른지 확인하기 위해 이러한 그룹을 수시로 다시 방문해야 합니다.  
...
모든 링에서 업데이트가 완료되는 데 60일을 초과할 수 없습니다.  
...
## <a name="managed-updates-visibility-and-control"></a>관리되는 업데이트: 표시 유형 및 제어

관리되는 서비스는 조직 전체에서 업데이트를 오케스트레이션합니다. 그러나 필요한 경우 개입할 가시성과 제어가 있습니다. 방법은 다음과 같습니다. 

- 업데이트 실패 시 Microsoft 관리 서비스 운영 팀과 함께 티켓이 자동으로 생성됩니다. 운영 팀은 오류를 수정하고 필요한 경우 사용자를 참여시키는 단계를 수행합니다.  
- 문제를 일으키는 업데이트가 표시되면 **일시 중지** 단추를 사용하여 업데이트를 일시 중지할 수 있습니다. 일시 중지 단추를 누르면 조사할 운영 센터에 대한 티켓이 생성됩니다. 인시던트 대응을 신속하게 처리할 수 있도록 업데이트를 일시 중지하는 동안 세부 정보를 제공해야 합니다.  
...
> [!NOTE]
> 일반적인 업데이트 관리 전략으로 **"강제 업데이트"는 권장하지 않습니다**. 유효성 검사 단계에 있는 업데이트를 푸시하는 경우 이미 알고 있는 문제가 발생할 수 있습니다. 이러한 경우 이러한 회의실에 대한 인시던트 해결은 최선을 다할 것입니다.  
