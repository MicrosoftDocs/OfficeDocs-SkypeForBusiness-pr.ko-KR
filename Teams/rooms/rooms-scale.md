...
이 문서에서는 Microsoft 엔드포인트 구성 관리자를 사용하여 Microsoft Teams Rooms 배포를 만드는 데 필요한 모든 정보를 제공합니다.
...
구성 관리자를 통해 Microsoft Teams Rooms를 배포하려면 다음 구성 조건 및 요구 사항을 충족해야 합니다.

### <a name="microsoft-endpoint-configuration-manager-requirements"></a>Microsoft 엔드포인트 구성 관리자 요구 사항

-   Microsoft 엔드포인트 구성 관리자 버전은 1706 이상이 되어야 합니다. 1710 이상을 사용하는 것이 좋습니다. 구성 관리자에서 [Windows 10에](/configmgr/core/plan-design/configs/support-for-windows-10#windows-10-as-a-client) 대한 지원을 확인하여 구성 관리자가 지원하는 Windows 10 버전에 대해 자세히 알아보아야 합니다.

-   Windows 10용 ADK(Windows 평가 및 배포 키트)의 지원되는 버전을 설치해야 합니다. 다양한 버전의 Configuration Manager에서 사용할 수 있는 [Windows 10 ADK](/configmgr/core/plan-design/configs/support-for-windows-10#windows-10-adk) 버전을 참조하고 배포에 올바른 버전이 포함되어야 합니다.
...
-   동일한 Microsoft Teams [](/configmgr/osd/deploy-use/use-multicast-to-deploy-windows-over-the-network)Rooms 이미지를 여러 단위에 동시에 배포할 가능성이 있는 경우 멀티캐스트 지원을 사용하도록 설정하는 것이 좋습니다.

### <a name="networking-requirements"></a>네트워킹 요구 사항

-   네트워크에는 Microsoft Teams Rooms 단위가 배포될 서브넷에 대한 자동 IP 주소 배포를 위해 구성된 DHCP(동적 호스트 구성 프로토콜) 서버가 필요합니다.
...
-   스위치 및 VLA(가상 런)를 포함한 네트워크는 PXE를 지원하도록 구성해야 합니다. IP 도우미 및 PXE 구성에 대한 자세한 내용은 네트워크 공급업체를 참조하세요. 또는 PXE [](/configmgr/osd/deploy-use/use-bootable-media-to-deploy-windows-over-the-network) 지원을 사용하도록 설정하지 않은 경우 배포에 부팅 가능한 미디어를 사용할 수 있습니다.
    ...
    > [!NOTE]
    > Surface Pro 디바이스의 경우 네트워크에서 부팅(PXE 부팅)은 Microsoft의 이더넷 어댑터 또는 도킹 스테이션을 사용하는 경우만 지원됩니다. 타사 이더넷 어댑터는 Surface Pro에서 PXE 부팅을 지원하지 않습니다. 자세한 [내용은 이더넷 어댑터](/surface/ethernet-adapters-and-surface-device-deployment) 및 Surface 배포를 참조하세요.

## <a name="configure-microsoft-endpoint-configuration-manager-for-operating-system-deployment"></a>운영 체제 배포를 위한 Microsoft 엔드포인트 구성 관리자 구성

이 문서에서는 구성 관리자 배포가 정상인 것으로 가정하고 구성 관리자를 처음부터 배포하고 구성하는 데 필요한 모든 단계를 자세히 설명하지 않습니다. Microsoft [엔드포인트](/configmgr/) 구성 관리자에 대한 설명서 및 구성 지침은 훌륭한 리소스입니다. 아직 구성 관리자를 배포하지 않은 경우 이러한 리소스를 시작하는 것이 좋습니다.
...
3.  구성 [관리자에서 Windows 10에 대한 지원을 검토합니다.](/configmgr/core/plan-design/configs/support-for-windows-10#windows-10-as-a-client) 배포를 업그레이드해야 하는 경우 설치하려는 업데이트를 선택한 다음 **다운로드를 선택합니다.**
...
2.  Microsoft Teams Rooms 배포를 서비스할 배포 지점 서버를 선택한 다음 속성을 **선택합니다.**
...
> [!NOTE]
> 부팅 가능한 USB 미디어를 만들어 PXE 지원이 없는 환경에 대한 Configuration Manager 작업 시퀀스 기반 배포를 시작할 수 있습니다. 부팅 가능한 미디어에는 Windows PE로 부팅하고 나머지 배포 프로세스에 대한 Configuration Manager에 연결할 수 있도록 부팅 이미지, 선택적 미리 시작 명령 및 필요한 파일 및 Configuration Manager 이진만 포함되어 있습니다. 자세한 내용은 부팅 가능한 [미디어 만들기 를 참조하세요.](/configmgr/osd/deploy-use/create-bootable-media#BKMK_CreateBootableMedia)

## <a name="create-configuration-manager-packages"></a>Configuration Manager 패키지 만들기

> [!IMPORTANT]
> 각 SRS 설치 관리자 버전에 필요한 운영 체제 버전은 모든 MSI 릴리스에 따라 변경됩니다. 특정 MSI에 대한 최상의 운영 체제 버전을 확인하려면 콘솔 설정 스크립트를 한 번 실행합니다. 자세한 내용은 Microsoft 엔드포인트 구성 관리자를 사용하여 [Microsoft Teams Rooms 배포를 참조합니다.](rooms-scale.md)

Configuration Manager는 Microsoft Teams Rooms 단위를 배포하고 구성하기 위해 여러 패키지가 필요합니다.

다음 패키지를 만들고 구성한 다음 배포 지점 서버 역할이 할당된 Configuration Manager 사이트 시스템에 배포해야 합니다.

| **패키지 이름**                     | **유형**               | **설명**                                                                           |
|--------------------------------------|------------------------|-------------------------------------------------------------------------------------------|
| SRS v2 - SRS 애플리케이션 패키지     | 소프트웨어 패키지       | Microsoft Teams Rooms 배포 키트에 대한 패키지                                      |
| SRS v2 - Sysprep 패키지             | 소프트웨어 패키지       | Microsoft Teams Rooms 단위를 Unattended.xml 사용자 지정 패키지            |
| SRS v2 - Set-SRSComputerName 패키지 | 소프트웨어 패키지       | 배포 중에 컴퓨터 이름을 할당하는 HTA(HTML 애플리케이션)에 대한 패키지    |
| SRS v2 - SRS 설정 구성         | 소프트웨어 패키지       | Microsoft Teams Rooms 앱의 배포를 구성하는 패키지                          |
...
Microsoft 엔드포인트 구성 관리자 중앙 관리 사이트 또는 기본 사이트에서 또는 패키지 원본 파일을 호스트하는 데 사용하는 서버 공유에서 다음 폴더 구조를 만들 수 있습니다.
...
2. 명령 프롬프트 창을 열고 명령 프롬프트에서 **MMASetup-AMD64.exe /C를** 입력하여 **SRS v2 - Microsoft** 모니터링 에이전트 패키지 폴더로 패키지를 추출합니다.

3. Configuration Manager 콘솔에서 **Software Library** 애플리케이션 \> **관리 패키지로 이동한** 다음 패키지 \>  **만들기 를 선택합니다.**

4. 다음 정보를 입력하여 패키지를 만들 수 있습니다.

   - 이름<strong>: SRS v2 - Microsoft 모니터링 에이전트 패키지</strong>

   - 제조업체<strong>: Microsoft Corporation</strong>

   - 버전<strong>: 8.1.11081.0(다운로드한</strong> 설치 파일의 버전을 입력합니다)

   - 이 **패키지에** 원본 파일이 포함된 확인란을 선택하고 **SRS v2 - Microsoft Monitoring Agent Package** 폴더에 대한 경로를 입력한 다음 다음을 **선택합니다.**
...
3. 필수 Windows 업데이트 패키지를 동일한 폴더에 다운로드합니다.
   > [!NOTE]
   > 이 문서가 게시된 [당시에는 KB4056892만](http://download.windowsupdate.com/c/msdownload/update/software/secu/2018/01/windows10.0-kb4056892-x64_a41a378cf9ae609152b505c40e691ca1228e28ea.msu) 필요합니다. [Microsoft Teams Rooms 콘솔](console.md)구성 을 확인하여 다른 업데이트가 필요한지 확인합니다.
...
-   Microsoft Teams Rooms 단위는 도메인 구성원이 아닌 작업에서 작동하도록 구성됩니다.
...
1.  **에서 최신 버전의 Microsoft Teams Rooms 배포 키트를** 다운로드하고, 이를 <https://go.microsoft.com/fwlink/?linkid=851168> 작업대에 설치합니다.

2.  **C: \\ 프로그램 파일(x86) \\ Skype Room System 배포 키트에서** **SRS v2 - SRS 애플리케이션 패키지 폴더에** 콘텐츠를 복사합니다.
...
1.  Windows 10 Enterprise x64 미디어를 구하고 **install.wim** 파일을 운영 체제 **\\ Windows 10 Enterprise 폴더에 복사합니다.**

2.  구성 관리자 콘솔에서 **소프트웨어** 라이브러리 운영 체제 운영 체제 이미지로 이동한 다음 운영 체제 이미지 \>  \>  **추가를 선택합니다.**

3.  방금 복사한 **install.wim** 파일의 경로를 지정한 다음 다음을 **선택합니다.**

4.  Windows  10 Enterprise 이미지의 빌드 번호와 일치하게 버전 필드를 업데이트한 다음 다음을 **선택합니다.**
...
### <a name="create-surface-pro-device-driver-packages"></a>Surface Pro 디바이스 드라이버 패키지 만들기

Microsoft Teams Rooms는 Surface Pro 및 Surface Pro 4 모두에서 지원됩니다. 환경에 있는 각 Surface Pro 모델에 대한 드라이버 패키지를 만들어야 합니다.

> [!IMPORTANT]
> 드라이버는 Windows 10 Enterprise 빌드 및 Microsoft Teams Rooms 배포 키트 버전과 호환되어야 합니다. 자세한 내용은 Surface 디바이스에 대한 최신 펌웨어 및 [드라이버](/surface/deploy-the-latest-firmware-and-drivers-for-surface-devices) 다운로드 및 콘솔 [구성을 참조하세요.](console.md)
...
4.  **다음 UNC(네트워크** 경로)에서 모든 드라이버 가져오기 를 선택하고 원본 폴더(예: C: _Sources Surface Pro)를 선택한 다음 \\ 다음을 \\ \\ **선택합니다.**
...
8.  Surface Pro 모델과 일치하는 패키지 이름을 지정하고, 폴더 경로를 입력하여 드라이버 패키지 파일을 저장하고 확인을 선택한 다음 **다음을** **선택합니다.**
...
> [!NOTE]
> 있을 수 있는 다른 Surface Pro 모델에 대해 동일한 단계를 반복합니다. 자세한 내용은 [Configuration Manager에서 드라이버 관리를 참조하세요.](/configmgr/osd/get-started/manage-drivers)

### <a name="create-microsoft-teams-rooms-configuration-package"></a>Microsoft Teams Rooms 구성 패키지 만들기
...
> 
> Microsoft Teams Rooms 단위 배포를 시작하기 전에 모든 패키지를 배포해야 합니다.
> 
...
Configuration Manager를 사용하여 작업 시퀀스를 사용하여 대상 컴퓨터에 운영 체제 이미지를 배포하는 단계를 자동화합니다. 자동화된 형식의 Microsoft Teams Rooms 단위를 배포하려면 대상 Microsoft Teams Rooms 컴퓨터, 설치하려는 Windows 10 Enterprise 운영 체제 이미지 및 기타 애플리케이션 또는 소프트웨어 업데이트와 같은 기타 추가 콘텐츠를 시작하는 데 사용되는 부팅 이미지를 참조하는 작업 시퀀스를 만듭니다.
...
3.  **찾아보기를** 선택하고 1단계에서 사용한 공유 폴더 위치로 이동하고, **MICROSOFT Teams Rooms 배포(EN-US).zip** 파일을 선택한 다음 다음을 **선택합니다.**
...
1. 가져온 작업 순서를 선택한 다음 **편집을 선택합니다.**
    ...
    작업 시퀀스 편집기가 Microsoft Teams Rooms 단위를 배포하고 구성하는 데 필요한 각 순차 단계를 열고 표시됩니다.
...
2. 각 단계를 진행하고 권장되는 업데이트를 완료합니다.
   ...
   3. **SRS 컴퓨터 이름** 설정 : 이 단계에서는 배포 중에 Microsoft Teams Rooms 단위에 대한 컴퓨터 이름을 설정하는 UI를 제공하는 HTML 애플리케이션이 포함되어 있습니다.
   ...
   6. **네트워크 설정 적용**: 이 단계를 통해 작업 영역 또는 Active Directory 도메인 이름 및 조직 단위를 지정할 수 있습니다.
      > [!NOTE]
      > Actve Directory 도메인의 구성원으로 Microsoft Teams Rooms 단위를 배포하는 데 필요한 권장 작업에 대한 Skype Room System 도메인 가입 고려 사항을 참조하세요. [](domain-joining-considerations.md)
   7. **드라이버 적용:** 이 단계 및 하위 단계는 현재 사용하는 Surface Pro 모델을 기반으로 해당 디바이스 드라이버 및 펌웨어를 배포하는 데 사용됩니다. 각 단계를 업데이트하여 이 배포와 관련된 관련 드라이버 패키지를 지정합니다.
      -   각 드라이버 패키지는 Surface Pro 만들기 및 모델에 따라 관련 드라이버 및 펌웨어를 배포하기 위해 WMI(Windows Management Instrumentation) 필터를 활용하도록 구성됩니다.
      -   이러한 드라이버의 구성을 변경하지 않는 것이 좋습니다. 그렇지 않으면 배포가 실패할 수 있습니다.

   8. **Windows 및 Configuration Manager 설정**: 이 단계는 Configuration Manager 클라이언트를 배포하고 구성합니다. 기본 제공 Configuration Manager 클라이언트 패키지를 지정하려면 이 단계를 업데이트합니다.

   9. **루트 인증서 설치**: 이 단계는 도메인에 가입되지 않은 디바이스에 대한 루트 인증서를 배포하므로 선택 사항이며 기본적으로 사용하지 않도록 설정됩니다.
      -   Microsoft Teams Rooms 단위에 루트 인증서를 배포해야 하는 경우 이 단계를 사용하도록 설정합니다.
      -   이 단계를 수행해야 하는 경우 **SRS v2 – 루트** 인증서 패키지 및 **64비트** 파일 시스템 리디렉션 사용 안 을 선택해야 합니다.

   10. **모니터링 에이전트** 설치 및 구성: 이 단계에서는 Microsoft Azure Monitor 에이전트의 64비트 버전을 설치하고 에이전트를 구성하여 Log Analytics 작업 영역으로 연결합니다.
       -   이 단계는 기본적으로 비활성화됩니다. 모니터링 에이전트를 사용하여 Microsoft Teams Rooms 단위의 상태 모니터링을 진행하는 경우만 이 단계를 사용하도록 설정합니다.
       ...
       -   **SRS v2 – Microsoft 모니터링 에이전트 패키지** 및 **64비트** 파일 시스템 리디렉션 사용 안 하세요.
       -   Microsoft Teams Rooms 배포의 상태 모니터링에 대한 자세한 내용은 [Azure Monitor를](azure-monitor-plan.md)사용하여 Microsoft Teams Rooms 관리 계획, Azure [Monitor를](azure-monitor-deploy.md) 사용하여 Microsoft Teams Rooms 관리 배포 및 Azure Monitor를 사용하여 Microsoft Teams Rooms 디바이스 관리 를 [참조하세요.](azure-monitor-manage.md)

   11. **SRS v2 구성 파일** 복사: 이 단계에서는 Microsoft Teams Rooms 배포 키트에서 로컬 하드 드라이브에 필요한 설정 및 구성 파일을 복사합니다. 이 단계에서는 사용자 지정이 필요하지 않습니다.
       -   **SRS v2 – SRS 애플리케이션 패키지** 및 **64비트 파일** 시스템 리디렉션 사용 안 하세요.

   12. **Install-SRSv2-OS-Updates:** 이 단계에서는 Microsoft Teams Rooms 배포에 필요한 필수 운영 체제 업데이트를 배포합니다. 다음을 수행합니다.
       -   [필요한 업데이트를 확인하려면 Microsoft Teams Rooms 콘솔](console.md) 구성을 확인합니다.
   ...
   15. **컴퓨터 다시 시작:** 이 단계는 Windows 기능이 구성된 후에 컴퓨터를 다시 부트합니다. 이 단계에서는 사용자 지정이 필요하지 않습니다.

   16. **로컬 Skype 사용자 추가:** 이 단계에서는 Windows에 자동으로 로그인하고 Microsoft Teams Rooms 애플리케이션을 시작하는 데 사용되는 로컬 Skype 계정을 만듭니다. 이 단계에서는 연결된 소프트웨어 패키지가 없습니다. 이에 대한 사용자 지정이 필요하지 않습니다.

   17. **SRS 애플리케이션** 설정 및 구성: 이 단계는 운영 체제의 다음 부팅에 대해 Microsoft Teams Rooms 애플리케이션 설치를 구성합니다.
...
6. 다음 목록에 사용할 수 있도록  만들기에서 미디어 및 **PXE만** 선택한 다음 을 **선택합니다.**
   ...
   >
   > 이러한 값을 다른 것으로 설정하면 모든 컴퓨터가 부팅 시 Microsoft Teams Rooms 배포 이미지를 얻을 수 있습니다.
...
Microsoft 엔드포인트 구성 관리자 작업 시퀀스를 완료한 후 테스트 실행을 수행하여 작업 시퀀스가 Microsoft Teams Rooms 단위를 배포하고 구성할 수 있는지 유효성을 검사해야 합니다.

1.  지원되는 이더넷 어댑터 중 하나를 사용하거나 Surface dock을 사용하여 테스트 디바이스를 유선 네트워크에 연결합니다. PXE 부팅 기능이 환경에 맞게 구성되지 않은 경우 앞에서 만든 USB 플래시 드라이브의 부팅 이미지를 사용하여 USB에서 부팅하고 Configuration Manager에 연결할 수 있습니다. [](/configmgr/osd/deploy-use/create-bootable-media)
...
5.  Windows PE가 메모리에 로드하고 작업 순서 마법사가 시작됩니다. **다음을** 선택하여 계속합니다.

6.  앞에서 가져온 작업 순서를 선택한 다음 다음 을 **선택합니다.**

7.  디스크 구성이 적용된 후 디바이스에 대한 컴퓨터 이름을 지정하라는 메시지가 표시됩니다. 사용자 인터페이스는 Surface Pro 디바이스의 일련 번호에 따라 권장되는 컴퓨터 이름을 표시합니다. 제안된 이름을 수락하거나 새 이름을 지정할 수 있습니다. 컴퓨터 이름 할당 화면의 지침을 따릅니다. **수락을 선택하면** 배포가 시작됩니다.

8.  나머지 배포 프로세스는 자동으로 수행됩니다. 더 이상 사용자 입력을 요청하지 않습니다.

9.  배포 작업 순서가 디바이스 구성을 완료하면 Microsoft Teams Rooms 애플리케이션 설정을 구성하도록 요청하는 다음 구성 화면이 표시됩니다.

    ![Microsoft Teams Rooms 애플리케이션에 대한 초기 설정 화면](../media/room-systems-scale-image2.png)

10.  Surface Pro를 Microsoft Teams Rooms 본체에 연결하고 애플리케이션 설정을 구성합니다.

11.  [Microsoft Teams Rooms에](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2) 나열된 기능이 배포된 디바이스에서 작동하고 있는지 확인합니다.
...
| **배포 단계**                                                            | **작업 시퀀스 로그 경로**                         |
|---------------------------------------------------------------------------------|----------------------------------------------------|
| HDD 형식 이전의 WinPE                                                        | X: \\ Windows \\ Temp \\ smstslog \\ smsts.log             |
...
Configuration Manager 설치 문제를 추가로 해결하는 데 사용할 수 있는 로그 파일의 전체 목록은 Microsoft 엔드포인트 구성 관리자 로그 파일 참조 를 [참조하세요.](/configmgr/core/plan-design/hierarchy/log-files)