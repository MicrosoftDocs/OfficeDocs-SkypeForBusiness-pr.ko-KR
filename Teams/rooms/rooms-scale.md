...
<span data-ttu-id="8bd6d-104">이 문서에서는 Microsoft Endpoint Configuration Manager를 사용하여 Microsoft Teams Rooms 배포를 만드는 데 필요한 모든 정보를 제공합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-104">This article gives you all the necessary information to create your Microsoft Teams Rooms deployments by using Microsoft Endpoint Configuration Manager.</span></span>
...
2.  <span data-ttu-id="8bd6d-139">아직 설치되지 않은 설치된 빌드 및 적용 가능한 업데이트를 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-139">Check the installed build and applicable updates that haven't been installed yet.</span></span>
...
1.  <span data-ttu-id="8bd6d-153">구성 관리자 콘솔에서 관리  사이트 구성 \> **사이트로 이동한** 다음 \> 사이트를 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-153">In the Configuration Manager console, go to **Administration** \> **Site Configuration** \> **Sites**, and then select the site.</span></span>
...
6.  <span data-ttu-id="8bd6d-171">요청이 있는 경우 **예를** 선택하고 업데이트된 부팅 이미지를 배포 지점에 배포합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-171">When asked, select **Yes** and distribute the updated boot image to your distribution points.</span></span>
...
| <span data-ttu-id="8bd6d-194">SRS v2 - SRS 설정 구성</span><span class="sxs-lookup"><span data-stu-id="8bd6d-194">SRS v2 - Configure SRS Setup</span></span>         | <span data-ttu-id="8bd6d-195">소프트웨어 패키지</span><span class="sxs-lookup"><span data-stu-id="8bd6d-195">Software package</span></span>       | <span data-ttu-id="8bd6d-196">Microsoft Teams Rooms 앱 배포를 구성하는 패키지</span><span class="sxs-lookup"><span data-stu-id="8bd6d-196">Package to configure deployment of the Microsoft Teams Rooms app</span></span>                          |
...
-   <span data-ttu-id="8bd6d-223">SRS v2 - OS 업데이트 패키지</span><span class="sxs-lookup"><span data-stu-id="8bd6d-223">SRS v2 - OS Updates Package</span></span>
...
4. <span data-ttu-id="8bd6d-239">다음 정보를 입력하여 패키지를 만들 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-239">Enter the following information to create the package:</span></span>
...
1.  <span data-ttu-id="8bd6d-268">루트 인증서를 **SRS v2 – 루트** 인증서 패키지 폴더에 복사합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-268">Copy the root certificate into the **SRS v2 – Root Certificate Package** folder.</span></span>
...
5.  <span data-ttu-id="8bd6d-287">프로그램 **만들기 안 을 선택하고** 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-287">Select **Do not create a program**, and then select **Next**.</span></span>
...
6. <span data-ttu-id="8bd6d-314">설정 **확인 페이지를 검토하고** 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-314">Review the **Confirm the settings** page, and then select **Next**.</span></span>
...
1.  <span data-ttu-id="8bd6d-329">최신 드라이버 및 펌웨어를 다운로드합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-329">Download the latest drivers and firmware.</span></span>
...
12. <span data-ttu-id="8bd6d-343">가져온 모든 드라이버를 새로 만든 폴더로 이동하여 쉽게 탐색하고 작업을 할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-343">Move all the imported drivers to the newly created folder for easier navigation and operation.</span></span>
...
3.  <span data-ttu-id="8bd6d-371">운영 체제 패키지를 배포합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-371">Distribute operating system packages.</span></span>
    ...
    2.  <span data-ttu-id="8bd6d-374">패키지 목록을 검토하고 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-374">Review the list of packages, and then select **Next**.</span></span>
...
> [!NOTE]
> <span data-ttu-id="8bd6d-377">패키지 배포는 패키지 크기, Configuration Manager 계층 구조, 배포 지점 서버 수 및 네트워크에서 사용할 수 있는 대역폭에 따라 다소 시간이 걸릴 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-377">Package distribution might take some time, depending on the package size, Configuration Manager hierarchy, number of distribution point servers, and the bandwidth available in your network.</span></span>
> 
> <span data-ttu-id="8bd6d-378">Microsoft Teams 회의실 단위 배포를 시작하기 전에 모든 패키지를 배포해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-378">All the packages must be distributed before you can start deploying a Microsoft Teams Rooms unit.</span></span>
> 
> <span data-ttu-id="8bd6d-379">배포 상태 콘텐츠 상태 모니터링으로 진행하여 구성 관리자  콘솔에서 패키지 배포 상태를 \> **검토할** \> **수 있습니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-379">You can review the status of your package distribution in the Configuration Manager console by going to **Monitoring** \> **Distribution Status** \> **Content Status**.</span></span>

## <a name="configuration-manager-task-sequences"></a><span data-ttu-id="8bd6d-380">구성 관리자 작업 시퀀스</span><span class="sxs-lookup"><span data-stu-id="8bd6d-380">Configuration Manager task sequences</span></span>

<span data-ttu-id="8bd6d-381">구성 관리자에서 작업 시퀀스를 사용하여 운영 체제 이미지를 대상 컴퓨터에 배포하는 단계를 자동화합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-381">You use task sequences with Configuration Manager to automate the steps for deploying an operating system image to a destination computer.</span></span> <span data-ttu-id="8bd6d-382">자동으로 Microsoft Teams 회의실 단위를 배포하려면 대상 Microsoft Teams Rooms 컴퓨터, 설치하려는 Windows 10 Enterprise 운영 체제 이미지 및 기타 추가 콘텐츠(예: 다른 응용 프로그램 또는 소프트웨어 업데이트)를 시작하는 데 사용되는 부팅 이미지를 참조하는 작업 시퀀스를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-382">To deploy a Microsoft Teams Rooms unit in an automated fashion, you create a task sequence that references the boot image used to start the destination Microsoft Teams Rooms computer, the Windows 10 Enterprise operating system image that you want to install, and any other additional content, such as other applications or software updates.</span></span>

### <a name="import-the-sample-task-sequence"></a><span data-ttu-id="8bd6d-383">샘플 작업 시퀀스 가져오기</span><span class="sxs-lookup"><span data-stu-id="8bd6d-383">Import the sample task sequence</span></span>

<span data-ttu-id="8bd6d-384">샘플 작업 시퀀스를 다운로드하고 쉽게 가져오고 요구에 맞게 사용자 지정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-384">You can download and easily import a sample task sequence and customize it to meet your needs.</span></span>

1.  <span data-ttu-id="8bd6d-385">[**샘플**](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Skype/SfbOnline/downloads/Skype-Room-Systems-v2/SRS-v2-Configuration-Manager-Files.zip?raw=true) 작업 시퀀스를 다운로드하고 다운로드한 zip 파일을 공유 위치에 복사합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-385">[**Download**](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Skype/SfbOnline/downloads/Skype-Room-Systems-v2/SRS-v2-Configuration-Manager-Files.zip?raw=true) the sample task sequence, and copy the downloaded zip file to a shared location.</span></span>
2.  <span data-ttu-id="8bd6d-386">구성 관리자 콘솔에서 **소프트웨어** 라이브러리 운영 체제 작업 시퀀스로 이동한 다음 작업 시퀀스 \>  \>  **가져오기를 선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-386">In the Configuration Manager console, go to **Software Library** \> **Operating Systems** \> **Task Sequences**, and then select **Import Task Sequence**.</span></span>

3.  <span data-ttu-id="8bd6d-387">**찾아보기를** 선택하고, 1단계에서 사용한 공유 폴더 위치로 이동하고, **Microsoft Teams 회의실 배포(EN-US).zip** 파일을 선택한 다음, 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-387">Select **Browse**, go to the shared folder location you used in step 1, select the **Microsoft Teams Rooms Deployment (EN-US).zip** file, and then select **Next**.</span></span>

4.  <span data-ttu-id="8bd6d-388">작업을 **새로** **만들기로 설정하고** 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-388">Set **Action** to **Create New**, and then select **Next**.</span></span>

5.  <span data-ttu-id="8bd6d-389">설정을 확인하고 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-389">Confirm the settings, and then select **Next**.</span></span>

6.  <span data-ttu-id="8bd6d-390">닫기 **선택**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-390">Select **Close**.</span></span>

### <a name="validate-that-the-reference-packages-are-correctly-linked-to-each-task-sequence-step"></a><span data-ttu-id="8bd6d-391">참조 패키지가 각 작업 시퀀스 단계에 올바르게 연결되어 있는지 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-391">Validate that the reference packages are correctly linked to each task sequence step.</span></span>

1. <span data-ttu-id="8bd6d-392">가져온 작업 시퀀스를 선택한 다음 편집을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-392">Select the imported task sequence, and then select **Edit**.</span></span>

    <span data-ttu-id="8bd6d-393">작업 시퀀스 편집기가 열리면 Microsoft Teams 회의실 단위를 배포하고 구성하는 데 필요한 각 순차적 단계가 표시됩니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-393">The Task Sequence Editor opens and displays each sequential step that you need to deploy and configure a Microsoft Teams Rooms unit.</span></span>

2. <span data-ttu-id="8bd6d-394">각 단계를 단계별로 진행하고 권장 업데이트를 완료합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-394">Walk through each step and complete the recommended updates:</span></span>

   1. <span data-ttu-id="8bd6d-395">**Windows PE에서 다시 시작:** 이 단계가 다시 시작된 다음 컴퓨터를 Windows PXE로 부팅합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-395">**Restart in Windows PE**: This step restarts and then boots the computer into Windows PXE.</span></span> <span data-ttu-id="8bd6d-396">이 단계에서는 변경이 필요하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-396">No changes are required for this step.</span></span>

   2. <span data-ttu-id="8bd6d-397">**파티션 디스크 0 – UEFI:** 이 단계는 디스크 구성을 지우고 구성된 설정에 따라 파티션을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-397">**Partition Disk 0 – UEFI**: This step wipes the disk configuration and creates partitions based on the configured settings.</span></span> <span data-ttu-id="8bd6d-398">이 단계를 변경하지 않는 것이 좋습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-398">We recommend that you not make any changes to this step.</span></span>

   3. <span data-ttu-id="8bd6d-399">**SRS 컴퓨터** 이름 설정: 이 단계에서는 배포하는 동안 Microsoft Teams 회의실 단위의 컴퓨터 이름을 설정하는 UI를 제공하는 HTML 애플리케이션을 포함합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-399">**Set SRS Computer Name**: This step includes an HTML application to provide a UI to set a computer name for the Microsoft Teams Rooms unit during the deployment.</span></span>
      -  <span data-ttu-id="8bd6d-400">선택적 단계이지만 대체 프로세스를 통해 컴퓨터 이름을 관리하려는 경우 비활성화할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-400">This is an optional step, but it can only be disabled if you want to manage computer naming through an alternate process.</span></span>
      -  <span data-ttu-id="8bd6d-401">**SRS v2 - Set-SRSComputerName 패키지가** 선택되어 있는지 확인</span><span class="sxs-lookup"><span data-stu-id="8bd6d-401">Verify that the **SRS v2 - Set-SRSComputerName** package is selected.</span></span> <span data-ttu-id="8bd6d-402">그렇지 않은 경우 패키지를 찾아 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-402">If it isn't, browse to the package and select it.</span></span>

   4. <span data-ttu-id="8bd6d-403">**운영 체제** 적용: 이 단계에서는 배포할 운영 체제 이미지와 사용할 무인 Sysprep 응답 파일을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-403">**Apply Operating System**: This step specifies the operating system image to be deployed and the unattended Sysprep answer file to use.</span></span>
      -  <span data-ttu-id="8bd6d-404">올바른 Windows 10 Enterprise 운영 체제 이미지 파일이 선택되어 있는지 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-404">Verify that the correct Windows 10 Enterprise operating system image file is selected.</span></span>
      -  <span data-ttu-id="8bd6d-405">사용자 지정 설치에 무인 또는 **Sysprep 응답** 파일을 사용할 수 있는지 확인하고 **SRS v2 - Sysprep 패키지가** 선택되어 있는지 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-405">Verify that **Use an unattended or Sysprep answer file for a custom installation** is enabled, and the **SRS v2 - Sysprep Package** is selected.</span></span> <span data-ttu-id="8bd6d-406">또한 **파일** 이름이 다음으로 **설정되어unattend.xml.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-406">Also ensure that **File Name** is set to **unattend.xml**.</span></span>

   5. <span data-ttu-id="8bd6d-407">**Windows 설정 적용:** 이 단계에서는 Windows 설치에 대한 정보를 수집합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-407">**Apply Windows Settings**: This step gathers information about the Windows installation.</span></span>
      -  <span data-ttu-id="8bd6d-408">제품 키, 로컬 관리자 계정 암호 및 표준 시간대를 포함한 라이선스 및 등록 정보를 제공합니다(요구에 따라 다를 수 있습니다).</span><span class="sxs-lookup"><span data-stu-id="8bd6d-408">Provide licensing and registration information including the product key, local administrator account password, and time zone (depending on your needs).</span></span>

   6. <span data-ttu-id="8bd6d-409">**네트워크 설정 적용:** 이 단계를 통해 작업 영역 또는 Active Directory 도메인 이름 및 조직 구성 단위를 지정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-409">**Apply Network Settings**: This step allows you to specify a workgroup or Active Directory domain name and organizational unit.</span></span>
      > [!NOTE]
      > <span data-ttu-id="8bd6d-410">Actve Directory 도메인의 구성원으로 Microsoft Teams 회의실 단위를 배포하는 데 필요한 권장 작업에 대한 [Skype](domain-joining-considerations.md) 채팅방 시스템 도메인 가입 고려 사항을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-410">See [Skype Room System domain joining considerations](domain-joining-considerations.md) for recommended actions you need to take in deploying Microsoft Teams Rooms units as members of an Actve Directory domain.</span></span>
   7. <span data-ttu-id="8bd6d-411">**드라이버 적용:** 이 단계 및 하위 단계는 사용 중 Surface Pro 모델을 기반으로 해당 디바이스 드라이버 및 펌웨어를 배포하는 데 사용됩니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-411">**Apply Drivers:** This step and its sub-steps are used to deploy applicable device drivers and firmware based on the Surface Pro model you have.</span></span> <span data-ttu-id="8bd6d-412">이 배포와 연결된 관련 드라이버 패키지를 지정하도록 각 단계를 업데이트합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-412">Update each step to specify the relevant driver package associated with this deployment.</span></span>
      -   <span data-ttu-id="8bd6d-413">각 드라이버 패키지는 WMI(Windows Management Instrumentation) 필터를 활용하여 Surface Pro make 및 model에 따라 관련 드라이버 및 펌웨어를 배포하도록 구성됩니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-413">Each driver package is configured to leverage Windows Management Instrumentation (WMI) filters to deploy relevant drivers and firmware based on the Surface Pro make and model.</span></span>
      -   <span data-ttu-id="8bd6d-414">이러한 드라이버의 구성을 변경하지 않는 것이 좋습니다. 그렇지 않으면 배포가 실패할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-414">We highly recommend that you not alter the configuration of these drivers, otherwise deployment might fail.</span></span>

   8. <span data-ttu-id="8bd6d-415">**Windows 및 구성 관리자 설정:** 이 단계에서는 Configuration Manager 클라이언트를 배포하고 구성합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-415">**Set up Windows and Configuration Manager**: This step deploys and configures the Configuration Manager client.</span></span> <span data-ttu-id="8bd6d-416">기본 제공 Configuration Manager 클라이언트 패키지를 지정하려면 이 단계를 업데이트합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-416">Update this step to specify the built-in Configuration Manager Client Package.</span></span>

   9. <span data-ttu-id="8bd6d-417">**루트 인증서** 설치: 이 단계는 도메인에 가입되지 않은 디바이스에 대한 루트 인증서를 배포하므로 선택 사항이며 기본적으로 사용하지 않도록 설정됩니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-417">**Install Root Certificate**: This step distributes the root certificate for non–domain-joined devices, and therefore is optional and disabled by default.</span></span>
      -   <span data-ttu-id="8bd6d-418">Microsoft Teams 회의실 단위에 루트 인증서를 배포해야 하는 경우 이 단계를 사용하도록 설정합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-418">Enable this step if you need to deploy a root certificate to the Microsoft Teams Rooms units.</span></span>
      -   <span data-ttu-id="8bd6d-419">이 단계를 수행해야 하는 경우 **SRS v2 –** 루트 인증서 패키지 및 **64비트** 파일 시스템 리디렉션 사용 안 을 선택해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-419">If you do need to perform this step, verify that the **SRS v2 – Root Certificate Package** and **Disable 64-bit file system redirection** are selected.</span></span>

   10. <span data-ttu-id="8bd6d-420">**모니터링 에이전트** 설치 및 구성: 이 단계에서는 64비트 버전의 Microsoft Azure Monitor 에이전트를 설치하고 Log Analytics 작업 영역으로 연결하도록 에이전트를 구성합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-420">**Install and Configure Monitoring Agent**: This step installs the 64-bit version of the Microsoft Azure Monitor agent and configures the agent to connect to your Log Analytics workspace.</span></span>
       -   <span data-ttu-id="8bd6d-421">이 단계는 기본적으로 사용하지 않도록 설정되어 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-421">This step is disabled by default.</span></span> <span data-ttu-id="8bd6d-422">모니터링 에이전트를 사용하여 Microsoft Teams 회의실 단위의 상태 모니터링을 사용하려면 이 단계를 사용하도록 설정해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-422">Enable this step only if you're going to use the Monitoring Agent to monitor the health of your Microsoft Teams Rooms units.</span></span>
       -   <span data-ttu-id="8bd6d-423">이 단계를 편집하고 명령줄 매개 변수를 업데이트하여 작업 영역 **ID** 및 작업 영역 키를 **지정합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-423">Edit this step and update the command-line parameters to specify your **Workspace ID** and **Workspace Key**.</span></span>
       -   <span data-ttu-id="8bd6d-424">Operations Management Suite 작업 영역 ID 및 기본 키를 얻는 데 대한 자세한 내용은 [Azure 모니터링에](azure-monitor-deploy.md#configure-test-devices-for-azure-monitoring) 대한 테스트 디바이스 구성을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-424">See [Configure test devices for Azure Monitoring](azure-monitor-deploy.md#configure-test-devices-for-azure-monitoring) for more information about obtaining the Operations Management Suite Workspace ID and the primary key.</span></span>
       -   <span data-ttu-id="8bd6d-425">**SRS v2 – Microsoft Monitoring Agent 패키지** 및 사용 안 **64비트** 파일 시스템 리디렉션이 선택되어 있는지 확인</span><span class="sxs-lookup"><span data-stu-id="8bd6d-425">Verify that the **SRS v2 – Microsoft Monitoring Agent Package** and **Disable 64-bit file system redirection** are selected.</span></span>
       -   <span data-ttu-id="8bd6d-426">Microsoft Teams Rooms 배포의 상태 모니터링에 대한 자세한 내용은 [Azure Monitor를](azure-monitor-plan.md)사용하여 Microsoft Teams 회의실 관리 계획, Azure [Monitor를](azure-monitor-deploy.md) 사용하여 Microsoft Teams Rooms 관리 배포 및 [Azure Monitor를](azure-monitor-manage.md)사용하여 Microsoft Teams Rooms 디바이스 관리에 대한 자세한 정보를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-426">For more information about monitoring the health of your Microsoft Teams Rooms deployment, see [Plan Microsoft Teams Rooms management with Azure Monitor](azure-monitor-plan.md), [Deploy Microsoft Teams Rooms management with Azure Monitor](azure-monitor-deploy.md) and [Manage Microsoft Teams Rooms devices with Azure Monitor](azure-monitor-manage.md).</span></span>

   11. <span data-ttu-id="8bd6d-427">**SRS v2 구성** 파일 복사: 이 단계에서는 필요한 설정 및 구성 파일을 Microsoft Teams 회의실 배포 키트에서 로컬 하드 드라이브로 복사합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-427">**Copy SRS v2 Configuration Files**: This step copies the required setup and configuration files from the Microsoft Teams Rooms deployment kit to the local hard drive.</span></span> <span data-ttu-id="8bd6d-428">이 단계에서는 사용자 지정이 필요하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-428">No customization is required for this step.</span></span>
       -   <span data-ttu-id="8bd6d-429">**SRS v2 – SRS 애플리케이션** 패키지 및 **64비트** 파일 시스템 리디렉션 사용 안 을 선택해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-429">Verify that the **SRS v2 – SRS Application Package** and **Disable 64-bit file system redirection** are selected.</span></span>

   12. <span data-ttu-id="8bd6d-430">**Install-SRSv2-OS-Updates:** 이 단계에서는 Microsoft Teams Rooms 배포에 필요한 모든 필수 운영 체제 업데이트를 배포합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-430">**Install-SRSv2-OS-Updates**: This step deploys any mandatory operating system updates required with the Microsoft Teams Rooms deployment.</span></span> <span data-ttu-id="8bd6d-431">다음을 수행합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-431">Do the following:</span></span>
       -   <span data-ttu-id="8bd6d-432">필요한 [업데이트를 확인하려면 Microsoft Teams 회의실](console.md) 콘솔 구성을 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-432">Check [Configure a Microsoft Teams Rooms console](console.md) to see which updates are required.</span></span>
       -   <span data-ttu-id="8bd6d-433">**SRS v2 – OS 업데이트 패키지에** 필요한 모든 업데이트가 포함되어 있는지 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-433">Verify that your **SRS v2 – OS Updates Package** includes all the required updates.</span></span>
       -   <span data-ttu-id="8bd6d-434">**SRS v2 – OS 업데이트 패키지가** 선택되어 있는지 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-434">Verify that the **SRS v2 – OS Updates Package** is selected.</span></span>
       -   <span data-ttu-id="8bd6d-435">PowerShell 실행 정책이 우회로 **설정되어 있는지 확인**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-435">Verify that the PowerShell execution policy is set to **Bypass**.</span></span>

   13. <span data-ttu-id="8bd6d-436">**컴퓨터 다시 시작:** 이 단계는 필수 운영 체제 업데이트가 설치된 후 컴퓨터를 다시 시작합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-436">**Restart Computer**: This step reboots the computer after the mandatory operating system updates are installed.</span></span> <span data-ttu-id="8bd6d-437">이 단계에서는 사용자 지정이 필요하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-437">No customization is required for this step.</span></span>

   14. <span data-ttu-id="8bd6d-438">**Windows 구성 요소 구성: 이** 단계에서는 필요한 Windows 기능을 구성합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-438">**Configure Windows Components**: This step configures the required Windows features.</span></span> <span data-ttu-id="8bd6d-439">이 단계에서는 사용자 지정이 필요하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-439">No customization is required for this step.</span></span>

   15. <span data-ttu-id="8bd6d-440">**컴퓨터 다시 시작:** 이 단계는 Windows 기능이 구성된 후 컴퓨터를 다시 시작합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-440">**Restart Computer**: This step reboots the computer after the Windows features are configured.</span></span> <span data-ttu-id="8bd6d-441">이 단계에서는 사용자 지정이 필요하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-441">No customization is required for this step.</span></span>

   16. <span data-ttu-id="8bd6d-442">**로컬 Skype 사용자** 추가: 이 단계에서는 Windows에 자동으로 로그인하고 Microsoft Teams 회의실 응용 프로그램을 시작하는 데 사용되는 로컬 Skype 계정을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-442">**Add Local Skype User**: This step creates the local Skype account used to automatically sign in to Windows and start the Microsoft Teams Rooms application.</span></span> <span data-ttu-id="8bd6d-443">이 단계에서는 소프트웨어 패키지와 연결된 소프트웨어 패키지가 없습니다. 이에 대한 사용자 지정은 필요하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-443">This step doesn't have any software package associated with it, and no customization is required for it.</span></span>

   17. <span data-ttu-id="8bd6d-444">**SRS** 애플리케이션 설정 및 구성: 이 단계에서는 운영 체제의 다음 부팅에 대해 Microsoft Teams Rooms 애플리케이션 설치를 구성합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-444">**Set up and configure SRS application**: This step configures the Microsoft Teams Rooms application installation for the next boot of the operating system.</span></span>
       -   <span data-ttu-id="8bd6d-445">**SRS v2 – SRS 설정** 패키지 구성 및 **64비트** 파일 시스템 리디렉션 사용 안 하도록 설정이 선택되어 있는지 확인</span><span class="sxs-lookup"><span data-stu-id="8bd6d-445">Verify that the **SRS v2 – Configure SRS Setup Package** and **Disable 64-bit file system redirection** are selected.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="8bd6d-446">작업 시퀀스 단계가 제공된 순서에 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-446">It is very important that the task sequence steps must be in the provided order.</span></span> <span data-ttu-id="8bd6d-447">단계의 순서를 수정하거나 추가 단계를 구성하면 배포가 중단될 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-447">Modifying the order of steps, or configuring additional steps might break the deployment.</span></span>
>
> <span data-ttu-id="8bd6d-448">**SRS 애플리케이션** 단계 설정 및 구성은 작업 시퀀스의 마지막 단계가 되어야 합니다. 그렇지 않으면 배포가 실패할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-448">**Set up and configure SRS application** step must be the last step in the task sequence, otherwise the deployment might fail.</span></span>

### <a name="create-deployment-for-the-task-sequence"></a><span data-ttu-id="8bd6d-449">작업 시퀀스에 대한 배포 만들기</span><span class="sxs-lookup"><span data-stu-id="8bd6d-449">Create deployment for the task sequence</span></span>

1. <span data-ttu-id="8bd6d-450">작업 시퀀스를 선택한 다음 배포를 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-450">Select the task sequence, and then select **Deploy**.</span></span>

2. <span data-ttu-id="8bd6d-451">**찾아보기를** 선택하여 배포할 대상 컬렉션을 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-451">Select **Browse** to select target collection for deployment.</span></span>

3. <span data-ttu-id="8bd6d-452">알 **수 없는 모든 컴퓨터를 선택한** 다음 확인을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-452">Select **All Unknown Computers** and then select **OK**.</span></span>

4. <span data-ttu-id="8bd6d-453">다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-453">Select **Next**.</span></span>

5. <span data-ttu-id="8bd6d-454">목적 **드롭다운** **목록에서 사용 가능을** 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-454">Select **Available** on the **Purpose** drop down list.</span></span>

6. <span data-ttu-id="8bd6d-455">다음 목록에서 미디어 및  **PXE만** 선택하고 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-455">Select **Only Media and PXE** in the **Make available to the following** list, and then select **Next**.</span></span>
   > [!WARNING]
   > <span data-ttu-id="8bd6d-456">목적이 **사용** 가능으로 설정되어 있는 것이 **매우 중요합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-456">It is very important that **Purpose** is set to **Available**.</span></span> <span data-ttu-id="8bd6d-457">목적이 **필수로** **설정되어 있지** **않은지 확인**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-457">Make sure that the **Purpose** is **NOT** set to **Required**.</span></span> <span data-ttu-id="8bd6d-458">또한 다음에서 사용할 수 있도록 만들기에서 미디어 및 **PXE만** **선택해야 합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-458">Also make sure that you select **Only Media and PXE** in the **Make available to the following**.</span></span>
   >
   > <span data-ttu-id="8bd6d-459">이러한 값을 다른 값으로 설정하면 부팅 시 모든 컴퓨터에서 Microsoft Teams Rooms 배포 이미지를 얻을 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-459">Setting these values to something else might cause all computers to get the Microsoft Teams Rooms deployment image when booted.</span></span>
7. <span data-ttu-id="8bd6d-460">일정을 지정하지 말고 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-460">Do not specify any schedule and select **Next**.</span></span>

8. <span data-ttu-id="8bd6d-461">사용자 환경 섹션 내에서 아무 것도 **변경하지** 말고 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-461">Do not change anything within the **User Experience** section and select **Next**.</span></span>

9. <span data-ttu-id="8bd6d-462">경고 섹션 내에서 아무 것도 변경하지 **말고** 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-462">Do not change anything within the **Alerts** section and select **Next**.</span></span>

10. <span data-ttu-id="8bd6d-463">배포 지점 섹션 내에서 아무 것도 변경하지 **말고** 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-463">Do not change anything within the **Distribution Points** section and select **Next**.</span></span>

11. <span data-ttu-id="8bd6d-464">설정을 확인하고 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-464">Confirm the settings and then select **Next**.</span></span>

12. <span data-ttu-id="8bd6d-465">닫기 **선택**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-465">Select **Close**.</span></span>

<a name="validate-and-troubleshoot-the-solution"></a><span data-ttu-id="8bd6d-466">솔루션의 유효성 검사 및 문제 해결</span><span class="sxs-lookup"><span data-stu-id="8bd6d-466">Validate and troubleshoot the solution</span></span>
--------------------------------------

<span data-ttu-id="8bd6d-467">Microsoft Endpoint Configuration Manager 작업 시퀀스를 완료한 후 테스트 실행을 수행하여 작업 시퀀스가 Microsoft Teams 회의실 단위를 배포하고 구성할 수 있는지 유효성을 검사해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-467">After you've completed the Microsoft Endpoint Configuration Manager task sequences, you'll need to perform a test run to validate that the task sequence can deploy and configure Microsoft Teams Rooms units.</span></span>

1.  <span data-ttu-id="8bd6d-468">지원되는 이더넷 어댑터 중 하나를 사용하거나 Surface Dock을 사용하여 유선 네트워크에 테스트 디바이스를 연결합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-468">Connect the test device to the wired network by using one of the supported Ethernet adapters or using the Surface dock.</span></span> <span data-ttu-id="8bd6d-469">사용자 환경에 대해 PXE 부팅 기능이 구성되지 않은 경우 앞에서 만든 USB 플래시 [](https://docs.microsoft.com/configmgr/osd/deploy-use/create-bootable-media) 드라이브의 부팅 이미지를 사용하여 USB에서 부팅하고 Configuration Manager에 연결할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-469">If PXE boot functionality hasn't been configured for your environment, you can use the boot image on the USB flash drive [that you created earlier](https://docs.microsoft.com/configmgr/osd/deploy-use/create-bootable-media) to boot from USB and connect to Configuration Manager.</span></span>

2.  <span data-ttu-id="8bd6d-470">펌웨어에 액세스하고 PXE 부팅을 시작합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-470">Access the firmware and initiate PXE boot:</span></span>

    1.  <span data-ttu-id="8bd6d-471">Surface 디바이스의 전원이 꺼진지 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-471">Ensure the Surface device is powered off.</span></span>

    2.  <span data-ttu-id="8bd6d-472">볼륨 증가 **단추를 누를 수** 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-472">Press and hold the **Volume Up** button.</span></span>

    3.  <span data-ttu-id="8bd6d-473">전원 단추를 **누르고 니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-473">Press and release the **Power** button.</span></span>

    4.  <span data-ttu-id="8bd6d-474">디바이스가 부팅을 시작한 후 볼륨 증가 **단추를 해제합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-474">After the device begins to boot, release the **Volume Up** button.</span></span>

    5.  <span data-ttu-id="8bd6d-475">부팅 **구성을 선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-475">Select **Boot configuration**.</span></span>

    6.  <span data-ttu-id="8bd6d-476">다음 중 하나를 수행합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-476">Do one of the following:</span></span>

        -   <span data-ttu-id="8bd6d-477">**PXE 부팅을 선택하고** 목록 맨 위로 끌어서 놓습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-477">Select **PXE boot**, and drag it to the top of the list.</span></span> <span data-ttu-id="8bd6d-478">또는 네트워크 어댑터에서 왼쪽으로 스와이프하여 디바이스에 즉시 부팅할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-478">Alternatively, you can swipe left on the network adapter to boot to the device immediately.</span></span> <span data-ttu-id="8bd6d-479">부팅 순서에는 영향을 주지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-479">This won't affect the boot order.</span></span>
        -   <span data-ttu-id="8bd6d-480">부팅 미디어가 있는 USB 플래시 드라이브를 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-480">Select the USB flash drive that holds the boot media.</span></span>

3.  <span data-ttu-id="8bd6d-481">**종료를** 선택하고 지금 다시 **시작을 선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-481">Select **Exit**, and then select **Restart Now**.</span></span>

4.  <span data-ttu-id="8bd6d-482">메시지가 표시될 때 네트워크 부팅 **서비스에 대해 Enter를** 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-482">When prompted, select **Enter** for network boot service.</span></span>

5.  <span data-ttu-id="8bd6d-483">Windows PE가 메모리에 로드하고 작업 시퀀스 마법사가 시작됩니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-483">Windows PE will load into memory, and the Task Sequence Wizard will start.</span></span> <span data-ttu-id="8bd6d-484">다음을 **선택하여** 계속합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-484">Select **Next** to continue.</span></span>

6.  <span data-ttu-id="8bd6d-485">앞에서 가져온 작업 시퀀스를 선택하고 다음을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-485">Select the task sequence that you imported earlier, and then select **Next**.</span></span>

7.  <span data-ttu-id="8bd6d-486">디스크 구성이 적용된 후 디바이스의 컴퓨터 이름을 지정하라는 메시지가 표시됩니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-486">After the disk configuration is applied, you'll be prompted to specify a computer name for the device.</span></span> <span data-ttu-id="8bd6d-487">사용자 인터페이스는 Surface Pro 디바이스의 일련 번호를 기반으로 권장되는 컴퓨터 이름을 표시합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-487">The user interface will display a recommended computer name based on the serial number of the Surface Pro device.</span></span> <span data-ttu-id="8bd6d-488">제안된 이름을 수락하거나 새 이름을 지정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-488">You can either accept the proposed name or specify a new one.</span></span> <span data-ttu-id="8bd6d-489">컴퓨터 이름 할당 화면의 지침을 따릅니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-489">Follow the instructions on the computer name assignment screen.</span></span> <span data-ttu-id="8bd6d-490">**수락을 선택하면** 배포가 시작됩니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-490">When you select **Accept**, the deployment begins.</span></span>

8.  <span data-ttu-id="8bd6d-491">배포 프로세스의 나머지는 자동으로 수행하며 더 이상 사용자 입력을 요청하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-491">The rest of the deployment process is automatic and doesn't ask for any more user input.</span></span>

9.  <span data-ttu-id="8bd6d-492">배포 작업 시퀀스가 디바이스 구성을 완료하면 Microsoft Teams Rooms 애플리케이션 설정을 구성하도록 요청하는 다음 구성 화면이 표시됩니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-492">After the deployment task sequence finishes configuring the device, you'll see the following configuration screen that asks you to configure the Microsoft Teams Rooms application settings.</span></span>

    ![Microsoft Teams Rooms 응용 프로그램의 초기 설정 화면](../media/room-systems-scale-image2.png)

10.  <span data-ttu-id="8bd6d-494">Surface Pro를 Microsoft Teams 회의실 콘솔에 연결하고 애플리케이션 설정을 구성합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-494">Plug the Surface Pro into the Microsoft Teams Rooms console, and configure the application settings.</span></span>

11.  <span data-ttu-id="8bd6d-495">[Microsoft Teams](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2) 회의실에 나열된 기능이 배포된 장치에서 작동하고 있는지 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-495">Validate that the capabilities listed in [Microsoft Teams Rooms help](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2) are working on the deployed device.</span></span>


<span data-ttu-id="8bd6d-496">실패한 설치 문제를 해결하려면 CONFIGURATION Manager 작업 시퀀스에서 실행된 모든 단계를 기록하는 **SMSTS.log** 파일을 검사합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-496">To troubleshoot a failed installation, check the **SMSTS.log** file, which logs all the steps executed in a Configuration Manager task sequence.</span></span>

<span data-ttu-id="8bd6d-497">SMSTS.log 파일은 빌드 프로세스의 단계에 따라 여러 경로 중 하나에 저장됩니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-497">The SMSTS.log file is stored on one of a number of paths, depending on the stage of the build process.</span></span> <span data-ttu-id="8bd6d-498">SMSTS.log 파일의 경로를 식별하려면 다음 표를 참조합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-498">Check the following table to identify the path to the SMSTS.log file.</span></span>


| <span data-ttu-id="8bd6d-499">**배포 단계**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-499">**Deployment phase**</span></span>                                                            | <span data-ttu-id="8bd6d-500">**작업 시퀀스 로그 경로**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-500">**Task sequence log path**</span></span>                         |
|---------------------------------------------------------------------------------|----------------------------------------------------|
| <span data-ttu-id="8bd6d-501">WinPE, HDD 형식 이전</span><span class="sxs-lookup"><span data-stu-id="8bd6d-501">WinPE, before HDD format</span></span>                                                        | <span data-ttu-id="8bd6d-502">X: \\ Windows \\ Temp \\ smstslog \\ smsts.log</span><span class="sxs-lookup"><span data-stu-id="8bd6d-502">X:\\Windows\\Temp\\smstslog\\smsts.log</span></span>             |
| <span data-ttu-id="8bd6d-503">WinPE, HDD 형식 이후</span><span class="sxs-lookup"><span data-stu-id="8bd6d-503">WinPE, after HDD format</span></span>                                                         | <span data-ttu-id="8bd6d-504">C: \\ _SMSTaskSequence \\ Logs \\ Smstslog \\ smsts.log</span><span class="sxs-lookup"><span data-stu-id="8bd6d-504">C:\\_SMSTaskSequence\\Logs\\Smstslog\\smsts.log</span></span>    |
| <span data-ttu-id="8bd6d-505">Configuration Manager 에이전트가 설치되기 전에 배포된 운영 체제</span><span class="sxs-lookup"><span data-stu-id="8bd6d-505">Operating system deployed, before the Configuration Manager agent was installed</span></span> | <span data-ttu-id="8bd6d-506">c: \\ _SMSTaskSequence \\ Logs \\ Smstslog \\ smsts.log</span><span class="sxs-lookup"><span data-stu-id="8bd6d-506">c:\\_SMSTaskSequence\\Logs\\Smstslog\\smsts.log</span></span>    |
| <span data-ttu-id="8bd6d-507">운영 체제 및 배포된 Configuration Manager 에이전트</span><span class="sxs-lookup"><span data-stu-id="8bd6d-507">Operating system and the Configuration Manager agent deployed</span></span>                   | <span data-ttu-id="8bd6d-508">%windir% \\ System32 \\ ccm \\ logs \\ Smstslog \\ smsts.log</span><span class="sxs-lookup"><span data-stu-id="8bd6d-508">%windir%\\System32\\ccm\\logs\\Smstslog\\smsts.log</span></span> |
| <span data-ttu-id="8bd6d-509">작업 시퀀스 실행 완료</span><span class="sxs-lookup"><span data-stu-id="8bd6d-509">Task sequence execution complete</span></span>                                                | <span data-ttu-id="8bd6d-510">%windir% \\ System32 \\ ccm \\ logs \\ smsts.log</span><span class="sxs-lookup"><span data-stu-id="8bd6d-510">%windir%\\System32\\ccm\\logs\\smsts.log</span></span>           |

> [!TIP]
> <span data-ttu-id="8bd6d-511">작업 시퀀스 중에 **F8을** 선택하여 명령 콘솔을 연 다음 SMSTS.log 파일에 액세스할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-511">You can select **F8** at any time during the task sequence to open a command console, and then get access to the SMSTS.log file.</span></span>

<span data-ttu-id="8bd6d-512">PXE 부팅 문제를 해결하려면 PXE 작업과 관련이 있는 Configuration Manager 서버에서 다음 두 로그 파일을 검사합니다.</span><span class="sxs-lookup"><span data-stu-id="8bd6d-512">To troubleshoot PXE boot issues, check the two log files on the Configuration Manager server that are specific to PXE actions:</span></span>

-   <span data-ttu-id="8bd6d-513">구성 관리자 설치 로그 디렉터리에 있는 **Pxecontrol.log**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-513">**Pxecontrol.log**, located in the Configuration Manager installation logs directory</span></span>

-   <span data-ttu-id="8bd6d-514">MP(Configuration Manager 관리 지점) 로그 디렉터리에 있는 **Smspxe.log**</span><span class="sxs-lookup"><span data-stu-id="8bd6d-514">**Smspxe.log**, located in Configuration Manager Management Point (MP) logs directory</span></span>

<span data-ttu-id="8bd6d-515">구성 관리자 설치 문제를 추가로 해결하는 데 사용할 수 있는 로그 파일의 전체 목록은 Microsoft Endpoint Configuration Manager [로그 파일 참조를 참조하세요.](https://docs.microsoft.com/configmgr/core/plan-design/hierarchy/log-files)</span><span class="sxs-lookup"><span data-stu-id="8bd6d-515">For a complete list of the log files that you can use to further troubleshoot your Configuration Manager installation, see the Microsoft Endpoint Configuration Manager [Log file reference](https://docs.microsoft.com/configmgr/core/plan-design/hierarchy/log-files).</span></span>
