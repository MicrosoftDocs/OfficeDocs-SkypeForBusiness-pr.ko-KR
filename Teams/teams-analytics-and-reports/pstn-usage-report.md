---
title: PSTN 사용 현황 보고서 Microsoft Teams
author: SerdarSoysal
ms.author: serdars
manager: serdars
audience: Admin
...
ms.locfileid: "65171724"
---
# <a name="microsoft-teams-pstn-usage-report"></a>PSTN 사용 현황 보고서 Microsoft Teams
...
1. Microsoft Teams 관리 센터의 왼쪽 탐색 영역에서 **Analytics &** **reportsUsage** >  보고서를 클릭합니다. **보고서 보기** 탭의 **보고서** 아래에서 **PSTN 사용 현황 보고서를** 선택합니다.
...
들어오는 호출의 경우 호출자 번호가 난독화되고 발신 호출의 경우 호출 수신자 번호가 난독화됩니다. 이는 테넌트 관리 센터의 PSTN 및 직접 라우팅 보고서, 데이터 내보내기 및 Microsoft Graph 통해 사용할 수 있는 통화 로그에 적용됩니다.
...
1.    Teams 클라이언트에서 Teams User 1이라는 PSTN 엔드포인트의 PSTN 사용자 1, 호출 형식 Dr_In, 상관 관계 ID 57f28917-42k5-4c0c-9433-79734873f2ac, 공유 상관 관계 ID 없음
2.    PSTN 엔드포인트에서 PSTN 사용자 1이라는 Teams 클라이언트에서 사용자 1을 Teams 2c12b8ca-62eb-4c48-b68d-e451f518ff4와 Dr_Out 호출 형식으로, 공유 상관 관계 ID가 없습니다.
3.    Teams 클라이언트에서 Teams User 2라는 PSTN 엔드포인트의 PSTN 사용자 1 호출 형식 Dr_In f45e9a25-9f94-46e7-a457-84f5940efde9, 공유 상관 관계 ID f45e9a25-9f94-46e7-a457-84f5940efde9.
4.    상관 관계 ID가 "f45e9a25-9f94-46e7-a457-84f5940efde9"인 기존 호출 3. Teams User 2를 사용한 호출의 PSTN 사용자 1입니다. Teams 사용자 2가 Teams 또는 PSTN 사용자에 대한 호출을 전송(블라인드 또는 상담)한 경우 호출 형식 Dr_Out_User_Transfer 45a1da7c-9e97-481a-8a05-3fe19a9a77e0, 공유 상관 관계 ID f45e9a25-9f94-46e7-a457-84f5940efde9.

## <a name="exporting-the-reports"></a>보고서 내보내기
**내보내기를 클릭하여 Excel** 다음 **다운로드** 탭에서 **다운로드** 를 클릭하여 준비가 되면 보고서를 다운로드합니다. 내보내기 프로세스는 데이터의 수량에 따라 완료하는 데 몇 초에서 몇 분 정도 걸릴 수 있습니다.
...
> | 4 | AAD ObjectId | `uniqueidentifier` | Azure Active Directory 사용자의 ID를 호출합니다.<br/> 이 사용자 및 기타 사용자 정보는 봇 호출 형식에 대해 null/비어 있습니다(ucap_in, ucap_out). |
> | 5 | UPN | `nvarchar(128)` | Azure Active Directory UserPrincipalName(로그인 이름)입니다.<br/>이는 일반적으로 사용자의 SIP 주소와 동일하며 사용자의 전자 메일 주소와 같을 수 있습니다. |
...
> | 1 | AAD ObjectId | `uniqueidentifier` | Azure Active Directory 사용자의 ID를 호출합니다.<br/> 봇 호출 유형에 대해 이 정보 및 기타 사용자 정보는 null/비어 있을 수 있습니다. |
...
> | 5 | 초대 시간 | `datetimeoffset` | 초기 초대가 Teams 사용자 또는 봇에서 SBC로 아웃바운드로 보내거나 SBC에서 직접 라우팅의 SIP 프록시 구성 요소에서 Teams 또는 봇 호출에 대한 인바운드로 수신된 경우 |
> | 6 | 시작 시간 | `datetimeoffset` | SIP 프록시가 아웃바운드의 SBC(Teams/봇에서 PSTN 사용자에게)에서 최종 답변(SIP 메시지 "200 OK")을 받은 시간 또는 SIP 프록시가 인바운드 호출의 Teams 백 엔드 내의 다음 홉에 초대를 보낸 후(PSTN 사용자는 Teams/봇으로).<br/>실패하고 응답이 없는 호출의 경우 초대 또는 실패 시간과 같을 수 있습니다. |
...
> | 12 | 수신자 번호 | `nvarchar(32)` | 전화를 받은 사용자 또는 봇의 수입니다. 팀 사용자 호출에 대한 인바운드에서는 Teams 사용자가 되고, Teams 사용자 호출의 아웃바운드에서 PSTN 사용자가 됩니다. |
...
- [Microsoft Graph PSTN 통화 보고서](/graph/api/callrecords-callrecord-getpstncalls?view=graph-rest-1.0&tabs=http)
- [Microsoft Graph 직접 라우팅 보고서](/graph/api/callrecords-callrecord-getdirectroutingcalls?view=graph-rest-1.0&tabs=http)
