...
ms.locfileid: "60865466"
---
# <a name="coexistence-modes---reference"></a>공존 모드 - 참조

> [!Important] 
> - 2021년 7월 31일 비즈니스용 Skype Online을 사용 중지한 후, TeamsOnly 외의 공존 모드를 클라우드에 있는 사용자에게 할당할 수 없습니다. 사용 중지 이전의 경우와 비즈니스용 Skype 서버 프레미스에 있는 사용자는 TeamsOnly가  아니라 다른 모든 모드를 할당할 수 있습니다. 
...
모드는 아래에 나열되어 있습니다. 클라우드 사용자는 TeamsOnly 되어야 합니다. 그리고 홈온-프레미스 사용자가 TeamsOnly 외의 모드가 되어야 합니다. 
</br>
</br>

|모드|통화 및 채팅|모임 예정<sup>1</sup>|Teams & 채널|사용 사례|
|---|---|---|---|---|
|**TeamsOnly <sup>2</sup>**</br>*사용자에 프레미스 계정이 없는 경우만 비즈니스용 Skype 서버*|Teams|Teams|예|업그레이드할 최종 상태입니다. 하이브리드 구성이 검색되지 않는 한 새 테넌트의 기본값입니다.|
|아일랜드|둘 중 하나|둘 중 하나|예|하이브리드 조직의 기본 구성입니다. 단일 사용자가 두 클라이언트를 나란히 평가할 수 있습니다. 채팅 및 통화는 두 클라이언트 모두에 착륙할 수 있으므로 사용자는 항상 두 클라이언트를 모두 실행해야 합니다. 혼란스럽거나 회귀되는 비즈니스용 Skype 환경, 외부(페더링) 통신, PSTN 음성 서비스 및 음성 애플리케이션, Office 통합 및 기타 여러 통합이 계속 비즈니스용 Skype.|
