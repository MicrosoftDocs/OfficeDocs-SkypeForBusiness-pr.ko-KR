...
>
> **2021년 2월** 이후 Microsoft Teams 이 설정을 구성하지 않은 기존 & 기본적으로 게스트 액세스가 설정됩니다. 이 변경이 구현되면 해당 테넌트에서 게스트 액세스 Microsoft Teams 아직 구성하지 않은 경우 해당 기능이 활성화됩니다. 게스트 액세스가 조직에 대해 비활성화된 상태로 유지하려면 게스트 액세스 설정이 서비스  기본값 대신 Off로 설정되어 있는지 **확인해야 합니다.**
...
2. 사용자 **게스트 액세스**  >  **를 선택합니다.**

3. 에서 게스트 액세스 **허용을 on으로 Teams** **설정**

    ![게스트 액세스 스위치를 켜기 로 설정하도록 허용합니다.](media/guest-access-setting.png)

4. **통화,** 모임 **및** **메시징에서** 게스트를 허용할 기능에 따라 각 기능에 대해 On **또는** **Off를** 선택합니다.

      - **개인 전화 걸기** – 게스트가 피어 투 피어 전화를 걸 수 있도록 허용하려면 이를 **켬** 으로 설정합니다.
      - **IP 비디오** -  게스트가 통화 및 모임에서 비디오를 사용할 수 있도록 이 설정을 켜면 됩니다.
      ...
      - **지금 만나기** –  게스트가 지금 만나기 기능을 사용할 수 있도록 이 설정을 Microsoft Teams.
      - **보낸 메시지 편집** - 게스트가 이전에 보낸 메시지를 편집할 수 있도록 허용하려면 이를 **켬** 으로 설정합니다.
      - **보낸 메시지 삭제** -  게스트가 이전에 보낸 메시지를 삭제할 수 있도록 이 설정을 켜면 됩니다.
      - **채팅** – 게스트가 Teams에서 채팅을 사용할 수 있도록 허용하려면 이를 **켬** 으로 설정합니다.
      - **대화의 Giphy** - 게스트가 대화에서 Giphys를 사용할 수 있도록 이 설정을 켜면 됩니다.  Giphy는 사용자가 애니메이션 GIF 파일을 검색하고 공유할 수 있는 온라인 데이터베이스 및 검색 엔진입니다. 각 Giphy에는 콘텐츠 등급이 할당됩니다.
      ...
      - **대화의 mes** - 게스트가 대화에서 Memes를 사용할 수 있도록 이 설정을 켜기 
      - **대화의 스티커** - 게스트가  대화에서 스티커를 사용할 수 있도록 이 설정을 켜면 됩니다.
      - **메시지에 대한** 몰입형 판독기  - 게스트가 에서 몰입형 읽기를 사용할 수 있도록 이 설정을 [켜면](https://support.microsoft.com/topic/a700c0d0-bc53-4696-a94d-4fbc86ac7a9a)Teams.
