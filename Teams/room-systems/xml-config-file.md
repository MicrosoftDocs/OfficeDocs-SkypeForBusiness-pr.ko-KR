...
다음 표에서는이 샘플 SkypeSettings (파일 이름 필수) 구성 파일에 표시 되는 요소에 대해 설명 합니다.
...
|\<ContentCameraEnhancement>  | 부울 &#x2777; | |True (기본값)로 설정 된 경우, 화이트 보드 가장자리가 감지 되 고 적절 한 줌이 선택 되어 있으며 잉크 줄이 향상 되 고 화이트 보드에 쓰는 사용자가 투명 하 게 됩니다.  <br><br> 화이트 보드가 펜을 사용 하 여 그려지지 않고 카메라를 사용 하 여 스티커 메모, 포스터 또는 기타 미디어를 표시 하는 공간에 대 한 모임 참가자에 게 원시 비디오 피드를 보내려면이를 false로 설정 합니다.  |
...
부울 플래그 &#x2777; true, false, 0 또는 1 중 하나일 수 있습니다. 부울 또는 숫자 값을 비워 두면 XML 형식이 잘못 되어 설정이 변경 되지 않을 수 있습니다.
...
사용자 지정 테마 이미지 파일을 **C:\Users\Skype\AppData\Local\Packages\Microsoft.SkypeRoomSystem_8wekyb3d8bbwe\LocalState**에 배치 하 고 \<CustomThemeImageUrl\> 변수에 파일 이름 및 확장명을 입력 해야 합니다.
