...
2.  <span data-ttu-id="ddf37-144">Microsoft 팀 대화방 배포를 서비스할 배포 지점의 서버를 선택한 다음 **속성**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-144">Select the distribution point server that will serve the Microsoft Teams Rooms deployment, and then select **Properties**.</span></span>
...
### <a name="configure-a-boot-image"></a><span data-ttu-id="ddf37-159">부팅 이미지 구성</span><span class="sxs-lookup"><span data-stu-id="ddf37-159">Configure a boot image</span></span>
...
| <span data-ttu-id="ddf37-197">SRS v2-OS 업데이트 패키지</span><span class="sxs-lookup"><span data-stu-id="ddf37-197">SRS v2 - OS Updates Package</span></span>          | <span data-ttu-id="ddf37-198">소프트웨어 패키지</span><span class="sxs-lookup"><span data-stu-id="ddf37-198">Software package</span></span>       | <span data-ttu-id="ddf37-199">필수 운영 체제 업데이트를 배포 하기 위한 패키지</span><span class="sxs-lookup"><span data-stu-id="ddf37-199">Package to deploy mandatory operating system updates</span></span>                                      |
...
-   <span data-ttu-id="ddf37-226">SRS v2-SRS 응용 프로그램 패키지</span><span class="sxs-lookup"><span data-stu-id="ddf37-226">SRS v2 - SRS Application Package</span></span>
...
4. <span data-ttu-id="ddf37-239">패키지를 만들려면 다음 정보를 입력 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-239">Enter the following information to create the package:</span></span>
   ...
   - <span data-ttu-id="ddf37-241">제조 업체<strong>: Microsoft Corporation</strong></span><span class="sxs-lookup"><span data-stu-id="ddf37-241">Manufacturer<strong>: Microsoft Corporation</strong></span></span>
...
1.  <span data-ttu-id="ddf37-268">루트 인증서를 **SRS v2-루트 인증서 패키지** 폴더에 복사 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-268">Copy the root certificate into the **SRS v2 – Root Certificate Package** folder.</span></span>
...
5.  <span data-ttu-id="ddf37-287">**프로그램 만들지 않음을**선택 하 고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-287">Select **Do not create a program**, and then select **Next**.</span></span>
...
6. <span data-ttu-id="ddf37-314">**설정 확인** 페이지를 검토 하 고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-314">Review the **Confirm the settings** page, and then select **Next**.</span></span>
...
10. <span data-ttu-id="ddf37-341">**닫기를**선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-341">Select **Close**.</span></span>
...
5.  <span data-ttu-id="ddf37-355">**닫기를**선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-355">Select **Close**.</span></span>
...
2.  <span data-ttu-id="ddf37-365">드라이버 패키지를 배포 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-365">Distribute driver packages.</span></span>
    ...
    4.  <span data-ttu-id="ddf37-370">**다음**을 선택 하 고 **닫기를**선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-370">Select **Next**, and then select **Close**.</span></span>
...
> [!NOTE]
> <span data-ttu-id="ddf37-377">패키지 배포에는 패키지 크기, 구성 관리자 계층 구조, 배포 지점의 서버 수, 네트워크에서 사용할 수 있는 대역폭에 따라 약간의 시간이 걸릴 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-377">Package distribution might take some time, depending on the package size, Configuration Manager hierarchy, number of distribution point servers, and the bandwidth available in your network.</span></span>
> 
> <span data-ttu-id="ddf37-378">Microsoft 팀 회의실 단위 배포를 시작 하려면 먼저 모든 패키지를 배포 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-378">All the packages must be distributed before you can start deploying a Microsoft Teams Rooms unit.</span></span>
> 
> <span data-ttu-id="ddf37-379">\*\*\*\* \> **배포** 상태 \> **콘텐츠 상태**모니터링으로 진행 하 여 Configuration Manager 콘솔에서 패키지 배포의 상태를 검토할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-379">You can review the status of your package distribution in the Configuration Manager console by going to **Monitoring** \> **Distribution Status** \> **Content Status**.</span></span>

## <a name="configuration-manager-task-sequences"></a><span data-ttu-id="ddf37-380">Configuration Manager 작업 순서</span><span class="sxs-lookup"><span data-stu-id="ddf37-380">Configuration Manager task sequences</span></span>

<span data-ttu-id="ddf37-381">System Center Configuration Manager에서 작업 순서를 사용 하 여 운영 체제 이미지를 대상 컴퓨터에 배포 하는 단계를 자동화 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-381">You use task sequences with System Center Configuration Manager to automate the steps for deploying an operating system image to a destination computer.</span></span> <span data-ttu-id="ddf37-382">Microsoft 팀 회의실 단위를 자동화 된 방식으로 배포 하려면 대상 Microsoft 팀 공간 컴퓨터를 시작 하는 데 사용 되는 부팅 이미지, 설치할 Windows 10 Enterprise 운영 체제 이미지, 모든 작업 순서를 만듭니다. 다른 응용 프로그램 또는 소프트웨어 업데이트와 같은 기타 추가 콘텐츠</span><span class="sxs-lookup"><span data-stu-id="ddf37-382">To deploy a Microsoft Teams Rooms unit in an automated fashion, you create a task sequence that references the boot image used to start the destination Microsoft Teams Rooms computer, the Windows 10 Enterprise operating system image that you want to install, and any other additional content, such as other applications or software updates.</span></span>

### <a name="import-the-sample-task-sequence"></a><span data-ttu-id="ddf37-383">샘플 작업 순서 가져오기</span><span class="sxs-lookup"><span data-stu-id="ddf37-383">Import the sample task sequence</span></span>

<span data-ttu-id="ddf37-384">샘플 작업 순서를 다운로드 하 여 쉽게 가져와 필요에 맞게 사용자 지정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-384">You can download and easily import a sample task sequence and customize it to meet your needs.</span></span>

1.  <span data-ttu-id="ddf37-385">예제 작업 순서를 [**다운로드**](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Skype/SfbOnline/downloads/Skype-Room-Systems-v2/SRS-v2-Configuration-Manager-Files.zip?raw=true) 하 고 다운로드 한 zip 파일을 공유 위치에 복사 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-385">[**Download**](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Skype/SfbOnline/downloads/Skype-Room-Systems-v2/SRS-v2-Configuration-Manager-Files.zip?raw=true) the sample task sequence, and copy the downloaded zip file to a shared location.</span></span>
2.  <span data-ttu-id="ddf37-386">Configuration Manager 콘솔에서 **소프트웨어 라이브러리** \> **운영 체제** \> **작업 시퀀스로**이동한 다음 **작업 순서 가져오기를**선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-386">In the Configuration Manager console, go to **Software Library** \> **Operating Systems** \> **Task Sequences**, and then select **Import Task Sequence**.</span></span>

3.  <span data-ttu-id="ddf37-387">**찾아보기를**선택 하 고 1 단계에서 사용한 공유 폴더 위치로 이동한 다음 **Microsoft 팀 대화방 배포 (en-us) .zip** 파일을 선택 하 고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-387">Select **Browse**, go to the shared folder location you used in step 1, select the **Microsoft Teams Rooms Deployment (EN-US).zip** file, and then select **Next**.</span></span>

4.  <span data-ttu-id="ddf37-388">**새로 만들기** **작업** 을 설정 하 고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-388">Set **Action** to **Create New**, and then select **Next**.</span></span>

5.  <span data-ttu-id="ddf37-389">설정을 확인 하 고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-389">Confirm the settings, and then select **Next**.</span></span>

6.  <span data-ttu-id="ddf37-390">**닫기를**선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-390">Select **Close**.</span></span>

### <a name="validate-that-the-reference-packages-are-correctly-linked-to-each-task-sequence-step"></a><span data-ttu-id="ddf37-391">참조 패키지가 각 작업 순서 단계에 제대로 연결 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-391">Validate that the reference packages are correctly linked to each task sequence step.</span></span>

1. <span data-ttu-id="ddf37-392">가져온 작업 순서를 선택 하 고 **편집**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-392">Select the imported task sequence, and then select **Edit**.</span></span>

    <span data-ttu-id="ddf37-393">작업 순서 편집기가 열리고 Microsoft 팀 대화방 단위를 배포 하 고 구성 하는 데 필요한 각 순차적 단계가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-393">The Task Sequence Editor opens and displays each sequential step that you need to deploy and configure a Microsoft Teams Rooms unit.</span></span>

2. <span data-ttu-id="ddf37-394">각 단계를 살펴보고 권장 업데이트를 완료 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-394">Walk through each step and complete the recommended updates:</span></span>

   1. <span data-ttu-id="ddf37-395">**WINDOWS PE에서 다시 시작**:이 단계는 컴퓨터를 다시 시작 하 고 Windows PXE로 부팅 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-395">**Restart in Windows PE**: This step restarts and then boots the computer into Windows PXE.</span></span> <span data-ttu-id="ddf37-396">이 단계에서는 변경할 필요가 없습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-396">No changes are required for this step.</span></span>

   2. <span data-ttu-id="ddf37-397">**파티션 디스크 0 – UEFI**:이 단계에서는 디스크 구성을 닦아내기 하 고 구성 된 설정을 기반으로 파티션을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-397">**Partition Disk 0 – UEFI**: This step wipes the disk configuration and creates partitions based on the configured settings.</span></span> <span data-ttu-id="ddf37-398">이 단계는 변경 하지 않는 것이 좋습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-398">We recommend that you not make any changes to this step.</span></span>

   3. <span data-ttu-id="ddf37-399">**SRS 컴퓨터 이름 설정**:이 단계에는 배포 중 Microsoft 팀 공간 단위에 대 한 컴퓨터 이름을 설정 하는 UI를 제공 하는 HTML 응용 프로그램이 포함 됩니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-399">**Set SRS Computer Name**: This step includes an HTML application to provide a UI to set a computer name for the Microsoft Teams Rooms unit during the deployment.</span></span>
      -  <span data-ttu-id="ddf37-400">이 단계는 선택 사항 이지만 대체 프로세스를 통해 컴퓨터 명명을 관리 하려는 경우에만 사용 하지 않도록 설정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-400">This is an optional step, but it can only be disabled if you want to manage computer naming through an alternate process.</span></span>
      -  <span data-ttu-id="ddf37-401">**SRS V2 Set-SRSComputerName** 패키지를 선택 했는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-401">Verify that the **SRS v2 - Set-SRSComputerName** package is selected.</span></span> <span data-ttu-id="ddf37-402">그렇지 않으면 패키지를 찾아 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-402">If it isn’t, browse to the package and select it.</span></span>

   4. <span data-ttu-id="ddf37-403">**운영 체제 적용**:이 단계에서는 배포할 운영 체제 이미지와 무인 Sysprep 응답 파일을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-403">**Apply Operating System**: This step specifies the operating system image to be deployed and the unattended Sysprep answer file to use.</span></span>
      -  <span data-ttu-id="ddf37-404">올바른 Windows 10 Enterprise 운영 체제 이미지 파일이 선택 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-404">Verify that the correct Windows 10 Enterprise operating system image file is selected.</span></span>
      -  <span data-ttu-id="ddf37-405">**사용자 지정 설치에 무인 또는 sysprep 응답 파일을 사용** 하도록 설정 되어 있는지 확인 하 고 **SRS V2-Sysprep 패키지** 를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-405">Verify that **Use an unattended or Sysprep answer file for a custom installation** is enabled, and the **SRS v2 - Sysprep Package** is selected.</span></span> <span data-ttu-id="ddf37-406">또한 **파일 이름이** **unattend.xml**으로 설정 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-406">Also ensure that **File Name** is set to **unattend.xml**.</span></span>

   5. <span data-ttu-id="ddf37-407">**Windows 설정 적용**:이 단계에서는 windows 설치에 대 한 정보를 수집 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-407">**Apply Windows Settings**: This step gathers information about the Windows installation.</span></span>
      -  <span data-ttu-id="ddf37-408">제품 키, 로컬 관리자 계정 암호, 표준 시간대 (필요에 따라 다름)를 포함 하 여 라이선스 및 등록 정보를 제공 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-408">Provide licensing and registration information including the product key, local administrator account password, and time zone (depending on your needs).</span></span>

   6. <span data-ttu-id="ddf37-409">**네트워크 설정 적용**:이 단계에서는 작업 그룹 또는 Active Directory 도메인 이름 및 조직 구성 단위를 지정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-409">**Apply Network Settings**: This step allows you to specify a workgroup or Active Directory domain name and organizational unit.</span></span>
      > [!NOTE]
      > <span data-ttu-id="ddf37-410">Microsoft 팀 대화방 단위를 Actve 디렉터리 도메인의 구성원으로 배포 하는 데 필요한 권장 작업에 대 한 [Skype 대화방 시스템 도메인 참가 고려 사항을](domain-joining-considerations.md) 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="ddf37-410">See [Skype Room System domain joining considerations](domain-joining-considerations.md) for recommended actions you need to take in deploying Microsoft Teams Rooms units as members of an Actve Directory domain.</span></span>
   7. <span data-ttu-id="ddf37-411">**드라이버 적용:** 이 단계 및 해당 하위 단계는 보유 하 고 있는 Surface Pro 모델을 기반으로 적용 가능한 디바이스 드라이버와 펌웨어를 배포 하는 데 사용 됩니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-411">**Apply Drivers:** This step and its sub-steps are used to deploy applicable device drivers and firmware based on the Surface Pro model you have.</span></span> <span data-ttu-id="ddf37-412">각 단계를 업데이트 하 여이 배포와 연결 된 관련 드라이버 패키지를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-412">Update each step to specify the relevant driver package associated with this deployment.</span></span>
      -   <span data-ttu-id="ddf37-413">각 드라이버 패키지는 Surface Pro 만들기 및 모델을 기반으로 관련 드라이버와 펌웨어를 배포 하기 위해 WMI (Windows Management Instrumentation) 필터를 활용 하도록 구성 됩니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-413">Each driver package is configured to leverage Windows Management Instrumentation (WMI) filters to deploy relevant drivers and firmware based on the Surface Pro make and model.</span></span>
      -   <span data-ttu-id="ddf37-414">이러한 드라이버의 구성은 변경 하지 않는 것이 좋으며, 그렇지 않은 경우에는 배포가 실패할 것을 적극 권장 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-414">We highly recommend that you not alter the configuration of these drivers, otherwise deployment might fail.</span></span>

   8. <span data-ttu-id="ddf37-415">**Windows 및 구성 관리자 설정**:이 단계에서는 Configuration manager 클라이언트를 배포 하 고 구성 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-415">**Set up Windows and Configuration Manager**: This step deploys and configures the Configuration Manager client.</span></span> <span data-ttu-id="ddf37-416">이 단계를 업데이트 하 여 기본 제공 Configuration Manager 클라이언트 패키지를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-416">Update this step to specify the built-in Configuration Manager Client Package.</span></span>

   9. <span data-ttu-id="ddf37-417">**루트 인증서 설치**:이 단계는 도메인에 연결 되지 않은 디바이스에 대 한 루트 인증서를 배포 하므로 기본적으로 선택 및 비활성화 되어 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-417">**Install Root Certificate**: This step distributes the root certificate for non–domain-joined devices, and therefore is optional and disabled by default.</span></span>
      -   <span data-ttu-id="ddf37-418">Microsoft 팀 회의실 단위에 루트 인증서를 배포 해야 하는 경우이 단계를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-418">Enable this step if you need to deploy a root certificate to the Microsoft Teams Rooms units.</span></span>
      -   <span data-ttu-id="ddf37-419">이 단계를 수행 해야 하는 경우 **SRS v2-루트 인증서 패키지** 및 **사용 안 함 64 비트 파일 시스템 리디렉션이** 선택 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-419">If you do need to perform this step, verify that the **SRS v2 – Root Certificate Package** and **Disable 64-bit file system redirection** are selected.</span></span>

   10. <span data-ttu-id="ddf37-420">**모니터링 에이전트 설치 및 구성**:이 단계에서는 64 비트 버전의 Microsoft Azure 모니터 에이전트를 설치 하 고 에이전트를 로그 분석 작업 영역에 연결 하도록 구성 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-420">**Install and Configure Monitoring Agent**: This step installs the 64-bit version of the Microsoft Azure Monitor agent and configures the agent to connect to your Log Analytics workspace.</span></span>
       -   <span data-ttu-id="ddf37-421">이 단계는 기본적으로 비활성화 되어 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-421">This step is disabled by default.</span></span> <span data-ttu-id="ddf37-422">모니터링 에이전트를 사용 하 여 Microsoft 팀 회의실의 상태를 모니터링 하는 경우에만이 단계를 사용 하도록 설정 하세요.</span><span class="sxs-lookup"><span data-stu-id="ddf37-422">Enable this step only if you’re going to use the Monitoring Agent to monitor the health of your Microsoft Teams Rooms units.</span></span>
       -   <span data-ttu-id="ddf37-423">이 단계를 편집 하 고 명령줄 매개 변수를 업데이트 하 여 **작업 영역 ID** 와 **작업 영역 키**를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-423">Edit this step and update the command-line parameters to specify your **Workspace ID** and **Workspace Key**.</span></span>
       -   <span data-ttu-id="ddf37-424">Operations Management Suite 작업 영역 ID 및 기본 키를 가져오는 방법에 대 한 자세한 내용은 [Azure 모니터링에 대 한 테스트 장치 구성을](azure-monitor-deploy.md#configure-test-devices-for-azure-monitoring) 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="ddf37-424">See [Configure test devices for Azure Monitoring](azure-monitor-deploy.md#configure-test-devices-for-azure-monitoring) for more information about obtaining the Operations Management Suite Workspace ID and the primary key.</span></span>
       -   <span data-ttu-id="ddf37-425">**SRS v2-Microsoft Monitoring Agent 패키지** 및 **사용 안 함 64 비트 파일 시스템 리디렉션이** 선택 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-425">Verify that the **SRS v2 – Microsoft Monitoring Agent Package** and **Disable 64-bit file system redirection** are selected.</span></span>
       -   <span data-ttu-id="ddf37-426">Microsoft 팀 대화방 배포의 상태를 모니터링 하는 방법에 대 한 자세한 내용은 [Azure 모니터로 Microsoft 팀 대화방 관리 계획](azure-monitor-plan.md), [Azure 모니터로 microsoft 팀 대화방 관리 배포](azure-monitor-deploy.md) 및 [microsoft 관리를 참조 하세요. 팀 대화방 장치/Azure 모니터](azure-monitor-manage.md).</span><span class="sxs-lookup"><span data-stu-id="ddf37-426">For more information about monitoring the health of your Microsoft Teams Rooms deployment, see [Plan Microsoft Teams Rooms management with Azure Monitor](azure-monitor-plan.md), [Deploy Microsoft Teams Rooms management with Azure Monitor](azure-monitor-deploy.md) and [Manage Microsoft Teams Rooms devices with Azure Monitor](azure-monitor-manage.md).</span></span>

   11. <span data-ttu-id="ddf37-427">**SRS V2 구성 파일 복사**:이 단계에서는 Microsoft 팀 대화방 배포 키트의 필수 설정 파일과 구성 파일을 로컬 하드 드라이브에 복사 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-427">**Copy SRS v2 Configuration Files**: This step copies the required setup and configuration files from the Microsoft Teams Rooms deployment kit to the local hard drive.</span></span> <span data-ttu-id="ddf37-428">이 단계에서는 사용자 지정이 필요 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-428">No customization is required for this step.</span></span>
       -   <span data-ttu-id="ddf37-429">**Srs v2-Srs 응용 프로그램 패키지** 및 **사용 안 함 64 비트 파일 시스템 리디렉션이** 선택 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-429">Verify that the **SRS v2 – SRS Application Package** and **Disable 64-bit file system redirection** are selected.</span></span>

   12. <span data-ttu-id="ddf37-430">**설치-SRSv2-OS-업데이트**:이 단계는 Microsoft 팀 대화방 배포에 필요한 필수 운영 체제 업데이트를 배포 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-430">**Install-SRSv2-OS-Updates**: This step deploys any mandatory operating system updates required with the Microsoft Teams Rooms deployment.</span></span> <span data-ttu-id="ddf37-431">이렇게 하려면 다음을 수행합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-431">Do the following:</span></span>
       -   <span data-ttu-id="ddf37-432">[Microsoft 팀 대화방 콘솔 구성을](console.md) 선택 하 여 필요한 업데이트를 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-432">Check [Configure a Microsoft Teams Rooms console](console.md) to see which updates are required.</span></span>
       -   <span data-ttu-id="ddf37-433">**SRS v2-OS 업데이트 패키지** 에 필수 업데이트가 모두 포함 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-433">Verify that your **SRS v2 – OS Updates Package** includes all the required updates.</span></span>
       -   <span data-ttu-id="ddf37-434">**SRS v2-OS 업데이트 패키지가** 선택 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-434">Verify that the **SRS v2 – OS Updates Package** is selected.</span></span>
       -   <span data-ttu-id="ddf37-435">PowerShell 실행 정책이 **Bypass**으로 설정 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-435">Verify that the PowerShell execution policy is set to **Bypass**.</span></span>

   13. <span data-ttu-id="ddf37-436">**컴퓨터 다시 시작**:이 단계에서는 필수 운영 체제 업데이트가 설치 된 후 컴퓨터를 재부팅 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-436">**Restart Computer**: This step reboots the computer after the mandatory operating system updates are installed.</span></span> <span data-ttu-id="ddf37-437">이 단계에서는 사용자 지정이 필요 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-437">No customization is required for this step.</span></span>

   14. <span data-ttu-id="ddf37-438">**Windows 구성 요소 구성**:이 단계에서는 필요한 Windows 기능을 구성 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-438">**Configure Windows Components**: This step configures the required Windows features.</span></span> <span data-ttu-id="ddf37-439">이 단계에서는 사용자 지정이 필요 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-439">No customization is required for this step.</span></span>

   15. <span data-ttu-id="ddf37-440">**컴퓨터 다시 시작**:이 단계는 Windows 기능을 구성한 후 컴퓨터를 재부팅 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-440">**Restart Computer**: This step reboots the computer after the Windows features are configured.</span></span> <span data-ttu-id="ddf37-441">이 단계에서는 사용자 지정이 필요 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-441">No customization is required for this step.</span></span>

   16. <span data-ttu-id="ddf37-442">**로컬 Skype 사용자 추가**:이 단계는 Windows에 자동으로 로그인 하 여 Microsoft 팀 회의실 응용 프로그램을 시작 하는 데 사용 되는 로컬 Skype 계정을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-442">**Add Local Skype User**: This step creates the local Skype account used to automatically sign in to Windows and start the Microsoft Teams Rooms application.</span></span> <span data-ttu-id="ddf37-443">이 단계에는 연결 된 소프트웨어 패키지가 없으며 사용자 지정이 필요 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-443">This step doesn’t have any software package associated with it, and no customization is required for it.</span></span>

   17. <span data-ttu-id="ddf37-444">**SRS 응용 프로그램 설정 및 구성**:이 단계에서는 다음 운영 체제 부팅을 위해 Microsoft 팀 대화방 응용 프로그램 설치를 구성 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-444">**Set up and configure SRS application**: This step configures the Microsoft Teams Rooms application installation for the next boot of the operating system.</span></span>
       -   <span data-ttu-id="ddf37-445">**Srs v2-Srs 설치 패키지 구성** 및 **64 비트 파일 시스템 리디렉션 사용 안 함** 이 선택 되어 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-445">Verify that the **SRS v2 – Configure SRS Setup Package** and **Disable 64-bit file system redirection** are selected.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="ddf37-446">작업 순서 단계는 제공 되는 순서 대로 진행 되어야 한다는 것이 중요 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-446">It is very important that the task sequence steps must be in the provided order.</span></span> <span data-ttu-id="ddf37-447">단계의 순서를 수정 하거나 추가 단계를 구성 하면 배포가 중단 될 수가 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-447">Modifying the order of steps, or configuring additional steps might break the deployment.</span></span>
>
> <span data-ttu-id="ddf37-448">**SRS 응용 프로그램 설정 및 구성** 단계는 작업 순서의 마지막 단계 여야 하며 그렇지 않으면 배포가 실패할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-448">**Set up and configure SRS application** step must be the last step in the task sequence, otherwise the deployment might fail.</span></span>

### <a name="create-deployment-for-the-task-sequence"></a><span data-ttu-id="ddf37-449">작업 순서에 대 한 배포 만들기</span><span class="sxs-lookup"><span data-stu-id="ddf37-449">Create deployment for the task sequence</span></span>

1. <span data-ttu-id="ddf37-450">작업 순서를 선택 하 고 **배포**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-450">Select the task sequence, and then select **Deploy**.</span></span>

2. <span data-ttu-id="ddf37-451">**찾아보기를** 선택 하 여 배포용 대상 모음을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-451">Select **Browse** to select target collection for deployment.</span></span>

3. <span data-ttu-id="ddf37-452">**알 수 없는 모든 컴퓨터** 를 선택 하 고 **확인**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-452">Select **All Unknown Computers** and then select **OK**.</span></span>

4. <span data-ttu-id="ddf37-453">**다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-453">Select **Next**.</span></span>

5. <span data-ttu-id="ddf37-454">**용도** 드롭다운 목록에서 **사용 가능** 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-454">Select **Available** on the **Purpose** drop down list.</span></span>

6. <span data-ttu-id="ddf37-455">**다음을 사용 하 여 설정** 목록에서 **미디어 및 PXE만** 을 선택 하 고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-455">Select **Only Media and PXE** in the **Make available to the following** list, and then select **Next**.</span></span>
   > [!WARNING]
   > <span data-ttu-id="ddf37-456">**용도가** **사용 가능**으로 설정 되어 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-456">It is very important that **Purpose** is set to **Available**.</span></span> <span data-ttu-id="ddf37-457">**용도가** **필수**로 설정 **되지** 않았는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-457">Make sure that the **Purpose** is **NOT** set to **Required**.</span></span> <span data-ttu-id="ddf37-458">또한 **다음에 사용할 수 있도록 설정**에서 **미디어 및 PXE만** 선택 하도록 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-458">Also make sure that you select **Only Media and PXE** in the **Make available to the following**.</span></span>
   >
   > <span data-ttu-id="ddf37-459">이러한 값을 다른 항목으로 설정 하면 모든 컴퓨터에서 부팅할 때 Microsoft 팀 공간 배포 이미지가 표시 될 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-459">Setting these values to something else might cause all computers to get the Microsoft Teams Rooms deployment image when booted.</span></span>
7. <span data-ttu-id="ddf37-460">일정을 지정 하지 않고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-460">Do not specify any schedule and select **Next**.</span></span>

8. <span data-ttu-id="ddf37-461">**사용자 환경** 섹션 내에서 아무것도 변경 하지 말고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-461">Do not change anything within the **User Experience** section and select **Next**.</span></span>

9. <span data-ttu-id="ddf37-462">**알림** 섹션 내에서 아무것도 변경 하지 말고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-462">Do not change anything within the **Alerts** section and select **Next**.</span></span>

10. <span data-ttu-id="ddf37-463">**배포 지점** 섹션 내에서 아무것도 변경 하지 말고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-463">Do not change anything within the **Distribution Points** section and select **Next**.</span></span>

11. <span data-ttu-id="ddf37-464">설정을 확인 하 고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-464">Confirm the settings and then select **Next**.</span></span>

12. <span data-ttu-id="ddf37-465">**닫기를**선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-465">Select **Close**.</span></span>

<a name="validate-and-troubleshoot-the-solution"></a><span data-ttu-id="ddf37-466">해결 방법 유효성 검사 및 문제 해결</span><span class="sxs-lookup"><span data-stu-id="ddf37-466">Validate and troubleshoot the solution</span></span>
--------------------------------------

<span data-ttu-id="ddf37-467">System Center Configuration Manager 작업 순서를 완료 한 후에는 작업 순서가 Microsoft 팀 대화방 단위를 배포 하 고 구성할 수 있는지 확인 하기 위해 테스트 실행을 수행 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-467">After you’ve completed the System Center Configuration Manager task sequences, you’ll need to perform a test run to validate that the task sequence can deploy and configure Microsoft Teams Rooms units.</span></span>

1.  <span data-ttu-id="ddf37-468">지원 되는 이더넷 어댑터 중 하나를 사용 하거나 Surface dock를 사용 하 여 테스트 장치를 유선 네트워크에 연결 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-468">Connect the test device to the wired network by using one of the supported Ethernet adapters or using the Surface dock.</span></span> <span data-ttu-id="ddf37-469">PXE 부팅 기능이 환경에 맞게 구성 되지 [않은 경우 이전에 만든](https://docs.microsoft.com/sccm/osd/deploy-use/create-bootable-media) usb 플래시 드라이브의 부팅 이미지를 사용 하 여 usb에서 부팅 하 고 구성 관리자에 연결할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-469">If PXE boot functionality hasn’t been configured for your environment, you can use the boot image on the USB flash drive [that you created earlier](https://docs.microsoft.com/sccm/osd/deploy-use/create-bootable-media) to boot from USB and connect to Configuration Manager.</span></span>

2.  <span data-ttu-id="ddf37-470">펌웨어에 액세스 하 고 PXE 부팅 시작:</span><span class="sxs-lookup"><span data-stu-id="ddf37-470">Access the firmware and initiate PXE boot:</span></span>

    1.  <span data-ttu-id="ddf37-471">Surface 디바이스의 전원이 꺼져 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-471">Ensure the Surface device is powered off.</span></span>

    2.  <span data-ttu-id="ddf37-472">**볼륨 크게** 단추를 길게 누릅니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-472">Press and hold the **Volume Up** button.</span></span>

    3.  <span data-ttu-id="ddf37-473">**전원** 단추를 눌렀다가 놓습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-473">Press and release the **Power** button.</span></span>

    4.  <span data-ttu-id="ddf37-474">장치가 부팅 되기 시작한 후 **볼륨을 위로** 단추를 놓습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-474">After the device begins to boot, release the **Volume Up** button.</span></span>

    5.  <span data-ttu-id="ddf37-475">**부팅 구성을**선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-475">Select **Boot configuration**.</span></span>

    6.  <span data-ttu-id="ddf37-476">다음 중 하나를 수행 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-476">Do one of the following:</span></span>

        -   <span data-ttu-id="ddf37-477">**PXE 부팅**을 선택 하 고 목록 맨 위로 끕니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-477">Select **PXE boot**, and drag it to the top of the list.</span></span> <span data-ttu-id="ddf37-478">또는 네트워크 어댑터에서 왼쪽으로 살짝 밀어 장치에 바로 부팅할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-478">Alternatively, you can swipe left on the network adapter to boot to the device immediately.</span></span> <span data-ttu-id="ddf37-479">이는 부팅 순서에 영향을 주지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-479">This won’t affect the boot order.</span></span>
        -   <span data-ttu-id="ddf37-480">부팅 미디어를 보유 하는 USB 플래시 드라이브를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-480">Select the USB flash drive that holds the boot media.</span></span>

3.  <span data-ttu-id="ddf37-481">**끝내기**를 선택 하 고 **지금 다시 시작**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-481">Select **Exit**, and then select **Restart Now**.</span></span>

4.  <span data-ttu-id="ddf37-482">메시지가 표시 되 면 네트워크 부팅 서비스에 대해 **Enter** 를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-482">When prompted, select **Enter** for network boot service.</span></span>

5.  <span data-ttu-id="ddf37-483">Windows PE가 메모리에 로드 되 고 작업 순서 마법사가 시작 됩니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-483">Windows PE will load into memory, and the Task Sequence Wizard will start.</span></span> <span data-ttu-id="ddf37-484">**Next (다음** )를 선택 하 여 계속 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-484">Select **Next** to continue.</span></span>

6.  <span data-ttu-id="ddf37-485">이전에 가져온 작업 순서를 선택 하 고 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-485">Select the task sequence that you imported earlier, and then select **Next**.</span></span>

7.  <span data-ttu-id="ddf37-486">디스크 구성이 적용 된 후 장치에 대 한 컴퓨터 이름을 지정 하 라는 메시지가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-486">After the disk configuration is applied, you’ll be prompted to specify a computer name for the device.</span></span> <span data-ttu-id="ddf37-487">사용자 인터페이스에는 Surface Pro 디바이스의 일련 번호를 기준으로 권장 컴퓨터 이름이 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-487">The user interface will display a recommended computer name based on the serial number of the Surface Pro device.</span></span> <span data-ttu-id="ddf37-488">제안 된 이름을 그대로 사용 하거나 새로 지정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-488">You can either accept the proposed name or specify a new one.</span></span> <span data-ttu-id="ddf37-489">컴퓨터 이름 할당 화면의 지침을 따릅니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-489">Follow the instructions on the computer name assignment screen.</span></span> <span data-ttu-id="ddf37-490">**수락**을 선택 하면 배포가 시작 됩니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-490">When you select **Accept**, the deployment begins.</span></span>

8.  <span data-ttu-id="ddf37-491">나머지 배포 프로세스는 자동으로 진행 되며 사용자 입력을 더 이상 요구 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-491">The rest of the deployment process is automatic and doesn’t ask for any more user input.</span></span>

9.  <span data-ttu-id="ddf37-492">배포 작업 순서에 따라 디바이스 구성이 완료 되 면 Microsoft 팀 대화방 응용 프로그램 설정을 구성 하 라는 다음 구성 화면이 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-492">After the deployment task sequence finishes configuring the device, you’ll see the following configuration screen that asks you to configure the Microsoft Teams Rooms application settings.</span></span>

    ![Microsoft 팀 대화방 응용 프로그램의 초기 설정 화면](../media/room-systems-scale-image2.png)

10.  <span data-ttu-id="ddf37-494">Surface Pro를 Microsoft 팀 공간 콘솔에 연결 하 고 응용 프로그램 설정을 구성 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-494">Plug the Surface Pro into the Microsoft Teams Rooms console, and configure the application settings.</span></span>

11.  <span data-ttu-id="ddf37-495">[Microsoft 팀 대화방](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2) 에 나열 된 기능이 배포 된 장치에서 작동 하는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-495">Validate that the capabilities listed in [Microsoft Teams Rooms help](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2) are working on the deployed device.</span></span>


<span data-ttu-id="ddf37-496">실패 한 설치 문제를 해결 하려면 Configuration Manager 작업 순서에서 실행 되는 모든 단계를 기록 하는 **Smsts .log** 파일을 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-496">To troubleshoot a failed installation, check the **SMSTS.log** file, which logs all the steps executed in a Configuration Manager task sequence.</span></span>

<span data-ttu-id="ddf37-497">SMSTS .log 파일은 빌드 프로세스의 단계에 따라 여러 경로 중 하나에 저장 됩니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-497">The SMSTS.log file is stored on one of a number of paths, depending on the stage of the build process.</span></span> <span data-ttu-id="ddf37-498">다음 표를 참조 하 여 SMSTS .log 파일의 경로를 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-498">Check the following table to identify the path to the SMSTS.log file.</span></span>


| <span data-ttu-id="ddf37-499">**배포 단계**</span><span class="sxs-lookup"><span data-stu-id="ddf37-499">**Deployment phase**</span></span>                                                            | <span data-ttu-id="ddf37-500">**작업 순서 로그 경로**</span><span class="sxs-lookup"><span data-stu-id="ddf37-500">**Task sequence log path**</span></span>                         |
|---------------------------------------------------------------------------------|----------------------------------------------------|
| <span data-ttu-id="ddf37-501">WinPE, HDD 형식 앞</span><span class="sxs-lookup"><span data-stu-id="ddf37-501">WinPE, before HDD format</span></span>                                                        | <span data-ttu-id="ddf37-502">X:\\Windows\\\temp\\smstslog\\smsts. 로그</span><span class="sxs-lookup"><span data-stu-id="ddf37-502">X:\\Windows\\Temp\\smstslog\\smsts.log</span></span>             |
| <span data-ttu-id="ddf37-503">HDD 형식 이후 WinPE</span><span class="sxs-lookup"><span data-stu-id="ddf37-503">WinPE, after HDD format</span></span>                                                         | <span data-ttu-id="ddf37-504">C:\\_smstasksequence\\Logs\\Smstslog\\smsts. 로그</span><span class="sxs-lookup"><span data-stu-id="ddf37-504">C:\\_SMSTaskSequence\\Logs\\Smstslog\\smsts.log</span></span>    |
| <span data-ttu-id="ddf37-505">구성 관리자 에이전트를 설치 하기 전에 배포 되는 운영 체제</span><span class="sxs-lookup"><span data-stu-id="ddf37-505">Operating system deployed, before the Configuration Manager agent was installed</span></span> | <span data-ttu-id="ddf37-506">c:\\_smstasksequence\\Logs\\Smstslog\\smsts. 로그</span><span class="sxs-lookup"><span data-stu-id="ddf37-506">c:\\_SMSTaskSequence\\Logs\\Smstslog\\smsts.log</span></span>    |
| <span data-ttu-id="ddf37-507">운영 체제 및 구성 관리자 에이전트 배포 됨</span><span class="sxs-lookup"><span data-stu-id="ddf37-507">Operating system and the Configuration Manager agent deployed</span></span>                   | <span data-ttu-id="ddf37-508">% windir%\\System32\\ccm\\\\Smstslog\\smsts. i i 로그</span><span class="sxs-lookup"><span data-stu-id="ddf37-508">%windir%\\System32\\ccm\\logs\\Smstslog\\smsts.log</span></span> |
| <span data-ttu-id="ddf37-509">작업 순서 실행 완료</span><span class="sxs-lookup"><span data-stu-id="ddf37-509">Task sequence execution complete</span></span>                                                | <span data-ttu-id="ddf37-510">% windir%\\System32\\ccm\\은\\smsts .log를 기록 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-510">%windir%\\System32\\ccm\\logs\\smsts.log</span></span>           |

> [!TIP]
> <span data-ttu-id="ddf37-511">작업 순서 중 언제 든 지 **F8** 키를 선택 하 여 명령 콘솔을 연 다음 smsts. i f f f f f f f f f f f f f f f f f f f f</span><span class="sxs-lookup"><span data-stu-id="ddf37-511">You can select **F8** at any time during the task sequence to open a command console, and then get access to the SMSTS.log file.</span></span>

<span data-ttu-id="ddf37-512">PXE 부팅 문제를 해결 하려면 Configuration Manager 서버의 PXE 동작과 관련 된 두 로그 파일을 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="ddf37-512">To troubleshoot PXE boot issues, check the two log files on the Configuration Manager server that are specific to PXE actions:</span></span>

-   <span data-ttu-id="ddf37-513">Configuration Manager 설치 로그 디렉터리에 있는 **Pxecontrol .log**</span><span class="sxs-lookup"><span data-stu-id="ddf37-513">**Pxecontrol.log**, located in the Configuration Manager installation logs directory</span></span>

-   <span data-ttu-id="ddf37-514">**Smspxe .log**(Configuration Manager 관리 위치 (MP) 로그 디렉터리에 있음)</span><span class="sxs-lookup"><span data-stu-id="ddf37-514">**Smspxe.log**, located in Configuration Manager Management Point (MP) logs directory</span></span>

<span data-ttu-id="ddf37-515">Configuration Manager 설치 문제를 해결 하는 데 사용할 수 있는 전체 로그 파일 목록은 [System Center Configuration manager의 로그 파일](https://docs.microsoft.com/sccm/core/plan-design/hierarchy/log-files)을 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="ddf37-515">For a complete list of the log files that you can use to further troubleshoot your Configuration Manager installation, see [Log files in System Center Configuration Manager](https://docs.microsoft.com/sccm/core/plan-design/hierarchy/log-files).</span></span>
