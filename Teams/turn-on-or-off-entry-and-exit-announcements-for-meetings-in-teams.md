...
Office 365에서 오디오 회의를 설정 하는 경우 음성 회의 브리지가 제공 됩니다. 회의 브리지에는 사용자가 Microsoft 팀 모임에 전화를 걸 때 사용 하는 하나 이상의 전화 번호가 포함 될 수 있습니다. 
...
3. **브리지 설정** 창에서 모임 항목을 사용 하거나 사용 하지 않도록 설정 **하 고 알림을 종료**합니다. 이 옵션이 기본적으로 선택 되어 있습니다. 이 확인란을 선택 취소 하면 모임에 참가 한 사용자에 게 다른 사용자가 모임에 입장 하거나 떠날 때 알림이 표시 되지 않습니다.
    
4. **입력/종료 알림 유형에**서 **이름 또는 전화 번호** 또는 **톤**을 선택 합니다.
