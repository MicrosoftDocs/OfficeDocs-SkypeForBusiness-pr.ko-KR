...
> [!Note]
> 다양한 플랫폼의 각 클라이언트 기능에 대한 자세한 내용은 플랫폼별로 [Teams 기능을 참조하세요.](https://support.microsoft.com/office/teams-features-by-platform-debe7ff4-7db4-4138-b7d0-fcc276f392d3)
...
Microsoft Teams 데스크톱 클라이언트는 독립 실행형 [애플리케이션으로, 엔터프라이즈용 Microsoft 365 Apps에서도 사용할 수 있습니다.](https://docs.microsoft.com/deployoffice/teams-install) Teams는 32비트 및 64비트 버전의 Windows(8.1 이상), ARM Windows 10용 ARM64 및 Windows Server(2012 R2 이상) 및 macOS 및 Linux(형식 및 형식)에 사용할 수 `.deb` `.rpm` 있습니다. Windows에서 Teams는 .NET Framework 4.5 이상이 필요하며, 설치되어 있지 않으면 Teams 설치 관리자가 설치를 제안합니다. Linux에서 `apt`, `yum` 등의 패키지 관리자가 모든 요구 사항을 설치하려고 시도합니다. 하지만 그렇지 않을 경우에는 Linux에 Teams를 설치하기 전에 보고된 모든 요구 사항을 설치해야 합니다.
...
> [!NOTE]
> Chromebook에 Teams를 설치하는 방법에 대한 자세한 내용은 Chromebook에서 Teams를 설치하고 Microsoft Office [방법을 참조합니다.](https://support.office.com/article/how-to-install-and-run-microsoft-office-on-a-chromebook-32f14a23-2c1a-4579-b973-d4b1d78561ad)
...
> [!NOTE]
> "취소"를 선택하여 메시지를 해제해도 Windows 방화벽 구성이 변경됩니다. TCP 및 teams.exe 프로토콜에 대해 허용 작업을 사용하여 두 개의 인바운드 규칙이 생성됩니다.

사용자가 Teams에서 첫 번째 전화를 걸 때 Teams에서 방화벽 규칙을 만들지 못하게 하려는 경우 아래의 샘플 [PowerShell](#sample-powershell-script---inbound-firewall-rule) 스크립트 - 인바운드 방화벽 규칙을 사용하세요. 
...
> [!NOTE] 
> 버그를 찾았으면 클라이언트에서 `Report a Problem`를 사용하여 제출합니다. 알려진 문제는 조직의 [지원 팀을 참조합니다.](Known-issues.md)
