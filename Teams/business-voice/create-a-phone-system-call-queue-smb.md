...
- Phone System
description: 통화 요금제로 중소기업에 대한 호출 큐를 Microsoft 365 Teams 전화 방법에 대해 자세히 알아보습니다.
...
8. 닫기 **를 클릭합니다.** 팀에 추가한 사람들은 이제 팀 구성원이 됐고 팀이 팀 목록에 표시됩니다는 전자 메일을 받게 됩니다.
...
4. (선택 사항) 호출 **ID 할당에서** **추가를** 클릭하고, 자동 참석자에 대해 만든 리소스 계정을 검색하고 **추가를** 클릭한 다음 추가를 **클릭합니다.** 이렇게 하면 호출 에이전트가 호출할 때 주 줄의 발신자 ID가 호출 에이전트에 제공됩니다.
