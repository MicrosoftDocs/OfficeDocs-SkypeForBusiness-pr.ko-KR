...
모든 앱은 사용자 지정할 수 없습니다. 앱 개발자가 앱의 사용자 지정을 허용하는 경우에만 앱의 모양을 수정할 수 있습니다. 선택한 앱이 사용자 지정 가능한지 여부를 확인하려면 다음 단계를 수행합니다.
...
1. 앱 이름을 사용하여 사용자 지정하려는 앱을 검색합니다. 앱 개발자가 앱을 사용자 지정할 수 있는지 여부를 **사용자 지정 가능한** 열에서 확인합니다.
...
1. UI를 열어 개별 메타데이터 필드를 사용자 지정하려면 다음 단계 중 하나를 수행합니다.
   ...
   * 앱 이름을 선택하여 앱 세부 정보 페이지를 연 다음 **작업** > **사용자 지정** 을 선택합니다.
...
1. 앱을 사용자 지정한 후 **적용** 을 선택합니다. 변경 내용을 확인하려면 [미리 보기 앱 세부 정보를](#preview-app-customizations) 참조하세요. 변경 내용을 실행 취소하려면 [앱 세부 정보를 기본값으로 다시 설정하세요](#reset-app-details-to-default-values).

1. **게시** 를 선택하여 조직의 저장소에 사용자 지정된 앱을 게시합니다.

앱은 업데이트된 세부 정보가 포함된 **앱 관리** 페이지와 Teams 스토어 및 클라이언트(웹, 모바일 또는 데스크톱 클라이언트를 통해 사용 가능)에 나열됩니다. 수정 사항을 표시하는 데 몇 시간이 걸릴 수 있습니다.
...
* 사용자 지정 앱이 아닌 [타사 앱](deploy-apps-microsoft-teams-landing-page.md#third-party-apps-validated-by-microsoft) 만 [사용자](deploy-apps-microsoft-teams-landing-page.md#custom-apps) 지정할 수 있습니다.

* GCCH(Government Community Cloud High) 및 국방부(DoD) 환경에서는 앱을 사용자 지정할 수 없습니다.
...
* 앱 및 앱과 관련된 설명을 사용자 지정할 때 앱 개발자가 설명서 또는 사용 약관에 제공하는 지침을 따르는지 확인합니다. 타사 이미지를 사용하는 경우 저작권법을 준수합니다.

* 관리 제공된 사용자 지정 데이터는 가장 가까운 데이터 스토리지 지역에 저장됩니다.

* 사용 약관 또는 개인 정보 취급 방침에 대한 링크가 유효한지 확인해야 합니다.

* 앱 개발자가 더 이상 필드를 사용자 지정할 수 없는 경우 앱 세부 정보 페이지에 이러한 필드에 대해 관리자에게 알리는 메시지가 표시됩니다. 필드에 대한 모든 변경 내용은 원래 값으로 되돌려집니다.
...
* [앱 사용 현황 보고서에](teams-analytics-and-reports/app-usage-report.md)는 최종 사용자가 사용자 지정 앱을 사용하는 경우에도 게시자가 제공하는 앱의 원래 이름이 표시됩니다.

* Microsoft Graph 권한 동의 대화 상자에는 게시자가 제공한 앱의 원래 이름이 표시됩니다. 권한을 부여하면서 앱을 정확하게 식별하는 데 도움이 됩니다.

* 앱을 사용자 지정해도 앱 기능은 변경되지 않습니다.

사용자 지정 가능한 필드 중 일부에 대한 제한 사항은 다음과 같습니다.
...
| 앱을 사용자 지정하기 시작했지만 변경 내용을 저장하거나 적용할 수 없습니다. | 필드의 제한 사항을 준수합니다. UI에 대한 오류 및 [앱 사용자 지정의 제한 사항](#considerations-and-limitations-of-app-customization) 찾기 |
| 앱 페이지가 제대로 로드되지 않도록 관리합니다. 앱 목록이 표시되지 않습니다. | 사용 중인 관리 계정에 Teams 라이선스가 할당되어 있어야 합니다. |
...
* [앱 커뮤니티 게시물 브랜딩](https://techcommunity.microsoft.com/t5/microsoft-teams-blog/rebrand-apps-to-your-own-organization-s-branding-with-app/ba-p/2376296)
