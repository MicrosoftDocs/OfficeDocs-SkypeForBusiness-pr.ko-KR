...
ms.locfileid: "62457198"
---
# <a name="enable-location-based-routing-for-direct-routing"></a>직접 라우팅에 위치 기반 라우팅 사용

이 문서에서는 직접 라우팅에 Location-Based 라우팅을 사용하도록 설정하는 방법을 설명합니다. 이 문서의 단계를 수행하기 전에 직접 라우팅에 대한 계획 Location-Based 라우팅을 [](location-based-routing-plan.md) 읽고 네트워크 설정 구성에서 Location-Based 완료해야 [합니다](location-based-routing-configure-network-settings.md).

 직접 라우팅을 배포하고 네트워크 지역, 사이트 및 서브넷을 설정한 후 라우팅을 사용하도록 Location-Based 있습니다. 이 문서의 단계를 완료하려면 PowerShell cmdlet에 대해 잘 알고 있는 것이 필요합니다. 자세한 내용은 [PowerShell 개요 Teams 참조하세요.](teams-powershell-overview.md)
...
관리 센터 또는 [powerShell](#using-powershell)을 사용하여 Teams 라우팅을 사용하도록 Location-Based 수 있습니다.[](#using-the-microsoft-teams-admin-center)
...
1. PSTN 사용량을 설정하기 위해 [Set-CsOnlinePstnUsage](/powershell/module/skype/set-csonlinepstnusage?view=skype-ps) cmdlet을 사용합니다. 여러 사용의 경우 각 사용량을 콤마로 구분합니다.
...
2. 사용자를 적절한 PSTN 사용량과 연결하기 위한 음성 라우팅 정책을 만들 경우 [New-CsOnlineVoiceRoutingPolicy](/powershell/module/skype/new-csonlinevoiceroutingpolicy?view=skype-ps) cmdlet을 사용합니다.
    ...
    다음 예제에서는 두 개의 새 음성 라우팅 정책을 만들고 PSTN 사용량을 할당합니다. 
...
3. 라우팅 제한을 적용해야 하는 사용자에게 온라인 음성 라우팅 정책을 연결하려면 [Grant-CsOnlineVoiceRoutingPolicy](/powershell/module/skype/grant-csonlinevoiceroutingpolicy?view=skype-ps) cmdlet을 사용하세요.
...
1. 라우팅 Location-Based 제한을 적용해야 하는 네트워크 사이트에 음성 라우팅 정책을 연결하려면 [Set-CsTenantNetworkSite](/powershell/module/skype/set-cstenantnetworksite?view=skype-ps) cmdlet을 사용 합니다.

    ```PowerShell
    Set-CsTenantNetworkSite -Identity <site ID> -EnableLocationBasedRouting <$true|$false>  
    ```

    이 예제에서는 Location-Based 및 Hyderabad 사이트에 대한 라우팅을 사용할 수 있습니다. 
...
1. 각 게이트웨이 또는 네트워크 사이트에 대한 게이트웨이 구성을 만들하려면 [New-CsOnlinePSTNGateway](/powershell/module/skype/new-csonlinepstngateway?view=skype-ps) cmdlet을 사용하세요. 
...
2. 라우팅 Location-Based 적용해야 하는 게이트웨이에 대한 라우팅을 사용하도록 설정하려면 [Set-CSOnlinePSTNGateway](/powershell/module/skype/set-csonlinepstngateway?view=skype-ps) cmdlet을 사용 합니다. 
   ...
   다음 예제에서는 Location-Based 및 Hyderabad 사이트의 PSTN 게이트웨이에 연결된 각 게이트웨이에 대한 라우팅을 사용할 수 있습니다. 
...
PSTN Location-Based 우회를 방지하여 라우팅을 사용하도록 설정하려면 [Grant-CsTeamsCallingPolicy](/powershell/module/skype/grant-csteamscallingpolicy?view=skype-ps) cmdlet을 사용 합니다.
