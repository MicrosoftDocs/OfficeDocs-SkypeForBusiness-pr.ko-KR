...
- 미디어 최적화를 위해 Microsoft는 SBC의 공용 IP 주소를 팀의 전송 릴레이로만 열 것을 권장 합니다. 회사 네트워크 외부의 클라이언트의 경우 Microsoft는 SBC의 공용 IP 주소에 직접 도달 하는 대신 전송 릴레이를 사용 하는 것이 좋습니다.
...
## <a name="considerations-if-you-have-skype-for-business-phones-in-your-network"></a>네트워크에 비즈니스용 Skype 전화를 사용 하는 경우의 고려 사항  

네트워크에 직접 라우팅을 사용 하는 비즈니스용 Skype 끝점이 있는 경우 (예: 비즈니스용 Skype 클라이언트를 기반으로 한 팀 사용자는 3PIP 휴대폰을 사용할 수 있음), 이러한 사용자를 위해 제공 하는 트렁크의 미디어 바이패스는 꺼야 합니다.

이러한 사용자를 위해 별도의 트렁크를 만들고 온라인 음성 라우팅 정책을 할당할 수 있습니다.

상위 수준 구성 단계:

- 사용자에 게 3PIP 휴대폰이 있는지 여부에 따라 유형별로 사용자를 분할 합니다.
...
- 온라인 음성 라우팅 정책에서 사용자의 유형에 따라 올바른 트렁크를 할당 합니다.

아래 예제에서는이 논리를 보여 줍니다.

| 사용자 집합 | 사용자 수 | OVRP에 할당 된 트렁크 FQDN | 미디어 바이패스 사용 |
| :------------ |:----------------- |:--------------|:--------------|
팀 클라이언트 및 3PIP 휴대폰을 사용 하는 사용자 | 명 | sbc1.contoso.com:5061 | 해제 | 
팀 종단점만 있는 사용자 (팀에 대해 인증 된 새 전화 포함) | 980 | sbc2.contoso.com:5060 | false
