...
1. 관리 센터의 [왼쪽 탐색](https://admin.teams.microsoft.com)에서 Microsoft Teams **사용자로** >  **이동하세요**.
...
> [!NOTE]
> 사용자로부터 특수한 정책의 지정을 해지하기 위해 각 정책을 **전역(Org-wide default)으로 설정할 수 있습니다**.

사용자에게 정책을 할당하기 위해 다음을 할 수 있습니다.
...
먼저 [Get-CsUserPolicyAssignment](/powershell/module/teams/get-csuserpolicyassignment) cmdlet `PolicySource` 과 매개 변수를 함께 사용하여 사용자와 Teams 모임 브로드캐스트 정책에 대한 세부 정보를 얻습니다.
...
정책 할당의 상태를 확인하려면 적용을 선택하여 정책 할당을 제출한 후 사용자 페이지 맨 위에 나타나는  배너에서 활동 로그  **를 선택합니다**. 또는 관리 센터의 왼쪽 탐색에서 Microsoft Teams 대시보드로 이동한 다음 활동 로그에서 세부 정보 보기 **를 선택합니다**. 활동 로그는 지난 30일 동안 관리 센터를 통해 20명 Microsoft Teams 일괄 처리에 대한 정책 할당을 보여줍니다. 자세한 내용은 활동 로그에서 정책 할당 [보기를 참조합니다](activity-log.md).

### <a name="use-powershell-method"></a>PowerShell 메서드 사용

> [!NOTE]
> 현재 PowerShell을 사용하는 일괄 처리 정책 할당은 모든 정책 유형에 Teams 없습니다. 지원되는 정책 유형 목록은 [New-CsBatchPolicyAssignmentOperation](/powershell/module/teams/new-csbatchpolicyassignmentoperation) 을 참조하세요.
...
다음을 실행하여 일괄 처리 할당의 상태를 얻습니다. 여기서 OperationId는 주어진 일괄 처리에 대해 cmdlet에서 반환되는 작업 ID `New-CsBatchPolicyAssignmentOperation` 입니다.

```powershell
$Get-CsBatchPolicyAssignmentOperation -OperationId f985e013-0826-40bb-8c94-e5f367076044 | fl
```

출력에서 오류가 발생한 것으로 표시되는 경우 다음을 실행하여 속성에 있는 오류에 대한 자세한 정보를 얻 `UserState` 습니다.

```powershell
Get-CsBatchPolicyAssignmentOperation -OperationId f985e013-0826-40bb-8c94-e5f367076044 | Select -ExpandProperty UserState
```

자세한 내용은 [Get-CsBatchPolicyAssignmentOperation을 참조합니다](/powershell/module/teams/get-csbatchpolicyassignmentoperation).
