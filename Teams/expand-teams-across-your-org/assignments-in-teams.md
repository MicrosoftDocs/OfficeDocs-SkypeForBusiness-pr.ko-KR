...
[Turnitin은](https://www.turnitin.com/) 학업 무결성 서비스입니다. 이 서비스는 자체 약관 및 개인 정보 취급 방침을 적용하는 타사 서비스입니다. 귀하는 타사 제품 및 서비스를 사용할 책임이 있습니다.
...
**프롬프트에 따라 TurnitinApiKey가** 제공됩니다.
...
과제 제출과 연결된 학생의 파일은 문서 라이브러리에 *저장됩니다(학생 작업).* 교사가 생성하고 학생이 액세스할 수 있는 과제와 연결된 파일은 해당 수업 팀 사이트 SharePoint 다른 문서 라이브러리(클래스 파일)에 저장됩니다. 자사 통합은 할당 데이터를 동일한 클래스 팀 SharePoint 사이트에 저장할 수도 있습니다(명명: 할당 제목 *+ 타임스탬프).*
...
단일 학생의 데이터를 대량으로 내보내기 위해 학생이 참여하는 수업에서 [](/microsoft-365/education/deploy/configure-assignments-for-teams) 학생을 제거하기 전에 스크립트를 실행하고 ``userId`` 을 제공합니다. 학생이 사이트에서 제거된 경우 관리자는 스크립트를 실행하기 전에 학생을 다시 수업에 추가할 수 있습니다. 또는 관리자가 학생의 일부인 학생을 제공할 수 ``userId`` ``classId`` 있습니다.
...
단일 학생의 데이터를 대량으로 삭제하려면 학생이 참여하는 수업에서 학생을 제거하기 전에 스크립트를 실행하고 [](/microsoft-365/education/deploy/configure-assignments-for-teams) ``userId`` 을 제공합니다. 학생이 사이트에서 제거된 경우 관리자는 스크립트를 실행하기 전에 학생을 다시 수업에 추가할 수 있습니다. 또는 관리자가 학생의 일부인 학생을 제공할 수 ``userId`` ``classId`` 있습니다.

a를 제공하면 관리자가 특정 수업에서 학생에 대한 정보만 ``ClassId`` 삭제할 수 있습니다.

#### <a name="for-a-teacher"></a>교사의 경우

교사에 대한 과제의 데이터는 수업에서 공유하기 때문에 일괄 삭제 옵션이 없습니다. 대신 관리자는 수업에 자신을 추가하고, 앱으로 이동한 다음 과제를 삭제할 수 있습니다.

자세한 내용은 에 대한 할당 [구성을 Teams.](/microsoft-365/education/deploy/configure-assignments-for-teams)
