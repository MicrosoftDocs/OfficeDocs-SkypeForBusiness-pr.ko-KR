...
- 환자가 환자 포털 내에서 또는 SMS 통해 Teams 가상 약속에 가입할 수 있도록 합니다.
...
- **Microsoft 365 전역 관리자**: 통합을 담당하는 주 사용자입니다. 관리자는 커넥터를 구성하고, SMS 사용하도록 설정하고(필요한 경우) 구성을 승인할 Epic 고객 분석가를 추가합니다.
...
1. 전화 번호를 FHIR 기본 URL에 연결하려면 SMS 구성 섹션의 **전화** **번호** 아래에서 번호를 선택합니다. SMS 알림을 사용하도록 설정하려는 각 FHIR 기본 URL에 대해 이 작업을 수행합니다.
    ...
    커넥터를 처음 구성하는 경우 이전 단계에서 입력한 FHIR 기본 URL이 표시됩니다. 동일한 전화 번호를 여러 FHIR 기본 URL에 연결할 수 있습니다. 즉, 환자는 서로 다른 조직 및/또는 부서에 대해 동일한 전화 번호에서 SMS 알림을 받습니다.

1. 각 FHIR 기본 URL 옆에 **SMS 설정을** 선택하여 환자에게 보낼 SMS 알림 유형을 설정합니다.
...
디바이스 테스트에 대한 컨텍스트 토큰은 환자 통합 레코드에서 찾을 수 있습니다. Epic 고객 분석가는 Epic에서 가상 약속 구성을 완료하려면 이러한 레코드를 Epic에 제공해야 합니다. 자세한 내용은 [Epic-Microsoft Teams Telehealth 통합 가이드](https://galaxy.epic.com/Search/GetFile?Url=1!68!100!100100357)를 참조하세요.
...
조직의 의료 공급자는 Epic 공급자 앱(하이퍼스페이스, Haiku, Canto)의 Teams 사용하여 약속에 참여할 수 있습니다. **가상 방문 시작** 단추는 공급자 흐름에 포함되어 있습니다.
  ...
  ![환자와 가상 약속의 공급자 환경.](media/ehc-provider-experience-6.png)
...
커넥터는 SMS 문자 메시지, MyChart 웹 및 모바일의 링크를 통해 약속에 가입하는 환자를 지원합니다. 약속 시 환자는 **가상 방문 시작** 단추를 사용하거나 SMS 문자 메시지의 링크를 탭하여 MyChart에서 약속을 시작할 수 있습니다.

  ![가상 약속의 환자 환경.](media/ehc-virtual-visit-5.png)

환자 환경의 주요 기능:

- 환자는 [Teams 앱을 설치하지 않고도 데스크톱 및 모바일의 최신 웹 브라우저](../browser-join.md)에서 약속을 조인할 수 있습니다.
- 환자는 약속에 가입하기 전에 디바이스 하드웨어 및 연결을 테스트할 수 있습니다.

    :::image type="content" source="media/ehr-admin-epic-device-test.png" alt-text="디바이스 테스트 기능을 보여 주는 모바일 디바이스의 이미지입니다." lightbox="media/ehr-admin-epic-device-test.png":::
  
    디바이스 테스트 기능:

  - 환자는 스피커, 마이크, 카메라 및 연결을 테스트할 수 있습니다.
  - 환자는 테스트 호출을 완료하여 구성의 유효성을 완전히 검사할 수 있습니다.
  - 디바이스 테스트 결과를 EHR 시스템으로 다시 보낼 수 있습니다.
