...
| <span data-ttu-id="1d02d-112">매개 변수 이름</span><span class="sxs-lookup"><span data-stu-id="1d02d-112">Parameter name</span></span> | <span data-ttu-id="1d02d-113">값의 예</span><span class="sxs-lookup"><span data-stu-id="1d02d-113">Example of the value</span></span> | 
| :---------------------  |:---------------------- |
...
| <span data-ttu-id="1d02d-140">헤더를 통해</span><span class="sxs-lookup"><span data-stu-id="1d02d-140">Via Header</span></span> | <span data-ttu-id="1d02d-141">Via: SIP/2.0/TLS sbc1.adatum.biz:5058;alias;branch=z9hG4bKac2121518978</span><span class="sxs-lookup"><span data-stu-id="1d02d-141">Via: SIP/2.0/TLS sbc1.adatum.biz:5058;alias;branch=z9hG4bKac2121518978</span></span> | 
...
<span data-ttu-id="1d02d-211">연락처 및 Record-Route 모두 사용하는 경우 SBC 관리자는 해당 값을 동일하게 유지해야 하여 관리 오버헤드가 발생합니다.</span><span class="sxs-lookup"><span data-stu-id="1d02d-211">If both Contact and Record-Route are used, the SBC administrator must keep their values identical, which causes administrative overhead.</span></span> 
...
<span data-ttu-id="1d02d-276">미디어 우회 시나리오에서 동일한 메시지(100 시도, 180, 183)가 사용됩니다.</span><span class="sxs-lookup"><span data-stu-id="1d02d-276">The same messages (100 Trying, 180, 183) are used in the media bypass scenario.</span></span> 
...
### <a name="sip-proxy-send-the-refer-to-the-sbc-and-acts-as-a-transferor"></a><span data-ttu-id="1d02d-308">SIP 프록시는 SBC 참조를 보내고 전송자 역할을 합니다.</span><span class="sxs-lookup"><span data-stu-id="1d02d-308">SIP proxy send the Refer to the SBC and acts as a Transferor</span></span>
...
<span data-ttu-id="1d02d-323">SIP 프록시는 호스트 이름의 SIP 프록시 FQDN으로 구성된 SIP URI로 REFER-TO를 형성하고 다음 중 하나를 형성합니다.</span><span class="sxs-lookup"><span data-stu-id="1d02d-323">The SIP proxy will form the REFER-TO as a SIP URI comprised of a SIP proxy FQDN in the hostname and either one of the following:</span></span>

- <span data-ttu-id="1d02d-324">전송 대상이 전화 번호인 경우 URI의 사용자 이름 부분의 E.164 전화 번호</span><span class="sxs-lookup"><span data-stu-id="1d02d-324">An E.164 phone number in the username part of the URI in case the transfer target is a phone number, or</span></span>

- <span data-ttu-id="1d02d-325">전체 전송 대상 MRI 및 테넌트 ID를 각각 인코딩하는 x-m 및 x-t 매개 변수</span><span class="sxs-lookup"><span data-stu-id="1d02d-325">x-m and x-t parameters encoding the full transfer target MRI and tenant ID respectively</span></span> 

<span data-ttu-id="1d02d-326">REFERRED-BY 헤더는 다음 표와 같이 전송자 테넌트 ID 및 기타 전송 컨텍스트 매개 변수뿐만 아니라 전송자 MRI가 인코딩된 SIP URI입니다.</span><span class="sxs-lookup"><span data-stu-id="1d02d-326">The REFERRED-BY header is a SIP URI with transferor MRI encoded in it as well as transferor tenant ID and other transfer context parameters as shown in the following table:</span></span>

| <span data-ttu-id="1d02d-327">매개 변수</span><span class="sxs-lookup"><span data-stu-id="1d02d-327">Parameter</span></span> | <span data-ttu-id="1d02d-328">값</span><span class="sxs-lookup"><span data-stu-id="1d02d-328">Value</span></span> | <span data-ttu-id="1d02d-329">설명</span><span class="sxs-lookup"><span data-stu-id="1d02d-329">Description</span></span> |  
|:---------------------  |:---------------------- |:---------------------- |
| <span data-ttu-id="1d02d-330">x-m</span><span class="sxs-lookup"><span data-stu-id="1d02d-330">x-m</span></span> | <span data-ttu-id="1d02d-331">MRI</span><span class="sxs-lookup"><span data-stu-id="1d02d-331">MRI</span></span> | <span data-ttu-id="1d02d-332">CC에 의해 채워진 전송기/전송 대상의 전체 MRI</span><span class="sxs-lookup"><span data-stu-id="1d02d-332">Full MRI of transferor/transfer target as populated by CC</span></span> |
| <span data-ttu-id="1d02d-333">x-t</span><span class="sxs-lookup"><span data-stu-id="1d02d-333">x-t</span></span> | <span data-ttu-id="1d02d-334">테넌트 ID</span><span class="sxs-lookup"><span data-stu-id="1d02d-334">Tenant ID</span></span> | <span data-ttu-id="1d02d-335">x-t 테넌트 ID 선택적 테넌트 ID가 CC로 채워진 경우</span><span class="sxs-lookup"><span data-stu-id="1d02d-335">x-t Tenant ID Optional Tenant ID as populated by CC</span></span> |
| <span data-ttu-id="1d02d-336">x-ti</span><span class="sxs-lookup"><span data-stu-id="1d02d-336">x-ti</span></span> | <span data-ttu-id="1d02d-337">전송자 상관 관계 ID</span><span class="sxs-lookup"><span data-stu-id="1d02d-337">Transferor Correlation Id</span></span> | <span data-ttu-id="1d02d-338">전송자에 대한 호출의 상관 관계 ID</span><span class="sxs-lookup"><span data-stu-id="1d02d-338">Correlation ID of the call to the transferor</span></span> |
| <span data-ttu-id="1d02d-339">x-tt</span><span class="sxs-lookup"><span data-stu-id="1d02d-339">x-tt</span></span> | <span data-ttu-id="1d02d-340">전송 대상 호출 URI</span><span class="sxs-lookup"><span data-stu-id="1d02d-340">Transfer target call URI</span></span> | <span data-ttu-id="1d02d-341">인코딩된 호출 대체 URI</span><span class="sxs-lookup"><span data-stu-id="1d02d-341">Encoded call replacement URI</span></span> |

<span data-ttu-id="1d02d-342">참조 헤더의 크기는 이 경우 최대 400개 기호일 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="1d02d-342">The size of the Refer Header can be up to 400 symbols in this case.</span></span> <span data-ttu-id="1d02d-343">SBC는 최대 400개 기호 크기의 참조 메시지 처리를 지원해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="1d02d-343">The SBC must support handling Refer messages with size up to 400 symbols.</span></span>

> [!div class="mx-imgBorder"]
> <span data-ttu-id="1d02d-344">![잠정 답변으로 울리는 여러 엔드포인트를 보여주는 다이어그램](media/direct-routing-protocols-5.png)</span><span class="sxs-lookup"><span data-stu-id="1d02d-344">![Diagram showing multiple endpoints ringing with provisional answer](media/direct-routing-protocols-5.png)</span></span>

## <a name="session-timer"></a><span data-ttu-id="1d02d-345">세션 시간</span><span class="sxs-lookup"><span data-stu-id="1d02d-345">Session timer</span></span>
...
- <span data-ttu-id="1d02d-359">History-Info 링 및 통화 전달 사례에 대한 추가 헤더가 추가됩니다.</span><span class="sxs-lookup"><span data-stu-id="1d02d-359">History-Info header will be added for simultaneous ring and call forwarding cases.</span></span>
