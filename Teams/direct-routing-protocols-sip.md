...
들어오는 또는 아웃바운드 호출을 처리하기 전에 OPTIONS 메시지가 SIP 프록시와 SBC 간에 교환됩니다. 이러한 OPTIONS 메시지를 사용하면 SIP 프록시가 SBC에 허용되는 기능을 제공할 수 있습니다. 옵션 협상이 성공해야 합니다(200OK 응답). 호출을 설정하기 위해 SBC와 SIP 프록시 간의 추가 통신을 허용합니다. SIP 프록시에 대한 OPTIONS 메시지의 SIP 헤더는 아래 예제로 제공됩니다.
...
> [!NOTE]
> SIP 헤더는 사용 중 SIP URI에 userinfo를 포함하지 않습니다. [RFC 3261, 섹션 19.1.1에](https://tools.ietf.org/html/rfc3261#section-19.1.1)따라 URI의 userinfo 부분은 선택 사항이며 대상 호스트에 사용자에 대한 구상이 없는 경우 또는 hosst 자체가 식별되는 리소스인 경우 존재하지 않을 수 있습니다. @ 기호가 SIP URI에 있는 경우 사용자 필드는 비어 있지 말아야 합니다.
...
Microsoft SIP 프록시에 들어오는 모든 SIP 메시지(OPTIONS, INVITE)의 경우 연락처 헤더에는 다음과 같이 URI 호스트 이름에 쌍을 이은 SBC FQDN이 있어야 합니다.

구문: <sip:phone 또는 sip address@FQDN SBC;transport=tls> 

[RFC 3261, 섹션 11.1에](https://tools.ietf.org/html/rfc3261#section-11.1)따라 옵션 메시지에 연락처 헤더 필드가 있을 수 있습니다. 직접 라우팅에서 연락처 헤더가 필요합니다. 위의 형식의 INVITE 메시지의 경우 OPTIONS 메시지의 경우 userinfo를 SIP URI에서 제거할 수 있으며 다음과 같이 FQDN 형식만 전송됩니다.

구문: 연락처: <sip:FQDN의 SBC;transport=tls>

이 이름(FQDN)도 제시된 인증서의 일반 이름 또는 주체 대체 이름 필드에 있어야 합니다. Microsoft는 인증서의 일반 이름 또는 주체 대체 이름 필드에 이름의 와일드카드 값을 사용할 수 있습니다.   
...
직접 라우팅에 대한 규칙으로 FQDN을 사용하여 IP 대신 SIP URI를 채우는 것이 중요합니다. 호스트 이름을 FQDN이 아닌 IP로 표시하는 연락처 헤더를 사용하여 SIP 프록시로 들어오는 INVITE 또는 OPTIONS 메시지는 403 사용할 수 없는 연결을 거부합니다.
...
[RFC 3261, 섹션 8.1.1.8에](https://tools.ietf.org/html/rfc3261#section-8.1.1.8)따라 연락처 헤더는 새 대화 상자가 될 수 있는 모든 요청에 필요합니다. 이 Record-Route 프록시가 대화 상자의 향후 요청 경로를 유지하려는 경우 필요합니다. 프록시 SBC가 직접 라우팅에 대한 로컬 미디어 최적화와 함께 사용 중이면 프록시 SBC가 경로에 유지해야 하여 레코드 경로를 구성해야 합니다. [](https://docs.microsoft.com/MicrosoftTeams/direct-routing-media-optimization) 

프록시 SBC를 사용하지 않는 경우 연락처 헤더만 사용하는 것이 좋습니다.

- [RFC 3261, 섹션 20.30,](https://tools.ietf.org/html/rfc3261#section-20.30)Record-Route 프록시가 대화 상자에서 향후 요청의 경로를 유지하려는 경우 사용됩니다. 모든 트래픽이 Microsoft SIP 프록시와 쌍을 이루는 SBC 간에 들어가기 때문에 프록시 SBC가 구성되지 않은 경우 필수는 아 없습니다. 
...
- 우선 순위 1. 최상위 레코드 경로입니다. 최상위 수준 Record-Route FQDN 이름을 포함하는 경우 FQDN 이름은 아웃바운드 대화 상자 연결을 만드는 데 사용됩니다.
...
-   미디어 응답 – SDP(세션 설명 프로토콜)의 미디어 후보를 사용하여 SIP 프록시에서 메시지 183으로 변환됩니다. 메시지 183을 수신할 때 SBC는 SDP 메시지에 수신된 미디어 후보에 연결해야 합니다. 
...
- 재구성된 History-Info 헤더의 개별 기록 항목에는 URI의 호스트 부분으로 설정된 직접 라우팅 FQDN(sip.pstnhub.microsoft.com)과 결합된 전화 번호 매개 변수가 있습니다. 'user=phone'의 매개 변수가 SIP URI의 일부로 추가됩니다.  전화 컨텍스트 매개 변수를 제외한 원래 History-Info 헤더와 연결된 다른 모든 매개 변수는 다시 생성된 History-Info 전달됩니다.  

  > [!NOTE]
  > 개인 항목(RFC 4244의 섹션 3.3에 정의된 메커니즘에 따라 결정)은 SIP 트렁크 공급자가 신뢰할 수 있는 피어이기 때문에 다음과 같이 전달됩니다.
