...
시작하기 전에 포트 순서의 정보를 검토 [하는 것이 좋습니다.](port-order-overview.md) 전화 접속 회의 브리지, 자동 참석자 또는 기타 서비스 번호, 수신자 부담 전화 번호 또는 전화 번호로 전송해야 하는 999명 이상의 사용자(구독자) 전화 번호가 있는 Teams 조직의 전화 번호 관리를 참조하여 올바른 양식을 다운로드하여 보내야 합니다.[](../manage-phone-numbers-for-your-organization/manage-phone-numbers-for-your-organization.md)
...
> [!NOTE]
> 현재 이 마법사를 사용하여 영국, 미국 및 캐나다의 전화 번호를 **얻을 수 있습니다**. 다른 국가 및 지역에 대한 전화 번호를 얻하려면 포트 주문을 수동으로 [제출할 수 있습니다](manually-submit-port-order.md). 포트 순서를 수동으로 제출해야 하는 양식을 얻하려면 조직의 전화 번호 관리에서 드롭다운 목록에서 국가 또는 지역을 [선택합니다](../manage-phone-numbers-for-your-organization/manage-phone-numbers-for-your-organization.md).

1. 관리 센터의 왼쪽 탐색에서 Microsoft Teams **음성** >  번호로 **전화 있습니다**. 숫자 **를** 클릭한 다음 포트 **를 클릭하여 포** 터링 마법사를 시작하세요.
2. 시작 페이지의 정보를 검토한  다음 준비가되면 다음을 **클릭합니다**.
3. 위치 및 **숫자** 유형 선택 페이지에서 다음을 지정한 다음 다음을 **클릭합니다**.

    - **국가 또는 지역**: 숫자를 받고 있는 국가 또는 지역입니다.
    - **전화 번호 유형**: 지리적 번호 또는 무료 번호와 같은 숫자 유형입니다.
...
5. 번호 추가 **페이지에서** 파일 선택을 클릭 **하고 전송할** 전화 번호가 포함된 CSV 파일을 찾아 선택한 다음 다음을 **클릭합니다**.  

    > [!NOTE]
    > CSV 파일에 PhoneNumber라는 헤더가 있는 열이 하나만 있어야 합니다. 각 전화 번호는 별도의 행에 있어야 합니다. 숫자만 또는 E.164 형식일 수 있습니다.

6. 주문 **완료 페이지에서** 서명된 업로드 인증 편지  를 클릭하여 서명된 LOA(인증 편지)의 스캔된 복사본을 업로드합니다.
    ...
    4. 서명된 LOA를 스캔한 다음 업로드 서명  된 인증 편지를 클릭하여 업로드합니다.
...
포트 순서의 상태를 보려면 관리 센터의 왼쪽 Microsoft Teams **VoicePort** >  주문으로 이동한 다음 > 기록을 **클릭합니다**. 각 포트 주문 상태는 상태 열 **에 나열** 됩니다. 자세한 내용은 포트 주문의 상태는 어떻게 [하나요?](port-order-status.md)
