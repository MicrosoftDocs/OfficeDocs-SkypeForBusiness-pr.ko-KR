...
조직 전체 팀을 통해 전역 관리자는 다음과 같은 특성을 가진 공개 팀을 쉽게 만들 수 있습니다.
- 조직의 모든 사용자를 끌어오기
- 사용자가 조직에 가입하고 나갈 때 Active Directory를 사용하여 멤버 자격을 최신 상태로 유지합니다.

전역 관리자만 조직 전체의 팀을 만들 수 있습니다. 현재 조직 전체 팀은 사용자가 10,000명 이하인 조직으로 제한됩니다. 테넌트당 조직 전체 팀 5개로 제한됩니다. 팀을 만들 때 이러한 요구 사항이 충족되면 전역 관리자는 **처음부터 팀 빌드** 를 선택할 때 **조직 전체** 에서 옵션으로 표시됩니다.
...
조직의 디렉터리가 새 활성 사용자를 포함하도록 업데이트되거나 더 이상 회사에서 일하지 않는 사용자의 계정을 사용하지 않도록 설정하면 변경 내용이 자동으로 동기화되고 사용자가 팀에서 추가되거나 제거됩니다. 팀 구성원은 조직 전체의 팀을 떠날 수 없습니다. 팀 소유자는 필요한 경우 수동으로 사용자를 추가하거나 제거할 수 있습니다.
...
1. 팀으로 이동하여 일반 채널을 찾은 다음 **... 추가 옵션** > **채널 관리**.
2. **채널 설정** 탭에서 **사용 권한을** 클릭한 다음 **소유자만 메시지를 게시할 수 있도록** 선택합니다.

### <a name="turn-off-team-and-team-name-mentions"></a>@팀 및 @[팀 이름] 멘션 해제

전체 조직에 과부하가 되는 것을 방지하기 위해 @멘션을 줄입니다.

1. 팀으로 이동하여 ...를 클릭합니다 **. 추가 옵션** \> **팀 관리**.
2. **설정** 탭에서 **@mentions** \> 클릭하여 **구성원에게 @team 옵션 또는 @[팀 이름]을 표시하지** 않습니다.
...
- 봇과 커넥터가 채널 메시지를 제출할 수 있는지 여부를 제어합니다.
...
### <a name="is-there-a-way-to-create-an-organization-wide-team-other-than-using-the-teams-client"></a>Teams 클라이언트를 사용하는 것 외에 조직 전체 팀을 만드는 방법이 있나요?

전역 관리자만 Teams 클라이언트를 사용하여 조직 전체의 팀을 만들 수 있습니다. 조직에서 팀 만들기를 PowerShell 사용으로 제한하는 경우 권장되는 해결 방법은 전역 관리자를 팀을 만들 수있는 보안 사용자 그룹에 추가하는 것입니다.

자세한 내용은 [그룹을 만들 수 있는 사용자 관리를 참조하세요](/microsoft-365/admin/create-groups/manage-creation-of-groups).

이 해결 방법이 옵션이 아닌 경우 PowerShell을 사용하여 공용 팀을 만들고 전역 관리자를 팀 소유자로 추가할 수 있습니다. 그런 다음 전역 관리자가 팀 이름 옆에 있는 **기타 옵션** 을 클릭하고 **팀 편집** 을 클릭한 다음 개인 정보 보호를 **조직 전체로 변경하면 조직의 모든 사용자가 자동으로 추가됩니다**.

> [!NOTE]
> 팀 소유자만 **팀 편집** 옵션에 액세스할 수 있으며 전역 관리자만 **조직 전체** 옵션을 볼 수 있습니다.

### <a name="is-there-a-way-to-convert-an-existing-team-to-an-organization-wide-team"></a>기존 팀을 조직 전체 팀으로 변환할 수 있는 방법이 있나요?

전역 관리자는 Teams 클라이언트에서 편집하여 기존 팀을 조직 전체 팀으로 변환할 수 있습니다. 팀 이름으로 이동하여 **기타 옵션** > **팀 편집** 을 클릭합니다.

### <a name="can-i-create-an-organization-wide-team-using-a-team-template"></a>팀 템플릿을 사용하여 조직 전체 팀을 만들 수 있나요?

팀 템플릿은 조직 전체 팀을 만드는 데 사용할 수 없습니다. 이 기능에 대한 작업은 현재 진행 중입니다.
