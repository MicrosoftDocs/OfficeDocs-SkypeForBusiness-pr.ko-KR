...
1. 다음 링크 중 하나를 사용 하 여 VDI VM 운영 체제와 일치 하는 팀 MSI 패키지를 다운로드 합니다.
    ...
    > [!NOTE]
    > 정부용 클라우드의 경우 [Microsoft Endpoint Configuration Manager를 사용 하 여](msi-deployment.md) MSI 파일에 대 한 다운로드 링크를 통해 Microsoft 팀 설치를 참조 하세요.
...
2. 다음 명령 중 하나를 실행 하 여이 MSI를 VDI VM에 설치 합니다.
    ...
    - 컴퓨터 단위 설치
        ...
        > [!NOTE]
        > 이 예제에서는 또한 **ALLUSERS = 1** 매개 변수를 사용 합니다. 이 매개 변수를 설정 하면 제어판의 프로그램 및 기능 및 컴퓨터의 모든 사용자에 대 한 Windows 설정의 앱 & 기능에 팀 Machine-Wide 설치 프로그램이 표시 됩니다. 모든 사용자가 관리자 자격 증명을 사용 하는 경우 팀을 제거할 수 있습니다.
...
3. VDI VM에서 MSI를 제거 합니다. 팀을 제거 하는 방법에는 두 가지가 있습니다.

    - PowerShell 스크립트: [이 PowerShell 스크립트](scripts/powershell-script-deployment-cleanup.md) 를 사용 하 여 팀을 제거 하 고 사용자에 대 한 팀 폴더를 제거할 수 있습니다. 팀이 컴퓨터에 설치 되어 있는 각 사용자 프로필에 대해 스크립트를 실행 합니다.
    - 명령줄: 다음 명령을 실행 합니다.
...
- 팀은 사용자가 선택한 마지막 오디오 장치를 사용 하도록 전환 되지 않으며, 장치 연결이 끊어지면 다시 연결 됩니다.
- 보내는 화면 공유:
