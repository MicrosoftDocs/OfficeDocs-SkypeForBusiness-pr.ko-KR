...
|AllowPrivateCalling| 팀 클라이언트의 왼쪽에 있는 앱 바에서 호출 앱을 사용할 수 있는지 여부와 사용자가 개인 채팅에서 통화 및 비디오 통화 옵션을 볼 것인지를 제어 합니다. |팀의 왼쪽에 있는 앱 표시줄에서 호출 앱을 제거 하 고 개인 채팅에서 통화 및 비디오 통화 옵션을 제거 하려면 False로 설정 합니다.|
...
|ScreenSharingMode | 사용자가 통화 또는 모임에서 화면을 공유할 수 있는 모드를 결정 합니다. | 사용자가 화면을 공유할 수 없도록 하려면 사용 안 함으로 설정 합니다. |
|AllowIPVideo |사용자의 모임 또는 통화에서 비디오를 사용할 수 있는지 여부를 결정 합니다. | 사용자가 비디오를 공유 하지 못하도록 하려면 False로 설정 합니다. |
| AllowAnonymousUsersToDialOut | 익명 사용자가 PSTN 번호로 전화를 걸 수 있는지 여부를 결정 합니다. | False로 설정 하 여 익명 사용자가 전화를 걸지 못하도록 합니다. |
| AllowAnonymousUsersToStartMeeting | 익명 사용자가 모임을 시작할 수 있는지 여부를 결정 합니다. | 사용자가 모임을 시작 하지 못하도록 하려면 False로 설정 합니다. |
| AllowOutlookAddIn |사용자가 Outlook 데스크톱 클라이언트에서 팀 모임을 예약할 수 있는지 여부를 결정 합니다. | False로 설정 하면 사용자가 Outlook 데스크톱 클라이언트에서 팀 모임을 예약 하지 못하도록 할 수 있습니다. |
| AllowParticipantGiveRequestControl|참가자가 화면 공유를 요청 하거나 제어권을 부여할 수 있는지 여부를 결정 합니다.| 사용자가 모임에서 제어권을 부여 하 고 요청 하지 못하도록 하려면 False로 설정 합니다. |
...
| AllowPowerPointSharing |사용자의 모임에서 PowerPoint 공유를 허용할지 여부를 결정 합니다. |False로 설정 하면 사용자가 모임에서 PowerPoint 파일을 공유 하지 못하도록 할 수 있습니다. |
...
| AllowSharedNotes | 사용자가 공유 노트를 사용할 수 있는지 여부를 결정 합니다. | False로 설정 하 여 사용자가 공유 메모를 작성 하지 못하도록 합니다. |
...
1. 모든 모임 정책이 비활성화 되어 있는 기본 제공 정책 옵션을 찾습니다. 다음과 같이 표시 됩니다.
...
1. 가상화 된 환경에서 팀을 사용할 모든 사용자에 게 AllOff 기본 제공 정책 옵션을 적용 합니다.
...
1. 다음 명령을 실행 하 여 VDI VM에 MSI를 설치 하거나 업데이트를 완료 합니다.
...
1. 다음 명령을 실행 하 여 VDI VM에서 MSI를 제거 하거나 업데이트 준비를 합니다.
