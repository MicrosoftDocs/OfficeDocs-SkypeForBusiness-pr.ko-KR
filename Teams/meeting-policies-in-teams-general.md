...
이 기능을 해제하면 사용자는 Outlook 새 모임을 만들 때 Teams 모임을 예약할 수 없습니다. 예를 들어 Windows 기반 Outlook에서는 **새 Teams 모임** 옵션이 리본에 표시되지 않습니다.
...
![Teams 모임 예약 옵션을 보여 주는 스크린샷](media/schedule-meeting-option.png)
...
**비공개 모임 예약을** 해제하고 **채널 모임 예약을 허용하는** 경우 Teams 사용자에 대해 **필요한 참석자 추가** 및 **채널 추가** 옵션을 사용할 수 없습니다. 이 설정은 기본적으로 켜져 있습니다.
...
이 정책은 기본적으로 설정되어 있으며 이끌이가 설정한 모임 및 웨비나를 등록하고 참석한 사람을 볼 수 있습니다. Teams 관리 센터에서 해제하려면 **MeetingsMeeting** >  **정책** 으로 이동하여 **Engagement 보고서** 설정을 **끄** 기로 설정합니다.
...
> [!NOTE]
> 관리자는 구성하지 않은 모임에 대한 출석 보고서를 볼 수 없습니다. 그러나 해당 모임의 24시간 이내에 지정된 모임에 대한 참가자 세부 정보를 볼 수 있습니다. Teams 관리 센터에서 **UsersManage** >  사용자로 이동합니다. 모임 이끌이의 표시 이름을 선택합니다. **모임 & 통화** 탭을 선택한 다음 적절한 모임 ID 또는 통화 ID를 선택합니다. 그런 다음 **참가자 세부 정보를** 선택합니다.

참여 보고서의 제한을 포함한 자세한 내용은 [Teams 모임 참석 보고서 보기 및 다운로드를](https://support.microsoft.com/office/view-and-download-meeting-attendance-reports-in-teams-ae7cf170-530c-47d3-84c1-3aedac74d310) 참조하세요.

## <a name="meeting-registration"></a>모임 등록

사용자별 정책에 해당합니다. 이 기능을 켜면 조직의 사용자가 웨비나를 설정할 수 있습니다. 이 정책은 기본적으로 사용하도록 설정됩니다.

Teams 관리 센터에서 이 정책을 편집하려면 **MeetingsMeeting** >  정책으로 이동합니다. 모임 등록을 해제하려면 정책을 **해제** 로 설정합니다.
...
- 익명 사용자를 포함한 모든 사용자가 조직의 사용자가 설정한 웨비 **나를 등록** 하고 참석하도록 허용하려면 Who 모든 사람에게 등록할 **수** 있습니다.
- 조직의 사용자만 웹 세미나를 **등록하고 참석할 수** 있도록 허용하려면 Who 조직의 **모든** 사용자에 등록할 수 있습니다.

기본적으로 **등록할 수 있는 Who** **모두** 로 설정됩니다. Teams 관리 센터에서 이 정책을 편집하려면 **MeetingsMeeting** >  정책으로 이동합니다.
