...
- [프레미스](#from-skype-for-business-online-with-on-premises-voice) 비즈니스용 Skype Cloud Connector Edition을 통해 비즈니스용 Skype 음성 기능을 제공하는 온라인 사용자입니다. TeamsOnly 사용자에게 PSTN 기능이 있는지 확인하려면 사용자의 Teams 직접 라우팅으로의 마이그레이션과 함께 사용자의 업그레이드를 조정해야 합니다.

- [온라인으로 비즈니스용 Skype](#from-skype-for-business-server-on-premises-with-enterprise-voice-to-direct-routing)PSTN 연결을 유지하는 Enterprise Voice 프레미스를 통해 프레미스에 있는 사용자입니다.  이 사용자를 Teams 사용자온-프레미스 비즈니스용 Skype 클라우드로 이동하고 사용자의 마이그레이션과 함께 직접 라우팅으로 이동하는 좌표를 조정해야 합니다. 

- [온라인으로 비즈니스용 Skype](#from-skype-for-business-server-on-premises-with-enterprise-voice-to-microsoft-calling-plan)Microsoft Enterprise Voice 프레미스를 사용하는 사용자입니다.  이 사용자를 마이그레이션하려면 Teams 사용자온-프레미스 비즈니스용 Skype 클라우드로 이동해야 합니다. 사용 가능한 지역에서 새 구독자 번호를 할당하는 Microsoft Calling Plan 또는 B로 해당 사용자의 전화 번호 포트를 A) 이동을 조정해야 합니다.
...
이 시나리오는 음성을 포함하는 가장 간단한 업그레이드 시나리오입니다. 

1. 사용자에게 라이선스가 할당되어 Teams 합니다. 기본적으로 라이선스 또는 라이선스를 Microsoft 365 Office 365 경우 Teams 활성화됩니다. 이전에 라이선스를 사용하지 않도록 Teams 경우 아무 작업도 필요하지 않습니다.
...
이 시나리오에서는 사용자가 이미 온라인에 비즈니스용 Skype 있습니다. 사용자의 PSTN 연결은 하이브리드 모드 또는 cloud Connector Edition에서 비즈니스용 Skype 서버 프레미스입니다. PSTN 기능을 사용하여 이러한 사용자를 TeamsOnly 모드로 마이그레이션하려면 직접 라우팅에 대해 사용자를 사용하도록 설정해야 합니다. 직접 라우팅을 사용하여 PSTN 트렁크는 SBC(On-Premises Session Border Controller)를 통해 직접 라우팅 서비스에 직접 연결됩니다.

기본 단계는 아래에 나열되어 있습니다.  1-4단계는 제안된 순서대로 나열되지만 순서대로 수행될 수 있습니다. 이러한 단계는 5단계 전에 완료해야 합니다.

1. 테넌트 전체 정책을 비즈니스용 Skype 모드 중 하나로 설정하는 경우 이전에 설명한 비즈니스용 Skype 섬 모드를 명시적으로 할당하여 기존 아일랜드 사용자를 할아버지 지정해야 합니다.

2. 직접 라우팅에 대한 테넌트 구성 직접 라우팅의 테넌트당 구성 [요약을 참조합니다.](#summary-of-per-tenant-configuration-of-direct-routing)

3. 원하는 경우 이러한 사용자에 대한 다양한 Teams 정책(예: TeamsMessagingPolicy, TeamsMeetingPolicy 등)을 구성합니다. 포시를 구성할 수 있습니다. 그러나 사용자가 업그레이드할 때 올바른 구성이 있는지 확인하려면 사용자가 TeamsOnly 모드로 업그레이드되기 전에 이러한 정책을 구성합니다.

4. 음성 마이그레이션을 위해 선택 사용자를 준비합니다. 
   - 필요한 경우 라이선스를 Teams 할당합니다.  사용자가 온라인온-프레미스 비즈니스용 Skype 이미 작동하고 있는 경우 사용자는 이미 비즈니스용 Skype 계획 2 및 Microsoft 전화 있습니다. 온라인 요금제 2 라이선스를 포함하여 비즈니스용 Skype 둘 다 사용하도록 설정합니다.  
...
이 시나리오에서는 사용자가 여전히 프레미스 비즈니스용 Skype 있습니다. 사용자의 PSTN 연결도 프레미스입니다. PSTN 기능을 사용하여 이 사용자를 TeamsOnly 모드로 마이그레이션하려면 직접 라우팅에 대해 사용자를 사용하도록 설정한 다음 사용자를 클라우드로 이동해야 합니다. 
 
기본 단계는 아래에 나열되어 있습니다. 1-5단계는 제안된 순서대로 나열되지만 순서대로 할 수 있습니다. 6단계 전에 1-5단계를 완료해야 합니다.
...
4. 원하는 경우 이러한 사용자에 대한 다양한 Teams 정책(예: TeamsMessagingPolicy, TeamsMeetingPolicy)을 구성합니다. 정책을 구성할 수 있습니다. 그러나 사용자가 업그레이드할 때 올바른 구성이 있는지 확인하려면 사용자가 TeamsOnly로 업그레이드되기 전에 이러한 정책을 구성합니다.

5. 필요한 Microsoft 365 Office 365 라이선스를 할당합니다.  사용자는 온라인 요금제 2와 Teams 비즈니스용 Skype 둘 다 전화 시스템. 온라인 비즈니스용 Skype 2를 사용하지 않도록 설정한 경우 다시 사용하도록 설정합니다.  

6. 사용자 업그레이드: 이러한 단계를 조정해야 합니다. 

   - 프레미스 비즈니스용 Skype 도구를 사용하여 -MoveToTeams 스위치로 Move-CsUser 실행합니다. -MoveToTeams 스위치를 지원하지 비즈니스용 Skype 서버 버전을 사용하는 경우 먼저 Move-CsUser 실행한 다음, 테넌트 원격 PowerShell 또는 관리 콘솔에서 TeamsOnly 모드를 Teams 할당합니다.
...
이 시나리오에서는 사용자가 여전히 프레미스 비즈니스용 Skype 있습니다. 사용자의 PSTN 연결도 프레미스입니다. PSTN 기능을 사용하여 이 사용자를 TeamsOnly 모드로 마이그레이션하려면 사용자를 클라우드로 이동하고 이전 통신사에서 해당 번호를 Microsoft 호출 계획으로 포트하거나 사용자에게 새 번호를 할당해야 합니다. 
...
3. 원하는 경우 이러한 사용자에 대한 다양한 Teams 정책(예: TeamsMessagingPolicy, TeamsMeetingPolicy 등)을 구성합니다. 정책을 구성할 수 있습니다. 그러나 사용자가 업그레이드할 때 올바른 구성이 있는지 확인하려면 사용자가 TeamsOnly로 업그레이드되기 전에 이러한 정책을 구성합니다. 

4. 필요한 Microsoft 365 Office 365 라이선스를 할당합니다.사용자는 온라인 요금제 2와 Teams 비즈니스용 Skype 둘 다 전화 시스템. 온라인 비즈니스용 Skype 2를 사용하지 않도록 설정한 경우 다시 사용하도록 설정합니다.  
...
5. 사용자에 대한 전화 번호를 얻습니다. (자세한 내용은 조직의 [전화 번호 관리를 참조합니다.](./manage-phone-numbers-for-your-organization/manage-phone-numbers-for-your-organization.md)
   ...
   - 번호를 다시 사용할 경우 이동통신사에 포터링 요청을 제출합니다.  
...
6. 사용자를 업그레이드하고 필요한 경우 LineUri를 할당합니다. 프레미스 비즈니스용 Skype 도구를 사용하여 -MoveToTeams Move-CsUser 실행합니다.  
    ...
    - Microsoft에서 새 번호를 사용하는 경우 Set-CsPhoneNumberAssignment를 사용하여 사용자가 온라인으로 이동한 후 사용자에 대한 LineUri를 변경해야 합니다.  

## <a name="summary-of-per-tenant-configuration-of-direct-routing"></a>직접 라우팅의 테넌트당 구성 요약 

1. 이 목록을 검토하여 SBC(세션 테두리 컨트롤러)가 직접 라우팅에서 [지원되는지 확인](direct-routing-border-controllers.md) 또한 올바른 버전의 펌웨어가 있는지 확인합니다.  
