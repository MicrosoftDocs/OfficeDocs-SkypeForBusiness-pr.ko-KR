...
다음 단계를 수행하세요.
- [CQD Teams를 다운로드하여 자동 전화 교환 &](https://aka.ms/TAPAACQAnalytics) 기록 보고서 서식 파일을 호출하고 컴퓨터의 디렉터리에 저장합니다.

- 템플릿을 두 번 클릭하면 Power BI Desktop이 실행됩니다.
...
## <a name="data-latency-any-aa--cq-analytics"></a>데이터 대기 시간 AA & CQ 분석
...
|AADirectorySearchMethod                 |string                   |호출에 사용되는 검색 방법:<br>§ abs_search_dtmf<br>§ abs_search_extension<br>§ abs_search_name|
...
|이름                                      |설명                            |
|:---------------------------------------|:--------------------------------------|
|수신 전화 원본                    |내부/외부 호출 원본에 의해 호출 배포      |
...
- 현재 통화 큐 및 자동 전화 걸기에는 통화 큐/자동 전화 요청 이름 대신 리소스 계정 ID가 표시됩니다.  자동 전화 걸기 또는 통화 큐에 대한 모든 트래픽을 표시하려면 자동 전화 걸기 또는 통화 큐에 할당된 모든 리소스 계정을 선택해야 합니다.

- 현재, 호출 큐/자동 전화 연결 데이터는 최종 사용자 식별 정보로 간주되어 데이터 개인 정보 보존 정책이 적용되어 28일 동안의 기록만 대시보드에서 사용할 수 있습니다.
