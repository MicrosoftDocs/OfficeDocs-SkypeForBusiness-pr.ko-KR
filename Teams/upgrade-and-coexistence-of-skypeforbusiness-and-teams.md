...
<sup>1</sup> 2017 이전에 출시 된 이전 비즈니스용 Skype 클라이언트는 TeamsUpgradePolicy를 준수 하지 않습니다. Office 채널에서 사용할 수 있는 최신 비즈니스용 Skype 클라이언트를 사용 하 고 있는지 확인 합니다.
...
또는 관리자가 SfBWithTeamsCollabAndMeetings 모드를 사용 하 여 비즈니스용 Skype에서 채팅 및 통화 기능을 종료 하면서 모임 예약을 먼저 팀으로 전환할 수 있습니다. 이 모드를 사용 하 여 조직에서 모임에 대 한 팀으로 전환 합니다. 예를 들어, 사용자가 아직 기존 PSTN 기능을 마이그레이션할 준비가 되어 있지 않은 경우에는 현재 Teamonly 모드로 이동할 준비가 되지 않은 경우입니다. 이 전환 시나리오를 [먼저 모임](meetings-first.md)이라고 합니다.
