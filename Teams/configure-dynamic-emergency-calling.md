...
ms.locfileid: "69814360"
---
# <a name="plan-and-configure-dynamic-emergency-calling"></a>동적인 긴급 전화 계획 및 구성 

Microsoft 통화 플랜, 운영자 연결, Teams Phone Mobile 및 직접 라우팅에 대한 동적 긴급 통화는 Teams 클라이언트의 현재 위치에 따라 긴급 통화를 구성 및 라우팅하고 보안 담당자에게 알리는 기능을 제공합니다.  

테넌트 관리자가 정의하는 네트워크 토폴로지(긴급 주소와 연결된 네트워크 요소)를 기반으로 Teams 클라이언트는 LIS(위치 정보 서비스)에 대한 요청에서 네트워크 연결 정보를 제공합니다. 일치하는 항목이 있는 경우 LIS는 클라이언트에 위치를 반환합니다.

Teams 클라이언트에는 긴급 통화의 일부로 위치 데이터가 포함됩니다. 그런 다음 응급 서비스 공급자가 이 데이터를 사용하여 적절한 PSAP(공공 안전 응답 지점)를 확인하고 PSAP 디스패처가 호출자의 위치를 가져올 수 있도록 해당 PSAP로 호출을 라우팅합니다.  

동적 긴급 호출의 경우 다음이 발생해야 합니다.

1. 네트워크 관리자는 네트워크 설정 및 LIS를 구성하여 네트워크/긴급 위치 맵을 만듭니다.

2. 시작 및 주기적으로 시작하는 동안 또는 네트워크 연결이 변경되면 Teams 클라이언트는 네트워크 연결 정보가 포함된 위치 요청을 네트워크 설정 및 LIS로 보냅니다.

   - 네트워크 설정 사이트 일치가 있는 경우 긴급 통화 정책이 해당 사이트에서 Teams 클라이언트로 반환됩니다. (정책에 대한 자세한 내용은 [긴급 정책 구성](#configure-emergency-policies)을 참조하세요.)

   - LIS 일치 항목이 있는 경우 Teams 클라이언트가 연결된 네트워크 요소의 긴급 위치가 Teams 클라이언트로 반환됩니다. 일치는 일치하는 첫 번째 결과가 반환되는 다음 순서로 수행됩니다.
       - Wap
       - 이더넷 스위치/포트
       - 이더넷 스위치
       - 서브넷
...
적절한 PSAP(공공 안전 응답 지점)로 자동 라우팅하는 기능은 Teams 사용자의 사용 국가에 따라 달라집니다.

Microsoft 통화 플랜, 운영자 연결 파트너 및 Teams Phone Mobile 파트너에는 미국 및 캐나다의 사용자를 위한 동적 긴급 라우팅 서비스가 포함됩니다.

그러나 직접 라우팅의 경우 긴급 통화를 라우팅하고 파트너 연결을 위해 추가 구성이 필요합니다. 관리자는 온라인 PSTN 게이트웨이 개체에서 PidfloSupported 매개 변수를 True로 설정하여 긴급 호출을 라우팅하는 PSTN 게이트웨이가 나가는 INVITE에 위치 정보를 추가하도록 구성되었는지 확인해야 합니다. 또한 관리자는 ERS(긴급 라우팅 서비스) 공급자(미국 및 캐나다)에 대한 연결을 구성 **하거나** ELIN(긴급 위치 식별 번호) 애플리케이션에 대한 SBC(세션 테두리 컨트롤러)를 구성해야 합니다. ERS 공급자에 대한 자세한 내용은 [직접 라우팅에 대해 인증된 세션 테두리 컨트롤러를 참조하세요](direct-routing-border-controllers.md).
...
> - Teams 앱 버전 1449/1.0.94.202111010101 이상이 포함된 Teams 전화.

> [!NOTE]
> 보안 데스크 알림을 포함한 동적 긴급 통화는 Teams 웹 클라이언트에서 지원되지 않습니다. 사용자가 Teams 웹 클라이언트를 사용하여 PSTN 번호를 호출하지 못하도록 하려면 Teams 통화 정책을 설정하고 **웹 PSTN 통화 허용** 설정을 해제할 수 있습니다. 자세한 내용은 [Teams의 통화 정책](teams-calling-policy.md) 및 [Set-CsTeamsCallingPolicy를 참조하세요](/powershell/module/skype/set-csteamscallingpolicy?view=skype-ps). 
...
- 운영자 연결 및 Teams 전화 모바일 사용자는&mdash;운송업체가 고객의 인벤토리에 업로드할 때 번호에 할당된 기능에 따라 달라집니다.
...
- 일치 항목을 찾을 수 없는 경우 긴급 주소를 수동으로 만들 수 있습니다. PIN 삭제 기능을 사용하여 이 작업을 수행할 수 있습니다. 

> [!NOTE]
> 몇 년 이상 된 긴급 주소는 네트워크 식별자에 할당할 수 없습니다. 이전 주소를 다시 만들어야 합니다.

Microsoft Teams 관리 센터에서 또는 PowerShell을 사용하여 긴급 주소를 추가하고 할당합니다. 자세한 내용은 [조직의 긴급 위치 추가](add-change-remove-emergency-location-organization.md) 및 [사용자에 대한 긴급 위치 할당을](assign-change-emergency-location-user.md) 참조하세요.
...
네트워크 설정에는 서브넷 컬렉션을 포함하는 사이트가 포함되며, 이러한 사이트는 사용자에게 동적 정책 할당에만 사용됩니다. 예를 들어 집이나 다른 Microsoft 위치에서 로밍하는 모든 사용자가 Redmond와 관련된 긴급 번호, 라우팅 및 보안 데스크로 구성되도록 긴급 통화 정책 및 긴급 통화 라우팅 정책을 "Redmond 사이트"에 할당할 수 있습니다.  
...
네트워크 설정의 일부 변경 내용(예: 새 주소, 네트워크 식별자 등)이 전파되고 Teams 클라이언트에 제공되려면 다소 시간이 걸릴 수 있습니다(최대 몇 시간).  

> [!Note]
> 서브넷은 LIS에서 정의할 수도 있으며 긴급 위치와 연결할 수도 있습니다.  LIS 서브넷은 클라이언트에 할당된 서브넷 IP 범위와 일치하는 네트워크 ID로 정의해야 합니다. 예를 들어 10.10.10.150/25 클라이언트 IP/마스크의 네트워크 ID는 10.10.10.128입니다. 자세한 내용은 [TCP/IP 주소 지정 및 서브넷 기본 사항 이해를 참조하세요](/troubleshoot/windows-client/networking/tcpip-addressing-and-subnetting).

> [!Important]
> 네트워크 구성 설정 조회는 Teams 클라이언트의 원본 IP 주소를 수정하는 클라우드 프록시 서비스 배포에서 지원되지 않습니다.



**통화 플랜, 운영자 연결 및 Teams 전화 모바일 사용자의 경우:**
...
>[!Important]
>서브넷이 네트워크 사이트의 일부로 사용되는 경우 동적 위치를 렌더링하려면 위치 정보 서비스에서 다시 정의해야 합니다.
...
- **긴급 통화 라우팅 정책 – 직접 라우팅에만 적용됩니다**. 이 정책은 긴급 번호, 원하는 경우 숫자당 마스크 및 숫자당 PSTN 경로를 구성합니다. 이 정책을 사용자, 네트워크 사이트 또는 둘 다에 할당할 수 있습니다. 자세한 내용은 [직접 라우팅에 대한 긴급 통화 라우팅 정책 관리를 참조하세요](manage-emergency-call-routing-policies.md).  
   ...
   (통화 플랜, 운영자 연결 및 Teams 전화 모바일 사용자는 Microsoft 365 또는 Office 365 사용 위치에 따라 국가의 긴급 번호로 긴급 통화를 자동으로 사용하도록 설정됩니다.)

- **긴급 통화 정책 - 통화 플랜, 운영자 연결, Teams 전화 모바일 및 직접 라우팅에 적용됩니다.** 이 정책은 긴급 전화를 걸 때 보안 데스크 알림 환경을 구성합니다. 알릴 사람과 알림을 받는 방법을 설정할 수 있습니다. 예를 들어 조직의 보안 데스크에 자동으로 알리고 긴급 통화에서 수신 대기하도록 합니다.  이 정책은 사용자 또는 네트워크 사이트 또는 둘 다에 할당할 수 있습니다. 자세한 내용은 [Teams에서 긴급 통화 정책 관리를 참조하세요](manage-emergency-calling-policies.md).

## <a name="enable-users-and-sites"></a>사용자 및 사이트 사용

긴급 통화 라우팅 정책 및 긴급 통화 정책을 사용자 및 사이트에 할당할 수 있습니다. 긴급 통화 라우팅 정책은 직접 라우팅에만 적용됩니다. (통화 플랜, 운영자 연결 또는 Teams 전화 모바일 사용자에게 이 정책을 할당할 수 있지만 정책은 적용되지 않습니다.)
...
"Contoso New York 긴급 통화 라우팅"이라는 정책을 사이트 1에 할당하려면 다음 명령을 사용합니다.
...
- **미국 또는 캐나다의 통화 플랜, 운영자 연결 및 Teams Phone Mobile 사용자는** 미리 정의된 테스트 긴급 번호 933을 사용하여 긴급 통화 구성의 유효성을 검사할 수 있습니다. 이 번호는 봇으로 라우팅됩니다. 그러면 호출자 전화 번호(통화 회선 ID), 긴급 주소 또는 위치 및 통화가 자동으로 PSAP로 라우팅되는지 또는 먼저 차단되는지 여부를 다시 에코합니다.
...
| 국방부 | -Teams 데스크톱에서 사용 가능 <br>-Teams 모바일 클라이언트에서 사용 가능 <br>-Teams 휴대폰에서 보류 중 |
