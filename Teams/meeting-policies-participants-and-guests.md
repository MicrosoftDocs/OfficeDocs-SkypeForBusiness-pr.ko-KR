...
|**초대된 사용자만**    |초대된 사용자와 모임 이끌이만 로비에서 기다리지 않고 직접 모임에 참가할 수 있습니다. 조직 내의 인증된 사용자, 게스트, 신뢰할 수 있는 조직의 사용자 및 익명 사용자를 포함한 다른 모든 사용자는 로비에서 기다려야 합니다. Teams 클라이언트 모임 옵션 페이지에서 "초대할 사람"로 표시됩니다. 메일 그룹의 일부로 추가된 사용자는 로비를 통과해야 합니다.      |
...
- 모임 이끌이가 **모임에서 모든** **사용자를 위해** 채팅을 켜거나 **익명 사용자를 제외한 모든 사용자에 대해** 채팅을 켜면 사용자의 개별 정책이 적용되고 설정된 **모든** 사용자가 모임에서 채팅할 수 없습니다.
- 이끌이가 **모임에서 채팅** 을 **모든 사용자에 대해 해제** 하도록 설정한 경우 이끌이의 정책이 적용되고 아무도 모임에서 채팅할 수 없습니다.
...
이 설정은 모임을 만들거나 이끌이가 업데이트할 때 적용됩니다. 기본적으로 이 설정은 꺼져 있습니다. [Q](/manage-qna-for-meetings)&A에 대해 자세히 알아보세요.
