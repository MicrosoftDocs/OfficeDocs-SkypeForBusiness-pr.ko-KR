---
title: 자동 전화 교환 및 통화 큐 Teams 계획
author: CarolynRowe
ms.author: crowe
manager: serdars
...
각 통화 큐에 대해 큐의 에이전트가 통화 수신을 옵트아웃할 수 있는지, Teams 현재 상태 표시에 따라 호출을 라우팅해야 하는지를 지정할 수 있습니다.
...
> [!NOTE]
> 리소스 계정은 로그인을 위해 사용하지 않도록 설정되며 계속 유지되어야 합니다. 채팅 및 현재 상태는 이러한 계정에 사용할 수 없습니다.
...
필요한 경우 각 큐에 대해 이러한 옵션의 조합을 사용할 수 있습니다. 메일 주소가 있는 그룹은 음성 메일에 사용할 수 있습니다. Teams 사용하면 에이전트 간의 공유 파일 스토리지 및 채팅, 음성 메일을 받을 수 있는 공통 사서함, LOB(기간 업무) 애플리케이션 또는 Power Apps 통합을 포함할 수 있는 확장 가능한 플랫폼 등 다양한 이점을 제공합니다.

구성을 시작하기 전에 큐에 호출 에이전트를 추가하는 전략을 선택하는 것이 좋습니다.

기존 자동 전화 교환 및 통화 큐 인프라가 있고 Teams 마이그레이션하는 경우 기존 전화 번호를 새 자동 전화 교환 및 통화 큐로 전송하는 계획이 필요합니다. 다른 공급자에서 번호를 이동하려면 [포트 순서](phone-number-calling-plans/port-order-overview.md) 를 만들어야 할 수 있습니다. 현재 서비스 중인 번호로 전환하기 전에 하나 이상의 새 전화 번호를 일시적으로 획득하고 자동 전화 교환 및 통화 큐 흐름을 테스트하는 것이 좋습니다.

*전화 회의 모드* 는 VOIP 호출 및 PSTN 호출을 에이전트에 연결하는 데 걸리는 시간을 크게 줄이는 Teams 통화 큐의 옵션입니다. 회의 모드가 작동하려면 통화 큐의 에이전트가 다음 클라이언트 중 하나를 사용해야 합니다.
...
2. 만들려는 각 리소스 계정에 대한 [전화 시스템 - 가상 사용자 라이선스](teams-add-on-licensing/virtual-user.md)를 가져옵니다. 이러한 라이선스는 무료이므로 나중에 리소스 계정을 변경하기로 결정한 경우 몇 가지 추가 라이선스를 추가로 받는 것이 좋습니다.

3. 만들려는 각 자동 전화 교환 및 호출 큐에 대한 [리소스 계정을 만듭니](manage-resource-accounts.md)다. 각 계정에 전화 시스템 - 가상 사용자 라이선스 및 선택적으로 서비스 번호를 할당합니다.
...
7. 내선 번호로 전화를 걸도록 허용하려는 경우 사용자의 확장 번호를 Azure Active Directory 프로필에 추가했는지 확인합니다.
...
[통화 큐 만들기 - 소규모 비즈니스 자습서](business-voice/create-a-phone-system-call-queue-smb.md)

[자동 전화 교환 설정 - 소규모 비즈니스 자습서](business-voice/create-a-phone-system-auto-attendant-smb.md)
