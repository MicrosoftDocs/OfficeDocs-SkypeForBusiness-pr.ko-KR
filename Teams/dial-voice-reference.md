...
전화 걸기 이름은 디렉터리 검색으로도 알려진 자동 참석자 기능입니다. 이 기능을 사용하면 자동 참석자에게 음성(음성 인식) 또는 해당 전화 키패드(DTMF) 응답을 사용하여 전체 또는 부분 이름을 입력하여 회사의 디렉터리를 검색하고, 해당 사람을 찾은 다음 통화를 전송할 수 있습니다. 자동 참석자에서 통화 흐름 설정을 구성할 때 이름에 따라 [다이얼을 설정합니다.](create-a-phone-system-auto-attendant.md#call-flow)

## <a name="searching-for-users"></a>사용자 검색

전화 걸기 이름을 사용하여 전화 접속을 하고자 하는 사용자는 전화 번호가 필요하거나 통화 요금제가 할당되지는 않지만 사용자가 전화 번호를 Enterprise Voice 사용할 비즈니스용 Skype 서버 **합니다.** 이름별로 전화를 걸면 다국적 조직에 대해 다른 국가 또는 지역에서 호스트되는 Microsoft Teams 사용자를 찾아 전송할 수도 있습니다. 관련 구성이 필요한 경우 자동 참석자에서 전화 걸기 이름을 명시적으로 사용하도록 설정합니다.

전화 걸기 확장은 디렉터리 검색의 일부인 자동 참석자 기능입니다. 이 기능을 사용하면 자동 참석자를 호출하는 사용자가 음성(음성 인식) 또는 DTMF(전화 키패드) 응답을 사용하여 도달하려는 사용자의 전화 확장을 입력한 다음 해당 사용자에게 전화를 전송할 수 있습니다. 전화 걸기를 사용하여 전화 접속을 하고자 하는 사용자는 전화 번호가 필요하거나 전화 요금제가 할당되지는 않지만 전화 번호가 Enterprise Voice 사용자에 대해 비즈니스용 Skype 서버 **합니다.** 또한 사용자에 대해 적절히 구성된 다이얼 플랜이 필요합니다. 확장을 통해 전화를 걸면 다국적 조직에 대해 다른 국가 또는 지역에서 호스팅되는 Microsoft Teams 사용자들에게 전화를 찾아 전송할 수도 있습니다. 관련 구성이 필요한 경우 자동 참석자에서 전화 걸기 확장을 명시적으로 사용하도록 설정합니다.
