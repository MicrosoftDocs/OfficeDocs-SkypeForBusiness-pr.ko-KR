...
이름으로 전화 걸기 기능은 디렉터리 검색으로도 알려진 자동 전화 접속 기능입니다. 이를 통해 자동 전화 회의에 전화를 걸면 음성(음성 인식) 또는 DTMF(전화 키패드) 응답을 사용하여 전체 또는 부분 이름을 입력하여 회사 디렉터리를 검색하고, 해당 사람을 찾은 다음, 통화를 전송할 수 있습니다. 자동 전화 접속에서 통화 흐름 설정을 구성할 때 이름으로 다이얼을 [설정합니다.](create-a-phone-system-auto-attendant.md#call-flow)
