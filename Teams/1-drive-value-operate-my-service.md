...
이 문서에서는 조직의 클라우드 음성 서비스를 성공적으로 운영하기 위한 요구 사항에 대한 개요를 제공합니다. 클라우드 음성 서비스를 올바르게 운영하여 조직에 고품질의 신뢰할 수 있는 환경을 제공하고 있는지 확인할 수 있습니다.
...
<tr><td><img src="media/audio_conferencing_image9.png" alt="An icon depicting the next steps"/><br/>다음 단계</td><td><ul><li>운영 가이드를 전체적으로 검토합니다.</li><li>클라우드 음성 워크로드의 품질과 안정성을 제공하기 위해 조직의 목표에 부합하는 운영 전략을 구현합니다.</li><li>[통화 품질 모니터링](monitor-call-quality-qos.md)을 검토합니다.</li><li> 클라우드 음성 배포가 최대 기능에서 작동하는지 확인하기 위해 정기적으로 환경 품질 검토를 수행하는 운영 전략을 구현합니다.</li></ul></td></tr>
</table>
...
Microsoft Teams는 Microsoft 365 또는 Office 365 기술을 결합하여 팀워크를 위한 허브를 제공합니다. 예를 들면 다음과 같습니다.
...
Microsoft Teams 서비스의 전반적인 상태를 이해하여 서비스에 영향을 주는 모든 이벤트를 조직의 다른 사람에게 사전에 경고할 수 있도록 하는 것이 중요합니다. 앞에서 설명한 대로 Teams는 Azure Active Directory, Exchange Online, SharePoint Online 및 비즈니스용 OneDrive 같은 다른 Microsoft 365 또는 Office 365 서비스에 의존합니다. 이 때문에 종속 서비스의 상태를 모니터링하는 것이 똑같이 중요합니다.
...
| Microsoft 365 또는 Office 365 관리자 앱                  | 모바일 디바이스에 푸시 알림을 제공합니다.  | 이동 중 서비스 인시던트에 대한 알림을 받아야 할 때 사용합니다.                  |
| Microsoft System Center               | Microsoft System Center와 통합           | 고급 모니터링 기능 및 알림 지원이 필요한 경우 를 사용합니다.                       |
| Microsoft 365 또는 Office 365 Service Communications API | Microsoft 365 또는 Office 365 서비스 상태에 프로그래밍 방식으로 액세스합니다.   | 타사 모니터링 도구와 통합해야 하거나 고유한 솔루션을 빌드하려는 경우에 사용합니다. |
...
Microsoft 365 또는 Office 365 관리자 앱은 Apple iOS, Android 및 Windows(PC 및 모바일)에서 사용할 수 있습니다. 앱은 서비스 관리자에게 서비스 상태 및 향후 변경 내용에 대한 정보를 제공합니다. 앱은 권고가 게시된 직후에 경고할 수 있는 푸시 알림을 지원합니다. 이렇게 하면 상태, 상태 및 서비스에 대한 향후 변경 내용을 최신 상태로 유지할 수 있습니다. 알림 지원을 통해 관리자에게 권장되는 모니터링 도구가 됩니다. 자세한 내용은 다음을 참조하세요.
...
Office 365 Service Communications API를 활용하여 Office 365 서비스 상태 및 변경 내용에 프로그래밍 방식으로 액세스하여 서비스 상태 및 향후 변경 내용을 모니터링할 수 있습니다. 이 API를 사용하여 사용자 고유의 모니터링 도구를 만들거나 기존 모니터링 도구를 Office 365 서비스 통신에 연결하여 환경을 모니터링하는 방법을 간소화할 수 있습니다. 자세한 내용은 [엔터프라이즈 개발자를 위한 Office 365 참조하세요](https://developer.microsoft.com/office).
...
변경 관리의 첫 번째 단계는 Teams에 대해 계획된 변경 내용을 모니터링하는 것입니다. 이러한 변경 내용을 모니터링하는 가장 좋은 방법은 현재 개발 중이거나 고객에게 배포되거나 완전히 시작된 기능을 나열하는 [Office 365 로드맵](https://products.office.com/business/office-365-roadmap)입니다. 제공된 필터를 사용하여 Teams 관련 기능을 검색하거나 추가 분석을 위해 Excel 파일에 로드맵을 다운로드할 수 있습니다. 각 기능에 대해 로드맵은 예상되는 릴리스 날짜와 함께 간단한 설명을 제공합니다.
...
사용 가능한 리소스에 익숙해지고 변경 사항을 모니터링할 수 있는 소유자를 할당해야 합니다.
...
초기 파일럿이 시작된 후에는 실제 Teams 사용량을 측정하기 위한 정기적인 주기를 설정하는 것이 중요합니다. 이를 통해 조직은 실제 사용량이 구상 단계에서 예측한 사용량과 어떻게 일치하는지에 대한 인사이트를 얻을 수 있습니다. 이 섹션에서는 Teams 사용에 중점을 두지만 전체 Office 365 사용량을 측정하고 평가하기 위한 광범위한 노력의 일환이어야 합니다.
...
사용량이 예상과 다른 경우 배포 문제 또는 채택 계획이 제대로 실행되지 않거나 다른 문제 때문일 수 있습니다. 사용량이 적은 실제 이유에 따라 서비스 관리자는 관련 팀과 협력하여 사용 장벽을 제거해야 합니다.

### <a name="measuring-usage-with-the-microsoft-365-admin-center"></a>Microsoft 365 관리 센터 사용량 측정

Teams의 사용량 현황 데이터는 보고 대시보드에서 사용할 수 있습니다. Teams 사용량 데이터는 세 가지 보고서에서 찾을 수 있습니다. 첫 번째 보고서는 Office 365 다양한 서비스를 사용하여 사용자가 통신하고 공동 작업하는 방법에 대한 제품 간 보기를 제공합니다. 이 보고서는 [활성 사용자 보고서 Office 365](https://support.office.com/article/Office-365-Reports-in-the-Admin-Center-Active-Users-FC1CF1D0-CD84-43FD-ADB7-A4C4DFA8112D) 여기에서 찾을 수 있습니다.
...
또한 **보고서 읽기** 권한자 역할은 보고서에 액세스해야 하지만 관리자 수준 권한이 필요한 작업을 수행하지 않는 사용자에게 사용할 수 있습니다. 이 역할을 할당하여 이해 관계자인 모든 사람에게 사용 현황 보고서를 제공하여 채택을 모니터링하고 추진합니다. 사용 가능한 다양한 역할에 대한 자세한 내용은 [Office 365 관리자 역할 정보를 참조하세요](https://support.office.com/article/About-Office-365-admin-roles-da585eea-f576-4f55-a1e0-87090b6aaa9d).
...
1.  품질 환경 검토를 수행하여 품질 유효성을 검사합니다(자세한 내용은 [Teams의 통화 품질 개선 및 모니터링](monitor-call-quality-qos.md) 참조).
...
다음 섹션에 설명된 NPS(Net Promotor Score)라는 사용자 감정을 평가하는 업계 전체 접근 방식을 사용할 수도 있습니다.

### <a name="nps"></a>Nps 

NPS(순 발기인 점수)는 업계 전반에 걸친 고객 충성도 메트릭이며 사용자 감정을 평가하는 데 사용할 수 있는 좋은 방법입니다. NPS는 "동료에게 Teams를 추천할 가능성이 얼마나 됩니까?"라는 두 가지 질문을 하고 자유형 질문인 "왜?"를 질문하여 계산할 수 있습니다.

NPS는 –100에서 100까지의 인덱스로, 회사의 제품 또는 서비스를 추천하려는 고객의 의지를 측정합니다. NPS는 이메일 또는 기타 전자 수단을 통해 사용자에게 전달되는 익명 설문 조사를 기반으로 합니다. NPS는 공급자와 소비자 간의 충성도를 측정합니다. 이는 사용자에게 1에서 10까지의 경험을 평가하도록 요청하는 하나의 질문으로 구성되며 추가 설명을 제공하는 옵션으로 구성됩니다. 그러면 사용자는 다음 등급에 따라 분류됩니다.
...
많은 핵심 계획 요소는 Microsoft Teams 서비스에 대한 고품질의 효율적인 경로를 보장하기 위해 네트워크 인프라를 최적화, 오른쪽 크기 조정 및 수정하는 데 사용됩니다. 계획 작업 및 요구 사항은 [네트워크 준비](3-envision-evaluate-my-environment.md#network-readiness) 지침에서 다룹니다. 업그레이드, 확장 또는 기타 비즈니스 요구 사항으로 인해 시간이 지남에 따라 네트워크가 발전하는 경우가 많습니다. 네트워크 계획 활동에서 Teams에 대한 요구 사항을 고려해야 합니다.
...
사용자 환경 개선에 가장 큰 영향을 미치는 주요 영역에서 수정 지침을 평가하고 제공하는 활동을 설명하는 [Teams의 통화 품질 개선 및 모니터링](monitor-call-quality-qos.md)을 참조하세요. 이 문서에서 제공하는 지침은 채택 및 영향을 최대화하기 위해 오디오에 중점을 두고 각 영역을 보고하고 조사하는 기본 도구로 CQD를 사용하는 데 중점을 둡니다. 오디오 환경을 개선하기 위해 네트워크에 대한 모든 최적화는 비디오 및 데스크톱 공유의 개선으로 직접 변환됩니다.

일찍부터 품질 챔피언을 지명하는 것이 좋습니다. 지명 된 후, 그들은 [모니터 통화 품질](monitor-call-quality-qos.md) 콘텐츠 및 관련 교육 자료를 숙지하기 시작해야합니다.
...
1.  **서비스 상태 유효성 검사:** 사용자에게 발생할 수 있는 문제는 Teams 서비스 또는 종속 서비스에 부정적인 영향을 미치는 이벤트와 관련이 있을 수 있습니다. 첫 번째 단계로 활성 서비스 문제가 없는지 확인하는 것이 좋습니다. [Office 365 서비스 상태를 확인하는 방법을 참조하세요](https://support.office.com/article/How-to-check-Office-365-service-health-932AD3AD-533C-418A-B938-6E44E8BC33B0).
...
5.  **Microsoft 지원 문의:** 온라인 또는 전화로 Teams 관련 문제에 대해 Microsoft 지원 문의할 수 있습니다. 자세한 내용은 [비즈니스 제품에 대한 고객 지원 문의를 참조하세요](/microsoft-365/admin/contact-support-for-business-products).
    프리미어 고객의 경우 [Microsoft Teams(프리미어 고객)에 대한 고객 지원 문의](https://support.microsoft.com/premier/contacts)의 지침에 따라 지원 요청을 시작할 수 있습니다.
...
#### <a name="common-tasks-for-audio-conferencing"></a>오디오 회의의 일반적인 작업
...
조직이 성장하거나 계약을 맺을 때 현재와 미래의 요구 사항에 대한 라이선스를 계획하는 것이 중요합니다. 클라우드 음성 기능 [전화 시스템](here-s-what-you-get-with-phone-system.md) 및 [오디오 회의에](/skypeforbusiness/skype-for-business-and-microsoft-teams-add-on-licensing#requirements-for-audio-conferencing) 대한 라이선스 외에도 기본 Teams 라이선스가 있습니다.

Teams의 경우 전화 시스템 라이선스에는 연결된 [통화 플랜](calling-plan-landing-page.md) 라이선스가 필요합니다. 플랜 라이선스를 호출하면 국내 및/또는 국제 전화 통화를 하고 받을 수 있습니다. 이러한 계획은 사용량 기반이며 연결된 분 풀이 있습니다. [통신 크레딧을](what-are-communications-credits.md) 프로비전하면 서비스가 부족해지지 않습니다.

오디오 회의를 통해 유료 전화 접속 회의 및 국내 전화 접속 회의 서비스를 사용할 수 있습니다. 무료 전화 접속 회의 또는 국내 이외의 전화 접속 시나리오로 인해 [통신 크레딧](what-are-communications-credits.md) 이 필요한 추가 요금이 발생할 수 있습니다.

통신 크레딧은 통화 플랜 및 오디오 회의 라이선스를 모두 보완할 수 있습니다. 통화 플랜 라이선스와 통신 크레딧은 모두 사용량 기준이므로 그에 따라 모니터링하고 프로비전해야 합니다.
...
Teams에서 번호를 획득하는 방법에는 두 가지가 있습니다. 다른 공급자의 전화 번호를 이식하거나 Microsoft의 번호 인벤토리에서 직접 번호를 프로비전할 수 있습니다. 두 방법 모두 [사용자의 전화 번호 가져오기](getting-phone-numbers-for-your-users.md)에 설명되어 있습니다.

Microsoft의 번호 인벤토리에서 프로비전할 수 있는 전화 번호 수에는 제한이 있습니다. 제한은 [몇 개의 전화 번호를 받을 수 있나요?](how-many-phone-numbers-can-you-get.md)에 자세히 설명된 여러 가지 요인에 따라 결정됩니다.
제한은 무료 서비스 번호, 유료 서비스 번호 및 구독자(사용자) 번호와 같은 숫자 유형에 따라 달라집니다. 각각에는 자체 제한이 있으며 독립적으로 관리되어야 합니다. 제한에 가까워지거나 한도에 도달한 경우 한도에 증분을 신청할 수 있습니다. 이 프로세스는 이전 단락의 문서에 설명되어 있습니다.
...
[Teams의 통화 품질 개선 및 모니터링](monitor-call-quality-qos.md) 에는 아래 그림과 같이 사용자 환경 개선에 가장 큰 영향을 미치는 주요 영역에서 수정 지침을 평가하고 제공하는 일련의 활동이 포함되어 있습니다.

![환경 품질 검토 중에 검사할 영역의 다이어그램.](media/plan-my-service-management-image2.png "경험 품질 검토 중에 검사할 주요 영역: 오디오, 안정성 및 사용자 설문 조사 결과.")
...
-   최적화되지 않은 오디오 또는 기본 제공 오디오 디바이스 사용

-   문제가 있는 서브넷 또는 네트워크 디바이스

[Teams의 통화 품질 개선 및 모니터링](monitor-call-quality-qos.md)에 제공된 지침은 채택 및 영향을 극대화하기 위해 오디오에 중점을 두고 설명된 각 영역을 보고하고 조사하는 기본 도구로 CQD(통화 품질 대시보드) Online을 사용하는 데 중점을 둡니다. 오디오 환경을 개선하기 위해 네트워크에 대한 모든 최적화는 비디오 및 데스크톱 공유의 개선으로 직접 변환됩니다.

품질 챔피언을 조기에 지명하는 것이 좋습니다. 후보에 오른 후 [Teams의 통화 품질 개선 및 모니터링](monitor-call-quality-qos.md)에서 콘텐츠를 숙지해야 합니다.
