...
- enabler-strategic
description: 네트워크를 설정하고, Teams, 라이브 이벤트 기능 및 일정을 사용하도록 설정하고, 비디오 배포 솔루션을 포함하여 라이브 이벤트에 대해 설정합니다.
appliesto:
- Microsoft Teams
...
> [!NOTE]
> 조직의 사용자는 사용자 지정 정책을 만들고 할당하지 않는 한 전역(Org-wide default) 정책을 얻습니다. 기본적으로 전역 정책에서는 Teams 사용자에 게 실시간 이벤트 예약을 사용할 수 있습니다. live 캡션과 자막(기록)이 꺼져 있는 경우 조직의 모든 사용자가 실시간 이벤트에 참석할 수 있으며 녹음/녹화 설정은 항상 녹음/녹화로 설정됩니다.
...
|**라이브 이벤트 일정**     |이 기능을 켜면 조직 사용자가 팀에서 라이브 이벤트를 만들고 예약할 수 있습니다. 사용자가 외부 앱 이나 장치를 사용하여 생성된 라이브 이벤트를 예약하도록 하려면 추가 단계를 수행해야 합니다. 자세한 내용은 [사용자가 외부 앱 또는장치를 사용하여 생성된 이벤트를 예약할 수 있도록 허용](#enable-users-to-schedule-events-that-were-produced-with-an-external-app-or-device)을 참조하세요.     |
|**참석자에 대한 전사** |이 설정은 팀에서 생성된 이벤트에만 적용할 수 있습니다. 이 기능을 켜면 이벤트 중에 실시간 이벤트 참석자가 실시간으로 캡션 및 자막을 볼 수 있습니다.         |
...
1. 조직의 사용자에 대해 Microsoft Stream을 활성화하세요. 스트림은 구독 또는 Microsoft 365 또는 Office 365 서비스로 사용할 수 있습니다. 스트림은 Business Essentials 또는 Business Premium 요금제에 포함되지 않습니다. 자세한 내용은 [Stream 라이센싱 개요](/stream/license-overview)를 참조하세요.
...
**Teams 외부에서 솔루션 구매 및 설정** Microsoft의 신뢰할 수 있는 비디오 제공 파트너를 활용하여 비디오 제공 확장에 대한 전문가의 도움을 받으세요. 비디오 전송 공급자를 사용하여 비디오 전송 공급자를 사용하도록 설정하려면 Teams SDN/eCDN 솔루션을 외부에서 구입하고 설정해야 Teams.
...
- **Kollective** 는 클라우드 기반의 스마트 피어링 배포 플랫폼은 기존 네트워크 인프라를 활용하여 다양한 형태(라이브 스트리밍 비디오, 주문형 비디오, 소프트웨어 업데이트, 보안 패치 등)로 컨텐츠를 보다 빠르고 안정적으로 제공하며 더 적은 대역폭을 사용합니다. Microsoft의 보안 플랫폼은 세계 최대 금융 기관의 신뢰를 받고 있으며 추가 하드웨어와 설정이 필요하지 않고 유지 관리가 쉽습니다. [자세히 알아보세요](https://kollective.com/microsoft-pilot/).
...
- 네트워크 최적화의 업계 표준인 **Riverbed는** 가속 솔루션을 Microsoft Teams 확장하고 있습니다.  이제 Microsoft 365 다른 선도적인 기업 SaaS 서비스와 함께 Teams 및 Stream을 포함한 365 트래픽을 자신 있게 가속화하여 어디서나 인력 생산성을 향상할 수 있습니다. Teams 및 Stream 가속은 Riverbed의 세계적 수준의 지원 및 지속적인 투자에 대한 모든 보장과 함께 사용할 수 있는 수월한 설정을 통해 사용할 수 있습니다.
