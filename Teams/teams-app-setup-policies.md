...
|---------|---------|
|![Teams 데스크톱 클라이언트의 앱 바.](media/app-setup-policies-desktop-app-bar.png).  |   ![Teams 모바일 클라이언트의 앱 바.](media/mobile-app-ui.png)      |

메시징 확장은 메시지 작성 영역의 맨 아래에서 사용할 수 있습니다.

앱을 고정하는 앱 설정 정책을 만들려면 다음 단계를 수행합니다.

1. [Microsoft Teams 관리 센터에 로그인합니다](https://admin.teams.microsoft.com).
...
1. **사용자 고정을** 켭니다.
   ...
   > [!NOTE]
   > **사용자 고정** 설정은 Microsoft 365 GCC(Government Community Cloud) 환경(GCC, GCC High 및 DoD)의 Teams 관리 센터에서 사용할 수 있지만 현재는 적용되지 않습니다.
...
1. Teams 관리 센터에 로그인하고 **Teams 앱** > **설정 정책에 액세스합니다**.
...
>[!NOTE]
> 이 절차를 사용하면 구성 가능한 탭이 있는 사용자 지정 앱을 설치할 수 없습니다.
...
전역 정책에서 설정을 편집하여 원하는 앱을 포함합니다. 조직의 여러 사용자 그룹에 대해 Teams를 사용자 지정하려면 하나 이상의 사용자 지정 정책을 만들고 할당합니다.
...
앱 설정 정책을 통해 모든 앱을 Teams에 고정할 수 있는 것은 아닙니다. 일부 앱은 이 기능을 지원하지 않을 수 있습니다. 고정할 수 있는 앱을 찾으려면 **고정된 앱 추가** 창에서 앱을 검색합니다. 개인 범위(정적 탭) 및 봇이 있는 탭은 Teams 데스크톱 클라이언트에 고정할 수 있으며 이러한 앱은 **고정된 앱 추가** 창에서 사용할 수 있습니다.
...
교육용 Teams 통화 앱을 사용할 수 없습니다. 새 사용자 지정 앱 설정 정책을 만들면 통화 앱이 앱 목록에 표시됩니다. 그러나 앱은 Teams 클라이언트에 고정되지 않으며 교육용 Teams 사용자에게 Teams의 통화 앱이 표시되지 않습니다.
...
사용자에 대해 고정된 모든 앱을 보려면 설치된 앱 수 및 Teams 클라이언트 창의 크기에 따라 다음을 수행해야 할 수 있습니다.

|Teams 데스크톱 클라이언트 |Teams 모바일 클라이언트 |
|---------|---------|
|Teams 측면의 앱 표시줄에서 ...를 선택합니다. **더 많은 앱**.| Teams 아래쪽에 있는 앱 바에서 위로 살짝 밉니다.|
...
#### <a name="my-organization-built-a-custom-teams-app-and-published-it-either-to-appsource-or-the-tenant-app-catalog-but-the-app-icon-isnt-displayed-as-expected-when-the-app-is-pinned-to-the-app-bar-in-teams-how-do-i-fix-it"></a>조직에서 사용자 지정 Teams 앱을 빌드하고 AppSource 또는 테넌트 앱 카탈로그에 게시했지만 앱이 Teams의 앱 표시줄에 고정될 때 앱 아이콘이 예상대로 표시되지 않습니다. 어떻게 할까요? 해결하시겠습니까?
