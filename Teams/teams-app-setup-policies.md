...
관리자는 앱 설정 정책을 사용하여 앱을 설치 및 고정하여 조직에서 가장 자주 사용하는 앱을 홍보하고 사용자가 사용자 지정 앱을 Teams 업로드할지 여부를 결정할 수 있습니다.

- **앱 고정:** 앱 설정 정책을 사용하면 고정할 앱을 선택하고, Teams 앱 모음 또는 작성 메시지 영역에서 사용자에게 표시되는 순서를 설정하고, 사용자가 자신의 앱을 고정할 수 있는지 여부를 제어할 수 있습니다. 자세한 내용은 [앱 고정](#pin-apps)을 참조하세요.
...
앱을 고정하면 타사 또는 조직의 개발자가 빌드한 앱을 포함하여 조직의 사용자가 필요로 하는 앱을 선보일 수 있습니다.
...
앱은 Teams 데스크톱 클라이언트의 왼쪽과 Teams 모바일 클라이언트(iOS 및 Android)의 맨 아래에 있는 막대인 앱 바에 고정됩니다.

|Teams 데스크톱 클라이언트  |Teams 모바일 클라이언트 |
|---------|---------|
|![Teams 데스크톱 클라이언트입니다.](media/app-setup-policies-desktop-app-bar.png).  |   ![Teams 모바일 클라이언트](media/mobile-app-ui.png)      |

메시징 확장은 메시지 작성 영역의 맨 아래에서 사용할 수 있습니다.

> [!NOTE]
> 교육용 Teams 경우 할당 앱이 현재 전역 정책에 나열되어 있지 않더라도 기본적으로 전역 정책에 고정되어 있다는 것을 알고 있어야 합니다.

앱을 고정하는 앱 설정 정책을 만들려면 다음 단계를 수행합니다.

1. [Microsoft Teams 관리 센터에](https://admin.teams.microsoft.com) 로그인합니다.

1. 왼쪽 창에서 **Teams** **appsSetup** >  정책으로 이동합니다.
...
개인 Teams 환경에서 사용자에 대해 기본적으로 설치되는 앱을 선택하고, 앱을 [메시징 확장](/microsoftteams/platform/messaging-extensions/what-are-messaging-extensions)으로 설치하고, 모임에 설치할 앱을 지정할 수 있습니다.

앱 설정 정책을 사용하여 다음 작업을 수행할 수 있습니다.

- 개인 Teams 환경에서 사용자를 위한 앱 설치
- 사용자를 위한 앱을 메시징 확장으로 설치
- 모임 이끌이를 위한 모임에 앱 설치

> [!NOTE]
> 할당된 앱 [사용 권한 정책이](teams-app-permission-policies.md) 허용하는 경우 사용자는 앱을 직접 설치할 수 있습니다.

앱을 설치하는 앱 설정 정책을 만들려면 다음 단계를 수행합니다.

1. Microsoft Teams 관리 센터의 왼쪽 탐색에서 **Teams** **appsSetup** >  정책으로 이동합니다.
...
5. **설치된 앱 추가** 창에서 사용자를 위해 자동으로 설치하려는 앱을 검색합니다. 앱 권한 정책을 통해 앱을 필터링할 수도 있습니다.
...
Microsoft Teams 관리 센터를 사용하여 사용자가 Teams 사용자 지정 앱을 업로드할 수 있는 사용자 지정 정책을 만들 수 있습니다.

사용자가 사용자 지정 앱을 Teams 업로드할 수 있도록 앱 설정 정책을 만들려면 다음 단계를 수행합니다.
...
4. 사용자가 **사용자 지정 앱을 Teams** 업로드할 수 있도록 할지 여부에 따라 업로드 사용자 지정 앱을 켜거나 끕니다.

> [!NOTE]
> [조직 전체](manage-apps.md#manage-org-wide-app-settings) 앱 설정에서 **타사 앱** 이 꺼져 있으면 이 설정을 변경할 수 없습니다.

## <a name="manage-app-setup-policies"></a>앱 설정 정책 관리

Microsoft Teams 관리 센터에서 앱 설정 정책을 관리합니다. 전역(조직 전체 기본값) 정책을 사용하거나 사용자 지정 정책을 만들고 할당합니다.  사용자 지정 정책을 만들고 할당하지 않으면 조직의 사용자에게 전역 정책이 자동으로 적용됩니다. 이러한 정책을 관리하려면 전역 관리자 또는 Teams 서비스 관리자여야 합니다.
...
타사 앱(Teams Store에서 다운로드할 수 있음)은 모바일에 표시되기 전에 승인해야 합니다. 관리자가 Microsoft for Mobile의 승인되지 않은 앱을 고정하면 Teams Desktop에 표시되지만 모바일에는 표시되지 않습니다. 자세한 내용은 [모바일 클라이언트를](/microsoftteams/platform/tabs/what-are-tabs#mobile-clients) 참조하세요.
...
관리자 핀은 항상 우선합니다. **사용자 고정** 옵션이 켜져 있으면 사용자는 고정된 앱을 관리자 고정 앱 아래에 유지합니다. **사용자 고정** 옵션이 해제된 경우 사용자는 기존 핀을 잃게 되고 관리자 고정 앱만 앱 표시줄에 표시됩니다.
