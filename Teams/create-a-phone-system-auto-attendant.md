...
![이전 스크린샷 이름의 설명선 번호 1의 아이콘은 ](media/teamscallout1.png)
 **Name** 자동 전화 교환의 표시 이름을 입력 합니다. 이름은 필수 이며 공백을 포함 하 여 최대 64 자를 포함할 수 있습니다. 여기에서 지정 하는 **이름은** **자동 전화 교환** 탭의 열에 나열 되어 있습니다.
 ...
 **Operator** (권장 사항). 호출자가 메뉴를 차단 하 고 지정 된 사람에 게 말할 수 있도록 **연산자** 옵션을 설정할 수 있습니다.
...
- **외부 전화 번호** 사용자가 지정 하는 외부 전화 번호로 발신자를 전송 합니다. 다음 사항에 유의 하세요.
...
![숫자 5의 아이콘, 이전 스크린샷의 설명선 음성 인식 기능을 ](media/teamscallout5.png)
 **사용** 하는 경우이 옵션을 선택 하면 음성 입력을 사용할 수 있습니다. 발신자는 [설정한 언어로](set-auto-attendant-languages-for-audio-conferencing-in-teams.md)음성 입력을 사용할 수 있습니다. 사용자가 휴대폰 키패드를 사용 하 여 선택 하 게 할 수만 있도록 하려면 음성 인식을 **Off**로 설정 합니다.
...
![이전 스크린샷에 표시 된 숫자 1, 설명선의 아이콘 ](media/teamscallout1.png) **인사말**을 선택 하지 않으면 나중에 선택한 작업 중 하나에서 통화를 처리 하기 전에 호출자가 메시지 또는 인사말이 들리지 않습니다. 

<!-- You can also upload an audio file (in .wav, mp3 or .wma formats), or create a custom greeting using Text-to-Speech.-->

![이전 스크린샷의 숫자 2, 설명선에서 ](media/teamscallout2.png) **오디오 파일 재생** 을 선택한 경우 **파일 업로드** 단추를 사용 하 여 오디오로 저장 된 녹음 된 인사말 메시지를 업로드할 수 있습니다. WAV,. MP3 또는. WMA 형식. 기록의 크기는 5mb를 초과할 수 없습니다.

![숫자 3의 아이콘, 이전 스크린샷의 설명선 ](media/teamscallout3.png) **인사말 메시지 입력** 이 옵션을 선택 하는 경우 제공 된 필드에 시스템에서 읽을 텍스트 (최대 1000 자)를 입력 합니다. 예를 들어 "Contoso 시작 '을 입력 합니다. 귀하의 전화는 귀하에 게 중요 합니다. " 출력은 텍스트 음성 통화 소프트웨어로 만들어집니다.
...
![숫자 4의 아이콘, 이전 스크린샷 ](media/teamscallout4.png) **리디렉션 호출** 의 설명선은 옵션에서 선택 하지 않고 선택한 대상에 호출자를 보냅니다. 다음과 같은 설정이 가능 합니다.
  ...
  - **조직의 사용자** 선택 하는 계정에는 Enterprise Voice에 사용할 수 있는 전화 시스템 라이선스가 있거나 Microsoft 365 또는 Office 365에서 통화 요금제를 할당 받아야 합니다. 전화를 음성 메일로 보낼 수 있도록 설정할 수 있습니다. **조직에서 사용자** 를 선택 하 고 전화를 음성 메일로 바로 착신 전환 하도록 계정을 설정 합니다.
...
![숫자 1, 이전 스크린샷의 설명선 ](media/teamscallout1.png) **메뉴 옵션** 을 선택 하면 오디오 파일을 사용할지 여부를 선택할 수 있습니다. 다이얼 패드 메뉴 옵션을 호출자에 게 제공 하기 위해 텍스트를 음성으로 렌더링 하도록 텍스트를 입력 합니다. **호출 리디렉션** 또는 **연결 끊기** 옵션을 대신 하 여 선택 합니다.


![숫자 2의 아이콘, 이전 스크린샷의 설명선에서 ](media/teamscallout2.png) **오디오 파일을 재생** 하면 호출자가 선택할 프롬프트 및 옵션을 설정할 수 있습니다. 
...
![메뉴 옵션을 설정 하기 위한 앞의 스크린샷에 표시 되는 숫자 3의 아이콘에는 ](media/teamscallout3.png) **+ 다이얼 키 할당** 을 클릭 하 고 다음 옵션에 대 한 정보를 입력 합니다.

![](media/teamscallout4.png)옵션에 대 한 이전 스크린샷**음성 명령** 열의 설명선 번호 4는 최대 64 자까지 입력할 수 있으며 "고객 서비스" 또는 "작업 및 Grounds" 등의 여러 단어를 포함할 수 있습니다.   음성 인식 기능을 사용 하도록 설정 하면 이름이 자동으로 인식 되 고, 발신자는 3을 눌러 "3" 이라고 말 하거나 "고객 서비스" 라고 말하여 키 3에 매핑된 옵션을 선택할 수 있습니다. 이 텍스트는 "교환원에 게 착신 전환 중" 처럼 서비스 확인 프롬프트에 대 한 텍스트를 음성으로 렌더링 하기도 합니다.

![이전 스크린샷에 있는 숫자 5의 아이콘으로 ](media/teamscallout5.png) 리디렉션 옵션은 해당 키를 누르거나 음성 인식을 사용 하 여 선택한 옵션 **을** 설정 합니다. 통화는 다음으로 전송 될 수 있습니다.
...
![이 섹션의 이전 스크린샷 디렉터리 검색에 있는 숫자 6의 아이콘 ](media/teamscallout6.png) **Directory search** 을 사용 하 여 자동 전화 교환에 대 한 **전화 걸기** 및 **내선** 번호로 전화를 걸 수 있습니다.   전화 걸기 범위 (선택 사항) 페이지에서 이러한 서비스에 포함 되지 않는 사용자를 설정할 수 있습니다. 디렉터리 검색은 기본적으로 **없음** 으로 설정 됩니다.
...
**내선** 번호 이 옵션을 사용 하도록 설정 하는 경우 발신자는 휴대폰 내선 번호를 입력 하 여 조직의 사용자와 연결할 수 있습니다. 전화 걸기 범위 (선택 사항) 페이지에서 **전화 접속을 통해** 사용할 수 있거나 사용할 수 없음으로 나열 되는 사용자를 선택할 수 있습니다. 휴대폰 시스템 라이선스가 있는 모든 온라인 사용자 또는 비즈니스용 Skype를 사용 하 여 온-프레미스에 호스팅되는 모든 사용자는 적격 사용자 이며, 내선 번호로 전화 접속을 통해 찾을 수 있습니다.
...
![숫자 1, 이전 스크린샷의 설명선 ](media/teamscallout1.png) **24/7 선택을** 클릭 하 여이 자동 전화 교환에 대 한 업무 시간을 모두 만들 수 있습니다.

![숫자 2의 아이콘, 이전 스크린샷의 설명선은 일정의 ](media/teamscallout2.png) 모든 변경 내용을 되돌린 다음 **기본으로 재설정** 옵션을 선택 하 여 오전 9:00에서 5:00 Pm 월요일부터 금요일까지 기본 정의 시간으로 반환 합니다.

![숫자 3의 아이콘, 이전 스크린샷의 설명선에서는 ](media/teamscallout3.png) 일정을 완전히 지우는 **모든 시간 지우기를** 선택 합니다. 이 옵션을 선택 하 고 설정 해제 한 시간을 유지 하는 것은 권장 되지 않으므로 업무 시간을 완전히 다시 실행 하려는 경우에만이 옵션을 사용 하세요.

![숫자 5의 이전 스크린샷 아이콘에 있는 설명선의 아이콘 ](media/teamscallout4.png) ![ , 특정 ](media/teamscallout5.png) 요일의 시작 또는 종료 시간을 사용자 지정 하기 위한 이전 스크린샷의 설명선이 표시 되는 목록에서 **시작** 날짜 또는 **종료** 시간을 클릭 하 고 새 시간을 선택 합니다.   이 목록을 사용 하 여 15 분 간격으로 업무 시간을 선택 하 고 여기에서 선택한 업무 시간은 **일반 정보** 페이지에서 설정한 표준 시간대를 기준으로 합니다.
...
![숫자 6의 아이콘, 이전에는 ](media/teamscallout6.png) 나누기를 설정 하는 설명선 (예: 점심 작업)을 선택 하 고 해당 요일에 대해 새 표 행을 **추가** 하는 것이 없으면 새 시작 및 종료 시간을 선택 합니다. 업무 시간 내에 여러 휴식을 설정할 수 있습니다.
...
![숫자 1, 위 스크린샷의 설명선 ](media/teamscallout1.png) 자동 전화 교환의 명절에 대 한 사용자 지정 통화 흐름을 설정 하는 아이콘을 클릭 하 고 **새 명절 통화 흐름** 보기 화면을 **추가** 합니다.
> [!TIP]
> 휴일을 만들려면 **조직 전체에 걸친 설정**명절의 화면으로 이동 하면  >  **Holidays**됩니다.  
...
![이전 스크린샷의 설명선 인 번호 1의 아이콘은 ](media/teamscallout1.png) 새 통화 흐름의 **이름을** 입력 합니다.

![이전 스크린샷의 설명선 인 번호 2의 아이콘 ](media/teamscallout2.png) 이미 공휴일을 만든 경우 **휴일** 풀 다운 메뉴에 표시 되 고 해당 항목을 선택할 수 있습니다. 필요에 따라 편집할 수 있는 사용 되지 않는 옵션이 표시 될 수 있습니다. 그렇지 않은 경우 풀 다운 목록의 아래쪽에 있는 **추가** 를 클릭 하 여 새 공휴일을 만듭니다.  휴일을 만드는 데 사용 되는 단계에 대해서는 [Microsoft 팀에서 휴일 설정을](set-up-holidays-in-teams.md) 참조 하세요. 
...
![숫자 3의 아이콘, 이전 스크린샷의 설명선은 ](media/teamscallout3.png) 휴일 통화 흐름에 사용할 수 있는 [인사말](#call-flow) 옵션은 업무 시간 중에 사용할 수 있는 옵션과 같습니다. 인사말이 재생 된 후 **에 수행할 수 있는 작업은** **연결 끊기** 또는 **리디렉션**만 가능 하 고 **리디렉션 옵션을** 선택 하면 해당 연산자가 사용 가능한 선택 항목 중 하나가 아님을 제외 하 고도 비슷합니다. 휴일 흐름과 관련 된 메뉴를 설정할 수 없습니다.
...
이 페이지에서는 사용자가 조직에 전화를 걸 때 디렉터리에 나열 된 사용자 이름으로 전화를 걸 수 있도록 설정할 수 있습니다. 이전 화면에서 기본적으로 전화 걸기 이름을 **Off** 로 설정 합니다. 내선 **번호로 전화 걸기를** 이전에 선택한 경우에는 확장명을 가진 모든 사용자를 사용할 수 있습니다.

![숫자 1의 아이콘, 이전 스크린샷의 설명선은 ](media/teamscallout1.png) **모든 온라인 사용자** 또는 **사용자 지정 사용자 그룹인** 이 섹션의 옵션을 **포함** 합니다.
...
![숫자 2의 아이콘, 이전 스크린샷의 설명선 ](media/teamscallout2.png) 이 섹션의 옵션을 **제외** 하면 조직의 디렉터리에서 특정 사용자 또는 사용자 그룹을 제외할 수 있습니다.
...
![1 ](media/teamscallout1.png) 개 이상의 기존 및 할당 되지 않은 리소스 계정을 자동 전화 교환에 추가 하는 이전 스크린샷의 아이콘 번호 1, **계정 추가** 및 검색을 차례로 클릭 하 고 제공 된 대화 상자에서 선택 합니다.

![새 수행자 요약 보기 스크린샷](media/aa-assigned.png)

![숫자 1, 이전 스크린샷에 표시 된 설명선의 아이콘을 ](media/teamscallout1.png) 선택 하 여 추가 리소스 계정을 추가 하려면 **+ 계정 추가**를 클릭 합니다.
