...
- **음성 메일** - 지정한 Microsoft 365 그룹과 연결된 음성 사서함입니다. 음성 메일 전사를 원하는지, "음색 다음에 메시지를 남겨주세요"를 선택할 수 있습니다. 시스템 프롬프트.
  - Microsoft 365 관리 센터 조직 외부 사용자가 지정한 Microsoft 365 그룹에 **대해 이 팀에 전자 메일을 보내도록** 허용을 사용하도록 설정합니다.
...
## <a name="whats-new-for-auto-attendants-in-the-past-six-months"></a>지난 6개월 동안 자동 전화 교환의 새로운 기능
...
4. 이 자동 전화 교환에 [지원되는 언어](create-a-phone-system-auto-attendant-languages.md) 를 지정합니다. 이 언어는 시스템 생성 음성 프롬프트에 사용됩니다.
...
대상에 다이얼 키를 할당하는 경우 **디렉터리 검색** 에 대해 **없음** 을 선택하는 것이 좋습니다. 디렉터리 검색이 수행되기 전에 다이얼 키가 일치합니다.  호출자가 특정 대상에 할당된 다이얼 키를 사용하여 이름 또는 확장을 입력하기 시작하면 이름 또는 확장명 입력을 완료하기 전에 해당 대상으로 라우팅됩니다. 디렉터리 검색을 위한 별도의 자동 전화 교환을 만들고 다이얼 키를 사용하여 기본 자동 전화 교환 링크를 사용하는 것이 좋습니다.
...
- 업무 시간은 하루 중 휴식 시간으로 설정할 수 있으며 업무 시간으로 설정되지 않은 모든 시간은 시간 후로 간주됩니다.
...
1. 통화 **의 연결 끊기**, **리디렉션** 또는 **재생 메뉴 옵션을** 선택할 수 있습니다.
...
- 직접 라우팅 번호의 경우 보낸 숫자는 다음과 같이 SBC의 P-Asserted-Identity(PAI) 설정을 기반으로 합니다.
  - 사용 안 함으로 설정하면 원래 발신자의 전화 번호가 표시됩니다. 사용 안 함이 기본 설정이며 권장되는 설정입니다.
...
3. 테스트는 자동 전화 교환이 전화를 받지 못하도록 하는 테넌트, 정책 또는 리소스 계정 구성을 식별하고 식별된 문제를 해결하는 단계도 제공합니다.
