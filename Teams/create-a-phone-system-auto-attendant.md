...
2. 연산자를 지정하려는 경우 연산자에 대한 호출 대상을 지정합니다. 이는 선택 사항입니다(하지만 권장). 발신자가  메뉴에서 나서 지정된 담당자에게 말하도록 연산자 옵션을 설정할 수 있습니다.
...
> [!NOTE]
> 전화 걸기 이름  및 내선  번호로 전화 걸기 기능을 모두 사용하려는 경우 기본 자동 전화 번호에서 다이얼 키를 할당하여 전화로 전화 걸기를 사용하도록 설정된 자동 전화 번호에 **도달할 수 있습니다.** 해당 자동 전화 접속자 내에서 1개 키(문자와 연결된 문자가 없는)를 할당하여 **내선** 번호 자동 전화 접속 전화 걸기에 도달할 수 있습니다.
