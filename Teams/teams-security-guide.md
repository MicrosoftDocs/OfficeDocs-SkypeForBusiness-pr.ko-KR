...
Teams는 Microsoft 365 및 Office 365 내 서버 통신에 MTLS(상호 TLS)를 사용하고 클라이언트에서 서비스로의 TLS도 사용합니다. MTLS를 사용하면 단일 대화 시간 내에 도청을 달성하기가 어렵거나 불가능합니다. TLS는 모든 당사자를 인증하고 모든 트래픽을 암호화합니다. TLS가 도청을 방지하지는 않지만 공격자는 암호화가 깨지지 않는 한 트래픽을 읽을 수 없습니다.

TURN 프로토콜은 실시간 미디어 용도로 사용됩니다. TURN 프로토콜은 트래픽 암호화를 요구하지 않으며 전송하는 정보는 메시지 무결성으로 보호됩니다. 도청에 열려 있지만 패킷의 원본 및 대상 주소를 보고 전송 정보(즉, IP 주소 및 포트)를 직접 추출할 수 있습니다. Teams 서비스는 절대로 일반 텍스트로 전송되지 않는 TURN 암호를 비롯한 몇 개의 항목에서 파생된 키를 사용하여 메시지가 유효한지 검사하는 식으로 데이터 유효성을 확인합니다. 미디어 트래픽에 SRTP가 사용되고 또한 암호화됩니다.
...
TLS는 모든 당사자를 인증하고 모든 트래픽을 암호화합니다. TLS를 사용하면 공격자가 특정 연결에 대해 IP 주소 스푸핑을 수행하는 것을 방지합니다(예: 상호 TLS 연결). 공격자는 여전히 DNS(Domain Name System) 서버의 주소를 스푸핑할 수 있습니다. 그러나 Teams의 인증은 인증서를 사용하여 수행되므로 공격자는 통신에 참여하는 당사자 중 하나를 스푸핑할 때 필요한 유효한 인증서를 보유하지 못합니다.

### <a name="man-in-the-middle-attack"></a>중간자(man-in-the-middle) 공격
...
SRTP를 사용하여 미디어 스트림을 암호화하면 Teams 오디오, 비디오 및 응용 프로그램 공유에 참여하는 두 엔드포인트 간의 미디어 트래픽에 대한 중간자 공격이 방지됩니다. 암호화 키는 TLS 1.2 및 AES-256(GCM 모드) 암호화 UDP/TCP 채널을 사용하는 독점 신호 프로토콜(Teams Call Signaling 프로토콜)을 통해 두 끝점 간에 협상됩니다.

### <a name="real-time-transport-protocol-rtp-replay-attack"></a>실시간 전송 프로토콜(RTP) 리플레이 공격
...
#### <a name="certificate-revocation-list-crl-distribution-points"></a>CRL(인증서 해지 목록) 배포 지점
...
TLS 및 MTLS 프로토콜은 인터넷상에서 암호화된 통신 및 엔드포인트 인증을 제공합니다. Teams는 이 두 프로토콜을 사용하여 신뢰할 수 있는 서버 네트워크를 만들고 그러한 네트워크를 통한 모든 통신을 암호화합니다. 서버 간의 모든 통신은 MTLS를 통해 발생합니다. 클라이언트에서 서버로의 나머지 또는 레거시 SIP 통신은 TLS를 통해 발생합니다.
...
Teams 데이터는 전송 중 암호화되고 Microsoft 데이터 센터에 저장됩니다. Microsoft는 TLS 및 SRTP와 같은 업계 표준 기술을 사용하여 사용자의 장치와 Microsoft 데이터 센터 간에 그리고 Microsoft 데이터 센터 간에 전송 중인 모든 데이터를 암호화합니다. 전송 중인 데이터에는 메시지, 파일, 모임 및 기타 콘텐츠가 포함됩니다. 또한 기업 데이터는 Microsoft 데이터 센터에서 암호화되어 있으므로 조직에서 eDiscovery와 같은 방법을 통해 보안 및 규정 준수 의무를 충족하기 위해 필요한 경우 콘텐츠를 복호화할 수 있습니다. Microsoft 365의 암호화에 대한 자세한 내용은 [Microsoft 365의 암호화](/microsoft-365/compliance/encryption)를 참조하세요.

### <a name="encryption-in-teams"></a>Teams의 암호화

Microsoft 365 내에는 여러 암호화 계층이 있습니다. Teams의 암호화는 나머지 Microsoft 365 암호화와 함께 작동하여 조직의 콘텐츠를 보호합니다. 이 문서에서는 Teams와 관련된 암호화 기술에 대해 설명합니다. Microsoft 365의 암호화 개요는 [Microsoft 365의 암호화](/microsoft-365/compliance/encryption)를 참조하세요.
...
Teams는 TLS 및 MTLS를 사용하여 인스턴트 메시지를 암호화합니다.
...
#### <a name="federal-information-processing-standard-fips"></a>연방 정보 처리 표준(FIPS)

Teams는 암호화 키 교환에 FIPS 호환 알고리즘을 사용합니다. FIPS 구현에 대한 자세한 내용은 [연방 정보 처리 표준(FIPS) 간행물 140-2](/microsoft-365/compliance/offering-fips-140-2)를 참조하세요.
...
- **최신 인증(MA)** 은 클라이언트-서버 통신을 위한 OAUTH 2.0의 Microsoft 구현입니다. 다단계 인증 및 조건부 액세스와 같은 보안 기능을 활성화합니다. MA를 사용하려면 온라인 테넌트와 클라이언트 모두 MA에 대해 활성화해야 합니다. PC와 모바일의 Teams 클라이언트와 웹 클라이언트는 모두 [MA를 지원](./sign-in-teams.md)합니다.

> [!NOTE]
> Azure AD 인증 및 권한 부여 방법에 대한 자세한 정보가 필요한 경우 이 문서의 소개 및 'Azure AD의 인증 기본 사항' 섹션이 도움이 될 것입니다.

Teams 인증은 Azure AD 및 OAuth를 통해 수행됩니다. 인증 프로세스는 다음으로 간소화할 수 있습니다.

- 사용자 로그인 > 토큰 발급 > 다음 요청 시 발급된 토큰을 사용합니다.

클라이언트에서 서버로의 요청은 OAuth를 사용하여 Azure AD에서 인증 및 권한 부여됩니다. 페더레이션 파트너가 발급한 유효한 자격 증명이 있는 사용자는 신뢰할 수 있으며 기본 사용자와 동일한 프로세스를 통과합니다. 그러나 관리자가 추가 제한을 설정할 수 있습니다.
...
### <a name="federation-safeguards-for-teams"></a>Teams용 페더레이션 보호 조치
...
Teams는 엔터프라이즈 사용자가 실시간 모임을 만들고 참석할 수 있는 기능을 제공합니다. 엔터프라이즈 사용자는 Azure AD, Microsoft 365 또는 Office 365 계정이 없는 외부 사용자를 이러한 모임에 초대할 수도 있습니다. 외부 파트너가 고용하여, 인증된 보안 ID를 보유한 사용자는 모임에 참가할 수 있을 뿐만 아니라, 발표자 역할을 하도록 승격될 수도 있습니다. 익명 사용자는 모임을 만들거나 발표자로서 모임에 참가할 수 없지만, 참가한 후에 발표자로 승격될 수 있습니다.
...
외부 사용자가 Teams 모임에 참가할 수 있도록 하면 유용할 수 있지만 몇 가지 보안 위험이 따릅니다. 이러한 위험을 해결하기 위해 Teams는 다음과 같은 안전 장치를 사용합니다.
...
모임 주최자는 참가자가 모임 도중에 발표 가능한지 여부를 제어합니다. 각 모임의 발표자가 다음 옵션으로 제한되도록 설정할 수 있습니다.
...
모임이 진행되는 동안 모임 옵션을 수정할 수 있습니다. 저장되면 몇 초 내에 실행 중인 모임에서 변경 내용이 눈에 띄게 됩니다. 이후의 모임에도 마찬가지로 효력을 미칩니다.
