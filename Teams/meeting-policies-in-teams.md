...
사용자별 정책에 해당합니다. 이 설정으로 사용자가 모임에서 화이트보드를 공유할 수 있는지 여부를 제어합니다. 익명, 게스트 및 외부 액세스 사용자를 포함한 외부 참가자는 모임 이끌이의 정책을 상속합니다.
...
사용자별 정책에 해당합니다. 이 설정으로 사용자가 모임에서 메모를 작성하고 공유할 수 있는지 여부를 제어합니다. 익명, 게스트 및 외부 액세스 사용자를 포함한 외부 참가자는 모임 이끌이의 정책을 상속합니다. **모임 메모** 탭은 참가자가 최대 100명인 모임에서 지원됩니다.
...
이 설정은 이끌이별 정책입니다. 이 설정으로 전화 접속한 참가자가 **자동으로 사용자 입장** 설정과 관계없이 바로 모임에 참가할지 아니면 대기실에서 기다릴지를 제어합니다. 기본적으로 이 설정은 꺼져 잇습니다. 이 설정을 끄면 전화 접속 사용자는 조직 사용자가 Teams 클라이언트에서 모임에 참가하여 입장을 허락할 때까지 대기실에서 기다립니다. 이 설정이 켜져 있는 경우 전화 접속 사용자는 자동으로 모임에 참가합니다.

> [!NOTE]
> 아무 사용자에 대해서든 기본 설정을 변경하는 경우 해당 사용자가 이끄는 모든 새 모임 및 사용자가 모임 옵션을 수정하지 않은 이전 모임에 변경된 설정이 적용됩니다.
