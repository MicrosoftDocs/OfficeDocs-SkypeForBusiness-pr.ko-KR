...
::: zone target="docs"
모임 정책은 조직에서 사용자가 예약한 모임 참가자가 사용할 수 있는 기능을 제어하는 데 사용됩니다. 자동으로 생성 되는 전역 (조직 차원의 기본) 정책을 사용 하거나 사용자 지정 정책을 만들고 할당할 수 있습니다. Microsoft 팀 관리 센터에서 또는 [PowerShell](teams-powershell-overview.md)을 사용 하 여 모임 정책을 관리할 수 있습니다.

> [!NOTE]
> 역할을 사용 하 여 모임 발표자 및 참석자의 사용 권한을 관리 하는 방법에 대 한 자세한 내용은 [팀 모임에서 역할](https://support.microsoft.com/office/roles-in-a-teams-meeting-c16fa7d0-1666-4dde-8686-0a0bfe16e019?ui=en-us&rs=en-us&ad=us)을 참조 하세요.
...
전역 정책에서 설정을 편집 하거나 하나 이상의 사용자 지정 정책을 만들어 할당할 수 있습니다. 사용자 지정 정책을 만들고 할당 하지 않으면 사용자가 글로벌 정책을 받습니다.
...
만든 전역 정책 및 사용자 지정 정책을 편집할 수 있습니다.

1. Microsoft 팀 관리 센터의 왼쪽 탐색 창에서 **모임**  >  **모임 정책** 으로 이동 합니다.
2. 정책 이름 왼쪽을 클릭 하 여 정책을 선택한 다음 **편집** 을 선택 합니다.
3. 여기서 원하는 대로 변경 합니다.
4. **저장** 을 선택 합니다.
...
**지금 모임** 허용은 사용자별 정책 이므로 모임이 시작 되기 전에 적용 됩니다. 이 설정은 사용자가 팀 채널에서 계획 되지 않은 모임을 시작할 수 있는지 여부를 제어 합니다. 이 설정을 사용 하면 사용자가 **모임 단추를 선택 하 여 계획** 되지 않은 모임을 시작 하거나 채널에서 모임을 예약할 수 있습니다. 기본값은 True입니다.
...
이 기능을 해제 하면 사용자가 예약할 수 없습니다. 팀이 Outlook에서 새 모임을 만들 때 모임 예를 들어 Windows의 Outlook에서는 **새 팀 모임** 옵션이 리본 메뉴에 표시 되지 않습니다.

### <a name="allow-channel-meeting-scheduling"></a>채널 모임 예약 허용

기존 AllowChannelMeetingScheduling 정책을 사용 하 여 팀 채널 일정에 만들 수 있는 이벤트 유형을 제어 합니다. 이것은 사용자별 정책으로, 모임이 시작 되기 전에 적용 됩니다. 이 설정은 사용자가 팀 채널에서 모임을 예약할 수 있는지 여부를 제어 합니다. 이 설정은 기본적으로 설정 되어 있습니다.

이 정책을 끄면 사용자가 새 채널 모임을 만들 수 없게 됩니다. 그러나 기존 채널 모임은 이벤트 구성 도우미를 통해 편집할 수 있습니다.
...
채널 게시물 페이지에서 다음 기능을 사용 하지 않도록 설정 합니다.

- 채널 회신 작성 상자에서 **모임 예약** 단추
  ![모임 예약 단추 회신 쓰기 상자](media/schedule-meeting-disabled-in-chat2.png)
  
- 채널 헤더에서 **모임 예약** 단추
  ![채널 머리글에서 모임 예약 단추](media/schedule-now-in-header.png)

채널 일정에서 다음을 수행 합니다.

- 채널 달력 머리글의 **새 이벤트 추가** 단추를 사용할 수 없게 됩니다.
  ![채널 달력 머리글 단추를 사용할 수 없음](media/add-new-event-disabled.png)

- 채널 일정에서 시간 블록을 끌어서 선택 하 여 채널 모임을 만들 수는 없습니다.

- 사용자는 바로 가기 키를 사용 하 여 채널 일정에서 모임을 만들 수 없습니다.
...
**개인 모임 예약 허용** 을 해제 하 고 **채널 모임 예약을 허용** 하는 경우 팀의 사용자는 **필수 참석자 추가** 및 **채널 추가** 옵션을 사용할 수 없습니다. 이 설정은 기본적으로 설정 되어 있습니다.

### <a name="allow-meet-now-in-private-meetings"></a>비공개 모임에서 모임 시작 허용

이것은 사용자별 정책으로, 모임이 시작 되기 전에 적용 됩니다. 이 설정은 사용자가 계획 하지 않은 비공개 모임을 시작할 수 있는지 여부를 제어 합니다.  이 설정은 기본적으로 설정 되어 있습니다.
...
- [IP 오디오 용 모드](#mode-for-ip-audio)
- [IP 비디오용 모드](#mode-for-ip-video)
...
|---------|---------|---------|
|Daniela | 전역   | 끄십시오 |
|Amanda | Location1MeetingPolicy | 등|
...
### <a name="mode-for-ip-audio"></a>IP 오디오 용 모드

이것은 사용자별 정책입니다. 이 설정은 모임 및 그룹 통화에서 오디오를 켤 수 있는지 여부를 제어 합니다. 이 설정에 대 한 값은 다음과 같습니다.

|값 설정 |결과가  |
|---------|---------|
|**송신 및 수신 오디오 사용**    |모임에서 발신 및 수신 오디오를 사용할 수 있습니다. 기본 설정입니다. |
|**비활성화**     |모임에서 발신 및 수신 오디오가 꺼집니다.     |

사용자에 대해 **사용 안 함으로** 설정 된 경우에도 해당 사용자는 모임을 예약 하 고 구성할 수 있지만 오디오를 사용할 수 없습니다. 모임에 참가 하려면 PSTN (공개 교환 전화 네트워크)을 통해 전화 접속 하거나 모임 전화를 걸어 휴대폰으로 참가 해야 합니다. 할당 된 정책이 없는 모임 참가자 (예: 익명 참가자)는이 설정이 **발신 및 수신 오디오** 로 기본적으로 활성화 됩니다. 팀 모바일 클라이언트에서이 설정을 사용 하지 않도록 설정한 경우 사용자는 PSTN을 통해 모임에 전화 접속 해야 합니다.
...
이것은 사용자별 정책입니다. 이 설정은 모임 및 그룹 통화에서 비디오를 켤 수 있는지 여부를 제어 합니다. 이 설정에 대 한 값은 다음과 같습니다.

|값 설정 |결과가  |
|---------|---------|
|**송신 및 수신 비디오 사용**    | 모임에서 보내고 받은 비디오를 사용할 수 있습니다. 기본 설정입니다. |
|**비활성화**     | 모임에서 보내고 받은 비디오가 꺼집니다. 팀 모바일 클라이언트에서는 사용자가 모임에서 비디오 또는 사진을 공유할 수 없습니다. <br><br>**Ip 오디오 용 모드** 를 사용할 수 없는 경우에는 **ip 비디오용 모드만** 사용할 수 없도록 유지 됩니다.  |
...
이는 구성 단위 및 사용자별 정책 조합입니다. 비디오는 모임에 대 한 주요 구성 요소입니다. 일부 조직에서는 관리자가 어떤 사용자의 모임이 비디오를 보유 하 고 있는지 더 자세히 관리할 수 있습니다. 이 설정은 사용자가 호스트 하는 모임 및 사용자가 시작 하는 1:1 및 그룹 통화에서 비디오를 켤 수 있는지 여부를 제어 합니다. 팀 모바일 클라이언트에서이 설정은 사용자가 모임에서 사진과 비디오를 공유할 수 있는지 여부를 제어 합니다.

이 정책 설정을 사용 하도록 설정한 사용자가 구성한 모임에는 참가자가 정책 설정을 사용 하도록 설정 되어 있는 경우 모임 참가자가 모임에서 비디오 공유를 허용 합니다. 배정 된 정책이 없는 모임 참가자 (예: 익명 및 페더레이션 참가자)는 모임 이끌이의 정책을 상속 합니다.

> [!NOTE]
> 이 설정은 나가는 동영상을 제어 하는 반면, **IP 비디오 설정 모드** 는 발신 및 수신 비디오를 모두 제어 한다는 점에 유의 하세요. 자세한 내용은 [우선 순위가 높은 IP 비디오 정책 설정을](#which-ip-video-policy-setting-takes-precedence) 확인 하 고 [모임 참가자를 위해 오디오/비디오를 관리](#manage-audiovideo-for-meeting-participants)하세요.

| 팀 데스크톱 및 웹 클라이언트 |팀 모바일 클라이언트  |
|:-------:|:-------:|
...
|---------|---------|---------|
|Daniela   | 전역   | 등       |
|Amanda    | Location1MeetingPolicy        | 끄십시오      |
...
#### <a name="which-ip-video-policy-setting-takes-precedence"></a>어떤 IP 비디오 정책 설정이 우선 적용 됩니다.

사용자의 경우 비디오에 대 한 가장 제한적인 정책 설정이 우선권을 갖습니다. 몇 가지 예를 살펴보겠습니다.

|IP 비디오 허용|IP 비디오용 모드|모임 환경|
|---------|---------|---------|
|이끌이: **설정**<br><br>참가자: **켜기** |참가자: **사용 안 함**        |**IP 영상 설정 모드가** 우선권을 갖습니다. 이 정책이 할당 된 참가자는 다른 사용자가 공유한 비디오를 켜거나 볼 수 없습니다.|
|이끌이: **설정**<br><br>참가자: **켜기** |참가자: **보내고 받은 비디오 사용**          |이 정책이 할당 된 참가자는 다른 사용자가 공유 하는 비디오를 켜거나 볼 수 있습니다.         |
|이끌이: **설정**<br><br>참가자: **끄기** |참가자: **보내고 받은 비디오 사용**         |**IP 영상 허용** 설정이 우선적으로 적용 됩니다. 참가자는 수신 비디오만 볼 수 있으며 보내는 비디오는 보낼 수 없습니다.         |
|이끌이: **설정**<br><br>참가자: **끄기** |참가자: **사용 안 함**         |**IP 영상 설정 모드가** 우선권을 갖습니다. 참가자는 수신 또는 발신 비디오를 볼 수 없습니다.|
|이끌이: **해제**    |       |구성 도우미에 대해 설정 되어 있지 않기 때문에 **IP 영상 허용** 설정이 우선권을 갖습니다. 이 정책이 할당 된 사용자가 구성한 모임에서는 아무도 비디오를 켤 수 없습니다.         |

### <a name="manage-audiovideo-for-meeting-participants"></a>모임 참가자를 위한 오디오/비디오 관리

|원하는 경우  |다음 정책 설정 설정  |
|---------|---------|
|모임 참가자가 오디오 및 비디오를 사용 하지 않도록 설정  |IP 오디오 모드: **사용 안 함**<br> IP 비디오용 모드: **사용 안 함**<br>IP 비디오 허용: 해당 없음       |
|모임 참가자에 게 들어오는 비디오 및 오디오만 허용  |IP 오디오 모드: **발신 및 수신 오디오 사용**<br> IP 비디오용 모드: **발신 및 수신 비디오 사용**<br>IP 비디오 허용: **해제**       |
|모임 참가자가 비디오를 사용할 수 없도록 설정 (참가자만 오디오를 사용 하는 경우)|  IP 오디오 모드: **송신 및 수신 오디오 사용**<br> IP 비디오용 모드: **사용 안 함**<br>IP 비디오 허용: 해당 없음
|모임 참가자에 게 오디오 및 비디오 사용    |IP 오디오 모드: **발신 및 수신 오디오 사용** (기본값)<br> IP 비디오용 모드: **발신 및 수신 비디오가 활성화 됨** (기본값)<br>IP 비디오 허용: **켜기** (기본값)    |

모임 이끌이 정책 및 사용자의 정책 간에 가장 제한적인 정책이 적용 됩니다. 예를 들어 이끌이에 비디오를 제한 하는 정책이 있고 사용자의 정책이 비디오를 제한 하지 않는 경우 모임 참가자는 모임 이끌이의 정책을 상속 하 고 모임에서 비디오에 액세스할 수 없습니다. 즉, 오디오만을 사용 하 여 모임에 참가할 수 있습니다.

> [!NOTE]
> 사용자가 전화로 참가 하기 위해 그룹 통화를 시작 하면 오디오를 **사용** 하는 것이 표시 되지 않습니다. 이는 해결 해야 하는 알려진 문제입니다. 이 문제를 해결 하려면 **기타 참가 옵션** 에서 **전화 오디오** 를 선택 합니다.  

#### <a name="teams-mobile-clients"></a>팀 모바일 클라이언트

팀 모바일 클라이언트 사용자의 경우 모임 중에 사진 및 비디오를 공유 하는 기능은 **ip 비디오 허용** 또는 **ip 비디오 모드** 설정에 의해서도 결정 됩니다. 우선 적용 되는 정책 설정에 따라 비디오와 사진을 공유 하는 기능을 사용할 수 없습니다. 이것은 별도의 [화면 공유 모드](#screen-sharing-mode) 설정을 사용 하 여 구성 하는 화면 공유에는 영향을 주지 않습니다. 또한 모바일 사용자가 셀룰러 연결을 통해 IP 비디오를 사용 하지 못하도록 [팀 이동성 정책을](https://docs.microsoft.com/powershell/module/skype/new-csteamsmobilitypolicy) 설정 하 여 WiFi 연결을 사용 해야 할 수도 있습니다.
...
> [!Note]
> 이 기능은 아직 개발 중입니다. 화면 공유는 참가자 별 정책입니다.

이 설정은 사용자의 모임에서 데스크톱 및/또는 창 공유를 허용할지 여부를 제어 합니다. 배정 된 정책이 없는 모임 참가자 (예: 익명, 게스트, B2B, 페더레이션 참가자)는 모임 이끌이의 정책을 상속 합니다.
...
|---------|---------|---------|
|Daniela   | 전역   | 등       |
|Babek    | Location1MeetingPolicy        | 끄십시오   |
...
이것은 사용자별 정책입니다. 조직에서이 사용자에 대해이 집합을 보유 하 고 있는지 여부는 모임 이끌이가 설정한 내용에 관계 없이 외부 참가자가 수행할 수 있는 작업을 제어 하지 않습니다. 이 매개 변수는 해당 조직의 모임 정책 내에서 설정한 공유자에 따라 외부 참가자가 제어권을 부여 하거나 공유자 화면 제어권을 요청할 수 있는지 여부를 제어 합니다. 팀 회의의 외부 참가자는 다음과 같이 분류할 수 있습니다.  
...
> [!NOTE]
>모임에 참가 하는 옵션은 각 팀 그룹의 설정 및 연결 방법에 따라 달라 집니다. 그룹에 오디오 회의가 있고이를 사용 하 여 연결 하는 경우 [오디오 회의](https://docs.microsoft.com/microsoftteams/audio-conferencing-in-office-365)를 참조 하세요. 팀 그룹에 오디오 회의가 없는 경우 [팀에서 모임 참가](https://support.office.com/article/join-a-meeting-in-teams-1613bb53-f3fa-431e-85a9-d6a91e3468c9)를 참조 하세요.

### <a name="let-anonymous-people-start-a-meeting"></a>익명 사용자가 모임을 시작 하도록 허용

Leaderless 전화 접속 회의 모임을 허용 하는 이끌이 별 정책입니다. 이 설정은 전화 접속 사용자가 참석자의 조직에서 인증 된 사용자 없이 모임에 참가할 수 있는지 여부를 제어 합니다. 기본적으로이 설정을 해제 하면 전화 접속 사용자가 조직의 인증 된 사용자가 모임에 참가할 때까지 대기실에서 대기 합니다.

> [!NOTE]
> 이 설정을 해제 한 상태에서 전화 접속 사용자가 모임에 먼저 참가 하 고 대기실에 배치 된 경우 조직 사용자는 팀 클라이언트와 모임에 참가 하 여 사용자가 대기실에서 작업을 허용 해야 합니다. 사용자에 게 전화를 걸 수 있는 로비 컨트롤이 없습니다.

### <a name="automatically-admit-people"></a>자동으로 사람들의 입장

이는 이끌이 별 정책입니다. 이 설정은 사용자가 인증 된 사용자가 참여 하 게 될 때까지 자신이 모임에 직접 참가 하거나 대기실에서 대기할지 여부를 제어 합니다. 이 설정은 전화 접속 사용자에 게는 적용 되지 않습니다.
...
> [!NOTE]
> 모임 옵션에는 "대기실를 무시할 수 있는 사람" 이라고 표시 됩니다. 사용자에 대 한 기본 설정을 변경 하면 해당 사용자가 구성한 모든 새 모임과 사용자가 모임 옵션을 수정 하지 않은 이전 모임에 모두 적용 됩니다.
...
|**이끌이만**    |모임 이끌이만 대기실에서 대기 하지 않고 바로 모임에 참가할 수 있습니다. 조직 내의 인증 된 사용자, 게스트 사용자, 신뢰할 수 있는 조직의 사용자, 익명 사용자를 포함 한 모든 사용자가 대기실에서 기다려야 합니다.           |

### <a name="allow-dial-in-users-to-bypass-the-lobby"></a>전화 접속 사용자가 대기실를 우회할 수 있도록 허용

이는 이끌이 별 정책입니다. 이 설정은 전화를 통해 전화를 거는 사용자가 모임에 직접 참가 하거나 **자동으로** 허용 되는 사용자 설정에 관계 없이 대기실에서 대기할지 여부를 제어 합니다. 이 설정은 기본적으로 꺼져 있습니다. 이 설정을 끄면 전화 접속 사용자가 팀 클라이언트와 모임에 참가 하 고 입장할 수 있을 때까지 대기실에서 대기 합니다. 이 설정을 사용 하면 조직 사용자가 모임에 참가할 때 전화 접속 사용자가 자동으로 모임에 참가 합니다.

> [!NOTE]
> 조직 사용자가 모임에 참가 하기 전에 전화 접속 사용자가 모임에 참가 하는 경우 조직 사용자가 팀 클라이언트를 사용 하 여 모임에 참가 하 고 입장할 때까지 로비에 배치 됩니다. 사용자에 대 한 기본 설정을 변경 하면 해당 사용자가 구성한 모든 새 모임과 사용자가 모임 옵션을 수정 하지 않은 이전 모임에 모두 적용 됩니다.
...
|**BlurandDefaultBackgrounds**     |사용자는 비디오 배경을 흐리게 하거나 기본 이미지 집합 중에서 선택 하 여 배경으로 사용할 수 있습니다. |
|**AllFilters**     |사용에는 비디오 배경을 흐리게 하거나 기본 이미지 집합에서 선택 하거나 배경으로 사용할 사용자 지정 이미지를 업로드 하는 옵션이 있습니다. |
