...
앱은 작업 공간 도구와 서비스를 통합하고 다른 사용자와 공동 작업할 수 있는 좋은 방법입니다. 앱은 최종 사용자가 일상적인 작업에서 생산성, 공동 작업 및 효율성을 높이는 데 도움이 됩니다. 조직은 앱을 사용하여 고객과 연결하고, 서비스를 제공하고, 정보를 공유합니다. 앱을 통해 사용자는 채팅, 모임 및 채널 Teams 보다 효과적으로 사용할 수 있습니다. 예를 들어 Teams 고정된 일정을 사용하여 다른 사용자와 빠르게 공동 작업하는 최종 사용자, Teams 채널에서 웹 서비스의 QoS를 사용자에게 알리는 봇 기능이 있는 앱, 채널의 다양한 최종 사용자에게 작업을 공유하고 할당하는 앱이 있습니다.

스토어에서 유효성이 검사되고 안전한 앱을 광범위하게 선택하면 최종 사용자가 조직에 매일 필요한 도구와 서비스에 액세스할 수 있습니다. Microsoft Teams 앱은 배포할 필요가 없는 웹 기반 SaaS 앱입니다. 최종 사용자는 사용자가 제공한 권한에 따라 Teams 앱을 사용할 수 있습니다. 관리자는 조직의 사용자에 대한 앱 사용을 승인하거나 차단합니다. 모임, 채팅 및 채널에서 모든 사용자에 대한 앱의 가용성을 제어합니다.

최종 사용자에게 필요한 앱을 제공하려면 앱 유형과 사용자가 해당 앱에 액세스하는 위치를 이해합니다. 앱 사용에 대한 자세한 내용은 [최종 사용자를 위한 앱 개요](https://support.office.com/article/overview-of-apps-in-teams-747492ee-7cdd-4115-a993-8c7e7f98a3d0)를 참조하세요.
...
최종 사용자가 Teams 사용할 수 있는 다양한 유형의 앱은 다음과 같습니다.

* [Teams 일부인 핵심 앱입니다](#core-apps).
* [Microsoft에서 만든 다른 앱](#microsoft-provided-apps).
* 파트너[별 타사 앱](#third-party-apps-validated-by-microsoft)(Microsoft에서 유효성 검사).
* 사용자 고유의 조직에서 만든 [사용자 지정 앱](#custom-apps)

## <a name="core-apps"></a>핵심 앱

활동 피드, Teams 채널, 채팅, 일정 및 통화와 같은 일부 기본 기능은 최종 사용자가 쉽게 액세스할 수 있도록 기본적으로 사용 가능하고 고정됩니다. 관리자는 [설치 정책을](/microsoftteams/teams-app-setup-policies) 사용하여 기본 동작을 수정할 수 있습니다.

:::image type="content" source="media/core-apps-pinned1.png" alt-text="핵심 앱은 기본적으로 Teams 고정된 앱입니다." lightbox="media/core-apps-pinned2.png":::

## <a name="microsoft-provided-apps"></a>Microsoft에서 제공하는 앱

Microsoft는 생산성과 협업을 개선하기 위해 많은 앱을 제공합니다. 사용자와 최종 사용자는 관리 센터에서 Publisher 나열되거나 팀 스토어에서 공급자로 나열된 Microsoft를 검색하여 이러한 앱을 찾을 수 있습니다.

Teams는 목록, 작업, 칭찬, 승인 등의 기본 제공 앱 집합을 제공합니다. 초기 Teams 롤아웃을 진행 시 Planner와 같은 Teams 추천 앱을 포함하는 것이 좋습니다.

:::image type="content" source="media/microsoft-apps-in-tac1.png" alt-text="Teams 관리 센터의 Microsoft 앱" lightbox="media/microsoft-apps-in-tac2.png":::

## <a name="third-party-apps-validated-by-microsoft"></a>Microsoft에서 유효성을 검사한 타사 앱

Microsoft에서 제공하는 앱 외에도 Microsoft에서 검증된 타사 앱을 사용할 수 있습니다. Microsoft는 이러한 앱을 Teams 스토어에서 사용할 수 있도록 하기 전에 이러한 앱의 기능과 보안의 유효성을 검사합니다. 앱 유효성 검사의 이점을 이해하려면 [타사 앱의 유효성 검사를 참조하세요](overview-of-app-validation.md).

:::image type="content" source="media/3p-apps-in-teams.png" alt-text="Teams 스토어의 타사 앱 예제":::

## <a name="custom-apps"></a>사용자 지정 앱

조직의 개발자가 만든 앱을 사용자 지정 앱이라고 합니다. 이러한 앱의 개발은 조직의 특정 요구 사항을 위해 의뢰되며 이러한 앱을 허용하거나 허용하지 않도록 제어할 수 있습니다. 조직의 개발자는 [Microsoft Power Platform](/microsoftteams/platform/samples/teams-low-code-solutions)과 Teams 통합을 사용하여 사용자 지정 로우 코드 솔루션을 신속하게 빌드할 수 있습니다.

관리자가 사용자 지정 앱을 사용하도록 허용하면 최종 사용자는 Teams 스토어의 왼쪽 탐색 영역에서 **조직용 빌드** 를 클릭하여 이러한 앱을 찾습니다.

:::image type="content" source="media/built-for-your-org1.png" alt-text="Teams 데스크톱 앱의 Teams 스토어에 있는 사용자 지정 앱" lightbox="media/built-for-your-org2.png":::

### <a name="understand-sideloading-of-custom-apps"></a>사용자 지정 앱의 테스트용 로드 이해

사용자 지정 앱을 개발하고 최종 사용자에게 배포하기 전에 개발자는 스토어에 추가하여 앱을 테스트하여 자체 테스트하거나 앱을 테스트하는 팀과 함께 테스트합니다. 이 메서드는 앱의 테스트용 로드라고 하며 사용자 지정 앱에만 적용됩니다.

개발자는 일반적으로 개발 미달 앱을 테스트하기 위해 특정 팀의 구성원이 앱을 사용할 수 있도록 앱을 테스트할 수 있습니다. 사이드로드가 허용되는 경우 관리자 승인이 필요하지 않습니다. 관리자는 모든 개발자에 대한 테스트용 로드를 허용하지 않을 수 있습니다.

테스트 [테넌](/microsoftteams/platform/concepts/build-and-test/prepare-your-o365-tenant)트에서 테스트용으로 로드를 허용하지 않는 경우 개발자는 여전히 앱을 테스트할 수 있습니다. 사용자 지정 앱 개발이 완료되면 개발자는 관리자에게 최종 사용자에게 사용자 지정 앱을 배포하도록 요청합니다. 자세한 내용은 [사용자 지정 앱을 게시하는 방법을 참조하세요](/microsoftteams/upload-custom-apps). 관리자는 특정 사용자에 대한 사용자 지정 앱 사용을 허용하거나 허용하지 않습니다.

### <a name="about-app-templates"></a>앱 템플릿 정보

Teams 대한 앱 템플릿은 Microsoft에서 인기 있는 사용 사례를 설명하고, 앱 개발 모범 사례를 소개하고, 개발자가 사용자 지정 앱을 만들기 위해 확장할 수 있는 오픈 소스 앱을 제공하기 위해 만든 기능적이고 프로덕션이 준비된 샘플 앱입니다. 조직 개발자는 GitHub 사용할 수 있는 코드를 간단하게 변경하여 조직의 요구에 맞게 앱 템플릿을 사용자 지정합니다. 관리자는 이러한 앱을 최종 사용자를 위한 사용자 지정 앱으로 제공합니다.

자세한 내용은 [Microsoft Teams 앱 템플릿을 참조하세요](https://adoption.microsoft.com/microsoft-teams/app-templates/).

## <a name="understand-app-capabilities"></a>앱 기능 이해

최종 사용자가 Teams 내에서 작업할 수 있는 풍부한 환경을 제공하기 위해 앱 개발자는 다음 앱 기능을 활용합니다. 메시징 확장을 사용하면 사용자가 웹 서비스 Teams 클라이언트와 상호 작용할 수 있습니다. 외부 시스템에서 작업을 검색하거나 시작합니다. 상호 작용의 결과를 풍부한 형식의 카드로 Teams 클라이언트에 보낼 수 있습니다. 모임 확장성 앱은 개발자의 앱을 모임 내에 통합하고 응답성이 뛰어난 모임 내 환경을 제공합니다.

봇을 챗봇 또는 대화형 봇이라고도 합니다. 간단하고 반복적인 작업을 실행하는 앱입니다. 봇 상호 작용은 빠른 질문과 대답이거나 서비스 또는 지원에 대한 액세스를 제공하는 복잡한 대화일 수 있습니다. 사용자는 일대일 또는 채널에서 조금 채팅할 수 있습니다. 예를 들어 Polly 앱을 사용하여 빠른 설문 조사를 만들고, 피드백을 받고, 펄스 검사를 수행할 수 있습니다.
...
웹후크 및 커넥터는 최종 사용자가 자주 사용하는 서비스(예: Jira Cloud 및 Bitbucket)에서 채널 대화로 직접 콘텐츠 및 업데이트를 제공합니다. 이 기능을 사용하는 앱은 외부 앱과 통신할 수 있으며 외부 서비스에서 알림 및 메시지를 보내거나 받을 수 있습니다.

메시징 확장은 최종 사용자가 대화에서 벗어나지 않고도 앱 콘텐츠를 삽입하거나 메시지에 대한 작업을 수행하는 바로 가기입니다. 메시징 확장에는 최종 사용자가 외부 콘텐츠를 빠르게 찾고 메시지 또는 작업 명령에 삽입할 수 있는 검색 명령이 있을 수 있습니다.

Teams 기능에 매핑된 일반적인 사용 사례를 보려면 [Teams 앱 기능에 사용 사례 매핑을 참조하세요](/microsoftteams/platform/concepts/design/map-use-cases).
