...
description: Teams Phone Mobile을 구성하는 방법에 대해 자세히 알아봅니다.
ms.custom:
- seo-marvel-apr2020
- seo-marvel-jun2020
appliesto:
...
1. Teams Phone Mobile을 지원하는 운영자를 선택합니다. **모든 연산자** 탭에서 지역 또는 서비스별로 사용 가능한 연산자를 필터링하여 Teams Phone Mobile을 지원하는 올바른 운영자를 찾습니다. 그런 다음, 사용하도록 설정할 연산자를 선택합니다.

2. **운영자 설정** 에서 선택한 연산자를 사용하여 사용하도록 설정할 국가를 선택합니다.

3. **연락처 정보를 제공합니다.** 전체 이름 및 전자 메일 주소를 포함한 연락처 정보는 운영자와 자동으로 공유됩니다. 나중에 이 정보를 변경할 수 있습니다. 또한 회사 크기를 제공해야 하며 전화 번호를 제공하는 옵션이 제공됩니다. 운영자는 이 정보를 사용하여 Teams Phone Mobile에 대한 자세한 내용을 문의합니다.
...
기존 회사 유료 SIM 사용 전화 번호를 Teams에 추가하려면 운영자에게 문의하여 적격 Teams Phone Mobile 구독이 있는지 확인하고 Teams에 번호를 업로드할 수 있습니다. 운영자가 주문을 완료한 후 해당 숫자를 사용자에게 할당할 수 있습니다. 

운영자의 웹 사이트를 찾으려면 [Microsoft 365 Teams Phone Mobile 디렉터리를 참조하세요](https://cloudpartners.transform.microsoft.com/practices/microsoft-365-for-operators/directory).
...
- [통화 플랜에서 Teams Phone Mobile로 번호를 이동합니다](#move-numbers-from-calling-plans-to-teams-phone-mobile).  

- [Operator Connect에서 Teams Phone Mobile로 번호를 이동합니다](#move-numbers-from-operator-connect-to-teams-phone-mobile).  

- [직접 라우팅에서 Teams Phone Mobile로 번호를 이동합니다](#move-numbers-from-direct-routing-to-teams-phone-mobile).  
...
1. **전화 시스템 라이선스 및 Teams Phone Mobile 추가 기능 라이선스를 할당합니다.** Microsoft 365 관리 센터 또는 PowerShell을 사용하여 사용자에게 전화 시스템 라이선스 및 Teams Phone Mobile 추가 기능 라이선스를 할당할 수 있습니다. 자세한 내용은 [사용자에게 Teams 추가 기능 라이선스 할당을 참조하세요](teams-add-on-licensing/assign-teams-add-on-licenses.md).

2. **Teams Phone Mobile을 사용하여 획득한 전화 번호가 할당되는 사용자는 TeamsOnly 모드에 있어야 합니다.** 조직이 TeamsOnly 모드인 경우 모든 사용자가 TeamsOnly 모드에 있습니다. 

   확인하려면 Teams 관리 센터에서 **Teams > Teams 업그레이드 설정** 으로 이동합니다. 조직이 아일랜드 모드인 경우 특정 사용자가 TeamsOnly 모드에 있는지 확인합니다. **사용자** 로 이동하여 사용자 계정을 선택합니다. **계정** 탭의 **Teams 업그레이드** 에서 공존 모드를 TeamsOnly로 설정해야 합니다.

3. **숫자를 획득합니다.** 운영자의 웹 사이트로 이동하거나 Teams Phone Mobile Service를 사용하도록 설정된 모바일 SIM 지원 전화 번호를 주문하고 얻으려면 연락하세요. 
...
### <a name="move-numbers-from-calling-plans-to-teams-phone-mobile"></a>통화 플랜에서 Teams Phone Mobile로 번호 이동

1. Teams Phone Mobile 및 Teams Phone Mobile 추가 기능 라이선스에 적합한 Microsoft 365 구독이 있는지 확인합니다. [각 사용자에 대해 이동할 전화 번호를 제거해야 합니다](assign-change-or-remove-a-phone-number-for-a-user.md#remove-a-phone-number-from-a-user). 

2. SIM 사용이 가능한 적격 무선 음성 요금제에서 Teams Phone Mobile에 번호를 이식하려면 운영자에게 문의하세요. 
...
### <a name="move-numbers-from-operator-connect-to-teams-phone-mobile"></a>Operator Connect에서 Teams Phone Mobile로 번호 이동

1. Teams Phone Mobile 및 Teams Phone Mobile 추가 기능 라이선스에 적합한 Microsoft 365 구독이 있는지 확인합니다. [각 사용자에 대해 이동할 전화 번호를 제거해야 합니다](assign-change-or-remove-a-phone-number-for-a-user.md#remove-a-phone-number-from-a-user). 기존 운영자 연결 공급자에게 문의하여 테넌트에서 전화 번호를 제거합니다.
...
### <a name="move-numbers-from-direct-routing-to-teams-phone-mobile"></a>직접 라우팅에서 Teams Phone Mobile로 번호 이동   

직접 라우팅에서 Teams Phone Mobile로 숫자를 이동하려면 다음 단계를 완료해야 합니다.
...
2. [직접 라우팅에서 Teams Phone Mobile로 숫자를 마이그레이션합니다](#migrate-the-numbers-from-direct-routing-to-teams-phone-mobile).
...
#### <a name="migrate-the-numbers-from-direct-routing-to-teams-phone-mobile"></a>직접 라우팅에서 Teams Phone Mobile로 번호 마이그레이션

숫자를 마이그레이션하려면 아래 단계를 수행합니다.  

> [!Important]
> 마이그레이션하는 동안 전화 번호가 서비스가 중단됩니다. 마이그레이션을 시작하기 전에 Teams Phone 통신사와 조정합니다.

- **온라인으로 할당된 기존 직접 라우팅 번호를 Teams Phone Mobile로 마이그레이션하려면 운영자에게** 문의하세요. 운영자의 웹 사이트를 찾으려면 [Microsoft 365 Teams Phone 모바일 디렉터리를 참조하세요](https://cloudpartners.transform.microsoft.com/practices/microsoft-365-for-operators/directory). 합의된 날짜 및 시간에 운영자는 직접 라우팅에서 Teams Phone Mobile로 번호를 마이그레이션합니다. 여기에는 테넌트에서 마이그레이션되는 전화 번호를 제거하고 Teams Phone Mobile과 연결된 새 전화 번호로 다시 추가하는 작업이 포함될 수 있습니다.

- **온-프레미스에 할당된 직접 라우팅 번호를 Teams Phone Mobile로 마이그레이션하려면** 다음 비즈니스용 Skype 서버 PowerShell 명령을 실행합니다.
...
SIM 사용이 가능한 적격 무선 음성 요금제에서 Teams Phone Mobile에 번호를 이식하려면 운영자에게 문의하세요.

운영자가 주문을 완료하면 테넌트에 번호를 업로드합니다. **음성 > 전화 번호** 로 이동하여 Teams 관리 센터에서 번호와 공급자를 볼 수 있습니다. 

#### <a name="assign-phone-numbers"></a>전화 번호 할당

Teams 관리 센터를 사용하거나 PowerShell을 사용하여 사용자에게 Teams 전화 휴대폰 번호를 할당할 수 있습니다. 자세한 내용은 [숫자 할당을 참조하세요](assign-change-or-remove-a-phone-number-for-a-user.md).
...
Teams 관리 센터를 사용하거나 PowerShell을 사용하여 사용자의 수신 통화 정책을 관리할 수 있습니다. 기본적으로 Teams Phone Mobile 사용자에 대한 수신 전화는 사용자의 SIM 지원 모바일 장치에서 Teams 앱을 먼저 울릴 것입니다. 
