...
QoS를 구현할 때 몇 가지 혼잡 관리 기능 (예: Cisco의 우선 순위 큐 및 클래스 기반 가중치 공정 대기열 [CBWFQ](https://www.cisco.com/en/US/docs/ios/12_0t/12_0t5/feature/guide/cbwfq.html#wp17641))과 혼잡 방지 기능 (예: 가중치가 지정 된 난수) 중 하나를 사용 하 여 여러 큐를 정의 합니다. 감지 [w 빨간색](https://en.wikipedia.org/wiki/Weighted_random_early_detection)).
...
이 방법을 사용 하면 네트워크 라우터가 들어오는 패킷을 검사 하 고 특정 포트 또는 포트 범위를 사용 하 여 패킷이 도착 하는 경우 특정 미디어 유형으로 식별 하 고 해당 형식의 큐에 배치 하 고 미리 정의 된 [DSCP](https://tools.ietf.org/html/rfc2474) 표시를 IP에 추가 합니다. 다른 장치에서 해당 트래픽 유형을 인식 하 고 대기열에 우선 순위를 지정할 수 있도록 하는 패킷 헤더
...
> [!NOTE]
> 비즈니스용 Skype Online의 원본 포트 범위 및 DSCP 표시를 기준으로 QoS를 이미 구성한 경우에는 동일한 구성이 팀에 적용 되 고, 매핑에 대 한 추가 클라이언트 또는 네트워크 변경이 필요 하지는 않지만 범위를 설정 해야 할 수 있습니다. [ ](meeting-settings-in-teams.md#set-how-you-want-to-handle-real-time-media-traffic-for-teams-meetings)비즈니스용 Skype Online에 대해 구성 된 항목과 일치 하도록 팀 관리 센터에서 사용 합니다.
...
[Microsoft 팀을 위한 조직의 네트워크 준비](prepare-network.md)
