...
2020년 3월에는 다운로드 가능한 CQD용 [Power BI 쿼리 템플릿](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Teams/downloads/CQD-Power-BI-query-templates.zip?raw=true)에 Microsoft Teams CQD(통화 품질 대시보드) PSTN 직접 라우팅 보고서를 추가했습니다. 
...
2.  사용자 피드백 맵에 빨간색 반점이 증가하는 경우 네트워크 효율성 비율 페이지 및 네트워크 매개 변수로 이동하여 변칙이 있는지 확인하고 MS 서비스 데스크 사용하여 티켓을 올릴 수 있습니다.
...
매시간 NER 양호한 백분율 추세 21:00 경에 딥이 발생하는 것을 확인할 수 있습니다. 그런 다음 다시 클릭하여 21시간으로 확대하고 효과 있는 통화 세부 정보를 확인하여 해당 시간에 실패한 통화 수와 통화 종료 이유가 무엇인지 확인합니다. SBC 문제에서 자체 문제 촬영을 시작하거나 문제가 SBC와 관련이 없는 경우 서비스 데스크 보고할 수 있습니다.
...
특정 날짜(예: 2020년 2월 14일의 대기 시간)에 대한 4개 차트(대기 시간, 지터, 패키지 손실률, 사후 다이얼 지연)가 급증하면 날짜 지점을 클릭합니다. 아래쪽의 시간별 추세 차트는 시간별 숫자를 표시하도록 새로 고쳐집니다. SCC를 확인하거나 MS 서비스 데스크 사용하여 티켓을 올릴 수 있습니다.
