...
사용자 설정은 응용 프로그램이 사용자 기준 응용 프로그램 동작을 사용자 지정하는 등 다양한 용도로 메타데이터를 저장하는 데 사용할 수 있는 키-값 쌍입니다. 각 사용자는 사용자 설정에 대한 저장소를 수신합니다. 소유자만 사용자 설정을 추가, 수정 및 제거할 수 있습니다.
  
 **전역 설정**
  
전역 설정은 시스템 사용자가 소유한 사용자 설정으로, 모든 사용자가 해당 설정에 대한 읽기 전용 액세스 권한을 습니다. 전역 설정은 상수입니다. 이 설정은 리포지토리를 만들 때 만들어지며 변경되지 않습니다.
