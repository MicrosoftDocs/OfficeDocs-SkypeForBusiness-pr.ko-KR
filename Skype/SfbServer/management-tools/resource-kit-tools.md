...
비즈니스용 Skype 서버 2015 Resource Kit는 비즈니스용 Skype 서버 2015에 필요한 사양을 충족하는 서버에 설치해야 합니다(일반적으로 비즈니스용 Skype 서버 2015 실행에 사용).

## <a name="resource-kit-tools-overview"></a>리소스 키트 도구 개요

다음은 비즈니스용 Skype 서버 2015 Resource Kit에서 제공하는 도구 목록입니다. 다음 섹션에서는 요구 사항 및 예제 사용법을 포함하여 각 도구에 대한 설명을 제공합니다.
...
- [서비스 Storage 가져오기](resource-kit-tools.md#Issd)
...
ABSConfig(주소 기록 서비스 구성 도구)는 관리자가 2015년 8월의 주소 비즈니스용 Skype 서버 사용자 지정하는 데 도움이 되는 관리 도구입니다. 또한 이 도구를 사용하면 비즈니스용 Skype 서버 2015 관리자가 기본 주소 기록 서비스 설정을 복원할 수 있습니다.
...
- 관리자가 Active Directory 도메인 서비스의 특성을 2015 또는 2015의 특성에 매핑할 비즈니스용 Skype 서버 합니다.
...
ABSConfig는 2015가 설치된 도메인에 가입된 컴퓨터에서만 비즈니스용 Skype 서버 수 있습니다. 비즈니스용 Skype 서버 2015의 경우 Enterprise Edition 설정하는 동안 주소 Front-End 서버의 모든 서버에서 이 도구를 실행할 수 있습니다.

#### <a name="network"></a>네트워크

컴퓨터가 데이터베이스 풀 및 백 엔드 Front-End 연결할 수 있습니다.
...
ABSConfig 도구를 사용하면 관리자에게 2015년 주소 비즈니스용 Skype 서버 쉽고 빠르게 사용자 지정할 수 있습니다.
...
대역폭 정책 서비스 모니터 도구는 네트워크 토폴로지의 일부인 컴퓨터에서 비즈니스용 Skype 서버 합니다.
...
명령 프롬프트에서 WanLinkLogCollector.exe 탐색기를 사용하여 Windows.
...
2. **종료 날짜** 보고서가 생성될 시간 표시 막대의 종료 날짜입니다. 예: 2010년 9월 30일
     ...
     ![대역폭 사용률 A의 시작 날짜 및 종료 날짜입니다.](../media/Reskit_2012_Tools_Documentation_Image4.jpg)
...
- **임시 파일 저장소 위치** 보고서를 생성하는 동안 중간 파일이 저장되는 임시 파일 위치입니다.
    ...
    ![대역폭 사용률 Anal의 파일 directories.](../media/Reskit_2012_Tools_Documentation_Image5.jpg)
...
통화 파크로미터는 현재 사용 중인 통화를 추적하는 도구입니다. 또한 궤도 및 CPS(통화 파기 서버) 사용에 대한 통계도 수집합니다. 이 명령줄 도구는 로컬 또는 원격으로 연결된 컴퓨터에서 CPS SQL Server 데이터베이스에 대한 읽기 및 쓰기 액세스를 모두 제공합니다.
 ...
 **컴퓨터** DBAnalyze는 2015가 설치된 도메인에 가입된 컴퓨터에서만 비즈니스용 Skype 서버 수 있습니다.
...
DBAnalyzer는 관리자에게 2015 데이터베이스를 빠르고 비즈니스용 Skype 서버 제공합니다.

## <a name="import-storage-service-data"></a>서비스 Storage 가져오기
<a name="Issd"> </a>

ImportStorageServiceData 리소스 키트 도구를 사용하면 LYSS(Storage Service)에서 플러시된 큐 및 끝점 데이터를 다시 Storage 있습니다.

### <a name="description"></a>설명

Storage 서비스에서 플러시된 데이터는 큐 항목 상태 또는 데이터베이스 크기에 따라 자동(주기적)일 수 있습니다. 풀 장애 조치(failover) cmdlet 또는 풀 장애 조치(failover) cmdlet이 호출하는 StorageServiceFullFlush cmdlet의 수동 호출로 인해 이 오류가 발생했습니다. 프런트 엔드의 LYSS(Storage Service) 데이터베이스 크기가 정상 수준보다 크면 데이터가 더 많이 내보내질 수 있기 때문에 데이터를 다시 전송하지 않는 것이 이상적입니다. 또한 오류로 인해 Storage 서비스 큐가 커질 수 있는 모든 문제를 먼저 해결해야 합니다(예: Exchange 끝점 오류, 네트워크 문제 또는 기타 문제).

 **시나리오 1:** 풀 장애 조치(failover) 중에 각 프런트 엔드에 대해 저장소 서비스에서 파일이 플러시될 수 있습니다. 장애 조치(failover)가 완료되면 도구를 실행하여 데이터를 다시 전송해야 합니다.

 **시나리오 2:** 데이터가 매일 자동으로 플러시되거나 Storage 서비스 데이터베이스가 특정 크기 임계값을 초과하는 경우(예: 60%, 80%, 90% 전체)에 대한 응답으로 데이터가 플러시됩니다. 이 자동 플러시된 데이터는 관리자가 정기적으로 다시 제공해야 합니다. 위의 상황에서는 모니터링 SCOM 팩이 배포되지 않은 경우 비즈니스용 Skype 서버 Storage 서비스에서 플러시되는 데이터와 관련된 Storage 이벤트가 있습니다. 32075(전체 플러시 작업이 시작), 32076(전체 플러시가 완료되었습니다), 32082(유지 관리 수준 플러시 시작), 32083(유지 관리 수준 플러시 완료), 32089(데이터베이스 채우기 때문에 플러시가 발생) 이러한 이벤트 ID는 RTM 릴리스에 해당합니다. 관리자가 이러한 이벤트를 볼 때 플러시된 파일이 있는 것입니다. 이 데이터는 이 도구를 사용하여 정기적으로 다시 가져와야 합니다(예: 주 1회).

온라인 서비스 릴리스의 경우 비즈니스용 Skype 서버 상태 모니터링 SCOM 팩이 배포된 경우 관리자가 플러시된 데이터를 다시 Storage 서비스로 다시 보내야 하는 새로운 경고가 Storage 있습니다. 경고를 트리거한 서버의 이벤트 로그에 Front-End 이벤트가 있습니다. 이 이벤트는 플러시된 데이터 파일이 있는 상위 경로와 경고 조건을 충족하는 파일의 수에 대한 설명을 제공합니다. 경고 조건은 적어도 Y일이 지난 특정 상위 경로 아래에 X 이상의 파일이 있는 것입니다. 여기서 X와 Y는 StorageService 내에 미리 설정되지만 APPCONFIG 파일을 변경하여 다시 정의할 수 있습니다. 상태 경고를 트리거할 수 있는 이벤트의 두 가지 예는 다음과 같습니다. 차이점은 부모 경로입니다. 한 가지 가능성은 웹 서비스 파일 공유에 있는 반면 다른 하나는 각 프런트 엔드의 로컬 응용 프로그램 데이터 디렉터리입니다. (예: c:\ProgramData\Microsoft\비즈니스용 Skype 서버 2015\StorageService). 그런 다음 관리자가 이 Reskit 도구를 실행합니다.
...
LCSSync 도구를 사용하면 다중 포리스트 환경에 비즈니스용 Skype 서버 2015 통신 소프트웨어를 배포할 수 있습니다. 이 도구는 Active Directory 도메인 서비스 연락처 개체로 서로 다른 사용자 포리스트의 사용자 및 그룹을 2015 또는 2015가 설치된 중앙 비즈니스용 Skype 서버 동기화하는 데 사용됩니다.

### <a name="description"></a>설명

 LCSSync는 중앙 포리스트의 동기화된 Active Directory 도메인 서비스 연락처 개체를 사용하여 사용자가 동기화된 Active Directory 도메인 서비스 연락처 개체를 비즈니스용 Skype 서버. 단일 로그인을 제공하려면 기본 사용자 계정이 2015년 10월에 대한 중앙 포리스트의 Active Directory 도메인 서비스 연락처 개체에 비즈니스용 Skype 서버 합니다. 이 도구는 해당 매핑을 수행하는 데 도움이 됩니다. 이 도구는 Microsoft Identity Integration Server에서 관리 에이전트를 만들기 위한 템플릿을 제공합니다.
...
LookupUserConsole 도구는 특정 사용자에 대한 비즈니스용 Skype 서버 라우팅 정보를 표시합니다. 이 정보는 배포 및 라우팅 문제를진단하는 데 개인을 지원하는 데 유용할 수 있습니다.
...
MSTurnPing 도구를 사용하면 비즈니스용 Skype 서버 2015 통신 소프트웨어의 관리자가 오디오/비디오 에지, 오디오/비디오 인증 서비스를 실행하는 서버 및 토폴로지에서 대역폭 정책 서비스를 실행하는 서버의 상태를 확인할 수 있습니다.
...
1. A/V 에지 서버 테스트: 이 도구는 다음을 수행하여 토폴로지의 모든 A/V 에지 서버에 대해 테스트를 수행합니다.
   ...
   - 오디오/비즈니스용 Skype 서버 인증 서비스가 시작되어 적절한 자격 증명을 발급할 수 있는지 확인

   - 오디오/비즈니스용 Skype 서버 에지 서비스가 시작되어 외부 에지의 리소스를 할당할 수 있는지 확인

2. 대역폭 정책 서비스 테스트: 이 도구는 토폴로지에서 대역폭 정책 서비스를 실행하는 모든 서버에 대해 다음을 수행하여 테스트를 수행합니다.

   - 비즈니스용 Skype 서버 대역폭 정책 서비스(인증)가 시작되어 적절한 자격 증명을 발급할 수 있는지 확인

   - 비즈니스용 Skype 서버 대역폭 정책 서비스(핵심)가 시작되어 대역폭 검사를 성공적으로 수행할 수 있는지 확인
...
- **AudioVideoEdgeServer** 테스트가 수행되는 경우 도구 출력은 다음과 같습니다.
  ...
  - 토폴로지에서 비즈니스용 Skype 서버/비디오 인증 서비스를 제공하는 컴퓨터의 테스트 결과
...
이 도구는 오디오/비디오 및 대역폭 정책 서비스를 비즈니스용 Skype 서버 서버의 상태를 확인하려는 2015 관리자에게 유용한 리소스가 될 수 있습니다.

## <a name="network-configuration-viewer"></a>네트워크 구성 뷰어
<a name="NCV"> </a>

네트워크 구성 뷰어는 비즈니스용 Skype 서버 2015 통신 소프트웨어 관리자가 지정된 대역폭 용량에 따라 음성 또는 화상 통화와 같은 실시간 통신 세션을 허용하도록 프로비전된 엔터프라이즈의 CAC(통화 허용 제어) 네트워크 토폴로지 보기에 사용할 수 있습니다. 비즈니스용 Skype 서버 2015 관리자는 2015와 함께 설치되는 대역폭 정책 서비스에서 적용하는 CAC 정책을 비즈니스용 Skype 서버 정의합니다.
 ...
 **CAC** 네트워크 토폴로지 링크(예: 사이트-지역, 지역-지역 및 사이트-사이트 링크)에 대한 사용자 지정 커넥터 정의: 비즈니스용 Skype 서버 2015 관리자는 아래 표시된 설정 옵션을 사용하여 CAC 네트워크 구성 WAN 링크에 대한 사용자 지정 그래픽 커넥터를 정의할 수 있습니다. 이렇게 하면 네트워크 구성에 프로비전되는 다양한 유형의 네트워크 링크를 차별화할 수 있습니다.

![도구.](../media/Reskit_2012_Tools_Documentation_Image29.jpg)

 **CAC 네트워크** 토폴로지 사이트 정보, 지역 정보 및 프로비전된 대역폭 정책 보기: 비즈니스용 Skype 서버 2015 관리자는 아래 표시된 옵션을 사용하여 관련 CAC 네트워크 지역 정보, 사이트 정보 및 CAC 대역폭 프로비전 정보를 볼 수 있습니다. 예를 들어 네트워크 지역 **또는** 네트워크 사이트 개체에서 정보를 클릭합니다.

![네트워크에 대한 사용자 지정 커넥터 정의](../media/Reskit_2012_Tools_Documentation_Image30.jpg)

### <a name="summary"></a>요약

이 도구는 배포에 대한 CAC 네트워크 비즈니스용 Skype 서버 그래픽 형식으로 보고자 하는 2015 관리자에게 유용한 리소스가 될 수 있습니다.

## <a name="response-group-agent-live"></a>응답 그룹 에이전트 Live
<a name="RGAL"> </a>

응답 그룹 응용 프로그램을 사용하면 에이전트가 기본 제공 웹 서비스를 사용하여 유용한 실시간 정보에 액세스할 수 있습니다. 안타깝게도 이 데이터의 그래픽 보기는 응용 프로그램 외부에서 사용할 수 없습니다. 응답 그룹 에이전트 Live Resource Kit 도구는 이 정보에 액세스할 수 있는 간단하고 그래픽적인 방법을 제공하여 다른 에이전트의 존재와 같은 실시간 비즈니스용 Skype 소프트웨어 정보로 개선하여 이 문제를 해결합니다.
...
응답 그룹 에이전트 라이브 도구는 응답 그룹 서비스 및 2015 비즈니스용 Skype 서버 SDK를 사용하여 구축됩니다. 응답 그룹 에이전트는 응답 그룹 서비스에서 사용할 수 있는 정보 및 기능(예: 그룹 구성원, 다른 에이전트의 현재 상태 및 대기 전화 수)을 제공합니다.
...
- **로그인/아웃:** 에이전트 그룹 페이지(비즈니스용 Skype 서버 2015에서 액세스할 수 있는)와는 다른 응답 그룹 에이전트 Live를 사용하면 에이전트만 한에 모든 에이전트 그룹에 로그인하거나 그룹에서 로그인할 수 있습니다. 이 응용 프로그램은 에이전트가 로그인 또는 아웃할 수 있는 세 가지 빠른 방법을 제공합니다.
...
응답 그룹 에이전트 Live를 사용하려면 .NET Framework 4.0이 필요합니다. 또한 현재 상태 및 연락처 카드 기능을 활용하려면 비즈니스용 Skype 설치해야 합니다(실행 중).
...
- 바로 가기: 로그인/아웃하기 위한 정확한 바로 가기를 사용자 정의할 수 있습니다. 유일한 제한은 두 바로 가기 모두 "Windows 로고" 키와 함께 다른 키가 포함되어야 하다는 것입니다.
...
빠른 로그인 및 로그인, 그룹 구성원 자격 및 기본 실시간 통계는 응답 그룹 서비스에서 응용 프로그램 외부에서만 사용할 수 있는 흥미로운 응답 그룹 에이전트 기능입니다. 응답 그룹 에이전트 Live Resource Kit 도구를 사용하여 비즈니스용 Skype 서버 2015 관리자는 에이전트에 보다 빠르고 그래픽적인 방식으로 작업을 수행할 수 있는 Windows 응용 프로그램을 제공할 수 있습니다.
...
SEFAUtil(보조 내선 기능 활성화)을 사용하면 비즈니스용 Skype 서버 2015 관리자 및 헬프데스크 에이전트가 비즈니스용 Skype 서버 2015 사용자를 대신하여 위임 벨 울림, 통화 전달, 동시 전화 울림, 팀 통화 설정 및 그룹 통화 pickup을 구성할 수 있습니다. 또한 관리자는 이 도구를 사용하여 특정 사용자에 대해 게시된 통화 라우팅 설정을 쿼리할 수 있습니다.
...
- Bob은 임원으로, 전화 통신을 비즈니스용 Skype 서버 이동한 바 있습니다. 기존 PBX 시스템에 위임이 있습니다. 비즈니스용 Skype 서버 2015로 이동하는 경우 관리자는 Bob의 라우팅에 기존 위임 구성을 반영하도록 구성할 수 있습니다.

- Alice는 출장 중이라 고객 중 한 명으로부터 중요한 전화를 걸고 있을 것으로 기대하고 있습니다. 그러나 이 부사장은 호텔에 있으며 컴퓨터에 액세스할 수 없습니다. 헬프데스크에 전화를 걸고 자신의 업무 번호로 걸려 올 모든 전화를 휴대폰 번호로 전달해 줄 수 있도록 요청합니다. 지원 담당자가 대신 구성을 할 수 있습니다.
...
- Mike는 Contoso의 신입 사원으로, 비즈니스용 Skype 서버 2015를 사용하도록 설정한 경우 모든 구성원이 팀 통화에 대해 구성된 새 팀에 합류하게 됩니다. 관리자는 자신의 팀 통화 그룹 설정을 설정하여 모든 새 팀 구성원을 포함할 수 있으며, 또한 관리자는 Mike를 팀의 각 구성원에 대한 팀 통화 그룹 구성원으로 추가할 수 있습니다.

- Contoso의 인적 자원 부서의 고객 서비스 관행은 첫 번째 통화 이후 모든 발신자에 대해 개인 서비스를 제공하는 것입니다. 부서의 모든 구성원이 서로 매우 가까이 있는 경우 팀 통화를 통해 모든 전화가 동시에 울리게 하는 것은 팀에 지장입니다. 팀 구성원을 방해하지 않고 최상의 서비스를 제공하기 위해 비즈니스용 Skype 서버 2015 관리자는 그룹 통화 선택 기능을 활용합니다. 관리자는 모든 부서 구성원을 Pickup 그룹에 추가하고 해당 부서에 Pickup 그룹 번호를 전달합니다. Samantha가 책상에 서지 못하면 Joe는 전화를 울리며 책상에서 전화를 계속 습니다.
...
2. 신뢰할 수 있는 응용 프로그램은 SEFAUtil 도구에 대한 토폴로지에서 정의해야 합니다. SEFAUtil을 새 신뢰할 수 있는 응용 프로그램으로 정의하기 위해 비즈니스용 Skype 서버 관리 셸을 사용하여 다음 cmdlet을 실행합니다.
   ...
   ```
    ...
    > [!NOTE]
    > 풀 FQDN: SEFAUtil 응용 프로그램을 호스팅할 서버 또는 풀의 FQDN(일반적으로 비즈니스용 Skype 프런트 엔드 서버 > 풀).
...
4. 필요한 경우 SEFAUtil 도구를 실행하기 위해 사용할 서버에 비즈니스용 Skype 서버 2015 Resource Kit Tools를 설치합니다(서버는 신뢰할 수 있는 응용 프로그램 풀의 일부임).

5. SEFAUtil이 제대로 실행되고 있는지 확인합니다. 이렇게하려면 관리자 권한으로 Windows 명령 프롬프트에서 도구를 실행하여 배포에 있는 사용자의 통화 전달 설정을 표시합니다. 기본적으로 도구는 "...\Program Files\비즈니스용 Skype 서버 2015\Reskit"에 있습니다. 사용자의 통화 전달 설정을 표시하려면 다음 명령을 사용합니다.
...
이 예에서는 전화 전달/응답 없음 대상 및 링 지연을 설정합니다. 여기서는 /server 스위치가 제공되지 않습니다. SEFAUtil은 2015에서 자동 비즈니스용 Skype 서버 시도합니다.
...
> [!NOTE]
> 사용자의 팀 통화 그룹에 구성원을 추가하면 사용자의 동시 벨 울림 집합이 자동으로 전환되어 팀 통화 그룹의 동시 벨이 울리게 됩니다.
...
SYSPrep.ps1 Windows PowerShell Server 2008 운영 체제 컴퓨터에 다음 비즈니스용 Skype 서버 2015 사전 Windows 설치하는 Windows 스크립트입니다.
...
- 비즈니스용 Skype 서버 2015 Core 파일
  ...
  스크립트 이름은 Microsoft Windows 운영 체제용 시스템 준비 도구와 유사하기는 하지만 서로 다릅니다. 이 스크립트는 2015에 필요한 필수 비즈니스용 Skype 서버 설치합니다. 이러한 선행 Windows SYSPrep 도구를 사용하여 서버 이미지를 만들 수 있습니다.
...
비즈니스용 Skype 서버 2015 관리자는 미지정 번호 공지 마이그레이션 도구를 사용하여 공지사항 응용 프로그램에서 제공하는 미지정 번호 구성을 원본 비즈니스용 Skype 서버 또는 풀에서 대상 비즈니스용 Skype 서버 또는 풀로 이동할 수 있습니다.
...
1. 이 스크립트는 관리 셸이 설치된 컴퓨터에서 비즈니스용 Skype 서버 실행해야 합니다.

2. 발표 응용 프로그램은 서버 또는 풀의 원본 및 대상 비즈니스용 Skype 배포해야 합니다.
...
웹 구성 데이터 도구를 사용하면 비즈니스용 Skype 서버 2015 통신 소프트웨어 관리자가 이끌이의 웹 회의와 연결된 데이터를 보다 세분화할 수 있습니다. 시나리오에는 타임스탬프 조건에 따라 특정 사용자의 모임 데이터를 삭제하는 능력이 포함됩니다.
...
4. 한 풀에서 다른 풀로 이동할 때 단일 사용자와 연결된 모든 웹 회의 데이터를 이동합니다.
    ...
    > [!NOTE]
    > Lync Server 2010용 리소스 키트 도구는 사용자가 한 풀에서 다른 풀로 이동할 때 단일 사용자와 연결된 모든 웹 회의 데이터를 이동할 수 있도록 지원했습니다. 이 기능은 이제 **MoveConferenceData** 매개 변수를 위해 이 도구에서 더 이상 사용되지 않습니다. 이 매개 변수에 대한 자세한 내용은 [Move-CsUser](/powershell/module/skype/move-csuser?) cmdlet을 참조합니다.
