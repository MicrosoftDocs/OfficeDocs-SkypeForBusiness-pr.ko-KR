...
[비즈니스용 Skype 서버 2015 용량](https://www.microsoft.com/download/details.aspx?id=51196) 계산기 및 [비즈니스용 Skype 서버 2019](https://www.microsoft.com/download/details.aspx?id=57509) 용량 계산기는 비즈니스용 Skype [계획](https://www.microsoft.com/download/details.aspx?id=50357) 도구 및 배포 설명서(비즈니스용 Skype 서버 [2015](../plan-your-deployment/plan-your-deployment.md) 배포 계획 및 사용자 계획)를 확장합니다.[ 비즈니스용 Skype 서버 2019 배포](../../SfBServer2019/plan/plan-your-deployment-2019.md)) 가이드를 검토하고 계획 도구를 사용하여 권장 토폴로지가 만들어진 후 계산기를 사용합니다.
...
계획 도구 및 용량 계획 계산기를 사용한 후 제안된 부하와 계획된 부하를 시뮬레이트하여 적절한 프로비전을 비즈니스용 Skype 서버 합니다. 시뮬레이트된 부하에서 스트레스 테스트를 수행하려면 비즈니스용 Skype 서버 및 [](https://www.microsoft.com/download/details.aspx?id=50367) 성능 도구에 비즈니스용 Skype 서버 [도구를 사용합니다](./stress-and-performance-tool/stress-and-performance-tool.md).
...
- 사용자 **설정 Enterprise Voice** 사용자에 대해 사용하도록 설정된 사용자의 백분율을 Enterprise Voice. 기본값은 60%입니다. 
    
- 시간 **당** 사용자당 평균 통화 수(최대)에 최대 부하 시간 동안 평균 사용자가 참여할 것으로 예상되는 시간당 통화 수를 입력합니다. 기본값은 4입니다. 
    
- 미디어 **우회** 를 사용하는 통화의 백분율에 중재 서버를 우회할 사용자가 걸은 통화의 백분율을 입력합니다. 기본값은 65%이지만, 지리적으로 분산되어 있는 경우나 집에서 일하는 사용자의 비율이 많은 경우 낮을 수 있습니다.
    
- **UC-PSTN** 통화에 참여한 음성 사용자의 백분율에 UC-PSTN 전화 통화인 조직의 통화 비율을 입력합니다. 기본값은 60%입니다.
    
- **UC-UC** 통화에 참여한 음성 사용자의 백분율은 UC-UC 통화에만 사용할 Enterprise Voice 사용자 비율을 보여줍니다. 이 번호는 **UC-PSTN** 통화에 사용하도록 설정된 음성 사용자의 백분율에 대해 입력한 개수에 따라 계산됩니다. 
    
  **회의**
  
- 동 **시** 회의의 사용자 비율에 동시에 회의에 참가할 사용자의 비율을 입력합니다. 기본값은 5%입니다. 
    
- 그룹 **IM만** 있는 회의의 백분율(음성 없음)에 인스턴트 메시징만 포함하며 오디오를 포함하지 않는 회의의 백분율을 입력합니다. 기본값은 10%입니다.
    
- 전화 **접속** 회의를 사용하는 사용자의 백분율에 전화 접속 회의를 한 번 사용할 회의 참가자의 백분율을 입력합니다. 기본값은 15%입니다.
    
- 음성 **을 사용하는 회의 비율** 에 오디오를 포함할 회의의 백분율을 입력합니다. 
    
  - 음성 회의의 20%에 일반 비디오도 포함되어 있는 경우 비디오 포함(다중 보기 **없음)** 확인란을 선택합니다.
    
  - 회의의 20%에 다중 보기 비디오도 포함하려면 다중 보기 포함 확인 **란을 선택합니다** .
    
  - 음성 회의의 50%에 응용 프로그램 공유도 포함하려면 응용 프로그램 공유 포함 확인 **란** 을 선택합니다.
    
  - 음성 회의의 20%에 프레젠테이션과 같은 데이터 업로드가 PowerPoint 경우 웹 회의 포함 확인란 **을** 선택합니다.
    
  **이동성**
  
- 모바일 **기능을 사용할** 수 있는 사용자의 백분율에 모바일 장치를 사용하여 모바일 장치에 연결할 수 있는 비즈니스용 Skype 서버 백분율을 입력합니다. 기본값은 40%입니다. 
...
- **프런트 엔드 CPU**: 테스트에 사용된 서버와 동일한 사양의 프런트 엔드 서버 한 대에서 전체 부하를 처리한 경우 CPU 사용량의 백분율입니다(이 문서 끝에 설명 참조).
