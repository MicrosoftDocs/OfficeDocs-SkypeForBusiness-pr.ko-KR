...
OMS를 사용하도록 클라우드 커넥터의 사내 환경을 구성해야 합니다. 이렇게하려면 다음과 같이 OMS 포털을 사용하여 찾을 수 있는 OMS 작업 영역 ID 및 키가 필요합니다. 설정 -- 연결된 원본 \> -- \> Windows Server:
...
2. 설정 - \> 데이터 - \> Windows 성능 카운터로 이동하여 다음에 대한 성능 카운터를 추가합니다. 
   ...
   - **비즈니스용 Skype 카운터**. 비즈니스용 Skype에서 제공하는 다양한 카운터가 있습니다. 중재 서버에 로그온하고 성능 모니터를 열면 이러한 카운터를 찾을 수 있습니다. 이러한 카운터는 "LS:"로 시작됩니다. 최소한 다음 용량 카운터로 시작하고 관심 있는 다른 카운터를 추가하는 것이 좋습니다.
     ...
     자세한 내용은 Log Analytics에서 Windows 및 Linux 성능 [데이터 원본을 참조하세요.](/azure/log-analytics/log-analytics-data-sources-performance-counters)
...
- [Azure의 Log Analytics 서비스에 Windows 컴퓨터 연결](/azure/log-analytics/log-analytics-windows-agents)