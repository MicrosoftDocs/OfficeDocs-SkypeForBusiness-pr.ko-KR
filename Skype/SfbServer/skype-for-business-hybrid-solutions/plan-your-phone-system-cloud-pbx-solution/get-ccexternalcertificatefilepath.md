...
다음 예에서는 중재 서버에 대한 인증서 집합을 보여줍니다.
...
배포 중이나 토폴로지 수정 시에는 에지 서버 인증서의 경로와 중재 서버의 경로를 선택적으로 지정해야 합니다. 게이트웨이와 중재 서버 간에 TLS를 사용하려면 중재 서버에 대한 인증서가 필요합니다. 경로를 변경하기 위해 Set-CcExternalCertificateFilePath cmdlet을 사용 합니다.
...
|**매개 변수**|**필수**|**유형**|**설명**|
|:-----|:-----|:-----|:-----|
|Target(대상)  <br/> |선택  <br/> | System.Management.Automation.SwitchParameter <br/> |요청된 파일 경로의 유형입니다. 유형은 다음과 같습니다.  <br/> EdgeServer(기본값)  <br/> MediationServer  <br/> |
