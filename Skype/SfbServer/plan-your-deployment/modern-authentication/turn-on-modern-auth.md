...
__종류 1 설명:__ 이는 비즈니스용 Skype __Server에 대해 MA가 설정 된__ 경우의 기본 시나리오입니다. 즉 MA가 구성 *되 면이 출발점을 사용할 수* 있습니다.
...
__5 설명:__ *외부*의 adal 클라이언트는 MA를 사용 하 고, adal을 지원 하지 않는 클라이언트는 레거시 인증 방법을 사용 합니다. 하지만 *내부적* 으로 *모든 클라이언트가* 레거시 인증 (모든 ADAL 지원 클라이언트 포함)을 사용 합니다.
...
> [!NOTE]
> 
> BlockWindowsAuthExternally 매개 변수를 사용 하 여 외부에서 NTLM을 차단 하는 경우에도 SIP 채널에 대 한 NTLM을 내부적으로 차단 합니다. 그러나 t e 2010 보다 새로운 비즈니스용 Skype 및 Lync 클라이언트는 로그인 하는 데 HTTP를 통해 내부적으로 NTLM을 사용 하 고 SIP를 통해 로그인 하는 인증서를 가져올 수 있습니다. 그러나 2010 보다 오래 된 클라이언트는 내부적으로이 상황에 로그인 할 수 없으며, 사용자가 보안 기능을 다시 시작할 수 있도록 이러한 응용 프로그램을 업그레이드 하는 것을 고려해 볼 수 있습니다.
...
- 명령 또는이를 설치 하는 데 필요한 CU (을 사용 하는 방법에 대해 자세히 알아보세요.
    ...
    - [비즈니스용 Skype 서버 2015 업데이트](https://support.microsoft.com/en-us/help/3061064/updates-for-skype-for-business-server-2015) (일반)
