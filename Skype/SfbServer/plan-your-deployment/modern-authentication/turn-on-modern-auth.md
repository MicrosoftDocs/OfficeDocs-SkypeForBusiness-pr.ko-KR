...
> [!IMPORTANT]
> 아래 표 및 설명에서 최신 인증은  __MA__ 로 약어되고 Windows 통합 인증은 Win으로  약어 __됩니다__. 미리 Windows 통합 인증은 NTLM 인증과 Kerberos 인증의 두 가지 방법으로 나타남을 예고합니다. 테이블을 제대로 읽으면 이 내용을 알아야 합니다.
...
__유형 2 설명:__ 이 토폴로지에서는 외부에서 *NTLM을* 차단하지만 ADAL을 지원하지 않는 클라이언트의 경우 NTLM 또는 Kerberos가 내부적으로 작동 *할 수 있습니다*. 클라이언트가 ADAL을 지원하는 경우 내부적으로 MA를 사용하게 됩니다.
...
__유형 5 설명:__ *외부* 적으로 최신 ADAL 클라이언트는 MA를 사용하며 ADAL을 지원하지 않는 클라이언트는 레거시 인증 방법을 사용하게 됩니다. 그러나 *내부적으로 모든 클라이언트**는* 레거시 인증(모든 ADAL 지원 클라이언트 포함)을 사용하게 됩니다.
...
이 `Set-CsAuthConfig` cmdlet은 등록자 및 웹 서비스 역할 모두에 대한 구성에 영향을 미치게 됩니다.
...
- 명령을 사용하는 방법 또는 설치에 필요한 CU에 대한 자세한 내용은 다음을 실행합니다.
    ...
    - [비즈니스용 Skype 서버 업데이트](https://support.microsoft.com/help/3061064/updates-for-skype-for-business-server-2015)(일반)
