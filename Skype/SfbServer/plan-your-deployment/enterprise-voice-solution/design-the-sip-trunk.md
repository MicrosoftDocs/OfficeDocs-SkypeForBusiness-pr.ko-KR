...
두 번째 방법을 사용할 경우 선택한 SBC 작성 및 모델이 인증되었는지 확인하고 PIDF-LO(Presence Information Data Format Location Object) 위치 데이터를 SIP INVITE의 일부로 전달하도록 지원하는지 확인합니다. 그렇지 않으면 해당 위치 정보가 제거된 상태로 통화가 긴급 서비스 제공자에게 도착합니다. 인증된 SBC에 대한 자세한 내용은 ["Microsoft Lync에](../../../SfbPartnerCertification/lync-cert/qualified-ip-pbx-gateway.md) 대해 자격을 갖춘 인프라" 및 "인증을 위한 전화 통신 [비즈니스용 Skype"를 참조합니다.](../../../SfbPartnerCertification/certification/infra-gateways.md) 
...
에서 SIP 트렁크를 배포하는 비즈니스용 Skype 서버 [sIP](sip-trunking.md)트렁크를 비즈니스용 Skype 서버. 다음 질문은 E9-1-1을 위한 SP 트렁크 배포 방법을 결정하는 데 유용합니다.
...
> 비즈니스용 Skype 서버 데이터 네트워크가 있는 경우, 각 분기에 SIP 트렁크를 배포하거나, 정전 중에 로컬 게이트웨이로 통화를 푸시하는 등 지점에서 음성 탄력을 처리하기 위한 몇 가지 전략을 제공합니다. 자세한 내용은 에서 [SIP 트렁크를 비즈니스용 Skype 서버.](sip-trunking.md)
