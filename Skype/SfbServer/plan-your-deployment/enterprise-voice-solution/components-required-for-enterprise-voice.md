...
- 신호 및 일부 구성에서 내부 비즈니스용 Skype 서버, Enterprise Voice 인프라와 PSTN(Public Switched Telephone Network) 게이트웨이 또는 SIP(Session Initiation Protocol) 트렁크 간의 미디어를 변환하는 하나 이상의 중재 서버. 중재 서버는 배포에서 가장 중요한 Enterprise Voice 구성 요소입니다. 자세한 내용은 의 중재 서버 구성 [요소를 비즈니스용 Skype 서버.](mediation-server.md)
    
    중재 서버는 프런트 엔드 서버와 함께 함께 설치하거나 독립 실행형 서버로 설치할 수 있습니다.
    
- SIP 트렁크 또는 PSTN 게이트웨이를 포함할 수 있는 PSTN 연결 구성 요소 자세한 내용은 에서 [PSTN 연결](pstn-connectivity.md)구성 요소를 비즈니스용 Skype 서버.
...
- 또한 일부 Enterprise Voice 구성 요소는 프런트 엔드 서버에서 실행됩니다. 이러한 구성 요소에 대한 [](front-end-server-voip.md) 자세한 내용은 비즈니스용 Skype 서버
