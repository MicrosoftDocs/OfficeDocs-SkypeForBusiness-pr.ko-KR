...
예를 들어 전화 접속 회의를 허용하려면 PSTN(Public Switched Telephone Network)에 연결하기 위한 중재 서버 및 게이트웨이를 배포해야 합니다. 웹 회의를 허용하려면 웹앱 서버에 연결할 수 비즈니스용 Skype 서버 수 있도록 Office 합니다. 외부 사용자가 회의에 참가하도록 허용하려면 에지 서버를 배포해야 합니다.
...
비즈니스용 Skype 서버 웹 회의 서비스는 모임 중에 공유되는 콘텐츠를 파일 저장소에 저장합니다. 배포의 일부로, Standard Edition 서버 또는 프런트 엔드 풀의 파일 저장소로 사용할 Enterprise Edition 지정해야 합니다. 기존 파일 공유를 파일 저장소로 사용하거나 파일 공유가 위치할 파일 서버의 FQDN(정규화된 도메인 이름) 및 새 파일 공유의 폴더 이름을 지정하여 새 파일 공유를 지정할 수 있습니다. 자세한 내용은 [에서 파일](../../deploy/install/create-a-file-share.md)공유 만들기를 비즈니스용 Skype 서버. 웹 회의 서비스는 콘텐츠를 파일 저장소에 저장하기 전에 암호화합니다.
...
웹 회의 기능을 사용하려면 웹앱 서버를 설치하고 Office Web Apps 서버와 통신하도록 비즈니스용 Skype 서버 구성해야 Office 합니다.

Office 웹앱 서버는 응용 프로그램, 서버 응용 프로그램 또는 다른 서버 응용 프로그램을 실행하지 비즈니스용 Skype 서버 SQL Server 컴퓨터에 설치해야 합니다. 해당 컴퓨터에 설치된 버전의 Office 없습니다. Web Apps Server를 Office 컴퓨터에는 특정 소프트웨어 집합(.NET Framework 4.5 및 Windows PowerShell 3.0 포함)도 설치되어 있어야 합니다. 이러한 요구 사항과 인증서 및 IIS(인터넷 정보 서비스) 구성에 대한 정보는 Microsoft Office Web Apps 배포 웹 사이트에서 자세히 [설명합니다.](/webappsserver/deploy-the-infrastructure-office-web-apps-server)

웹앱 서버에서 작동하도록 비즈니스용 Skype 서버 방법에 대한 자세한 내용은 Office Web Apps Server와의 Office [구성을 비즈니스용 Skype 서버.](../../deploy/deploy-conferencing/office-web-app-server.md)
...
오디오 회의를 네트워크에 배포한 경우 사용자는 헤드셋과 같은 오디오 장치가 있어야 오디오 회의에 참가할 수 있습니다. 또한 비디오 회의를 배포한 경우 사용자를 위해 웹캠과 같은 비디오 장치를 배포해야 합니다. 오디오 및 비디오 디바이스 모두에서 디바이스 배포 및 사용자 교육은 고려해야 하는 중요한 단계입니다. 자세한 내용은 [Plan for clients and devices을 참조하십시오.](../../plan-your-deployment/clients-and-devices/clients-and-devices.md) 최적의 사용자 환경을 보장하려면 모든 장치 유형에 대해 Microsoft에서 인증한 UC(통합 통신) 장치를 사용하는 것이 좋습니다. UC 인증 장치에 대한 자세한 내용은 에 대한 전화 및 [비즈니스용 Skype.](../../../SfbPartnerCertification/certification/devices-ip-phones.md)

## <a name="requirements-for-dial-in-conferencing"></a>전화 접속 회의 요구 사항

전화 접속 회의는 다양한 구성 요소를 포함하는 비즈니스용 Skype 서버 회의 작업의 선택적 기능입니다. 일부 구성 요소는 전화 접속 회의와 관련되며 또 다른 일부 구성 요소는 Enterprise Voice 구성 요소입니다. 이 섹션에서는 전화 접속 회의에 필요한 구성 요소의 요구 사항에 대해 설명합니다. 중재 서버 및 PSTN(Public Switched Telephone Network) 게이트웨이 [요구](../../plan-your-deployment/enterprise-voice-solution/mediation-server.md) 사항에 대한 자세한 내용은 비즈니스용 Skype 서버의 중재 서버 구성 요소 및 비즈니스용 Skype 서버 토폴로지 작성기에서 중재 [서버 배포를 비즈니스용 Skype 서버.](../../deploy/deploy-enterprise-voice/deploy-a-mediation-server.md)
...
|프런트 엔드 서버  <br/> |비즈니스용 Skype 서버 회의 알림 서비스  <br/> |5073  <br/> |TCP  <br/> |비즈니스용 Skype 서버 회의 알림(전화 접속 회의용)에 대한 들어오는 SIP 요청에 사용됩니다.  <br/> |
