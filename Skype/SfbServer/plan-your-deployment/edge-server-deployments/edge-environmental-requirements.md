...
- 들어오는 **트래픽과 나가** 는 트래픽에 대해 대칭 NAT를 구성해야 합니다. 대칭 NAT는 에지 서버에서 사용할 수 있는 유일한 비즈니스용 Skype 서버 NAT입니다.
...
- DNS **부하 분산** 을 사용하는 경우 확장된 통합 에지 서버에만 NAT를 사용할 수 있습니다. HLB(하드웨어 부하 분산)를 사용하는 경우 NAT 없이 공개적으로 라우팅 가능한 IP **주소를 사용해야** 합니다.
...
&sup1; Exchange 분산을 사용하는 UM(통합 메시징) 원격 사용자 장애 조치(failover)를 수행하려면 2013 이상이 Exchange 필요합니다.
  
### <a name="ip-address-requirements"></a>IP 주소 요구 사항

기본 수준에서 세 서비스에는 IP 주소가 필요합니다. 액세스 에지 서비스, 웹 회의 에지 서비스 및 A/V 에지 서비스 세 개의 IP 주소(각 서비스에 대해 하나씩)를 사용할 수도 있으며, 각 IP 주소를 사용하여 각 서비스를 다른 포트에 둘 수도 있습니다(이 중 일부에 대한 자세한 내용은 포트 및 방화벽 계획 [](edge-environmental-requirements.md#PortFirewallPlan) 섹션을 참조하세요). 단일 통합 에지 환경의 경우 그 정도입니다.
...
- **고가용** 성: 배포에 고가용성이 필요한 경우 풀에 에지 서버를 두 개 이상 배포해야 합니다. 단일 에지 풀이 최대 12대의 에지 서버를 지원할 것입니다(토폴로지 작성기에서는 테스트 또는 지원되지 않는 최대 20대의 에지 서버를 추가할 수 있습니다. 이렇게 하지 않는 것이 좋습니다). 에지 서버가 12대 이상 필요한 경우 에지 풀을 추가로 만들어야 합니다.
...
에지 비즈니스용 Skype 서버 배포할 때 DNS를 제대로 준비하는 것이 중요합니다. 올바른 레코드가 배치된 경우 배포는 훨씬 더 간단합니다. 개요를 확인한 다음 이러한 시나리오에 대한 DNS 레코드를 설명하는 몇 가지 표를 나열하기 위해 위의 섹션에서 토폴로지가 선택되었습니다. 필요한 경우 좀 [더 비즈니스용 Skype 서버](../../plan-your-deployment/network-requirements/advanced-edge-server-dns.md) 읽기를 위해 일부 고급 에지 서버 DNS 계획을 세울 것입니다.
...
- 외부 네트워크 어댑터에 하나의 IP 주소를 사용할 수 있습니다. 다른 포트(비즈니스용 Skype 서버)를 사용하여 서비스 간을 차별화해야 하지만 대체 포트를 차단할 수 있는 몇 가지 방화벽이 있기 때문에 권장되지 않습니다. 이에 [대한 자세한 내용은 포트 및 방화벽 계획](edge-environmental-requirements.md#PortFirewallPlan) 섹션을 참조하세요.
...
- contoso.com<span></span>: 공용 FQDN
    
- fabrikam.com<span></span>: 두 번째 공용 FQDN(SIP 도메인이 여러 개 있는 경우 요청할 사항의 데모로 추가)
    
- Contoso.net<span></span>: 내부 도메인
...
비즈니스용 Skype 서버 에지 서버 배포를 위한 포트 및 방화벽에 대한 계획을 수립하면 문제 해결과 스트레스를 비즈니스용 Skype 서버 수 있습니다. 따라서 프로토콜 사용법과 NAT 및 공용 IP 시나리오에 대해 열려 있는 포트, 인바운드 및 아웃바운드 포트를 나타내는 몇 가지 테이블을 나열할 것입니다. HLB(하드웨어 부하가 균형 조정된 시나리오)에 대한 별도의 테이블과 이에 대한 몇 가지 추가 지침도 제공됩니다. 자세한 내용은 특정 배포 관련 정보를 확인할 수 비즈니스용 Skype 서버 에 [](scenarios.md) 지 서버 시나리오도 제공합니다.
