...
비즈니스용 Skype 서버 2015 설치를 시작 하려면 서버와 사용자를 호스트할 Active Directory 도메인 서비스 스키마, 포리스트 및 도메인을 준비 해야 합니다. 비즈니스용 Skype 서버 배포 마법사는 스키마부터 시작 하 여 포리스트 준비로 이동 하 여 Active Directory를 준비 하는 데 필요한 단계를 안내 합니다. Active Directory 복제가 성공 했는지 확인 한 후에는 사용자 또는 서버를 호스트할 각 도메인을 준비 합니다.
