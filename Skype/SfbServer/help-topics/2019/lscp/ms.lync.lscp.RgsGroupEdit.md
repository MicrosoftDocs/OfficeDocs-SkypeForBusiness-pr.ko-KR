...
- **라우팅 메서드** 에이전트가 전화를 받는 순서를 결정하는 방법을 선택합니다.
  ...
  - 현재 상태와 관계없이 동시에 로그인한 모든 에이전트와 응답 그룹 응용 프로그램에 새 통화를 제공하려면 **Attendant를** 선택합니다. 에이전트로 구성된 전화 참석자 및 클라이언트 사용자는 대기 중이던 모든 통화를 볼 수 있으며 원하는 순서로 대기 중 전화에 응답할 수 있습니다. 통화를 수락하는 첫 번째 에이전트로 통화가 전송됩니다. 다른 전화 참석자 및 사용자는 더 이상 통화를 볼 수 없습니다.
