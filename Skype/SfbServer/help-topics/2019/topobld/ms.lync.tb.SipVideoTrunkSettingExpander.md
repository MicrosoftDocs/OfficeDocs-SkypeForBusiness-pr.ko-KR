---
title: SIP 비디오 트렁크 설정 확장기
ms.reviewer: ''
ms.author: v-cichur
author: cichur
manager: serdars
audience: ITPro
ms.topic: article
f1.keywords:
- CSH
ms.custom:
- ms.lync.tb.SipVideoTrunkSettingExpander
ms.prod: skype-for-business-itpro
localization_priority: Normal
ms.assetid: 39e21ee3-3b4e-4ef3-b977-5abeec63bcff
ROBOTS: NOINDEX, NOFOLLOW
description: 토폴로지 작성기에서 새 VIS 또는 비디오 게이트웨이를 정의하는 데 필요한 단계는 비즈니스용 Skype 서버에서 VIS 풀 만들기에서 온라인으로 설명됩니다.
ms.openlocfilehash: 0019fa835a4904482f913484f8c048da0dbf2465
ms.sourcegitcommit: c528fad9db719f3fa96dc3fa99332a349cd9d317
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 01/12/2021
ms.locfileid: "49805688"
---
# <a name="sip-video-trunk-setting-expander"></a>SIP 비디오 트렁크 설정 확장기
 
토폴로지 작성기에서 새 VIS 또는 비디오 게이트웨이를 정의하는 데 필요한 단계는 비즈니스용 Skype 서버에서 VIS 풀 만들기에서 온라인으로 [설명됩니다.](../../../deploy/deploy-video-interop-server/create-a-vis-pool.md)
  
비디오를 사용하는 서버에 Video Interop 서버 역할을 할당하는 Windows PowerShell 단계는 비즈니스용 Skype 서버의 VIS 서버 역할 배포에서 온라인으로 [설명됩니다.](../../../deploy/deploy-video-interop-server/deploy-the-vis-server-role.md)
  
웹 서버를 사용하는 새 Video Interop 서버의 구성 Windows PowerShell 비즈니스용 Skype 서버의 Video Interop 서버 구성에서 [온라인에 설명되어 있습니다.](../../../deploy/deploy-video-interop-server/configure-the-vis.md)
  
 VIS와의 상호 연동을 위해 Cisco GUI에서 설정해야 하는 비디오 게이트웨이의 설정은 비즈니스용 Skype 서버와의 상호 연산을 위한 [CUCM](../../../deploy/deploy-video-interop-server/configure-cucm-for-interoperation.md)구성에서 온라인으로 설명됩니다.
  
 비디오 게이트웨이 및 VIS와의 상호를 위해 Cisco GUI에서 설정해야 하는 VTC의 설정은 비즈니스용 Skype 서버와의 상호 연산을 위한 [VTC](../../../deploy/deploy-video-interop-server/configure-a-vtc-for-interoperation.md)구성에서 온라인으로 설명됩니다.
  
## <a name="see-also"></a>참고 항목

[비즈니스용 Skype 서버의 비디오 Interop 서버 계획](../../../plan-your-deployment/video-interop-server.md)
