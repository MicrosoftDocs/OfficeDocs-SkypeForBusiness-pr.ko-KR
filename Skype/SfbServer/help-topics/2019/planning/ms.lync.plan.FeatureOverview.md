...
계획 도구의 **중앙 사이트** 페이지를 사용하여 비즈니스용 Skype 서버 배포를 디자인할 수 있습니다. 중앙 집중식 배포 또는 분산된 배포를 두 개 만들 수 있습니다. 중앙 집중식 배포에는 조직의 모든 비즈니스용 Skype 사용자가 있는 하나의 중앙 사이트만 있습니다. 분산 배포에는 두 개 이상의 중앙 사이트가 있습니다. 여러 중앙 사이트에 비즈니스용 Skype 서버를 배포하는 경우 계획 도구에서 각 중앙 사이트의 사용자 수를 입력합니다.
...
- **전화 접속 회의**
    ...
    전화 접속 회의를 사용하면 PSTN의 전화로 A/V에 참가할 수 있습니다. 전화 접속 회의를 사용하려면 회의 수행자 및 회의 공지 서비스 응용 프로그램을 배포해야 합니다.
...
- **XMPP 기반 서비스 공급자와의 페더럴**
    ...
    비즈니스용 Skype 서버 2015에서는 완전히 통합된 XMPP 프록시(에지 서버에 배포)와 프런트 엔드 서버에 배포된 XMPP 게이트웨이를 도입했습니다. XMPP 프록시 추가 및 구성과 XMPP 게이트웨이를 배포하면 비즈니스용 Skype 서버 사용자가 IM(인스턴트 메시징) 및 현재 상태용 XMPP 기반 파트너의 연락처를 추가할 수 있습니다.
...
- **Exchange 통합 메시징**
    ...
    Exchange UM(통합 메시징)은 음성 메일과 전자 메일을 단일 메시징 인프라로 결합합니다. 비즈니스용 Skype 서버 2015는 Exchange UM을 사용하여 전화 응답, 구독자 액세스, 전화 알림 및 자동 전화 교환 서비스를 제공합니다. 이러한 서비스를 사용하는 경우 공유 Active Directory 토폴로지에서 Exchange UM 및 비즈니스용 Skype 서버를 통합해야 합니다.
...
- **Exchange Archiving 통합**
    ...
    Exchange에 있는 사용자가 사서함을 In-Place 경우 비즈니스용 Skype 서버 저장소를 Exchange 저장소와 통합하는 옵션을 선택할 수 있습니다.
...
- **장치 업데이트 웹 서비스**
    ...
    장치 업데이트 웹 서비스는 조직 외부에 배포된 비즈니스용 Skype와 같은 모든 장치를 업데이트하는 자동화된 방법을 제공합니다.
...
- **회의 참석자**
    ...
    회의 전화 회의 응용 프로그램은 타사 오디오 회의 공급자의 서비스가 없는 전화 사용자에게 오디오 회의 기능을 제공합니다.
    
- **회의 공지**
    
    회의 알림 응용 프로그램은 사용자가 전화 회의에 입장 또는 퇴장할 때 신호음과 음소거되거나 음소거되지 않은 전화 사용자에게 알림을 생성합니다.
