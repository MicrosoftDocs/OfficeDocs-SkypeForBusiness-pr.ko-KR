...
영구 채팅 서버 또는 영구 채팅 서버 준수 기능에 대 한 데이터베이스를 제공할 준수 SQL Server 저장소를 구성 합니다.
  
 **SQL server 저장소**: 기존 SQL server를 선택 하 고 필요에 따라 영구 채팅 인스턴스를 선택할 수도 있습니다.
  
**새로 만들기** 를 클릭 하 여 새 SQL Server를 정의 하 고 영구 채팅 준수 데이터에 대 한 새 인스턴스를 선택 합니다.
  
Sql Server 데이터베이스 및 영구 채팅 준수 데이터에 대 한 미러링된 데이터베이스를 제공할 선택적 인스턴스를 구성 하려면 **이 확인란을** 선택 합니다.
  
영구 채팅 준수 SQL Server에 대 한 SQL server 미러로 사용할 sql server 및 선택적 인스턴스를 **미러링 sql server 저장소** 목록에서 선택 합니다.
...
**새로 만들기** 를 클릭 하 여 새 SQL server 감시를 정의 합니다 (선택적으로 영구 채팅 준수 SQL Server 미러링 모니터 서버에 대 한 인스턴스).
