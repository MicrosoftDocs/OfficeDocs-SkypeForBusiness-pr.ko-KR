---
title: AV MCU와 프론트 엔드 연결
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
manager: serdars
ms.date: 11/17/2018
audience: ITPro
ms.topic: article
f1.keywords:
- ms.lync.tb.AssociateAvMcuWithFrontEndPage
ms.prod: skype-for-business-itpro
localization_priority: Normal
ms.assetid: 5a436e41-c902-4968-8d6e-8e7c0adf9f40
description: 회의를 사용 하는 모든 Enterprise Edition 프런트 엔드 풀에는 A/V 회의 서비스 collocated 있어야 하며, 독립 실행형 A/V 회의 풀을 사용 해야 합니다. 현재 A/V 회의 서비스 collocated를 보유 하 고 있는 기존의 모든 Enterprise Edition 프런트 엔드 풀은이 새로운 독립 실행형 A/V 회의 풀을 대신 사용 하도록 선택 될 수 있습니다.
ms.openlocfilehash: 93a6874f13e7d7a79052af8771c5432310780a04
ms.sourcegitcommit: 19f534bfafbc74dbc2d381672b0650a3733cb982
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 02/03/2020
ms.locfileid: "41684961"
---
# <a name="associate-av-mcu-with-front-end"></a><span data-ttu-id="e9097-104">AV MCU와 프론트 엔드 연결</span><span class="sxs-lookup"><span data-stu-id="e9097-104">Associate AV MCU With Front End</span></span>
 
<span data-ttu-id="e9097-105">회의를 사용 하는 모든 Enterprise Edition 프런트 엔드 풀에는 A/V 회의 서비스 collocated 있어야 하며, 독립 실행형 A/V 회의 풀을 사용 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="e9097-105">All Enterprise Edition Front End pools with Conferencing enabled must have an A/V Conferencing service collocated, or must use a stand-alone A/V Conferencing pool.</span></span> <span data-ttu-id="e9097-106">현재 A/V 회의 서비스 collocated를 보유 하 고 있는 기존의 모든 Enterprise Edition 프런트 엔드 풀은이 새로운 독립 실행형 A/V 회의 풀을 대신 사용 하도록 선택 될 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="e9097-106">Any existing Enterprise Edition Front End pools that currently have an A/V Conferencing service collocated may be selected to use this new stand-alone A/V Conferencing pool instead.</span></span>
  

