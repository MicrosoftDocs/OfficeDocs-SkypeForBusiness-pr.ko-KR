...
> [!TIP]
> 나중에 A/V 회의 풀을 구현하려는 경우 **다중 컴퓨터 풀**을 선택합니다. 풀이 부하 분산된 둘 이상의 컴퓨터로 정의되었더라도 단일 컴퓨터 풀을 만들고 단일 컴퓨터에 대한 풀 FQDN을 만들 수 있습니다. 나중에 더 많은 컴퓨터를 풀에 추가할 준비가 되 면 작성기를 다시 수행 하 여 새 풀 구성원을 정의 하 고 새 토폴로지를 게시 한 다음 비즈니스용 Skype 서버 배포 마법사를 통해 새 A/V 회의 풀 구성원을 설정 해야 합니다. A/V 회의 서버 풀은 풀을 만들기 위해 부하 분산 장치가 필요하지 않다는 점에서 고유합니다. A/V 회의 풀은 내부적으로 부하 분산을 수행하며 추가 하드웨어가 필요하지 않습니다. 
