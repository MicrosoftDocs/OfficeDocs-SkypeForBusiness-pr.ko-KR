...
영구 채팅 SQL Server 또는 영구 채팅 서버 준수 저장소에 대한 백업 데이터베이스를 제공할 백업 준수 SQL Server 구성합니다.
  
 **SQL Server 저장소:** 기존 SQL Server 영구 채팅 인스턴스를 선택합니다.
  
새로 **고치려면** 새 SQL Server 영구 채팅 백업 준수 데이터에 대한 새 인스턴스(선택 사항)를 정의합니다.
  
저장소 SQL Server 사용 **확인란을** 선택하여 영구 채팅 백업 준수 데이터에 SQL Server 데이터베이스 및 선택적 인스턴스를 구성합니다.
  
영구 채팅 **SQL Server** 준수 정책에 대한 SQL Server 미러로 사용할 SQL Server 및 선택적 인스턴스를 저장하는 미러링 SQL Server.
...
새로 **고침을** 클릭하여 미러링 SQL Server 미러링된 영구 채팅 백업 준수 준수에 대한 인스턴스를 선택적으로 SQL Server 정의합니다.
  
이전 풀 정의 대화 상자로 돌아가려면 **뒤로** 를 클릭합니다.
  
이 **풀의** 백업 저장소 구성에 대한 옵션 입력을 완료하고 SQL Server 채팅 서버 풀 정의를 계속하려면 다음을 클릭합니다.
...
[비즈니스용 Skype 서버 2015에서 영구 채팅 서버에 대한 준수 서비스 구성](../../manage/persistent-chat/configure-compliance.md)
