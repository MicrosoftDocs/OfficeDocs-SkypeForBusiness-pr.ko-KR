...
- **번호 범위** 첫 번째 필드에 미지정 번호 범위의 시작 번호를 입력합니다. 두 번째 필드에 범위의 끝 번호를 입력합니다.
  ...
  - 숫자는 정규식(tel:)?()?과 \+ 일치해야 하나요? [1-9]\d {0,17} (;ext=[1-9]\d {0,9} )?. 즉, 번호가 문자열 tel: (해당 문자열을 지정하지 않으면 자동으로 추가됩니다), 더하기 기호(+) 및 숫자 1-9로 시작할 수 있습니다. 전화 번호는 최대 17자리이며 ;ext= 뒤에 내선 번호가 오는 형식으로 내선 번호를 추가할 수 있습니다.
