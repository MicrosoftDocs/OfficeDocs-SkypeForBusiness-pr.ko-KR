...
> [!NOTE]
> 장치 업데이트 웹 서비스에서 구성한 로그 파일 유지 기간(일)보다 오래된 로그 파일을 자동으로 삭제할 시간을 구성할 수도 있습니다(기본적으로 10일 이상 된 로그 파일이 자동으로 삭제됨). 이 설정은 비즈니스용 Skype 서버 제어판을 사용하여 수정할 수 없습니다. 대신 비즈니스용 Skype 서버 관리 셸을 사용해야 합니다. 만료된 로그 파일을 삭제할 시간을 지정하려면 **New-CsDeviceUpdateConfiguration** cmdlet에 -LogCleanUpTimeOfDay 매개 변수를 사용합니다. 자세한 내용은 [New-CsDeviceUpdateConfiguration을 참조합니다.](/powershell/module/skype/new-csdeviceupdateconfiguration?view=skype-ps)
