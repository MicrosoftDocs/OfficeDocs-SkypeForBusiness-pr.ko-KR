---
title: 2013에서 상태 및 모니터링 비즈니스용 Skype 서버
ms.reviewer: ''
ms.author: v-cichur
author: cichur
manager: serdars
audience: ITPro
ms.topic: article
ms.prod: skype-for-business-itpro
f1.keywords:
- NOCSH
localization_priority: Normal
ms.assetid: 0b9b44c4-06ad-4b89-b87d-62448f936ab8
description: '요약: 이 문서의 모니터링 및 상태 구성 작업에 대해 비즈니스용 Skype 서버.'
ms.openlocfilehash: 259e3a939f53320d1bb8066bfa02a96c314c1d37dfb7e5d0ccbcd7a5d872a573
ms.sourcegitcommit: a17ad3332ca5d2997f85db7835500d8190c34b2f
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/05/2021
ms.locfileid: "54297184"
---
# <a name="manage-health-and-monitoring-in-skype-for-business-server"></a>2013에서 상태 및 모니터링 비즈니스용 Skype 서버

**요약:** 2013의 모니터링 및 상태 구성 작업에 대해 비즈니스용 Skype 서버.

이 섹션의 항목에서는 비즈니스용 Skype 서버 제어판 및 관리 셸에서 수행할 수 있는 모니터링 및 상태 구성 작업을 비즈니스용 Skype 서버 단계별 절차를 제공합니다.

## <a name="in-this-section"></a>이 섹션의 내용

- [CDR(통화 정보 기록)비즈니스용 Skype 서버](call-detail-recording-cdr.md)

- [QoE(QoE) 비즈니스용 Skype 서버](quality-of-experience.md)

- [모바일 기능을 통해 성능 비즈니스용 Skype 서버](monitor-mobility-performance.md)

- [2016에서 모니터링 보고서 비즈니스용 Skype 서버](monitoring-reports.md)

## <a name="see-also"></a>참고 항목



[모니터링 계획](/previous-versions/office/lync-server-2013/lync-server-2013-planning-for-monitoring)

[모니터링 배포](/previous-versions/office/lync-server-2013/lync-server-2013-deploying-monitoring)