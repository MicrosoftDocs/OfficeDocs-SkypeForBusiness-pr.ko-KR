...
사용자 활동 보고서에는 유용한 정보가 많지만 이러한 정보를 찾기가 어려운 경우도 있습니다. 예를 들어 지정된 기간 동안 조직에서 진행되는 모든 사용자 활동은 사용자 활동 보고서에 포함됩니다. 즉, 보고서 내에 묻어 있는 것은 실제로 어떤 사용자가 어떤 방식으로 비즈니스용 Skype 서버 정보입니다.

> [!NOTE]
> 기술적으로는 일부 사용자 활동이 기록되지 않을 수 있습니다. 비즈니스용 Skype 서버 모든 전화 통화에 대한 정보를 유지하기 위해 노력하는 동안 해당 통화에 대한 정보가 데이터베이스에 기록되지 않은 채로 통화가 걸 수 있습니다. 비즈니스용 Skype 서버 매우 정확하지만 이러한 방식이 어떻게 사용되는지 정확하게 비즈니스용 Skype 서버 수 있습니다. 모든 통화의 100%가 기록된다고 보장할 수 없는 사실은 비즈니스용 Skype 서버 모니터링을 청구 시스템으로 사용할 수 없는 이유를 설명하고 있습니다. 둘째, 모니터링 보고서는 레코드를 1,000개까지만 표시할 수 있습니다. 따라서 사용자 활동의 양과 작업 기간에 따라서는 쿼리가 데이터베이스에 실제로 저장되어 있는 모든 데이터를 반환하지는 않을 수도 있습니다. 
...
이러한 정보를 확인하려면 모니터링 보고서에서 검색된 데이터를 Excel 스프레드시트로 내보내면 됩니다. 그런 다음 해당 스프레드시트 및/또는 쉼표로 구분된 값 파일을 사용하여 사용자 활동 보고서에서 가능한 방식으로 데이터를 분석합니다. 예를 들어 보고서 데이터를 Excel로 내보낸 다음 쉼표로 구분된 값 파일로 내보냈다고 가정하겠습니다. 이때 다음 명령을 사용하여 .CSV 파일에서 Windows PowerShell 가져올 수 있습니다.
...
| **Modality** <br/>         | 사용할 수 있는 형식은 선택된 활동 유형에 따라 다릅니다. 활동 유형이 피어 투 피어인 경우 IM을 선택할 수 있습니다. 파일 전송 응용 프로그램 공유 음성 또는 비디오(모달)  <br/> 활동 유형이 전화 회의인 경우 IM 전화 수 있습니다. 웹 회의 응용 프로그램 공유 음성/비디오 회의 또는 전화 통신 회의.  <br/>                                                                                                                                                                                                                                            |
