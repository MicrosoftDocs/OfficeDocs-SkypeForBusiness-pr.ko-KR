...
ms.locfileid: "54325105"
---
# <a name="peer-to-peer-activity-diagnostic-report-in-skype-for-business-server"></a>피어 투 피어 활동 진단 비즈니스용 Skype 서버
 
**요약:** 자세한 내용은 2013의 피어 투 피어 활동 진단 비즈니스용 Skype 서버.
  
피어 투 피어 활동 진단 보고서는 피어 투 피어 통신 세션의 성공 및 실패에 대한 정보를 제공합니다. 각 비즈니스용 Skype 서버 오류 종류를 구분합니다.
  
- **예상 오류입니다.** 예상 오류는 일반적으로 가장 기술적 의미에서만 실패입니다. 예를 들어 누군가에게 전화를 걸었지만 부재 중이라 전화에 응답할 수 없는 경우를 가정해 보겠습니다. 통화에 응답하지 않은 것이기 때문에 기술적으로 통화는 실패로 간주됩니다. 반면 이 오류는 예상된 오류입니다. 비즈니스용 Skype 서버 전화에 응답할 수 없는 경우 전화에 응답하지 않을 것으로 예상됩니다. 마찬가지로 오프라인 상태이거나 인스턴트 메시징을 지원하지 않는 전화에만 로그온되어 있는 사용자에게 인스턴트 메시지를 보내면 예상되는 오류가 발생합니다.
    
- **예기치 않은 오류입니다.** 예기치 않은 오류는 이름에서 의미하는 정확히, 상황에 따라 발생할 것으로 예상되지 않는 오류입니다. 예를 들어 누군가에게 전화를 걸고 해당 사람이 통화에 응답할 수 있는 경우를 가정해 보겠습니다. 그러나 비즈니스용 Skype 서버 통합 메시징에 대한 연결이 끊어지기 때문에 통화를 음성 메일로 Exchange 실패합니다. 이 오류는 예기치 않은 오류입니다. 통화가 항상 음성 메일로 라우팅될 수 있습니다. 일반적으로 예기치 않은 오류는 실제 오류입니다. 즉, 사용자 교육이나 유사한 조치를 통해 해결할 수 없는 문제입니다.
...
이 경우 2024+469+16=총 세션 수는 2,509개가 되지만 총 세션 열에는 총 세션 수가 2,521개로 표시됩니다. "누락된" 12개 세션은 시스템에서 성공 또는 실패로 분류하지 못한 세션입니다. 타사 제품에 익숙하지 않은 새로운 진단 코드가 도입되는 경우도 비즈니스용 Skype 서버. 이와 같은 경우 해당 제품을 사용하여 전화를 걸고 해당 진단 코드를 보고하는 경우를 항상 성공, 예상 오류 또는 예기치 않은 오류로 분류할 수 있는 것은 아닙니다.
  
## <a name="accessing-the-peer-to-peer-activity-diagnostic-report"></a>피어 투 피어 활동 진단 보고서 액세스

피어 투 피어 진단 보고서는 모니터링 보고서 홈 페이지에서 액세스합니다. 다음 메트릭 [](failure-distribution-report.md) 중 하나를 클릭하여 오류 비즈니스용 Skype 서버 보고서에 액세스할 수 있습니다.
...
|**Modality** <br/> | 발생한 통신 활동의 유형을 나타냅니다. 다음 중 하나를 선택합니다. <br/>  [모두] <br/>  인스턴트 메시징 <br/>  파일 전송 <br/>  응용 프로그램 공유 <br/>  오디오 <br/>  비디오 <br/> |
