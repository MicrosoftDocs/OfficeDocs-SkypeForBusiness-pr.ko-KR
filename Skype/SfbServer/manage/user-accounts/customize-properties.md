...
7. **전화 통신** 에서 다음을 수행합니다.
   ...
   - PC 대 PC 오디오 통신을 비롯한 서비스 정책 클래스에 따라 비즈니스용 Skype 인프라를 사용하여 사용자의 전화 통화를 라우팅하려면 **Enterprise Voice.** **줄 URI** 에서 Enterprise Voice의 전화 번호를 지정합니다. **다이얼 플랜 정책** 및 **음성 정책** 에서 사용자에게 적절한 정책을 지정합니다. 사용자가 건 전화 번호를 E.164 형식으로 변환하는 정규화 규칙을 지정하려면 **위치 정책** 에서 적절한 위치 프로필을 선택합니다.
