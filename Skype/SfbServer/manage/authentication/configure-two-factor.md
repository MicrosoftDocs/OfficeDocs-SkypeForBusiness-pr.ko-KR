...
---
# <a name="configure-two-factor-authentication-in-skype-for-business-server"></a>비즈니스용 Skype 서버에서 2단계 인증 구성

**요약:** 비즈니스용 Skype 서버에서 2단계 인증을 구성합니다.
...
## <a name="configure-an-enterprise-root-certificate-authority-to-support-smart-card-authentication"></a>스마트 카드 인증을 지원하도록 엔터프라이즈 루트 인증 기관 구성

다음 단계에서는 스마트 카드 인증을 지원하도록 엔터프라이즈 루트 CA를 구성하는 방법을 설명합니다.

엔터프라이즈 루트 CA를 설치하는 방법에 대한 자세한 내용은 엔터프라이즈 루트 인증 기관 설치를 [참조하세요.](https://go.microsoft.com/fwlink/p/?LinkID=313364)

1. 도메인 관리자 계정을 사용하여 엔터프라이즈 CA 컴퓨터에 로그인합니다.

2. 시스템 관리자를 시작하고 인증 기관 웹 등록 역할이 설치되어 있는지 확인합니다.

3. 관리 도구 **메뉴에서** 인증 기관 관리 **콘솔을** 갑니다.

4. 탐색 창에서 인증 **기관을 확장합니다.**

5. 인증서 **템플릿을 마우스 오른쪽 단추로 클릭하고** 새로 **고른** 다음 **발급할 인증서 템플릿을 선택합니다.**
...
12. 사용 권한을 다음과 같이 변경합니다.
    ...
    - 읽기/등록(허용) 권한이 있는 Domain Users 그룹 추가

## <a name="configure-windows-8-for-virtual-smart-cards"></a>가상 스마트 카드에 대해 Windows 8 구성

2단계 인증 및 스마트 카드 기술을 배포할 때 고려해야 할 한 가지 요소는 구현 비용입니다. Windows 8은 다양한 새로운 보안 기능을 제공하며, 가장 흥미로운 새 기능 중 하나는 가상 스마트 카드에 대한 지원입니다.

사양 버전 1.2를 충족하는 TPM(신뢰할 수 있는 플랫폼 모듈) 칩이 탑재된 컴퓨터의 경우 이제 조직에서는 하드웨어에 추가 투자를 하지 않고도 스마트 카드 로그온의 이점을 얻을 수 있습니다. 자세한 내용은 [Windows 8에서 가상 스마트 카드 사용을 참조하세요.](https://go.microsoft.com/fwlink/p/?LinkId=313365)

### <a name="to-configure-windows-8-for-virtual-smart-cards"></a>가상 스마트 카드에 대해 Windows 8을 구성합니다.

1. 비즈니스용 Skype 사용 가능 사용자의 자격 증명을 사용하여 Windows 8 컴퓨터에 로그인합니다.

2. Windows 8 시작 화면에서 커서를 화면의 오른쪽 아래 모서리로 이동합니다.
...
8. 명령 프롬프트에서 다음 명령을 사용하여 새 가상 스마트 카드를 생성합니다.
  ...
  ```
    ...
    > [!NOTE]
    > 가상 스마트 카드를 만들 때 사용자 지정 PIN 값을 제공하기 위해 대신 /pin 프롬프트를 사용합니다.
...
1. 비즈니스용 Skype 사용 가능 사용자의 자격 증명을 사용하여 Windows 8 Workstation에 로그인합니다.

2. 실행 Internet Explorer.

3. 인증 기관 웹 **등록** 페이지(예: https://MyCA.contoso.com/certsrv)

    > [!NOTE]
    > Internet Explorer 10을 사용하는 경우 호환성 모드에서 이 웹 사이트를 보아야 할 수 있습니다.
...
7. 인증서 템플릿 섹션에서 **Smartcard** **사용자를** 선택하고 다음 값을 사용하여 고급 인증서 요청을 완료합니다.
  ...
  - 추가 **옵션에서** 다음 값을 확인할 수 있습니다.
    ...
    - 요청 **형식의 경우** **CMC를 선택합니다.**

    - 해시 **알고리즘의 경우** **sha1을 선택합니다.**
...
다음 섹션에서는 다단계 인증을 지원하도록 AD FS 2.0(Active Directory Federation Services)을 구성하는 방법을 설명합니다. AD FS 2.0을 설치하는 방법에 대한 자세한 내용은 [AD FS 2.0](https://go.microsoft.com/fwlink/p/?LinkId=313374)단계별 및 방법 가이드를 참조하세요.

> [!NOTE]
> AD FS 2.0을 설치할 때 Windows Server Manager를 사용하여 Active Directory Federation Services 역할을 추가하지 않습니다. 대신 [Active Directory Federation Services 2.0 RTW 패키지를 다운로드하여 설치합니다.](https://go.microsoft.com/fwlink/p/?LinkId=313375)

### <a name="to-configure-ad-fs-for-two-factor-authentication"></a>2단계 인증에 대해 AD FS를 구성합니다.
...
4. 다음 명령을 실행하여 수동 인증을 사용하도록 설정할 각 서버와 파트너 관계를 설정하고 배포와 관련한 서버 이름을 대체합니다.
...
7. 비즈니스용 Skype 서버에 대해 새 트러스트가 만들어졌습니다.

8. 다음 명령을 실행하여 신뢰할 수 있는 Windows PowerShell 트러스트에 대한 실행 권한 부여 규칙을 만들고 할당합니다.
...
10. AD FS 2.0 관리 콘솔에서 신뢰하는 사용자 트러스트 마우스 오른쪽 단추를 클릭하고 클레임 규칙 **편집을 선택합니다.**

11. 발행 권한 **부여** 규칙 탭을 선택하고 새 권한 부여 규칙이 만들어졌습니다.

12. 발행 **변환 규칙** 탭을 선택하고 새 변환 규칙이 성공적으로 만들어졌습니다.
...
양식 기반 인증을 사용하면 사용자가 사용자 이름/암호를 사용하여 또는 스마트 카드와 PIN을 사용하여 인증할 수 있는 웹 페이지를 개발할 수 있습니다. 이 항목에서는 AD FS 2.0을 사용하여 전송 계층 보안 클라이언트 인증을 구현하는 방법에 대해 중점적으로 다를 수 있습니다. AD FS 2.0 인증 유형에 대한 자세한 내용은 [AD FS 2.0: 로컬](https://go.microsoft.com/fwlink/p/?LinkId=313384)인증 유형을 변경하는 방법을 참조하십시오.
...
3. C:\inetpub\adfs\ls로 찾아보기

4. 기존 사용자 지정 파일의 백업 복사본을 web.config 합니다.

5. 메모장에서 web.config 기존 파일 열기
...
다음 섹션에서는 수동 인증을 지원하도록 비즈니스용 Skype 서버를 구성하는 방법을 설명합니다. 2단계 인증을 사용하도록 설정한 사용자는 실제 또는 가상 스마트 카드와 유효한 PIN을 사용하여 비즈니스용 Skype 클라이언트를 사용하여 로그인해야 합니다.
...
다음 단계에서는 수동 인증을 사용하도록 설정되는 Director, 엔터프라이즈 풀 및 Standard Edition 서버에 대한 사용자 지정 웹 서비스 구성을 만드는 방법을 설명합니다.

### <a name="to-create-a-custom-web-service-configuration"></a>사용자 지정 웹 서비스 구성을 만들하려면

1. 비즈니스용 Skype 관리자 계정을 사용하여 비즈니스용 Skype 서버 프런트 엔드 서버에 로그인합니다.

2. 비즈니스용 Skype 서버 관리 셸을 실행합니다.

3. 비즈니스용 Skype 서버 관리 셸 명령줄에서 다음 명령을 실행하여 수동 인증을 사용하도록 설정될 각 Director, Enterprise Pool 및 Standard Edition 서버에 대해 새 웹 서비스 구성을 생성합니다.
  ...
  ```
    ...
    > [!CAUTION]
    > WsFedPassiveMetadataUri FQDN의 값은 AD FS 2.0 서버의 페더ation 서비스 이름입니다. 페더링 서비스 이름 값은 탐색 창에서 서비스를 마우스 오른쪽 단추로 클릭한 다음  페더링 서비스 속성 편집을 선택하여 AD FS 2.0 관리 콘솔에서 찾을 수 **있습니다.**
...
비즈니스용 Skype 웹 서비스에 대해 인증서 인증을 사용하지 않도록 설정하면 비즈니스용 Skype 클라이언트는 덜 선호되는 인증 유형(예: Kerberos 또는 NTLM)을 사용하여 등록자 서비스에 인증합니다. 클라이언트가 웹틱킷을 검색할 수 있도록 허용하려면 인증서 인증이 필요합니다. 그러나 등록자 서비스에 대해 Kerberos 및 NTLM을 사용하지 않도록 설정해야 합니다.

다음 단계에서는 수동 인증을 사용하도록 설정되는 에지 풀, 엔터프라이즈 풀 및 Standard Edition 서버에 대한 사용자 지정 프록시 구성을 만드는 방법을 설명합니다.

### <a name="to-create-a-custom-proxy-configuration"></a>사용자 지정 프록시 구성을 만들하려면

1. 비즈니스용 Skype 서버 관리 셸 명령줄에서 다음 명령을 실행하여 수동 인증을 사용하도록 설정될 각 비즈니스용 Skype 서버 에지 풀, 엔터프라이즈 풀 및 Standard Edition 서버에 대해 새 프록시 구성을 생성합니다.
...
[비즈니스용 Skype 클라이언트 및 비즈니스용 Skype 서버에서 2단계 인증 사용](use-two-factor.md)
