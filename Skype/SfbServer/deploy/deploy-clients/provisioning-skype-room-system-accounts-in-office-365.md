...
이 항목을 읽고 Microsoft 365 또는 Office 365에서 Skype 룸 시스템 계정을 프로비전하는 방법을 알아보습니다.
  
다음 섹션에서는 Skype 룸 시스템 계정 프로비전에 대한 설명을 제공합니다.
  
## <a name="microsoft-365-and-office-365-prerequisites"></a>Microsoft 365 및 Office 365의 선행 기술
...
- 테넌트 원격 관리자에게는 다음 PowerShell 액세스 권한이 있어야 합니다.
  ...
  - Windows Azure Microsoft 365 또는 Office 365 디렉터리 액세스에 Windows PowerShell Active Directory 모듈을 설치합니다.
    
Skype 룸 계정의 경우 다음 라이선스가 필요합니다.
  
- Skype 모임을 사용하도록 설정하려면 비즈니스용 Skype Online 계획 2 또는 Office 365 E1 또는 E3 라이선스가 필요합니다.
    
- 전화 번호로 채팅방을 사용할 수 있도록 Enterprise Voice 기능을 사용하여 방에 권한을 부여하려면 전화 시스템 라이선스 또는 Office 365 E5가 있는 비즈니스용 Skype Online 계획 2가 필요합니다(1).
...
Exchange에 이미 계획 기능을 제공하는 리소스 공간 사서함이 있을 수 있습니다. 또는 Skype 룸 시스템 배포를 용이하게 하기 위해 리소스 사서함을 처음으로 만들 수도 있습니다. 어떤 경우든 테넌트에서 사용할 방 계정을 식별해야 합니다. Exchange Online 프로비전 및 비즈니스용 Skype 프로비전 섹션에서는 두 종류의 계정에 대한 지침을 제공합니다. 예를 들어 다음 두 방이 있으며 이 두 방 모두에 대해 Skype 룸 시스템을 배포할 수 있습니다.
...
이전 명령은 계정을 사용하도록 설정하여 Skype 룸 시스템 사용에 대한 새 Exchange 리소스 사서함 계정을 설정하거나 만들 수 있습니다.
  
사서함을 만들고 나면 Exchange Online PowerShell의 Set-CalendarProcessing cmdlet을 사용하여 사서함을 구성할 수 있습니다. 자세한 내용은 단일 포리스트의 3-6단계를 참조하세요.
...
비즈니스용 Skype Online에 대한 라이선스를 할당한 후 로그인하고 비즈니스용 Skype 클라이언트를 사용하여 계정이 활성 상태인지 확인할 수 있습니다.
...
2. 비즈니스용 Skype에 대해 Skype 룸 시스템 계정을 사용하도록 설정하려면 다음 명령을 실행합니다.
   ...
   ```
    ...
    다음 명령을 사용하여 이 속성을 반환하면 비즈니스용 Skype 사용자가 있는 RegistrarPool 주소를 기존 계정 중 하나에서 얻을 수 있습니다.
...
>[!NOTE] 
>Skype 룸 시스템 계정에는 MFA(다단계 인증)가 지원되지 않습니다. 

## <a name="password-expiration"></a>암호 만료

Microsoft 365 또는 Office 365에서 다른 암호 만료 정책을 구성하지 않는 한 모든 사용자 계정에 대한 기본 암호 만료 정책은 90일입니다. Skype 룸 시스템 계정의 경우 다음 단계에 따라 암호 사용 기간 미정 설정을 선택할 수 있습니다.
  
1. 테넌트 Windows Azure 자격 증명을 사용하여 Active Directory 세션을 만들 수 있습니다.
