...
---
# <a name="skype-room-system-single-forest-on-premises-deployments"></a>Skype 룸 시스템 단일 포리스트-프레미스 배포
 
이 항목을 읽고 단일 포리스트에 Skype 룸 시스템을 배포하는 방법을 배워야 합니다.
  
이 섹션에서는 단일 포리스트의 Exchange Server 배포에서 호스팅되는 비즈니스용 Skype 서버에서 Skype 채팅방 시스템 계정을 프로비전하는 단계를 간략하게 설명합니다.
...
2. 새 사서함을 만들 계획인 경우 단일 포리스트의 경우 다음 명령을 실행합니다.
...
5. 선택 사항: 사용자에게 비즈니스용 Skype 회의실에 대한 정보와 모임을 예약하고 참가할 때 예상할 수 있는 정보를 제공하는 모임 수락 텍스트를 구성합니다. 
...
1. 다음 명령을 실행하여 비즈니스용 Skype 서버 풀에서 Skype 룸 시스템 계정을 사용하도록 설정할 수 있습니다.
...
> [!NOTE]
> Skype 회의실 Enterprise Voice 계정을 사용하도록 설정하는 경우 조직에 적합한 제한된 음성 정책을 구성해야 합니다. 비즈니스용 Skype 회의실이 공개적으로 사용 가능한 리소스인 경우 누구나 이 리소스를 사용하여 예약된 모임 또는 애드업으로 모임에 참가할 수 있습니다. 모임에 참가한 후 어떤 번호로도 전화를 걸 수 있습니다. 비즈니스용 Skype 서버에서 전화 접속 기능은 사용자의 음성 정책(이 경우 모임에 참가하는 데 사용되는 Skype 채팅방 시스템 계정)을 사용 합니다. 이전 버전의 Lync Server에서는 이끌이의 음성 정책이 사용됩니다. 따라서 이전 버전의 Lync Server 사용자가 회의실을 예약하고 Skype 회의실 계정을 초대하는 경우 이끌이가 해당 번호로 전화를 걸 수 있는 한 누구나 비즈니스용 Skype 회의실을 사용하여 모임에 참가하고 모든 국가/지역 또는 국제 전화 번호로 전화를 걸 수 있습니다. 
