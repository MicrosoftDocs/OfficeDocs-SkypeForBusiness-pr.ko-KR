...
비즈니스용 Skype 서버 2013에서 응답 그룹 워크플로를 디자인하고 Enterprise Voice. 헌트 그룹 워크플로와 대화형 워크플로에 모두 설명되어 있습니다.
...
8. 페더레이션 사용자가 그룹을 호출할 수 있도록 허용하려면 **페더레이션 사용** 확인란을 선택합니다. 또한 페더전을 사용하도록 구성된 응답 그룹 응용 프로그램에 적용되는 외부 액세스 정책도 있어야 합니다.
    ...
    > [!NOTE]
    > 비즈니스용 Skype Online에서 호스팅된 사용자는온-프레미스 배포에서 호스트된 응답 그룹에 전화를 걸 수 없습니다. 이는 하이브리드 배포와 비즈니스용 Skype Online 배포와 함께 사내 배포가 페더임되는 경우 모두에 해당합니다.
...
14. (선택 사항) **설명에** 워크플로에 대한 설명을 비즈니스용 Skype의 연락처 카드에 표시하려는 경우 입력합니다.
...
19. 다음 중 하나를 수행하여 사용할 업무 시간 일정의 유형을 선택합니다.
    ...
    - 미리 정의된 업무 시간 일정을 사용하려면 **미리 설정된 일정 사용** 을 클릭한 다음 드롭다운 메뉴에서 사용할 일정을 선택합니다.
      ...
      > [!NOTE]
      > 이 옵션을 선택하려면 이전에 미리 설정된 일정이 하나 이상 있어야 합니다. **New-CSRgsHoursOfBusiness** cmdlet을 사용하여 미리 설정된 일정을 정의할 수 있습니다. 자세한 내용은 [(Optional) Define Response Group business hours in Skype for Business을 참조하세요.](optional-define-response-group-business-hours.md)
...
24. **5단계 휴일 지정** 아래에서 응답 그룹이 근무하지 않는 요일을 정의하는 하나 이상의 휴일 집합에 대한 확인란을 클릭합니다.
    ...
    > [!NOTE]
    > 워크플로를 구성하기 전에 휴일 및 휴일 집합을 정의해야 합니다. **New-CsRgsHoliday** 및 **New-CsRgsHolidaySet** cmdlet을 사용하여 휴일 및 휴일 집합을 정의할 수 있습니다. 자세한 내용은 [(Optional) Define Response Group holiday sets in Skype for Business을 참조하세요.](optional-define-response-group-holiday-sets.md)
...
6. 업무 시간과 휴일을 정의하려는 경우 워크플로를 만들거나 수정하기 전에 정의해야 합니다. 자세한 내용은 [(선택 사항) 비즈니스용 Skype에서](optional-define-response-group-business-hours.md) 응답 그룹 업무 시간 정의 및 (선택 사항) 비즈니스용 Skype에서 응답 그룹 휴일 집합 [정의를 참조하세요.](optional-define-response-group-holiday-sets.md)
...
9. 워크플로를 만들거나 수정합니다. 워크플로를 만들 경우 **New-CsRgsWorkflow 를 사용하세요.** 워크플로를 수정하려면 **Set-CsRgsWorkflow 를 사용하세요.** 명령줄에 다음을 입력합니다.
...
14. (선택 사항) **설명에** 비즈니스용 Skype의 연락처 카드에 표시하려는 워크플로에 대한 설명을 입력합니다.
...
19. 다음 중 하나를 수행하여 사용할 업무 시간 일정의 유형을 선택합니다.
    ...
    - 미리 정의된 업무 시간 일정을 사용하려면 **미리 설정된 일정 사용** 을 클릭한 다음 드롭다운 메뉴에서 사용할 일정을 선택합니다.
      ...
      > [!NOTE]
      > 이 옵션을 선택하려면 이전에 미리 설정된 일정이 하나 이상 있어야 합니다. **New-CsRgsHoursOfBusiness** cmdlet을 사용하여 미리 설정한 일정을 정의합니다. 자세한 내용은 [(Optional) Define Response Group business hours in Skype for Business을 참조하세요.](optional-define-response-group-business-hours.md) 미리 설정된 일정을 선택한 경우 응답 그룹을 사용할 수 있는 요일 및 시간으로 **요일**, **시작** 및 **종료** 가 자동으로 채워집니다.
...
28. **7단계 대화형 음성 응답 구성** 의 **사용자에게 다음 텍스트 또는 녹음된 메시지가 재생됩니다** 머리글 아래에서 다음과 같이 발신자에게 제공할 질문을 지정합니다.
    ...
    - 질문이 포함된 미리 녹음된 오디오 파일을 사용하려면 녹음 선택 을  클릭한 다음 녹음 링크를 클릭하여 파일을 업로드합니다. 새 브라우저 창에서 찾아보기를 클릭하고 오디오 파일을 선택한 다음 열기 를 **클릭합니다.** **업로드를** 클릭하여 파일을 로드한 다음 선택적으로 텍스트 상자에 질문을 입력할 수 있습니다(이렇게 하면 질문과 발신자 응답이 응답 에이전트로 전달될 수 있습니다).
