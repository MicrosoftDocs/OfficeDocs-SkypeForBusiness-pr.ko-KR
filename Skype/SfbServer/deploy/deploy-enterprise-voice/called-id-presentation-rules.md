...
2. 변환 규칙 정의를 시작하고 10단계에서 비즈니스용 Skype 서버 트 [](configure-trunk-with-media-bypass.md) 렁크를 미디어 우회로 트렁크 구성 또는 9단계까지 비즈니스용 Skype 서버 트렁크 구성 [](configure-trunk-without-media-bypass.md) 의 단계를 수행합니다.

3. **새 변환 규칙** 또는 **변환 규칙 편집** 페이지의 **이름** 아래에서 변환 대상 숫자 패턴을 설명하는 이름을 입력합니다.

4. (선택 사항) 설명 **에** 변환 규칙에 대한 설명을 입력합니다(예: 미국 국제 장거리 전화 걸기).

5. 대화 상자의 **변환 규칙 작성** 섹션에 있는 다음 필드에 값을 입력합니다.

   - **시작 숫자**: (선택 사항) 패턴을 일치시킬 선행 자릿수를 지정합니다. 예를 들어 이 필드에 +를 입력하면 +로 시작하는 E.164 형식의 번호가 일치됩니다.

   - **길이**: 일치 패턴의 자릿수를 지정하고 해당 패턴이 이 길이의 숫자와 정확히 일치하는지, 적어도 이 길이인지 또는 어떤 길이인지를 선택합니다. 예를 들어 11을 입력하고 드롭다운 목록에서 최소 11자릿수의 숫자와 일치하게 선택합니다.
...
6. **정규식 입력** 에 다음을 입력합니다.
   ...
   - **변환 규칙** 에 변환된 숫자 형식의 패턴을 지정합니다.
     ...
     예를 들어 이 패턴 일치에 ^\+(\d{9}\d+)$를  입력하고 **변환** 규칙에 011$1을 입력하면 규칙은 +441235551010 011441235551010.
