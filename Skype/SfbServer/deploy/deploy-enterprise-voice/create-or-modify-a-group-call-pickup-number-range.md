...
3. **New-CsCallParkOrbit를** 사용하여 새 통화 선택 그룹 번호를 만들 수 있습니다. **Set-CsCallParkOrbit를** 사용하여 기존 통화 선택 번호 범위를 수정합니다.
   ...
   ```
    ...
    > [!IMPORTANT]
    > 이 cmdlet을 사용하여 처음에 잘못된 형식을 지정하고 그룹 범위가 아직 사용되지 않은 경우 번호 범위에 할당된 형식을 변경합니다. 번호 범위를 CallPark에서 GroupPickup으로 변경하거나 그 반대로 변경하고 번호 범위가 이미 사용 중이면 통화 파킹 또는 그룹 통화 선택이 해당 번호 범위에 대해 작동하지 않습니다. 예를 들어 번호 범위를 CallPark에서 GroupPick으로 변경하는 경우 통화 파킹된 응용 프로그램은 더 이상 통화를 파킹하는 데 해당 파킹된 통화 번호 범위를 사용할 수 없습니다.
