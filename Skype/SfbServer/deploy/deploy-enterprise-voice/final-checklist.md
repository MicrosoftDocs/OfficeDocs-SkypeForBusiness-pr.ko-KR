...
- CAC가 사용하도록 설정되어 있는지 확인합니다. 이 설정은 2016년 8월 1일부로 [비즈니스용 Skype 서버.](enable-call-admission-control.md)
    
- CAC가 모든 중앙 사이트에서 사용하도록 설정되었는지 확인합니다. 이 수행은 토폴로지 작성기에서 수행될 수 있습니다. 게시할 때 경고가 생성되는 경우  *무시하지*  마십시오.
    
- 엔터프라이즈 네트워크에서 관리된 모든 서브넷이 네트워크 구성 설정에서 구성되었는지 확인합니다. 또한 네트워크 지역, 사이트 및 서브넷 배포에 설명된 각 서브넷을 네트워크 사이트에 연결해야 [비즈니스용 Skype.](deploy-network.md)
