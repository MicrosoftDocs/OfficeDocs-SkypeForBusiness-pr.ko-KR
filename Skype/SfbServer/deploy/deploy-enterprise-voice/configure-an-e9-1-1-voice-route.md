...
E9-1-1을 배포하려면 먼저 긴급 통화 음성 경로를 구성해야 합니다. 음성 경로를 만드는 데 대한 자세한 내용은 [Create or modify a voice route in 비즈니스용 Skype](create-or-modify-a-voice-route.md). 예를 들어 배포에 기본 SIP 트렁크와 보조 SIP 트렁크가 포함된 경우 두 개 이상의 경로를 정의할 수 있습니다. 
...
4. 다음 cmdlet을 실행하여 이전 단계에서 만든 PSTN 사용 레코드를 사용하여 새 음성 경로를 만들 수 있습니다.
    ...
    번호 패턴은 위치 정책의 긴급 전화 문자열 설정에 사용되는 번호 패턴  과 같아야 합니다. 긴급 통화에 "+"비즈니스용 Skype 추가하기 때문에 "+" 기호가 필요합니다. "Co1-pstngateway-1"은 E9-1-1 서비스 공급자 또는 ELIN 게이트웨이 서비스 ID의 SIP 트렁크 서비스 ID입니다. 다음 예에서는 음성 경로의 이름으로 "EmergencyRoute"를 사용 합니다.
