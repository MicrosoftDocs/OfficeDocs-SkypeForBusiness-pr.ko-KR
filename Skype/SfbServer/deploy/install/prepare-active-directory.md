...
4. **1단계: 스마마 준비**
    ...
    e. **완료를** 클릭하여 Schema 준비 마법사를 닫고 Active Directory 준비 단계로 돌아온다.
...
5. **2단계: Schema 파티션 복제 확인**
    ...
    e. Schema 컨테이너 아래에서 **CN=ms-RTC-SIP-SchemaVersion을 검색합니다.** 이 개체가 존재하고 **rangeUpper** 특성의 값이 1150이고 **rangeLower** 특성 값이 3이면 해당 스마마가 성공적으로 업데이트 및 복제되었습니다. 이 개체가 존재하지 않는 경우 **또는 rangeUpper** 및 **rangeLower** 특성의 값이 지정되지 않은 경우 해당 스마가 수정되지 않은 것이거나 복제되지 않은 것입니다.
