...
*    **호출 유형은** 통화가 PSTN 발신 또는 들어오는 호출인지 여부와 사용자가 호출하거나 오디오 회의에 의해 배치된 호출과 같은 호출 유형인지 여부입니다. 볼 수 있는 호출 유형은 다음입니다. 
     ...
     **요금제 호출 유형** 
     *    **user_in(사용자가** 인바운드 PSTN 호출을 수신) 
     *    **user_out(사용자가** 아웃바운드 PSTN 호출을 배치) 
     *    **user_out_conf(사용자가** 3차원 전화 통화와 같은 통화에 2명 이상의 PSTN 참가자를 추가) 
     *    **user_out_transfer(사용자가** PSTN 번호로 전화를 전송) 
     *    **user_out_forwarding(사용자가** PSTN 번호로 전화를 전달)

     **오디오 회의 통화 유형**
     *    **conf_in(오디오** 회의 브리지에 대한 인바운드 호출). 이 호출 형식의 레코드의 경우 사용자 **ID** 열에 지정된 사용자는 모임 이끌이에 해당합니다.
     *    **conf_out(일반적으로** 회의에 PSTN 번호를 추가하는 오디오 회의 브리지의 아웃바운드 호출). 이 호출 형식의 레코드의 경우 사용자 **ID** 열에 지정된 사용자는 모임 이끌이에 해당합니다.
...
*    **번호 형식은** 사용자의 전화 번호, 서비스 또는 무료 전화 번호의 전화 번호 유형입니다.  
*    **시작 시간(UTC)은** 호출이 시작되거나 배치된 시간입니다. 
*    **기간은** 통화가 연결된 기간입니다.  
*    **ConfID는** 오디오 회의의 회의 ID입니다. 
*    **요금은** 계정에 청구되는 통화의 금액 또는 비용입니다. 
*    **통화는** 통화 비용을 계산하는 데 사용되는 통화 유형입니다. 
*    **기능은** 호출에 사용되는 라이선스입니다. 볼 수 있는 라이선스 유형은 다음을 참조할 수 있습니다. 
     *    **MCOPSTNPP** - 통신 크레딧 <br/> **MCOPSTN1** - 국내 통화 계획(미국 3000분/ EU 1200분 요금제) 
     *    **MCOPSTN2** - 국제 통화 계획 
     *    **MCOPSTN5** - 국내 통화 계획(120분 통화 계획) 
     *    **MCOPSTN6** - 국내 통화 계획(240분 통화 계획) 참고: 제한된 가용성
     *    **MCOMEETADD** - 오디오 회의
     *    **MCOMEETACPEA** - 분당 오디오 회의 지불
     
> [!NOTE]
> 통화 또는 회의 구독에 포함되지 않은 분당 유료 호출만 포함하기 위해 보고서를 실행하려면 "MCOPSTNPP"를 사용하여 보고서를 필터링합니다. 이렇게 하면 분당 모든 유료 호출의 항목화가 표시됩니다.  분당 유료 오디오 회의의 경우 "MCOPSTNPP"가 아닌 "MCOMEETACPEA"로 필터링합니다.  

> [!NOTE]
> 일부 필드에 "데이터 없음"이 표시될 수도 있습니다. "데이터 없음"은 필드가 호출 유형 또는 기능에 적용되지 않습니다. 
...
내보낼 파일에는 온라인 보고서에서 사용할 수 없는 추가 필드가 포함되어 있습니다. 이러한 방법은 문제 해결 및 자동화된 워크플로에 사용할 수 있습니다.
...
> | 13 | 시작 시간 | `datetimeoffset` | 통화 시작 시간(UTC, [ISO 8601)](https://en.wikipedia.org/wiki/ISO_8601) |
> | 14 | 종료 시간 | `datetimeoffset` | 통화 종료 시간(UTC, [ISO 8601)](https://en.wikipedia.org/wiki/ISO_8601) |
