...
Windows 10 주년 업데이트 또는 Windows Server 2016을 사용하는 경우 이미 5.1 Windows PowerShell 있습니다. 이 애플리케이션은 해당 운영 체제와 함께 사전 설치됩니다.
  
사용할 Microsoft PowerShelll 버전을 확인하려면 Windows 7 또는 Windows Server 2008 R2 또는 컴퓨터의 경우 다음을 Windows Server 2012 합니다.
...
3. 그런 다음 콘솔 창에 다음과 유사한 정보를 표시해야 합니다.
    ...
    반환된 버전 번호가 5.1이면 5.1 Windows PowerShell 실행됩니다. 반환된 버전 번호가 5.1이 아닌 경우 5.1 Windows PowerShell 설치해야 합니다. Microsoft Windows Management Framework 5.1을 포함하는 Windows PowerShell 5.1을 [다운로드할 수 있습니다.](https://www.microsoft.com/download/details.aspx?id=54616)
  
5.1이 Windows PowerShell 확인한 후 PowerShell이 원격 스크립트를 실행하도록 구성되어 있는지 확인해야 합니다. 이를 위해 관리자 권한으로 PowerShell을 시작합니다. Windows 7에서 Windows Server 2008 R2, Windows Server 2012 또는 Windows Server 2012 R2에서는 다음을 실행합니다.
...
1. 참 표시줄에 액세스하고 **검색을** 클릭한 다음 을 **마우스 오른쪽 단추로 Windows PowerShell.** Windows 키를 누르고 C를 눌러 모든 Windows 8 컴퓨터(터치 스크린 또는 비터치 화면)의 참 표시줄에 빠르게 액세스할 수 있습니다.
...
현재 Windows PowerShell 5.1을 실행하고 있지 않은 경우 Microsoft 다운로드 센터에서 5.1 Windows Management Framework 다운로드하고 설치해야 합니다. 5.1 및 Windows 원격 관리(WinRM) 3.0을 Windows PowerShell 설치 패키지입니다. 예를 들어 Windows 7 SP1을 실행하고 있으며 아직 5.1 버전으로 업데이트하지 않은 경우 이 설치 패키지가 Windows PowerShell 수 있습니다. Windows Server 2016 또는 Windows 10주년 업데이트를 실행하는 경우 5.1 버전에 설치하지 Windows PowerShell 없습니다. Windows PowerShell 5.1은 해당 운영 체제에 미리 설치됩니다.
  
5.1 Windows Management Framework 설치하기 전에:
  
- 올바른 버전의 설치 패키지를 다운로드한지 확인합니다. Windows 7 SP1의 64비트 버전을 실행하는 경우 파일 다운로드를 Win7AndW2K8R2-KB3191566-x64.ZIP. 32비트 버전의 Windows 7을 실행하는 경우 파일을 다운로드하여 Win7-KB3191566-x86.ZIP.
    
- 컴퓨터에서 Windows 7을 실행하는 경우 Windows 7 서비스 팩 1을 설치한지 확인합니다.

실행 중인 Windows 버전이 확실하지 않은 경우 또는 Windows 7 서비스 팩 1을 설치한지 확실하지 않은 경우 **시작,** 컴퓨터 마우스 오른쪽 단추로 **클릭한** 다음 **속성을 클릭합니다.** 이 정보는 시스템 대화 상자에 보고됩니다.
...
기본 인증이 true로 설정되어 있는 경우 PowerShell을 사용하여 비즈니스용 Skype Online에 연결할 준비가 된 것입니다.
