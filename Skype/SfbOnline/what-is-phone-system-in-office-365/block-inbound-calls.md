...
비즈니스용 Skype 온라인 통화 계획은 이제 PSTN(공용 전환 전화 네트워크)에서 인바운드 통화 차단을 지원합니다. 이 기능을 사용하면 테넌트에 들어오는 모든 PSTN 호출의 발신자 ID를 일치하는 목록에 대해 확인할 수 있도록 테넌트 전역 번호 패턴 목록을 정의할 수 있습니다. 일치하는 경우 들어오는 호출이 거부됩니다.
...
숫자 패턴은 ```CsInboundBlockedNumberPattern``` ```New```명령, , ```Get```및 ```Set```를 통해 관리됩니다 ```Remove```. 특정 패턴의 활성화를 전환하는 기능을 포함하여 이러한 cmdlet을 사용하여 주어진 패턴을 관리할 수 있습니다.
...
전체 호출 ```CsTenantBlockingCallingNumbers``` 차단 기능을 보고 활성화하는 것은 명령 및 를 통해 관리 ```Get``` 됩니다 ```Set```.

- [Get-CsTenantBlockedCallingNumbers](/powershell/module/skype/get-cstenantblockedcallingnumbers) 는 사용(True/False)을 포함하여 전역 차단된 번호 목록에 대한 매개 변수를 반환합니다. 기능을 켜거나 끄는 것 이외에 수동으로 수정할 수 없는 단일 전역 테넌트 정책이 있습니다.
...
이 예제에서는 및 ```-Enabled``` 매개 변수 ```-Description``` 는 선택 사항입니다.
...
이 예제에서는 매개 변수 ```-Identity``` 가 필요합니다.

```powershell
Remove-CsInboundBlockedNumberPattern -Identity “<identity>”
```
 
ID를 알 수 ```Get-CsInboundBlockedNumberPattern``` 없는 경우 cmdlet을 사용하여 먼저 적절한 패턴을 찾고 ID를 적어 냅니다. 그런 다음 ```Remove-CsTenantBlockedNumberPattern``` cmdlet을 실행하고 적절한 ID 값을 전달합니다.
...
명령을 통해 ```CsTenantBlockNumberExceptionPattern``` ```Set``````New``````Get```차단된 숫자 패턴에 예외를 추가할 수 있습니다.```Remove```
...
- [Get-CsTenantBlockedNumberExceptionPattern](/powershell/module/skype/get-cstenantblockednumberexceptionpattern) 은 테넌트 목록에 추가된 모든 숫자 예외 패턴 목록을 반환합니다.
- [Set-CsTenantBlockedNumberExceptionPattern](/powershell/module/skype/set-cstenantblockednumberexceptionpattern) 은 테넌트 목록의 숫자 예외 패턴에 하나 이상의 매개 변수를 수정합니다.
...
이 예제에서는 새 숫자 예외 패턴이 만들어지며 기본적으로 사용 가능한 패턴을 추가합니다. 및 ```-Enabled``` 매개 ```-Description``` 변수는 선택 사항입니다.
...
이 예제에서는 -Identity 매개 변수가 선택 사항입니다. 매개 변수 ```-Identity``` 를 지정하지 않으면 이 cmdlet은 테넌트에 입력된 모든 숫자 예외 패턴 목록을 반환합니다.
...
이 예제에서는 -Identity 매개 변수가 필수입니다. cmdlet ```Set-CsTenantBlockedNumberExceptionPattern``` 을 사용하면 주어진 숫자 패턴 ID에 대해 하나 이상의 매개 변수를 수정할 수 있습니다.
...
이 예제에서는 매개 변수 ```-Identity``` 가 필요합니다. 이 cmdlet은 테넌트 목록에서 주어진 숫자 패턴을 제거합니다.  ID를 알 수 ```Get-CsInboundBlockedNumberPattern``` 없는 경우 cmdlet을 사용하여 먼저 적절한 패턴을 찾고 ID를 적어 냅니다. 그런 다음 ```Remove-CsTenantBlockedNumberExceptionPattern``` cmdlet을 실행하고 적절한 ID 값을 전달합니다.테스트하고 유효성을 검사하기 전에 복제 시간을 허용합니다.  
...
이 예제에서는 및 ```-Phonenumber``` 매개 변수가 ```-Tenant``` 필요합니다. 매개 ```-PhoneNumber``` 변수는 + 또는 -와 같은 추가 문자가 없는 숫자 문자열이 해야 합니다. TRPS에서 는 ```-Tenant parameter``` 선택 사항입니다. 결과 매개 ```isNumberBlocked``` 변수는 테넌트에서 숫자가 차단된 경우 True 값을 반환하고, 차단되지 않은 경우 False를 반환합니다.
