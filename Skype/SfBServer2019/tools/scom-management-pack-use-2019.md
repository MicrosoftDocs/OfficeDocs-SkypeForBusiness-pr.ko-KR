...
|구성|지원 여부|
|:-----|:-----|
|Windows Server 2008 R2 운영 체제  <br/> Windows Server 2012 R2 운영 체제   |예. 2019 비즈니스용 Skype 서버 및 가상 트랜잭션 감시자 노드에서 모두 사용할 수 있습니다.   |
|클러스터된 서버   |지원되지 않습니다.   |
|에이전트 없는 모니터링   |지원되지 않습니다.   |
|가상 환경   |예.   |
|도메인에 가입된 서버 역할   |모든 내부 비즈니스용 Skype 서버 2019 서버 역할은 도메인에 가입되어 있어야 합니다.   |
...
- **클라이언트 로그인에 대한 자동 검색** 비즈니스용 Skype 서버 2019에 로그인하는 클라이언트 응용 프로그램은 종종 로그인할 서버를 자동으로 검색합니다. 가상 트랜잭션은 이제 자동 검색이 올바르게 구성되어 있는지 확인하도록 지원됩니다.
...
|통화 안정성 경고   |비즈니스용 Skype 서버 2019 서버에서 작성한 CDRS(통화 정보 기록)에는 사용자가 통화에 연결할 수 있는지 또는 통화가 종료되는 이유가 반영됩니다. 통화 안정성 경고는 CDR 데이터베이스를 쿼리하여 많은 사용자가 피어 투 피어 통화 또는 기본 회의 기능에 대한 연결 문제를 경험하는 경우를 나타내는 경고를 생성합니다.  <br/> 시나리오 범위에는 오디오 통화, 피어 투 피어 IM(인스턴트 메시징) 및 기타 회의 기능이 포함됩니다.   |
...
|8    |MCX 피어 투 피어 인스턴트 메시지(모바일)   |Lync Server 2010의 2011년 9월 릴리스에서 2019년 9월 비즈니스용 Skype 사용 가능   |
...
|14    |전화 접속 회의   |2015 비즈니스용 Skype 서버 이상에서 사용 가능  |
|15    |응용 프로그램 공유 회의   |2015 비즈니스용 Skype 서버 이상에서 사용 가능  |
|16    |UCWA 회의(웹 모임 참가)   |2015 비즈니스용 Skype 서버 이상에서 사용 가능  |
...
|20   |XMPP 인스턴트 메시징(페더ation)   |Lync Server 2013 이상에서 사용 가능   |
| 21   |비디오 Interop 서버   |2015 비즈니스용 Skype 서버 이상에서 사용 가능   |
...
|:-----|:-----|
|비즈니스용 Skype 서버 배포   |조직에서 비즈니스용 Skype 서버 2019의 배포를 나타났습니다.   |
