...
2. 도메인의 콘솔 트리에서 정방 지원 영역 확장을 확장한 다음 비즈니스용 Skype 서버 2019가 설치될 도메인을 마우스 오른쪽 단추로 클릭합니다. 
    
3. **새 호스트(A 또는 AAAA)** 를 클릭합니다.
    
4. 이름을 **클릭하고** 비즈니스용 Skype 서버 2019 풀의 호스트 이름을 입력합니다(도메인 이름은 레코드가 정의된 영역으로 가정되어 A 레코드의 일부로 입력할 필요가 없습니다).
