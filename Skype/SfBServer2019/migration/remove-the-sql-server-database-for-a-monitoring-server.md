...
description: 모니터링 서버를 제거한 후 서버 데이터를 SQL Server 데이터베이스를 제거할 수 있습니다. 다음 절차에 따라 토폴로지 작성기에서 정의를 제거한 다음 데이터베이스 서버에서 데이터베이스 및 로그 파일을 제거합니다.
ms.openlocfilehash: 23f58166c6a24680772d50c6bc484ba1bd02d754
ms.sourcegitcommit: 556fffc96729150efcc04cd5d6069c402012421e
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/26/2021
ms.locfileid: "58605757"
---
# <a name="remove-the-sql-server-database-for-a-monitoring-server"></a>모니터링 서버용 SQL Server 데이터베이스 제거

모니터링 서버를 제거한 후 서버 데이터를 SQL Server 데이터베이스를 제거할 수 있습니다. 다음 절차에 따라 토폴로지 작성기에서 정의를 제거한 다음 데이터베이스 서버에서 데이터베이스 및 로그 파일을 제거합니다.
...
2. 토폴로지 작성기에서  공유 구성 요소로 이동한 다음 SQL Server 저장소에서 제거되거나 다시 구성된 모니터링 서버와 연결된 SQL Server 인스턴스를 마우스 오른쪽 단추로 클릭한 다음 삭제를 **클릭합니다.**
    
3. 토폴로지를 게시한 후 복제 상태를 확인합니다.
    
## <a name="to-remove-the-database-files-from-the-sql-server"></a>SQL Server에서 데이터베이스 파일을 제거하려면

1. SQL Server 기반 서버에서 데이터베이스를 제거하려면 데이터베이스 파일을 제거할 SQL Server 서버에 대한 SQL Server sysadmins 그룹의 구성원이 되어야 합니다.
...
3. 명령줄에 다음을 입력합니다.
   ...
   ```
    ...
    여기서 는 데이터베이스  _\<FQDN\>_ 서버의 FQDN(FQDN)이며 선택적 명명된  _\<instance\>_ 데이터베이스 인스턴스입니다. 
