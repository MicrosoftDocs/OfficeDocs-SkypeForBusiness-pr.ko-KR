...
2.  **비즈니스용 Skype 하이브리드 토폴로지 설정**
    ...
    > [!Note]
    > 보이스 메일 서비스 마이그레이션을 위해 사용자를 온라인으로 마이그레이션할 필요는 없습니다. 그러나 온-프레미스 사용자는 전화 시스템 음성 메일 서비스를 활용 하기 위해 하이브리드 토폴로지를 설정 해야 합니다.

3. **자동 전화 교환 마이그레이션 계획**
    
    관리자는 언제 든 지 자동 전화 교환 (ExchUMO에서 클라우드 자동 전화 교환) 마이그레이션을 시작할 수 있습니다. 자세한 내용은 [클라우드 자동 전화 교환 설정을](https://docs.microsoft.com/microsoftteams/create-a-phone-system-auto-attendant) 참조 하세요. Microsoft는 고객이 마이그레이션에 필요한 것으로 간주할 수 있는 추가 자동 전화 교환 기능을 계속 제공 하며, 관리자는 기능 집합을 평가 하 고 그에 따라 자동 전화 교환 인스턴스를 마이그레이션해야 합니다. 기능 목록 비교는 [ExchUMO 및 Azure 클라우드 기반 서비스 기능 행렬](#exchumo-and-azure-cloud-based-services-feature-matrix)을 참조 하세요.

4. **보이스 메일 마이그레이션 후 유효성 검사 및 테스트 계획**

    보이스 메일 마이그레이션은 Microsoft 구동 방식입니다. 필수 구성 요소 하이브리드 토폴로지가 설정 된 경우 관리자는 아무런 작업도 수행 하지 않아도 됩니다. Microsoft는 사용자의 보이스 메일 마이그레이션이 중단 되지 않도록 필요한 유효성 검사 및 테스트를 수행 합니다. 관리자는 해당 측면에서 테스트 및 유효성 검사를 수행 하는 것이 좋습니다. 권장 테스트 계획에 대 한 [관리자를 위한 제안 된 테스트 계획 및 마이그레이션 후 유효성 검사](#suggested-test-plan-and-post-migration-validation-for-admins) 를 참조 하세요. 
...
6. **예정 된 마이그레이션에 대 한 옵트인 고려**
    ...
    관리 되는 마이그레이션을 선택 하면 Microsoft 365 관리 포털 메시지 센터에서 마이그레이션 전 30 일 알림이 표시 되지 않습니다.
 
    계획 된 마이그레이션을 위해 옵트인 (opt in) 하려면 관리자의 전자 메일 주소에서 다음 정보를 사용 하 여 [cvm@microsoft.com](mailto:cvm@microsoft.com) 전자 메일 요청을 보냅니다.
...
Office 365 테 넌 트 관리자는 Microsoft 클라우드 자동 전화 교환 서비스에서 Exchange UM Online 자동 전화 교환을 다시 만들고, Exchange UMO 서비스를 2020 년 2 월 28 일 이전에 해당 사용자의 온-프레미스 전화 번호로 전환 해야 합니다. 됨. 새 클라우드 자동 전화 교환을 성공적으로 마이그레이션하고 테스트 하는 데 권장 되는 지침입니다. 많은 수의 자동 전화 교환이 있는 경우 [EXCHANGE UM 자동 전화 교환을](https://github.com/NathanJBennett/ExUMAAMigrationToCloudAA) 사용 하 여 자동 전화 교환의 대량 마이그레이션을 단순화할 수 있습니다.
...
| VM | 사용자 기능 | 개인 인사말 녹음 (PSTN) — 휴대폰에서 재생 |  | 개 | 피지    |
...
| VM | 발신자 기능  | 발신자 환경-보호 된 보이스 메일 | 발신자는 기록 된 메시지를 보호 된 것으로 표시 하는 옵션을 선택할 수 있습니다.| 개 | 피지    |
| VM | 발신자 기능  | 발신자 환경-비공개 보이스 메일 | 발신자는 기록 된 메시지를 비공개로 표시 하는 옵션을 선택할 수 있습니다.  | 개 | 피지    |
...
사용자가 클라우드 음성 메일로 마이그레이션 되었는지 확인 하려면 음성 메일을 사용자에 게 유지 하 고 Outlook에서 메시지 본문을 확인 합니다.  클라우드 보이스 메일 메시지에는 다음과 같은 바닥글이 있습니다.
...
|이전 메시지에 대 한 액세스 | 변경 내용 없음<br>사용자가 지원 되는 모든 끝점에서 이전 보이스 메일 메시지에 액세스할 수 있습니다. |
