...
> [!Note]
> Lync Server 2010는 지원 되지 않습니다. 2010 서버를 배포 하는 경우에는 서버 업그레이드를 계획 하거나 사용자를 Microsoft 팀으로 마이그레이션하는 것을 고려해 야 합니다.  
...
|---------|-------|--------|----|--------|------|
| V  | 서비스 기능| 타사 PBX 지원    | Exchange UM Online에서 SIP notify 메시지를 사용 하 여 MWI (메시지 대기 표시기)와 같은 타사 PBX에 제공 되는 모든 기능 포함 | N   | 피지    |
| V | 서비스 기능  | 비즈니스용 Skype 서버 지원   |  | 피지 | 피지    |
| V | 서비스 기능 | Microsoft 팀 지원|  | 피지 | N    |
| V | 서비스 기능 | eDiscovery 및 보류  | 보안 및 규정 준수  | 피지 | 피지    |
| V | 서비스 기능 | Exchange 규칙 지원 | 보안 및 규정 준수  | 피지 | 피지    |
| V | 사용자 기능 | PSTN 전화 접속 액세스  | 구독자 액세스  | N | 피지    |
| V | 사용자 기능 | 대리인  | 부재 중 전화 전자 메일  | N | 피지    |
| V | 사용자 기능 | PSTN Outlook Voice Access   | 구독자 액세스  | N | 피지    |
...
| V | 사용자 기능 | 기본 인사말을 TTS로 덮어쓰도록 사용자 설정  |  | 피지 | N    |
| V | 사용자 기능 | 개인 인사말 녹음 (인증 된 장치) |  | 피지 | 피지    |
| V | 사용자 기능 | 녹음 개인 인사말 (PSTN)-전화에서 재생 |  | N | 피지    |
| V | 사용자 기능 | 기록을 사용 하지 않도록 설정할 수 있는 사용자 설정 |  | N | 피지    |
| V | 사용자 기능 | 기록  |  | 피지 | 피지    |
| V | 사용자 기능 | SIP notify 메시지를 사용한 MWI (메시지 대기 표시기) |  | N | 피지    |
| V | 사용자 기능 | Outlook의 MP3 오디오 파일 형식    |  | 피지 | 피지    |
| V | 사용자 기능 | 가변 속도 재생 컨트롤 |  | 피지 | 피지    |
| V | 사용자 기능 | 음성 메일 전달  | 다른 사용자에 게 받은 음성 메일 전달 | 피지 | 피지    |
| V | 사용자 기능 | 사용자 그룹에 음성 메시지 보내기  |음성 메일 브로드캐스트   | N | 피지   |
| V | 사용자 기능 | SMS를 사용한 음성 메일 알림    | 사용자에 게 새 음성 메일이 있을 때 SMS 수신 가능    | N | 피지    |
...
| A | 서비스 기능 | AA 지원 타사 PBX    |  | N | 피지    |
| A | 서비스 기능 | 비즈니스용 Skype 서버 지원   |  | 피지 | 피지    |
| A | 서비스 기능 | Microsoft 팀 지원|  | 피지 | N    |
...
| A | 서비스 기능 | AA에 클라우드 PSTN 번호 할당 |  | 피지 | N    |
...
| A | 테 넌 트-관리 기능 | 웹 포털 관리   |  | 피지 | N    |
| A | 테 넌 트-관리 기능 | PowerShell cmdlet  |  | 피지 | 피지    |
| 팩스| 서비스 기능 | 팩스 통합|  | N | 피지    |
