...
많은 대기업에는 온-프레미스 AD 포리스트가 여러 개 있고, 경우에 따라 고객은 둘 이상의 Exchange 및/또는 비즈니스용 Skype 서버(또는 Lync Server) 배포가 있을 수 있습니다. 또한 온-프레미스 포리스트 하나만 있는 조직에서도 비즈니스 합병 또는 인수를 통해 유사한 상황에 처할 수 있습니다. 이러한 고객은 클라우드로 전환할 때 주어진 사내 워크로드의 여러 인스턴스를 단일 조직 또는 Microsoft 365 Office 365 합니다. 이 문서에서는 UC 워크로드를 Microsoft 클라우드(예: Microsoft Teams 및/또는 비즈니스용 Skype Online)로 이동하려는 비즈니스용 Skype(또는 Lync)의 여러 온-프레미스 배포가 있는 조직을 위해 이러한 통합을 달성하는 방법에 대해 설명합니다.

[!INCLUDE [sfbo-retirement-skype](../../Hub/includes/sfbo-retirement.md)]

지금까지는 이러한 상황에 처한 고객은 온-프레미스 배포를 먼저 통합한 다음 클라우드로 이동해야 했습니다. 이 문서에서는 여전히 옵션이지만, 여러 비즈니스용 Skype 배포를 사용하는 조직이 한 번의 배포를 한 번의 Microsoft 365 또는 Office 365 조직으로 마이그레이션할 수 있도록 하는 새로운 기능을 기반으로 하는 솔루션에 대해 설명합니다. 이 새로운 기능을 사용하는 경우에도 비즈니스용 Skype Online 및 Microsoft Teams 단일 비즈니스용 Skype 또는 Office 365 하이브리드 모드에서 여러 Microsoft 365/Lync 포리스트를 지원하지 않습니다. 
...
다음 주요 요구 사항이 충족되는 경우 여러 비즈니스용 Skype 배포를 사용하는 모든 조직에 대해 단일 Microsoft 365 또는 Office 365 조직의 모든 사용자를 클라우드로 통합할 수 있습니다.
...
여러 포리스트에 비즈니스용 Skype 배포하는 고객은 공유 SIP 주소 공간 기능을 사용하여 단일 하이브리드 비즈니스용 Skype 포리스트의 모든 사용자를 개별적으로 Microsoft 365 또는 Office 365 조직으로 완전히 마이그레이션한 다음, 다음 온-프레미스 비즈니스용 Skype 배포를 마이그레이션하기 전에 해당 온-프레미스 배포와 하이브리드를 사용하지 않도록 설정해야 합니다. 클라우드로 마이그레이션하기 전에 온-프레미스 사용자는 동일한 사용자의 온-프레미스 디렉터리에 표시되지 않는 사용자와 페더레이션 상태로 유지됩니다.  

## <a name="canonical-example-of-cloud-consolidation"></a>클라우드 통합의 정형 예

온라인 또는 온라인에서 온라인으로 통합하려는 비즈니스용 Skype 두 개의 별도의 Microsoft Teams 배포를 비즈니스용 Skype 고려합니다.


|원래 상태 세부 정보 |원하는 상태 세부 정보 |
|---------|---------|
|<ul><li>2개의 비즈니스용 Skype AD 포리스트에 있는 독립적인 프레미스 배포<li>1대의 포리스트가 온라인과 하이브리드에 비즈니스용 Skype 있습니다. <li> Orgs가 서로 페더더러 <li>사용자가 이러한 포리스트에서 동기화되지 않습니다.<li> 조직에 조직 또는 Microsoft 365 Office 365 Azure AD에 디렉터리를 동기화하고 있을 수 있습니다.</ul>|<ul> <li>1 Microsoft 365 또는 Office 365 조직<li>더 이상 사내 배포가 아니기 때문에 하이브리드가 남지 않습니다.<li>사내의 모든 사용자는 비즈니스용 Skype Online에 있으며 선택적으로 Teams 전용 사용자일 수 있습니다. <li>어디에서도 사용할 수 있는 비즈니스용 Skype 공간 없음 <li>사용자가 여전히 사내 인증을 사용 중입니다.</ul> |
...
1.  아직 Microsoft 365 없는 Office 365 조직을 만들 수 있습니다.
2.  두 배포의 모든 관련 SIP 도메인이 모든 Microsoft 365 또는 Office 365 확인해야 합니다.
3.  하이브리드 또는 비즈니스용 Skype 하이브리드 배포를 하나 Microsoft 365 Office 365. 이 예제에서는 OriginalCompany를 사용하게 될 것입니다. <span> com.
4.  [먼저 커넥트 포리스트에 대해 AAD](configure-azure-ad-connect.md) 설정을 사용하도록 설정(OriginalCompany). <span> com) 
5.  조직에 Teams 경우 [TeamsUpgradePolicy에](/powershell/module/skype/grant-csteamsupgradepolicy) 대한 테넌트 전체 정책을 SfBWithTeamsCollab 또는 다른 SfB 모드 중 하나(SfBOnly 또는 SfBWithTeamsCollabAndMeetings)로 설정하십시오. 이는 프레미스에 남아 있는 사용자에게만 통화 및 채팅을 Teams 중요합니다.
...
7.  다른 사내 배포에서 호스팅하는 모든 SIP 도메인의 경우(이 경우 AcquiredCompany). <span> com) PowerShell에서 [비즈니스용 Skype 온라인에서](/powershell/module/skype/disable-csonlinesipdomain) 이러한 SIP 도메인을 사용하지 `Disable-CsOnlineSipDomain` 않도록 설정하세요. 이 기능은 2018년 12월 이후의 새로운 기능입니다.
...
9.  하이브리드 배포(OriginalCompany) <span> com) 계정이 [비즈니스용 Skype](move-users-between-on-premises-and-cloud.md) 온라인에 있도록 사용자를 비즈니스용 Skype 클라우드로 사용자를 이동(Teams 전용인지 여부는 비즈니스용 Skype) 이제 조직은 그림 **[B처럼 보입니다.](#figure-b)** 그림 A에서 변경된 주요 사항은 다음입니다.
    - 이제 두 가지 모두에 있는 사용자가 AAD에 있습니다.
    - AcquiredCompany. <span> com은 사용하지 않도록 설정된 온라인 SIP 도메인입니다.
    - 일부 사용자는 온라인에서 온라인 또는 비즈니스용 Skype 이동했습니다Teams. 자주색 사용자 A를 참조합니다.
10. 모든 사용자를 클라우드로 이동한 [](cloud-consolidation-disabling-hybrid.md) 후 OriginalCompany에 대한 비즈니스용 Skype 하이브리드를 사용하지 않도록 설정하십시오. <span> com from Office 365:  
    - 조직에서 분할 도메인을 Microsoft 365 Office 365 않습니다.
    - OriginalCompany에서 사용자 또는 Microsoft 365 Office 365 기능을 사용하지 않도록 설정합니다. <span> com on-premises.
    - OriginalCompany에 대한 DNS 레코드를 업데이트합니다. <span> com을 Microsoft 365 또는 Office 365.
...
13. 닫힌 페더링을 사용하는 경우 순수 온라인 테넌트의 모든 SIP 도메인(.microsoftonline.com 제외)을 동일한 Microsoft 365 또는 동일한 도메인에 있는 허용 도메인으로 \* Office 365.  변경이 적용되기까지 다소 시간이 걸릴 수 있으며 초기에 이 작업을 수행하면 손상되지 않습니다. 따라서 14단계로 이동하기 전에 이 작업을 잘 수행해보는 것이 좋습니다.
...
16. 원하는 경우 사용자를 사내에서 클라우드로 [마이그레이션합니다.](move-users-between-on-premises-and-cloud.md) 사용자를 [TeamsOnly](/microsoftteams/teams-and-skypeforbusiness-coexistence-and-interoperability) 모드로 직접 마이그레이션하거나 먼저 온라인으로 마이그레이션할 비즈니스용 Skype 있습니다. 이 상태의 경우 조직은 그림 **[D와 같이 됩니다.](#figure-d)**
...
- 하이브리드 조직의 일부 사용자가 클라우드로 이동했습니다(보라색 음영으로 표시된 사용자 A). 이러한 사용자는 비즈니스용 Skype 온라인 사용자일 수도 Teams 및 페더임이 지원되는 사용자만 사용할 수 있습니다.<br><br>
    ![그림 B 다이어그램](../media/cloudconsolidationfigb.png)

##### <a name="figure-c"></a>그림 C:

- OriginalCompany의 모든 사용자입니다. <span> com은 이제 클라우드에 있습니다(비즈니스용 Skype Online에 홈). 또한 이 두 개만 사용할 Teams 좋습니다.
...
- AcquiredCompany 간에 공유 SIP 주소 공간을 사용할 수 있습니다. <span> com 및 Microsoft 365 또는 Office 365.
...
위의 정식 예제의 단계에서는 조직이 현재 상태의 Microsoft 또는 현재 상태가 없는 두 개의 페더전된 Office 365 가정합니다. 그러나 일부 조직에는 기존 Microsoft 365 Office 365 수 있으며 위의 순서에 서로 다른 진입점이 있을 수 있습니다. 일반적인 구성은 네 가지입니다.

- 조직 또는 조직이 없는 여러 개의 페더 Microsoft 365 Office 365. 이 경우 1단계에서 시작합니다.
...
- 별도의 비즈니스용 Skype 조직과 페더미드하는 순수 Teams Online 조직(Teams 또는 사용하지 않을 수 있는)비즈니스용 Skype. 이 조직이온-프레미스 조직에 대해 온라인 SIP 도메인을 사용하지 않도록 설정하고 커넥트 비즈니스용 Skype 조직에 대해 AAD 도메인을 활성화하면 1-11단계를 완료한 그림 **[C에](#figure-c)** 표시된 가상의 조직과 같이 표시됩니다.

## <a name="limitations"></a>제한 사항

- 조직에 관련된 조직 또는 Microsoft 365 Office 365 있어야 합니다. 조직이 두 개 이상인 시나리오에서는 통합이 지원되지 않습니다.
- 포리스트가 한 비즈니스용 Skype 모드(공유 SIP 주소 공간)에 있을 수 있습니다. 다른 모든 비즈니스용 Skype 포리스트는 전적으로 비즈니스용 Skype 유지되어야 하며, 서로 페더 Microsoft 365 또는 Office 365 합니다.
...
- 이전 단계 중 일부의 순서를 조정할 수 있습니다. 충족해야 하는 주요 요구 사항은 이러한 모든 요구 사항이 충족될 경우 다음을 충족하는 것입니다.
    - 단일 AAD 테넌트와 비즈니스용 Skype 포리스트 동기화를 위한 두 개 이상의 사내 포리스트
...
사용자를 하이브리드 환경의 클라우드로 이동하는 경우 사용자를 전용 또는 TeamsOnly 모드로 비즈니스용 Skype 수 있습니다. *사용자를 TeamsOnly 모드로 이동하는 경우 먼저 이 섹션을 읽어야 합니다.*
...
- 클라이언트 Teams UX는 아직 TeamsUpgradePolicy의 SfB 모드를 존중하지 않습니다. 예를 들어 이러한 모드에서는 현재는 Teams 채팅 시작이 가능하기는 하지만 앞으로는 이 모드가 불가능합니다. 이로 인해 사용자들이 혼동을 일으킬 수 있습니다. 경우에 따라 때때로 Teams 경우에 따라 비즈니스용 Skype 수 있습니다. 아직 프레미스에 있는 사용자에 대해 TeamsMessagingPolicy 및 TeamsCallingPolicy를 통해 통화 및 채팅을 별도로 사용하지 않도록 설정하는 것이 좋습니다.
