...
Azure AD Connect는 [여러 포리스트에서 동기화를](https://docs.microsoft.com/en-us/azure/active-directory/connect/active-directory-aadconnect-topologies)지원 합니다. 그러나 AAD에 대 한 Azure AD Connect 동기화 인스턴스만 지원 합니다. 따라서 Azure AD가 한 포리스트에 이미 설치 되어 있는 경우에는 추가 포리스트에서 동기화 되도록 AAD 연결의 기존 인스턴스를 업데이트 해야 합니다.
