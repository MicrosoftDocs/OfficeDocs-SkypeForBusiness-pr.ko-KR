...
- Adm_Skype4B_Online
ms.custom: ''
description: 다음 섹션에서는 리소스/사용자 포리스트 모델에 여러 포리스트가 있는 환경을 구성하여 하이브리드 시나리오에서 비즈니스용 Skype 기능을 제공하는 방법에 대한 지침을 제공합니다.
ms.openlocfilehash: 38e41a2ae845120bbdc49419d47b264f69c0a5fe
ms.sourcegitcommit: 9879bc587382755d9a5cd63a75b0e7dc4e15574c
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 07/21/2021
ms.locfileid: "53510699"
---
...
다음 섹션에서는 리소스/사용자 포리스트 모델에 여러 포리스트가 있는 환경을 구성하여 하이브리드 시나리오에서 비즈니스용 Skype 기능을 제공하는 방법에 대한 지침을 제공합니다. 
...
- 하이브리드 구성에서 지원되는 Lync Server 및 비즈니스용 Skype 서버 버전에 대한 자세한 내용은 [Plan hybrid connectivity between 비즈니스용 Skype 서버 and Microsoft 365 or Office 365.](plan-hybrid-connectivity.md) [](plan-hybrid-connectivity.md#server-version-requirements)
...
비즈니스용 Skype 사용자가 프레미스 또는 Exchange 홈으로 사용할 수 있습니다. 비즈니스용 Skype 온라인 사용자는 최적의 환경을 Exchange Online 정보를 사용해야 합니다. 그러나 이 필수는 아닙니다. Exchange 두 경우 모두 비즈니스용 Skype 구현할 필요는 없습니다.
  
## <a name="configure-forest-trusts"></a>포리스트 트러스트 구성

리소스 포리스트 토폴로지에서 리소스를 호스팅하는 리소스 비즈니스용 Skype 서버 액세스하는 사용자의 계정이 포함된 각 계정 포리스트를 신뢰해야 합니다. 사용자 포리스트가 여러 개 있는 경우 포리스트 간 인증을 사용하도록 설정하려면 이러한 각 포리스트 트러스트에 대해 이름 접미사 라우팅을 사용하도록 설정해야 합니다. 자세한 내용은 [Managing Forest Trusts를 참조하십시오.](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc772440(v=ws.11)) 다른 Exchange Server 배포한 경우 해당 포리스트가 비즈니스용 Skype 기능을 제공하는 경우 포리스트를 호스팅하는 포리스트는 Exchange 포리스트를 신뢰해야 비즈니스용 Skype 서버. 예를 들어 Exchange 포리스트에 배포된 경우 이는 해당 구성에 계정과 비즈니스용 Skype 포리스트 간의 양측 트러스트가 필요하다는 의미입니다.
  
## <a name="synchronize-accounts-into-the-forest-hosting-skype-for-business"></a>계정을 호스트하는 포리스트에 계정을 비즈니스용 Skype

비즈니스용 Skype 서버 포리스트(리소스 포리스트)에 배포되지만 하나 이상의 다른 포리스트(계정 포리스트)에 있는 사용자에게 기능을 제공하는 경우 다른 포리스트의 사용자는 포리스트가 배포된 포리스트에서 비활성화된 사용자 비즈니스용 Skype 서버 표시되어야 합니다. id 관리 제품(예: Microsoft Identity Manager)을 배포하고 구성하여 계정 포리스트의 사용자를 배포된 포리스트로 비즈니스용 Skype 서버 합니다. 사용자를 사용하지 않도록 설정한 사용자 개체로 비즈니스용 Skype 서버 포리스트에 동기화해야 합니다. 사용자가 Active Directory 연락처 개체로 동기화될 수 없습니다Azure Active Directory 커넥트 연락처를 Azure AD와 올바르게 동기화하지 못하기 때문에 Skype.
...
## <a name="create-a-microsoft-365-or-office-365-organization"></a>조직 Microsoft 365 또는 Office 365 만들기

그런 다음 배포에 사용할 Microsoft 365 Office 365 조직을 프로비전해야 합니다. 자세한 내용은 Microsoft의 클라우드 서비스 에 대한 구독, 라이선스, 계정 및 [테넌트 를 참조하세요.](/office365/enterprise/subscriptions-licenses-accounts-and-tenants-for-microsoft-cloud-offerings) 
...
이 배포는 R2 AD FS의 표준 Windows Server 2012 진행하기 전에 작동해야 합니다. 자세한 내용은 [How To Install AD FS 2012 R2 For Microsoft 365 or Office 365.](https://blogs.technet.com/b/rmilne/archive/2014/04/28/how-to-install-adfs-2012-r2-for-office-365.aspx) 
  
배포한 후 앞에서 선택한 원본 앵커와 일치하게 클레임 규칙을 편집해야 합니다. AD FS MMC의 신뢰 파티 트러스트에서 id **플랫폼** 또는 Microsoft 365 플랫폼을 마우스 Microsoft Office 365 마우스 오른쪽 단추로 클릭한 다음 클레임 규칙 **편집을** **선택합니다.** 첫 번째 규칙을 편집하고 ObjectSID를 **employeeNumber로 변경합니다.** 
  
![다중 포리스트 규칙 편집 화면](../../sfbserver/media/f5d485bd-52cc-437f-ba71-217f8902056c.png)
  
## <a name="configure-aad-connect"></a>AAD 구성 커넥트

리소스 포리스트 토폴로지에서는 리소스 포리스트와 모든 계정 포리스트의 사용자 특성을 Azure AD로 동기화해야 합니다. 이렇게 하는 가장 간단하고 권장되는 방법은 Azure AD가 사용자 계정을 사용하도록 커넥트 포리스트 및 사용자 계정이 포함된 포리스트에서 사용자 ID를 동기화하고 병합하도록 하는 비즈니스용 Skype.  자세한 내용은 [Configure Azure AD 커넥트 for 비즈니스용 Skype and Teams.](configure-azure-ad-connect.md)

AAD 커넥트 계정과 리소스 포리스트 간에는 프레미스에서 동기화를 제공하지 않습니다. 이 구성은 앞에서 설명한 대로 Microsoft Identity Manager 또는 유사한 제품을 사용하여 별도로 구성해야 합니다.
  
마쳤고 AAD 커넥트 때 메타버스에서 개체를 보면 다음과 비슷한 것을 볼 수 있습니다. 
  
![다중 포리스트 메타버스 개체 화면](../../sfbserver/media/16379880-2de3-4c43-b219-1551f5dec5f6.png)
  
녹색으로 강조 표시된 특성은 Microsoft 365 또는 Office 365, 노란색은 사용자 포리스트에서, 파란색은 리소스 포리스트에서 병합됩니다. 
  
이 사용자는 테스트 사용자로, AAD 커넥트 사용자가 sourceAnchor 및 cloudSourceAnchor를 식별하고 앞에서 선택한 employeeNumber인 Microsoft 365 또는 Office 365의 리소스 포리스트 개체를 식별한 것입니다. 그런 다음 이 개체를 위의 개체에 병합할 수 있습니다. 
  
자세한 내용은 을(를) 통해 프레미스 Azure Active Directory. [](/azure/active-directory/hybrid/whatis-hybrid-identity) 
  
AAD 커넥트 다음을 제외하고 기본값을 사용하여 설치해야 합니다. 
