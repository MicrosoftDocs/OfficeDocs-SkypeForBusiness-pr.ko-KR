...
비즈니스용 Skype 서버 (또는 Lync Server)가 온-프레미스이 고 팀 또는 비즈니스용 Skype Online을 사용할 계획인 조직은이 문서에서 설명 하는 대로 온-프레미스 디렉터리를 Office 365와 동기화 하도록 Azure AD Connect를 구성 해야 합니다. 오피스.  여기에는 비즈니스용 Skype 온-프레미스에서 팀으로 직접 이동 하는 조직이 포함 됩니다. 특히 비즈니스용 Skype 온-프레미스를 사용하는 조직은 올바른 msRTCSIP 특성이 Azure AD로 동기화되는지 확인해야 합니다. 
...
3. 여러 포리스트에 여러 비즈니스용 Skype 서버를 배포 특정 요구 사항을 충족되면 조직은 이러한 여러 배포를 단일 Office 365 테넌트로 통합할 수 있습니다.
